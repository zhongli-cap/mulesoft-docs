...
Access the documentation for the link:/mule-user-guide/v/3.9/database-connector[Database Connector] to take advantage of the newly-designed functionality.
====
...
NOTE: The JDBC connector supports multi-resource transactions. Multi-resource transactions are also supported by the link:/mule-user-guide/v/3.9/database-connector[Database Connector]. Consider also the use of link:/mule-user-guide/v/3.9/xa-transactions[XA Transactions] to handle this use case.
...
+
[NOTE]
JDBC Inbound Endpoints can only be set to a one-way exchange pattern. JDBC Outbound Endpoints can be set to either a one-way or request-response exchange pattern. For details about exchange patterns, see link:/anypoint-studio/v/6/[Anypoint Studio Essentials].
...
For details on setting the properties for a JDBC endpoint using an XML editor, consult the link:/mule-user-guide/v/3.9/jdbc-transport-reference[JDBC Transport Reference].
