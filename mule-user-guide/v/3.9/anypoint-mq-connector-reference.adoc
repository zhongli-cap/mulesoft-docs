...
|Fetch Retry Attempts |Number of times to retry prefetching in case of an error. Default value is -1 (infinite).
|Fetch Retry Wait Time |The duration in milliseconds to wait after a prefetching error before retrying.
|===
