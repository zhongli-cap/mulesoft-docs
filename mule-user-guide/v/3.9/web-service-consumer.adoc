...
For more information on best design-time practices with DataSense and DataWeave, see the link:/anypoint-studio/v/6/datasense[DataSense documentation].
...
[NOTE]
As with the CXF component, you can optimize transmission of SOAP messages using the Message Transmission Optimization Mechanism (MTOM) by checking the MTOM Enabled box, or by defining an expression that triggers MTOM activation. See link:https://docs.mulesoft.com/mule-user-guide/v/3.9/using-mtom[Using MTOM].
