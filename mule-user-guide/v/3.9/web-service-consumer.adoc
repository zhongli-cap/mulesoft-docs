...
[WARNING]
The component formerly known as the *SOAP Connector* is now known as the *link:/mule-user-guide/v/3.9/cxf-component-reference[CXF Component]*. It should not be confused with the Web Service Consumer.

<<Complete Code Example>>

== Prerequisites

This document assumes that you are familiar with Anypoint Studio; review the chapter on link:/anypoint-studio/v/6/[Anypoint Studio Essentials] to learn more about developing with Mule's graphical user interface. This document also assumes you are familiar with link:/mule-user-guide/v/3.9/dataweave[DataWeave], link:/mule-user-guide/v/3.9/global-elements[Global Elements], and link:/anypoint-studio/v/6/datasense[DataSense].
...
can use the link:/mule-user-guide/v/3.9/cxf-module-reference[CXF module] to call the service.
...
To do this:
+
** Click the link *Search WSDL in Exchange* to open the link:/anypoint-exchange/anypoint-exchange[Anypoint Exchange] and select a WSDL definition residing there.
...
[NOTE]
As with the CXF component, you can optimize transmission of SOAP messages using the Message Transmission Optimization Mechanism (MTOM) by checking the *Enable MTOM* box, or by defining an expression that triggers MTOM activation. See link:https://docs.mulesoft.com/mule-user-guide/v/3.9/using-mtom[Using MTOM].
...
As stated above, when used in conjunction with a DataSense-enabled Web Service Consumer, DataWeave automatically extracts the payload or header metadata that you can use to map and or transform to a different data format or structure through the link:/mule-user-guide/v/3.9/dataweave[DataWeave language]. When you have a configured, DataSense-enabled Web Service Consumer in your flow, you can drop a DataWeave transformer next to it – preceding or following it – and DataWeave automatically extracts the information that DataSense collected from the WSDL about the expected format and structure of the message. 
...
[TIP]
--
For a full list of elements, attributes and default values, see the link:/mule-user-guide/v/3.9/web-service-consumer-reference[Web Service Consumer Reference].
--
+
...
[TIP]
Learn more about passing JVM arguments to your Mule instance link:/mule-user-guide/v/3.9/passing-additional-arguments-to-the-jvm-to-control-mule[here].
...
. Add an link:/mule-user-guide/v/3.9/http-connector[HTTP Connector] global element in your project, configure it with the necessary security attributes
+
[source, xml, linenums]
----
...
* Learn more about how to use the link:/mule-user-guide/v/3.9/dataweave[DataWeave transformer].
