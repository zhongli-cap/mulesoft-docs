...
Below are a series of examples that demonstrate various data transformation approaches expressed in link:/mule-user-guide/v/3.9/dataweave-language-introduction[DataWeave]. DataWeave code is typically written inside the *Transform Message* component, which is accessible in Anypoint Studio.

[NOTE]
To step through basic DataWeave use cases and exercises in actual Mule applications, see the link:/mule-user-guide/v/3.9/dataweave-quickstart[DataWeave Quickstart Guide].
...
[TIP]
The link:/mule-user-guide/v/3.9/dataweave-quickstart[DataWeave Quickstart Guide] walks you this same transformation, showing you how you can create it automatically via the drag-and-drop UI. It then goes into tweaking the code to make it more interesting.
...
Follow along to see how to use the link:/mule-user-guide/v/3.9/dataweave-operators#map[`map`] operator. The `as` operator is also featured in this example. Take an XML list of books and use DataWeave to output a JSON version of the book list. It goes through each `<book>` element from the XML input, simply using the `map` operator.

You can ensure the transformation generates the correct type for each element by using the `as` operator (used for type coercion - see an https://docs.mulesoft.com/mule-user-guide/v/3.9/dataweave-operators#coerce-to-number[`as` operator example]).
...
In this example, the input contains sensitive information that should be removed. The transform replicates the inbound structure but uses a simple link:/mule-user-guide/v/3.9/dataweave-operators#remove[`remove`] operator to take away specific key:value pairs.

The example goes through the whole set of elements in the input using the link:/mule-user-guide/v/3.9/dataweave-operators#map[`map`] operator.
...
In this example, the input contains the same sensitive information as in the previous one, but instead of entirely removing the key:value pairs that contain it, the values are replaced with the string. The transform replicates the inbound structure but uses a simple link:/mule-user-guide/v/3.9/dataweave-language-introduction#when-otherwise[`when`] operator to replace values when specific keys occur.
...
The link:/mule-user-guide/v/3.9/dataweave-operators#map[`map`] operator carries out the same steps for each element in the input array. Through the use of @, attributes are injected into the XML.

[TIP]
See link:/mule-user-guide/v/3.9/dataweave-selectors#attribute-selector-expressions[Attribute selector expressions] on how to select XML attributes in DataWeave.
...
The example goes through the whole set of elements in the input using a link:/mule-user-guide/v/3.9/dataweave-operators#map[`map`] operator. It also uses the link:/mule-user-guide/v/3.9/dataweave-language-introduction#when-otherwise[`when`] operator to only select instances that include the field 'insurance'.
...
The example goes through the whole set of elements in the input using  a link:/mule-user-guide/v/3.9/dataweave-operators#map-object[`mapObject`] operator. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the link:/mule-user-guide/v/3.9/dataweave-language-introduction#when-otherwise[`when`] operator together with an link:/mule-user-guide/v/3.9/dataweave-operators#and[`and`] to pick out the specific keys that need to change, and an https://docs.mulesoft.com/mule-user-guide/v/3.9/dataweave-operators#coerce-to-number[example use] of the `as` operator to coerce its type to string.
...
This example takes an XML input and parses it into a different XML arrangement. After a single `<header>` element is copied, a link:/mule-user-guide/v/3.9/dataweave-operators#map[`map` operation] carries out the same steps for each 'item': several fields are passed on without any changes, then the discount and subtotal fields are calculated with references to constants defined in the link:/mule-user-guide/v/3.9/dataweave-language-introduction#the-dataweave-header[header directives] of the transform. A single set of subtotal, tax and total elements are created by performing a link:/mule-user-guide/v/3.9/dataweave-operators#reduce[*reduce*] operation over all of the items in the "items" array, performing calculations that sometimes involve constants defined in the header. The link:/mule-user-guide/v/3.9/dataweave-operators#as[`as`] operator is also used to coerce to a number and then performs link:/mule-user-guide/v/3.9/dataweave-operators#basic-math-operations[basic math] on these numbers.
...
This example takes in an XML file that is grouped separating two types of elements: teachers and students, the transform uses the link:/mule-user-guide/v/3.9/dataweave-operators#group-by[*Group By* operation] to make it into an XML where the top level grouping is "class". Within each class, the students and teachers that are registered in it are listed.

The example goes through the whole set of elements in the input using both a link:/mule-user-guide/v/3.9/dataweave-operators#map-object[*map object* operation] and a link:/mule-user-guide/v/3.9/dataweave-operators#map[*map* operation]. Note that they differ in that map object processes both key and value of its input, rather than just the keys. It also uses the link:/mule-user-guide/v/3.9/dataweave-operators#filter[*filter* operation] to only show the attendees that have the class subject listed as a hobby. As each student may have multiple hobbies, the link:/mule-user-guide/v/3.9/dataweave-selectors#multi-value-selector[*Multi value* selector] is used to refer to them all.
...
In this example, there are three different input JSON files, these three all arrive in one single Mule message, occupying the payload and two link:/mule-user-guide/v/3.9/dataweave-selectors#flow-variables-from-a-mule-message[flow variables]. The payload contains an array of book objects, one flow variable has a set of currency exchange rates, and the other one a query. The transform filters the first input using the conditions passed in the third input, then performs a link:/mule-user-guide/v/3.9/dataweave-operators#map[`map`] to deal with each remaining object separately. Within this map, it defines two variables: *it* and *props*. Through the use of @, attributes are added into the XML tags. A second map operation inside the first one calculates the price of each book for each of the currencies provided in the second input. Another map operation displays each element in the `author` array as a separate <author></author> tag.


[TIP]
See link:/mule-user-guide/v/3.9/dataweave-selectors#flow-variables-from-a-mule-message[flow variables] for how to access flow variables from the input and link:/mule-user-guide/v/3.9/dataweave-selectors#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
The example defines a link:/mule-user-guide/v/3.9/dataweave-types#functions-and-lambdas[function] in the link:/mule-user-guide/v/3.9/dataweave-language-introduction#dataweave-header[dataweave header] and then calls it on the body. It goes through the whole set of elements in the input using  a link:/mule-user-guide/v/3.9/dataweave-operators#map-object[`mapObject`] operator. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the link:/mule-user-guide/v/3.9/dataweave-language-introduction#when-otherwise[`when` and `otherwise`] operators to pick out only instances where an XML attribute exists, and to do nothing otherwise.
...
When producing an output in link:/mule-user-guide/v/3.9/dataweave-formats#xml[XML], DataWeave by default outputs every value as enclosed between an opening and a closing tag:
