...
|`int/Short/Long/BigInteger/Flat/Double/BigDecimal`|link:/mule-user-guide/v/3.9/dataweave-types#number[:number]
...
Note that copybook schemas must always begin with a `01` entry, for example:

.Copybook Schema Example
----
01  MAILING-RECORD.
...
A separate record type is generated for each `01` definition in your copybook (there must be at least one 01 definition for the copybook to be usable, so add one using an arbitrary name at the start of the copybook if none is present).
