...
*Note:*
Track feature additions, compatibility, limitations and API version updates with each release of the connector using the link:/release-notes/kafka-connector-release-notes[Connector Release Notes]. Review the connector operations and functionality using the link:http://mulesoft.github.io/mule-kafka-connector/[Technical Reference] and in https://www.anypoint.mulesoft.com/exchange/?search=kafka[Anypoint Exchange].
...
You can install this connector in Anypoint Studio using the instructions in link:/anypoint-studio/v/6/import-asset-exchange-task[To Import an Asset from Exchange].
