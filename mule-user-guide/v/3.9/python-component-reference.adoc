...
[TIP]
Placing a Python Component is actually the same as placing a link:/mule-user-guide/v/3.9/script-component-reference[Script component] and then selecting the Python engine.
...
When deploying to a customer managed Mule runtime (one that's deployed on-premises), then you can set the `python.path` property directly in the `wrapper.conf` file. See link:/mule-user-guide/v/3.9/configuring-properties#system-properties[system properties] for a better understanding of how to set it.
