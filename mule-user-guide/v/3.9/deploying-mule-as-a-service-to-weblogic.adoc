= Deploying Mule as a Service to Weblogic
:keywords: deploy, deploying, weblogic

These instructions assume you have downloaded and installed Oracle WebLogic Application Server version 10.3. +
WebLogic 8.x and 9.x are also supported, but these instructions are specific to version 10.3.
...
. _Required_ <<Modify your WebLogic Configuration>> by adding elements to the *weblogic.xml* file.
. <<Configuring Logging>>.
...
The `web.xml` file configures the Mule application as a web application by specifying your Mule configuration file and other options for starting Mule in the WebSphere container. +
Create a `web.xml` file for your Mule application as per the example below.
...
Mule uses log4j2. both WebLogic and the Mule runtime can be configured to use log4j.
