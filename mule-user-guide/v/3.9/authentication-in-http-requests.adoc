...
For example, the redirectionURLis http://localhost:8082/callback in the link:/mule-user-guide/v/3.9/authentication-in-http-requests#creating-the-mule-client-app[previous example]:
...
When the response is in JSON format, the parameters are automatically extracted and you can use link:/mule-user-guide/v/3.9/mule-expression-language-mel[MEL expressions] to reference these values in the Mule Message that was generated from the response to the request to the token URL, as shown in the previous Github example.
...
You can customize when Mule performs one of these requests to obtain a new access token using a link:/mule-user-guide/v/3.9/mule-expression-language-mel[MEL Expression]. The expression is evaluated against the Mule Message that is generated from the response of the HTTP Request Connector call.
...
By default, the token manager uses an in-memory object store to store the credentials. You can customize the token manager object store by using the objectStore-ref attribute. link:/mule-user-guide/v/3.9/mule-object-stores[See how to configure a custom object store].

== See Also

* HTTP Request Connector
* HTTP Listener Connector
* See a link:/mule-user-guide/v/3.9/http-connector-reference[reference] of the available XML configurable options in this connector
