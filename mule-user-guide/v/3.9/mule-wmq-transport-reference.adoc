...
* WMQ Transport supports the WMQ 7 Client Driver, which can be used to connect to WMQ Server versions 7.0, 7.1, 7.5, 8.0, and 9.0.
* WMQ Transport does not support WMQ 8 or 9 Client Drivers, as these drivers use com.ibm.mq.jms.JMSC interface that was deprecated after WMQ 7.0.
* This connector supports Mule 3.8 and later with IBM MQ 8 and 9, using the WMQ 7 driver. This connector was not tested on Mule 3.7 using IBM MQ 8 or 9. If you use the WMQ 7 driver in Mule 3.7, it may work, but this configuration is not supported. 
