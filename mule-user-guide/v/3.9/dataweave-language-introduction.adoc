...
The link:/mule-user-guide/v/3.9/dataweave[DataWeave] Language is a powerful template engine that allows you to transform data to and from any kind of format (XML, CSV, JSON, Pojos, Maps, etc). In Anypoint Studio, this language can be used in:
...
* Any Mule component that accepts Mule Expression Language, through link:/mule-user-guide/v/3.9/mel-dataweave-functions[MEL DataWeave Functions]
...
* link:/mule-user-guide/v/3.9/dataweave-formats#java[`application/java`]
* link:/mule-user-guide/v/3.9/dataweave-formats#csv[`application/csv`]
* link:/mule-user-guide/v/3.9/dataweave-formats#csv[`text/csv`]
* link:/mule-user-guide/v/3.9/dataweave-formats#json[`application/json`]
* link:/mule-user-guide/v/3.9/dataweave-formats#json[`text/json`]
* link:/mule-user-guide/v/3.9/dataweave-formats#xml[`application/xml`]
* link:/mule-user-guide/v/3.9/dataweave-formats#xml[`text/xml`]
* link:/mule-user-guide/v/3.9/dataweave-formats#flat-file[`text/plain`]
* `application/dw`
...
You can define a link:/mule-user-guide/v/3.9/dataweave-types#functions-and-lambdas[function] in the header, you can then call it in any part of the DataWeave body, including arguments.
...
See link:/mule-user-guide/v/3.9/dataweave-types#functions-and-lambdas[Functions and Lambdas] for more on this.
...
* link:/mule-user-guide/v/3.9/dataweave-types#string[*String*] : Double quoted ("Hello") or Single quoted ('Hello')
* link:/mule-user-guide/v/3.9/dataweave-types#boolean[*Boolean*] : Literals true or false
* link:/mule-user-guide/v/3.9/dataweave-types#number[*Number*] : Decimal and Integer values are supported (ex: 2.0)
* link:/mule-user-guide/v/3.9/dataweave-types#dates[*Date*] : IS0-8601 enclosed by "|" (ex:|2003-10-01T23:57:59Z|)
* link:/mule-user-guide/v/3.9/dataweave-types#regular-expressions[*Regex*] : Regex expression enclosed by "/" (ex:/(\d+)-(\d+)/)
...
See link:/mule-user-guide/v/3.9/dataweave-types#array[DataWeave types] for more details on arrays.
...
See link:/mule-user-guide/v/3.9/dataweave-types#object[DataWeave types] for more details on objects.
...
Operators can be classified based on their link:https://en.wikipedia.org/wiki/Arity[arity] as Unary, Binary or Ternary. See link:/mule-user-guide/v/3.9/dataweave-operators[DataWeave Operators] for a full reference.
...
A selector allows for the navigation and querying the multiple levels of a data-structure to reference a certain value or set of values. See link:/mule-user-guide/v/3.9/dataweave-selectors[DataWeave Selectors] for a full reference.
...
[TIP]
For simpler use cases where all you need is a boolean result based on if a value matches or not, see the link:/mule-user-guide/v/3.9/dataweave-operators#matches[Matches Operator].
...
In this example, the first field evaluates the type of 'payload.a' and returns a different string with the type name depending on what type it matches with (this could be easier done through the link:/mule-user-guide/v/3.9/dataweave-operators#type-of[Type Of operator]), the second field returns an object with the same type name as a string and a reference to the evaluated value.
...
In this example, the payload includes two elements in an array, and in both cases the link:/mule-user-guide/v/3.9/dataweave-operators#map[Map operator] to cycle through the array. It then evaluates each element against a regular expression and outputs a different object depending on what kind of match is found.
...
Returns the present moment in link:/mule-user-guide/v/3.9/dataweave-types#dates[(:datetime)] type.
...
[TIP]
See link:/mule-user-guide/v/3.9/dataweave-selectors[DataWeave Selectors] for a list of possible selectors to use here.


=== Random

Returns a random number of type link:/mule-user-guide/v/3.9/dataweave-types#number[(:number)] between 0 and 1
...
[NOTE]
The `lookup` function does not support calling link:/mule-user-guide/v/3.9/flows-and-subflows#types-of-flows[subflows].

== Calling Global MEL Functions from DataWeave Code

If you define a global link:/mule-user-guide/v/3.9/mule-expression-language-mel[Mule Expression Language] (MEL) function in your Mule project, you can then invoke it anywhere in your DataWeave code, without need for any special syntax.
...
|link:/mule-user-guide/v/3.9/dataweave-operators#AND[AND]                              |Conditional And         | 7
|link:/mule-user-guide/v/3.9/dataweave-operators#OR[OR]                                |Conditional OR          | 8
|link:/mule-user-guide/v/3.9/dataweave-operators#default[Default], <<Pattern Matching>>, link:/mule-user-guide/v/3.9/dataweave-operators#matches[Matches], link:/mule-user-guide/v/3.9/dataweave-operators#map[Map], link:/mule-user-guide/v/3.9/dataweave-operators#map-object[Map Object], link:/mule-user-guide/v/3.9/dataweave-operators#group-by[Group By], link:/mule-user-guide/v/3.9/dataweave-operators#filter[Filter]             |Default Value / Pattern Matching / Binary Operators | 9
...
* For a reference about all of the operators that are available for the `using` operator, see link:/mule-user-guide/v/3.9/dataweave-operators[DataWeave Operators]
* For a listing and details about all of the types you can use, see link:/mule-user-guide/v/3.9/dataweave-types[DataWeave Types]
* For details on the different formats you can process with DataWeave and the parameters you can configure for each format, see link:/mule-user-guide/v/3.9/dataweave-formats[DataWeave Formats]
* For details on how you can select certain components of the incoming message, see link:/mule-user-guide/v/3.9/dataweave-selectors[DataWeave Selectors]
