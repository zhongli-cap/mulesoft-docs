...
This module enables Mule to use the JBoss transaction manager (previously Arjuna) to configure link:/mule-user-guide/v/3.9/xa-transactions[XA transactions]. Developers can configure one Transaction Manger per Mule instance. For more information, see link:http://www.jboss.org/jbosstm/[JBoss Transactions], link:/mule-user-guide/v/3.9/deploying-mule-to-jboss[Deploying Mule to JBoss], link:/mule-user-guide/v/3.9/transaction-management[Transaction Management].
