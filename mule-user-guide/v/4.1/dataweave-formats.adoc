...
You can append reader properties to the mime type (`outputMimeType`) attribute for components in your Mule app. Listeners and Read operations accept these settings. For example, this On New File listener example identifies the `,` separator for a CSV input file:
