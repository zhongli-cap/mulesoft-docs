...
A typical example of a message source is an link:/mule-user-guide/v/3.9/anypoint-connectors[*Anypoint Connector*]. 
...
Under certain conditions, flows do not need to be triggered by message sources. For instance, a link:/mule-user-guide/v/3.9/flow-reference-component-reference[Flow Reference Component] can trigger a different flow in the same application. Similarly, the link:/mule-user-guide/v/3.9/async-scope-reference[Async Scope] can trigger a another flow that executes asynchronously, (that is, in parallel with the originating flow).
...
For further details, see link:/mule-user-guide/v/3.9/flow-processing-strategies[Flow Processing Strategies].
...
Studio provides four pre-packaged error handling strategies to handle exceptions thrown at various points during the message processing sequence. For details, see link:/mule-user-guide/v/3.9/error-handling[Error Handling].
...
* Call various other link:/mule-user-guide/v/3.9/flows-and-subflows[flows]

== See Also

* *NEXT STEP:* Learn about how link:/mule-user-guide/v/3.9/flows-and-subflows[flows and subflows] work together in an application.
* Examine an link:/mule-user-guide/v/3.9/flow-architecture-advanced-use-case[advanced use case] showing a more complex flow architecture.
