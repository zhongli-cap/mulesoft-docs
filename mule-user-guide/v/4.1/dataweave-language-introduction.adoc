...
You can use the `${file::filename}` syntax for any XML tag that expects an expression. For example, see the `when expression="${file::logic.dwl}"` in the Choice router here:
