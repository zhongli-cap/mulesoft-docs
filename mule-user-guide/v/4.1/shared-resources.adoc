...
To associate an existing app with a domain (Anypoint Studio):

. In the Project Explorer view, double click on the Mule app *Properties* > *Mule Project*. Notice a *Domain* field, which by default is set to `default`. This value gives each project its independent domain. Change this value. The dropdown list on this field displays all of the domains you've created in the current workspace.
...
[NOTE]
====
If you are creating your apps outside Studio, you need to include the domain property in the `mule-artifact.json` file to associate an existing app:
