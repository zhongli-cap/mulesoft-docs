...
This page assumes that you are familiar with link:/mule-fundamentals/v/3.6/mule-concepts[Mule flows] and basic link:/mule-fundamentals/v/3.6/mule-application-architecture[application architecture].
...
|*Queued-Asynchronous* |*Yes.* You can override an implicitly selected queued-asynchronous processing strategy by explicitly declaring a synchronous processing strategy (or, in rare use cases, a different kind of asynchronous processing strategy) instead. <<Specifying a Processing Strategy>>. |*Yes.* You can fine-tune the queued-asynchronous processing strategy to meet your needs. <<Fine-Tuning a Queued-Asynchronous Processing Strategy>>. |*Yes.* A synchronous link:/mule-fundamentals/v/3.6/flows-and-subflows[flow or subflow] processes a selected block of message processors synchronously, regardless of the processing strategy of the main flow. |*Yes.* <<Creating a Custom Processing Strategy>>.
|===
...
. create an link:/mule-fundamentals/v/3.6/flows-and-subflows[asynchronous flow] by wrapping a flow-ref element in an async scope so that the contents of the flow will be processed asynchronously with the triggering flow
...
* Review the different kinds of link:/mule-fundamentals/v/3.6/flows-and-subflows[Flows and Subflows] in Mule.
