...
[TIP]
Instead of building a proxy application from scratch, consider link:https://docs.mulesoft.com/api-manager/setting-up-an-api-proxy[Setting Up and Deploying a Proxy].

== Deciding How to Proxy Your Web Service

While many times you can proxy web services without using CXF (see link:https://docs.mulesoft.com/mule-user-guide/v/3.6/proxying-web-services[Proxying Web Services]), there are several cases where you might want to use CXF proxies:
...
[TIP]
To send outbound requests to external web services, it's highly recommended that you use the link:https://docs.mulesoft.com/mule-user-guide/v/3.6/web-service-consumer[Web Service Consumer] instead.
