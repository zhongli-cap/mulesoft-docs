...
In its most basic form, an endpoint acts as a flow-level inbound or outbound implementation of a link:https://docs.mulesoft.com/mule-user-guide/v/3.6/connecting-using-transports[transport] and its underlying data source or message channel. For example:
...
An outbound endpoint can also be dynamic. This means that the endpoint's URI is the value of an link:https://docs.mulesoft.com/mule-user-guide/v/3.6/mule-expression-language-mel[expression], which is evaluated just before a message is sent to it. This allows the target of a message to be determined by its contents or the value of any message property. Dynamic endpoints can use either of the endpoint formats shown above.
...
link:https://docs.mulesoft.com/mule-user-guide/v/3.6/using-transformers[Transformers] can be configured on an endpoint encapsulating transformation logic in an endpoint that can then be reused as required.
...
The following reference tables list the attributes that can be configured for the link:https://docs.mulesoft.com/mule-user-guide/v/3.6/generic-connector[generic endpoint] in Mule. 
