...
In its most basic form, an endpoint acts as a flow-level inbound or outbound implementation of a link:/mule-user-guide/v/3.6/connecting-using-transports[transport] and its underlying data source or message channel. For example:
...
The following reference tables list the attributes that can be configured for the link:/mule-user-guide/v/3.6/generic-connector[generic endpoint] in Mule. 
