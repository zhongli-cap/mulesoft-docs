...
* link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-listener-connector[Listen for HTTP requests] that arrive at a certain address and provide an HTTP response to these
* link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-request-connector[Send HTTP requests] to a certain address and receive the returned response

Through additional configuration, the connector allows you to:

* Use link:https://docs.mulesoft.com/mule-user-guide/v/3.6/tls-configuration[TLS] encryption to send or receive *HTTPS* requests 
* Send link:https://docs.mulesoft.com/mule-user-guide/v/3.6/authentication-in-http-requests[Authenticated Requests], through *Basic Authentication, Digest and OAuth*
...
* As an link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-listener-connector[HTTP Listener Connector]
* As an link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-request-connector[HTTP Request Connector]
...
For information about migrating a project that uses instances of the old HTTP endpoint-based Connector to the new HTTP operation-based Connector, see link:https://docs.mulesoft.com/mule-user-guide/v/3.6/migrating-to-the-new-http-connector[Migrating to the New HTTP Connector]. The old HTTP endpoint-based Connector still does work with Mule 3.6 Runtime, but it's deprecated and will be eventually removed.

*Tip*: You can edit the log4j2 configuration file to make the logging of the HTTP connector's activity verbose. See link:https://docs.mulesoft.com/mule-user-guide/v/3.6/logging-in-mule[Logging in Mule] for instructions.

== Debugging

Gaining greater visibility into HTTP inbound and outbound behavior can be achieved by enabling underlying library loggers with log4j2. This section assumes you're comfortable adjusting log levels with log4j2. If you have not adjusted logging levels in the past, read link:https://docs.mulesoft.com/mule-user-guide/v/3.7/logging-in-mule#configuring-custom-logging-settings[configuring custom logging settings] before continuing.
