...
[NOTE]
Note that high performance is implemented differently on link:https://docs.mulesoft.com/runtime-manager/cloudhub[CloudHub], so this section applies only for on-premises deployments.
...
* If your application processes messages from a non-transactional transport, use a link:https://docs.mulesoft.com/mule-user-guide/v/3.6/reliability-patterns[reliability pattern] to move them to a transactional store such as a VM or JMS store.
...
* Implement link:https://docs.mulesoft.com/mule-user-guide/v/3.6/reliability-patterns[reliability patterns] to create high reliability applications.
...
* If your link:https://docs.mulesoft.com/mule-user-guide/v/3.6/endpoint-configuration-reference[custom message source] does not use a message receiver to define node http://en.wikipedia.org/wiki/Polling_(computer_science)[polling], then you must configure your message source to implement a ClusterizableMessageSource interface. +
...
* link:https://docs.mulesoft.com/mule-user-guide/v/3.6/installing-an-enterprise-license[Install an Enterprise License] to begin using the Mule Management Console to manage clusters in production.
