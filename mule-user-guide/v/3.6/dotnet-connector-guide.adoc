...
This document assumes that you are familiar with Mule, the link:/mule-user-guide/v/3.6/anypoint-studio-essentials[Anypoint Studio interface], and link:/mule-user-guide/v/3.6/global-elements[Global Elements]. Further, it is assumed that you are familiar with the Microsoft .NET Framework.
