...
In Studio, to instance the connector as an HTTP Listener Connector (and not an link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-request-connector[HTTP Request Connector]), you must place it into the *Source* section of a flow (ie: as the first element in the flow):
...
By default, the connector configuration has its host set to `0.0.0.0`, this address is in fact a shortcut to simultaneously listen on all active IP addresses, (including `localhost`). In order to deploy a project to link:https://docs.mulesoft.com/runtime-manager/cloudhub[CloudHub], you must set the hosts of all the HTTP connectors to `0.0.0.0`. Otherwise, your application will run into conflicts with the inner workings of CloudHub
...
When using an HTTP Listener Connector, whenever there are outbound properties included in the Mule Message that arrives to the end of the flow, each outbound property is transformed into an HTTP response header. It's easy to add properies to the Mule Message by using the link:https://docs.mulesoft.com/mule-user-guide/v/3.6/property-transformer-reference[Property Transformer].
...
In the example above, two headers will be added to every response to a request to the app. Both headers take their values from link:https://docs.mulesoft.com/mule-user-guide/v/3.6/mule-expression-language-date-and-time-functions[dateTime MEL expressions].
...
. Set the variable's name to `headersOut` and its value to a map through the following link:https://docs.mulesoft.com/mule-user-guide/v/3.6/mule-expression-language-mel[MEL expression]:  `#[['date': server.dateTime, 'expires' : server.dateTime.plusHours(1)]]`
...
See link:https://docs.mulesoft.com/mule-user-guide/v/3.6/tls-configuration[TLS Configuration] for more details.
...
See link:https://docs.mulesoft.com/mule-user-guide/v/3.6/configuring-the-spring-security-manager[Configuring the Spring Security Manager] for more details.
...
* To send HTTP requests, see  link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-request-connector[HTTP Request Connector]
...
* See a link:https://docs.mulesoft.com/mule-user-guide/v/3.7/http-connector-reference[full Reference] of the available XML configurable options in this connector
* link:https://docs.mulesoft.com/mule-user-guide/v/3.6/authentication-in-http-requests[Authentication in HTTP Requests] +
* To update allplications that use the old HTTP endpoint based connector, see link:https://docs.mulesoft.com/mule-user-guide/v/3.6/migrating-to-the-new-http-connector[Migrating to the New HTTP Connector]
* Consult a reference to the deprecated predecessor of this element,  link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-transport-reference[HTTP Transport Reference]
