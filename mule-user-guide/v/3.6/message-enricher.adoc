...
You can make the source a namespace by simply populating the field with a link:https://docs.mulesoft.com/mule-user-guide/v/3.6/mule-expression-language-mel[Mule Expression Language MEL] expression, for example with an xpath expression that takes arguments from the Mule message.

You can find information on using a variable with any enricher listed on these pages:

* link:https://docs.mulesoft.com/mule-user-guide/v/3.6/non-mel-expressions-configuration-reference[Default enrichers] are loaded at runtime
...
* API documents for http://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/enricher/MessageEnricher.html[org.mule.core.enricher.MessageEnricher]
