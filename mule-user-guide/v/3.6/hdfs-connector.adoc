...
To use the HDFS connector in your Mule application, you must configure a global HDFS element that can be used by all the HDFS connectors in the application (read more about  link:/mule-fundamentals/v/3.6/global-elements[global elements] ).
...
* Access HDFS connectorlink:/release-notes/hdfs-connector-release-notes[release notes].
