...
In Studio, to instance this connector as an HTTP Request Connector (and not an link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-listener-connector[HTTP Listener Connector]), you must place it into the *Process* section of a flow (ie: anywhere except the beginning).
...
|Enable Datasense |The *Enable DataSense* checkbox allows Studio to use link:https://docs.mulesoft.com/mule-user-guide/v/3.6/datasense[DataSense] to expose metadata from the RAML file and interact with it on other elements in Studio.
|===
...
For this example, you must first link:https://docs.mulesoft.com/mule-user-guide/v/3.6/variable-transformer-reference[generate a variable] named `customMap`. If you assign that variable a value through the following MEL expression:
...
For this example you need to link:https://docs.mulesoft.com/mule-user-guide/v/3.6/variable-transformer-reference[create a variable] named `customMap`. If you set that variable to the MEL expression `#[{'p1':'v1', 'p2':'v2'}]`, it will generate a GET request to ++http://www.example.com/test+/v1/v2+
...
An HTTP response is mapped to the Mule Message in exactly the same way that the HTTP request is mapped to a Mule Message in the link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-listener-connector[HTTP Listener Connector], except that the following elements don't apply to HTTP responses:
...
* Consult a reference to the deprecated predecessor of this element, the HTTP endpoint link:https://docs.mulesoft.com/mule-user-guide/v/3.6/http-request-connector[HTTP Transport Reference]
