...
[WARNING]
====
If you deploy multiple applications through a link:https://docs.mulesoft.com/mule-user-guide/v/3.6/shared-resources[Shared Resources] structure, don't set anything in the properties files, as there might potentially be conflicts between the various apps that share a domain. Instead, set environment variables over the scope of the deployed app, its domain, and other apps under that domain.

As explained in link:https://docs.mulesoft.com/mule-user-guide/v/3.6/shared-resources[Shared Resources], in Studio you can create these variables through the *Environment tab* of the *Run Configurations menu*, reachable via the drop-down menu next to the Play button.
====
...
For information on configuration builders, see link:https://docs.mulesoft.com/mule-user-guide/v/3.6/about-configuration-builders[About Configuration Builders].
...
See link:https://docs.mulesoft.com/runtime-manager/managing-applications-on-cloudhub[Managing Applications on CloudHub] and link:https://docs.mulesoft.com/runtime-manager/secure-application-properties[Secure Application Properties] for more details.
