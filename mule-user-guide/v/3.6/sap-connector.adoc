...
Mule supports SAP integration through our link:/mule\-user\-guide/v/3\.6/anypoint-connectors[Anypoint Connector] for SAP, which is an SAP-certified Java connector that leverages the SAP Java Connector (JCo) libraries.  The SAP connector enables Mule applications to:
...
If you intend to employ an SAP endpoint in conjunction with a link:https://docs.mulesoft.com/anypoint-studio/v/6/datamapper-user-guide-and-reference[DataMapper transformer] to map and transform data, you can make use of Anypoint Studio's link:/mule\-user\-guide/v/3\.6/datasense[DataSense] functionality.
...
Port 3300 is predefined by SAP, so if you need to validate other port numbers based on your SAP instance number, you can check the complete list of link:/mule\-user\-guide/v/3\.6/sap-jco-server-services-configuration[service-to-port mappings].
...
In this case, you must know the configuration property _as defined by SAP_. You can check link:/mule\-user\-guide/v/3\.6/sap-jco-extended-properties[the configuration properties list].
...
|*Extended Properties* |jcoClientExtendedProperties-ref |Advanced |A Reference to `java.util.Map`, which contains additional JCo connection parameters. As necessary, consult the link:/mule\-user\-guide/v/3\.6/sap-jco-extended-properties[complete list of parameters]. | 
|===
