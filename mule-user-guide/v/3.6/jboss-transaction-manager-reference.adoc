...
This module enables Mule to use the JBoss transaction manager (previously Arjuna) to configure link:/mule\-user\-guide/v/3\.6/xa-transactions[XA transactions]. Developers can configure one Transaction Manger per Mule instance. For more information, see http://www.jboss.org/jbosstm/[JBoss Transactions].
