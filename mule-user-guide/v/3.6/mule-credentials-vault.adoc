...
[NOTE]
This topic introduces the idea of Global Elements; if you are unfamiliar with this functionality, access link:/mule-fundamentals/v/3.6/global-elements[Understand Global Mule Elements] to learn more before proceeding.
...
It is recommended to never store the secret key value to disk in any file. Instead, a Operations Adminstrator should manually enter these secret properties to the command line command when starting the Mule runtime into which the application will be deployed.
...
In CloudHub, the Operations Administrator can enter each environment's name/value pair into the Propeties tab of the application's deployment configuration.
