...
Developing in Maven is greatly simplified by the link:https://docs.mulesoft.com/mule-user-guide/v/3.6/maven-tools-for-mule-esb[Maven Tools for Mule ESB] plugin, which provides Maven archetypes for building Mule applications.
...
This document assumes that you are working with Maven outside of Anypoint Studio, and thus will be using archetypes to build Mule applications. See link:https://docs.mulesoft.com/mule-user-guide/v/3.6/using-maven-with-mule[Using Maven with Mule] for an overview of using Maven to manage your Mule projects.

[NOTE]
====
Note that you generally do not need to modify your settings.xml file if you are link:https://docs.mulesoft.com/mule-user-guide/v/3.6/using-maven-with-mule[working with Maven through Studio], as Studio keeps track of the dependencies you'll need and attempts to manage them for you in the application's associated POM file. If dependency errors occur, you may need to manually adjust your POM file, and, in some cases, in your settings.xml. Refer to the Maven documentation for the http://maven.apache.org/pom.html[pom.xml] and http://maven.apache.org/settings.html[settings.xml] for specific instructions.
====
...
* Learn about link:https://docs.mulesoft.com/mule-user-guide/v/3.6/maven-tools-for-mule-esb[Maven tools for Mule], a Mule plugin to develop Mule applications in Maven.

* Control instances of Mule using the link:https://docs.mulesoft.com/mule-user-guide/v/3.6/mule-esb-plugin-for-maven[Mule plugin for Maven].
