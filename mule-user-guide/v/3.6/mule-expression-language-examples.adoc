...
This document assumes you are familiar with Mule Expression Language's link:/mule-user-guide/v/3.6/mule-expression-language-basic-syntax[basic syntax] and are comfortable building and running Mule applications using link:/mule-user-guide/v/3.6/anypoint-studio-essentials[Mule Studio] or XML.

If you aren't yet familiar with how to access information about the Mule messages that pass through your applications, consider following this link:/mule-user-guide/v/3.6/mule-message-tutorial[tutorial], which walks you through both examining your Mule message and its data structure and writing simple MEL expressions.
...
In this example, like in the previous one, the Mule application saves a CSV file with user data and returns a greeting. The call to the service includes two parameters, `username` and `age`. The service stores these two parameters and adds a third boolean parameter that evaluates if the user is above a certain age (if `age ``> 18`). In this case, however, the mapping between input and output fields and the generation of the third field is performed by a link:/mule-user-guide/v/3.6/datamapper-user-guide-and-reference[DataMapper] component.
...
[TIP]
For your convenience, you may download the link:/documentation/download/attachments/122752119/mel_example_project.zip?version=1&modificationDate=1397239701353[complete project]. Note that this project is configured specifically for the **Mule 3.6.0 runtime**. Please refer to the 3.5.X documentation for a 3.5.X compatible version.
...
* Want to learn more about how to get information about the Mule message so that you can work with it using MEL? Try out this link:/mule-user-guide/v/3.6/mule-message-tutorial[tutorial]. 
