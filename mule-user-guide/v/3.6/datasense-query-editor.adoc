...
This document assumes that you are familiar with the link:/anypoint-studio/v/6/index[Visual Editor in Anypoint Studio], as well as the http://www.mulesoft.org/documentation/display/current/Anypoint+Connectors[Anypoint Connectors], in particular the Salesforce cloud connector used in the example queries. This document also references link:/mule-user-guide/v/3.6/anypoint-connectors[Connectors], link:/mule-user-guide/v/3.6/global-elements[Global Elements], and link:/mule-user-guide/v/3.6/datasense[DataSense]. To learn more about developing with Mule ESB's graphical user interface, review link:/anypoint-studio/v/6/index[Getting Started with Anypoint Studio].
