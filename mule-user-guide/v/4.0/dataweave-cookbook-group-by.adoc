...
The examples take input that is grouped under one field and transform it into a new structure that groups data under another field.

Both examples use these operators:

* `groupBy` to organize the fields by `subject`
* `mapyObject` and `map` to map the fields from the input to the new hierarchy.
