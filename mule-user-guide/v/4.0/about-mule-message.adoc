...
The Mule message is the data that passes through flows inside your Mule app.
...
Attributes are in the message header of the Mule event. They provide process-related information about the message. Examples of attributes include standard file metadata and HTTP headers.
...
** `attributes.fileName`: For the size of a file, such as `1079.`
* HTTP header examples:
** `attributes.headers.statusCode`: For the HTTP status code, such as `200`.
** `attributes.headers."content-type"`: For the HTTP content type, such as `application/json; charset=utf-8`.

Note that the quotation marks in `headers."content-type"` are necessary to avoid a parsing error. The only valid identifiers for attribute keys are numbers, characters and underscores. Without the quotation marks, the name is parsed like this `attributes.headers.content - type`.
