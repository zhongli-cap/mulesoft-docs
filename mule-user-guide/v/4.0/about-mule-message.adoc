...
Attributes are stored in the Mule message header. Depending on the connector source or operation that requests or handles them  (such as the HTTP Listener, JMS Listener, or Read operation to a connector), attributes can be standard HTTP headers, JMS properties, file metadata, or some other sort of metadata. In the case of HTTP, the attributes are an instance of `HttpAttributes`.
