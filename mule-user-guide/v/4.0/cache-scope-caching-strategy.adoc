...
* Cache "miss": If there is no cached response event (a cache “miss”), the Cache scope processes the message.
* Cache "hit": If there is a cached response event (a cache “hit”), the Cache scope offers the cached response event rather than processing the message.
...
The default strategy should only be used for testing. It uses an `InMemoryObjectStore` (a container for cached data).

. A message enters the Cache scope.
. Cache scope determines whether the message’s payload is consumable.
+
A consumable payload (such as a streaming payload) can only be read once before it is lost and cannot be cached.
+
* If the message payload is consumable, the Cache scope processes the message. Nothing is cached, and the caching strategy is abandoned.
* If the message payload is not consumable, the Cache scope continues to the next step in the caching strategy.
...
+
. The Cache scope compares the newly generated key to cached responses that it has previously processed and saved in an `InMemoryObjectStore`.
+
In other words, the Cache scope searches for a _cache hit_ that it can offer instead of processing the message.
+
...
== About Object Stores and Clustering
...
This changed in 3.5.0 to the current caching strategy.
////

With the exception of the default caching strategy, caching strategies use an ObjectStore instance to provide a unified cache for all cluster nodes without extra modifications.

Mule provides the `MuleObjectStoreManager` for create object stores.

Note that you should not use the `TextFileObjectStore` (from `apidocs.org.mule.util.store`).

If you want to provide a cache for each cluster node, you need to configure your application to use a managed store that controls an in-memory store. The configuration for the managed store should be:
...
. In your Mule app, add Cache scope to a flow. 
+
...
By default, Mule stores all cached responses in an InMemoryObjectStore. xref:ccs[Create a Caching Strategy] and define a new link:/mule-user-guide/v/3.9/mule-object-stores[object store] if you want to customize the way Mule stores cached responses.
...
. Executes, and returns the answer to, the Fibonacci equation (see below) using a number (_n_) provided by the caller as a request header. The images below show it as a query parameter, but it should be passed as a request header. Use a tool like link:https://www.getpostman.com/[Postman] to pass it the following equation text:  +
...
// link:/connectors/v/latest/object-store-module-reference[Object Store Module Reference]

// link:https://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/util/store/package-summary.html[mule.util.store]
