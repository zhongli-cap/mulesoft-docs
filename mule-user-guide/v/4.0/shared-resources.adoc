...
|*pom.xml* |This is the dependencies file descriptor. It's just a maven pom file with requires plugins for domains projects.
...
+
Provide a name for the project and select a runtime. Optionally create a .gitignore file if you plan to share this project via git or github.


=== Defining Shared Resources

You can configure the domain project that you just created in the previous step by defining shared resources in the the  `mule-domain-config.xml` file. You can define multiple resources in this configuration file. In Anypoint Studio, there are two different views for editing this file:

* Configuration file: This view should be the most familiar for those used to the XML editor in regular Studio Mule projects
