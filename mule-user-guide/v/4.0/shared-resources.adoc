...
* Ensure consistency between applications upon any changes because the configuration is only set in one place.

== Prerequisites

This document assumes that you are using Anypoint Studio with Mule runtime 4.0 or newer, or that you are building your applications using Studio and deploying them to Mule Standalone *4.0* runtime or newer. 
...
|`pom.xml` |This is the dependencies file descriptor. It's just a Maven POM file with required plugins for domains projects.
|`/mule-artifact.json` |This file is a descriptor for the domain artifact.
|===

=== Domain Artifact

The following files are the ones that you can find when you package a domain into a jar file:
...
+
Provide a name for the project, and select a runtime. Optionally, create a `.gitignore` file if you plan to share this project through git or GitHub.


=== Defining Shared Resources

You can configure the domain project that you just created in the previous step by defining shared resources in the `mule-domain-config.xml` file. You can define multiple resources in this configuration file. In Anypoint Studio, there are two different views for editing this file:

* Configuration file: This view should be the most familiar for those accustomed to the XML editor in regular Studio Mule projects.
* Global Elements: This view provides a graphical interface for defining shared resources.

You can add modules/connectors dependencies to you domain project so you can use them as shared resources.
...
In Anypoint Studio, to associate an existing application with a domain:

. In the project explorer view, double click on the Mule application *Properties* > *Mule Project*. Notice a *Domain* field, which by default is set to `default`. This value gives each project its independent domain. Change this value. The dropdown list on this field displays all of the domains you've created in the current workspace.

image:assign+domain.png[assign+domain]

Note that when picking a domain, the Server Runtime of your project automatically becomes that of the domain because these must always match.

[NOTE]
====
If you're creating your applications outside Studio, then to associate an existing application with a domain, edit the `mule-artifact.json` file to include the domain property:
...
Share link:/mule-user-guide/v/3.9/tls-configuration[TLS configurations] allows shared <<HTTP,listener configurations>> to use the same TLS configuration. +
...
* Note that although shared resources is limited to the selected connectors and libraries covered in this document, there are ways to share other configuration fragments in Mule. Refer to link:http://docs.mulesoft.com/mule-user-guide/v/3.9/sharing-custom-configuration-fragments[Sharing Custom Configuration Fragments] for details.

== See Also

Access reference documentation for:

* link:http://docs.mulesoft.com/mule-user-guide/v/3.9/vm-transport-reference[VM]
* link:http://docs.mulesoft.com/mule-user-guide/v/3.9/database-connector[DB]
* link:http://docs.mulesoft.com/mule-user-guide/v/3.9/jms-transport-reference[JMS]
* link:http://docs.mulesoft.com/mule-user-guide/v/3.9/http-connector[HTTP Connector]
* link:http://docs.mulesoft.com/mule-user-guide/v/3.9/mule-wmq-transport-reference[WMQ]
* link:http://docs.mulesoft.com/mule-user-guide/v/3.9/jboss-transaction-manager-reference[JBoss]
* link:http://docs.mulesoft.com/mule-user-guide/v/3.9/setting-environment-variables[Set Environment Variables]
