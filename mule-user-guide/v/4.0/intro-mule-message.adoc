...
== Attachments
Inbound and outbound attachments have been removed. Affected connectors now handle
the attachment concept on their own:

* HTTP now relies on DataWeave's multipart format support.
* Email and Web Service Consumer allow attachments to be explicitly added using
DataWeave.
