= Example: Merging Multiple Inputs
...
payload[0]  map (bookPrice) -> using (id = bookPrice.bookId) {
	bookId:	id,
    title:  bookPrice.title,
	price: 	bookPrice.price as Number,
	(payload[1] filter ($.*bookId contains id)  map (authorByBook) -> {
		author:authorByBook.author
   })
}
----
