...
* Data selectors that access fields in a input data structure. The script selects keys in a JSON object from the message payload input (for example, with `payload.title`) and then outputs their values.
...
You write DataWeave expressions and scripts within message processors (components, connectors, or modules) when creating apps in Anypoint Studio, Design Center, or as handwritten XML configuration files.
