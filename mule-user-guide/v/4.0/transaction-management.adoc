...
In this example the try scope demarcates a transaction including the DB operation and the VM operation. Only both will be applied or none if there's an error raised within the try scope.
...
It's also possible to start a transaction from a message source. In that case the transaction will spawn the whole flow. This is mostly useful when working with messaging connectors to prevent the message consuption if there was a problem processing it so we can later retry.
...
Specific transaction configuration for connectors can be found in the connector documentation pages.
