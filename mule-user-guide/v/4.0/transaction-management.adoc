...
Specific transaction configuration for connectors can be found in the connector documentation.

For JMS Ack:

* MANUAL: This is JMS `{@link Session#CLIENT_ACKNOWLEDGE}` mode. You must configure the Ack manually within the flow.
* DUPS_OK: JMS message is Acked automatically but in a lazy fashion that might lead to duplicates.
