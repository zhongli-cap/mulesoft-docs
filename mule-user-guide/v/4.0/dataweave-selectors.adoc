...
%output application/json
---
payload.items.[variables.item]
----

The above example receives a payload that contains a list of items, and a variable that indicates which of these items to take from the payload.
...
link:/mule-user-guide/v/4.0/mule-expression-language-mel[Mule Expression Language (MEL)] expression.
...
You can take any *Outbound Properties* in the Mule message that arrives to the DataWeave transformer and use it in your transform body. To refer to it, simply call it through the matching link:/mule-user-guide/v/4.0/mule-expression-language-mel[Mule Expression Language (MEL)] expression.
