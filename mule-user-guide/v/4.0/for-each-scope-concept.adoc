...
* For non-Java input, such as XML or JSON, you need to use a DateWeave expression to split data. You use the Collection (`collection`) field for this purpose. Here, the field is set to iterate over an array stored in `payload.topics`.
+
