...
This DataWeave script takes the entire payload of the JSON input shown above and transforms it to XML.
...
A good mental model for approaching the transformation script is to normalize the input to the JSON-like <<dataweave-formats#format_dataweave, `application/dw`>> format. This can help you determine whether an error is a formatting or a scripting error. So, for example, you might transform your `application/json` input to `application/dw` if you get an error when trying to transform it to XML (`application/xml`).
