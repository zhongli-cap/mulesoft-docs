...
As mentioned above, there is now a single thread pool for all flows in an application. This decouples overall tuning from flow-specific behavior. Mule allocates threads in proportion to how many CPU cores your machine has. You can tune this thread count by using the self-documented `conf/scheduler-pools.conf` file.
