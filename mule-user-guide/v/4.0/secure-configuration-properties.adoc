...
You can define more than one configuration file to read properties from. To do so, just define a `<secure-properties:config />` tag for each file you want to load. They do not need to have the same algorithm, mode or key to be decrypted.
