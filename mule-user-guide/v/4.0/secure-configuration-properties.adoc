...
You can link:_attachments/somejarfile.jar[download the jar file] for this tool that will allow you to encrypt or decrypt single values, and also complete files (both, YAML and Properties files). You can run it in the command line as:
...
In the case of using the `file` mode, the output will be a file with the same properties, but it's values encrypted. For example, if the input file is: 
...
Running the script: `java -jar secure-properties-tool.jar file encrypt Blowfish CBC mulesoft example_in.yaml example_out.yaml`, the output file will be: 
