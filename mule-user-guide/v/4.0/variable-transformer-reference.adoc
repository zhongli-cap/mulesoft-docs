...
The Variable component can set or remove a variable on the Mule event within the the flow where you set it. You access the variable through the DataWeave identifier `vars`. For example, access an integer `myInt` using `#[vars.myInt]`.
