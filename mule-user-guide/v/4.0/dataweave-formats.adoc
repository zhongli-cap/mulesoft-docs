...
01  MAILING-RECORD.
    ...
    05  ZIP                     PIC 9(5).
----
Note that copybook schemas must always begin with a `01` entry.

When you import the schema, the Transform component converts the schema to a flat file that it stores in the `src/main/resources/schema` folder of your Mule project. In flat file format, the copybook schema above looks like this:
