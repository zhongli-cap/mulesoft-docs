= Formats that are Supported by DataWeave
...
// TODO: IS :enum correct? Or is it now Enum?
=== :enum
...
// TODO IS THE INFO BELOW ACCURATE?
...
// TODO: mentioned as a Beta feature... IS THIS ACCURATE AND WHAT WE WANT TO DO?
Excel support is a beta feature for Mule Runtime 4.0.2. It is not supported on earlier versions of the runtime. Only `.xlsx` files are supported (Excel 2007), but `.xls` files are not supported on any Mule runtime version.
...
Or via the UI of the Transform component:
...
JSON data structures are mapped to DataWeave data structures. They share a lot of similarities.
...
* link:/mule-user-guide/v/4.0/dataweave-flat-file-schemas[DataWeave Flat File or EDI Schemas]
