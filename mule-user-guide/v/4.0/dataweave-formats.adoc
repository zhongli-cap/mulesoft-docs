...
DataWeave can read and write many types of data formats. DataWeave can also read and write format-specific properties, such as a path to a schema file (the `schemaPath`) for a Cobol copybook or, for CSV input and output, `separator` (delimiter) or `encoding` properties.
