...
The examples here normalize the date separators (`/`, `.`, and `-`) between different date formats so that all of them use the same separator (`-`). Both examples use a function that is defined in the DataWeave header.
...
.DataWeave (returns dates as String types)
...
.DataWeave (returns dates as Date types)
[source,Dataweave,linenums]
----
%dw 1.0
%output text/xml
...
link:/mule-user-guide/v/4.0/dataweave-types#functions-and-lambdas[Functions and Lambdas]
