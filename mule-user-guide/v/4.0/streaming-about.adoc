...
Mule 4.0 introduces Repeatable Streams. Since each component that consumes a stream expects to receive a new stream, as a component consumes the stream, Mule saves its content into a temporary buffer. The runtime then feeds the component from the temporary buffer, ensuring that each component receives the full stream, regardless of how much of the stream was already consumed by any prior component. +
