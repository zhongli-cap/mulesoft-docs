...
Note that the `tagStart` parameter is required when using flat file structures. Using it, along with `tagLength`, is the only way for the parser to distinguish different segments. The the only supported value for `tagStart` now is 0.
