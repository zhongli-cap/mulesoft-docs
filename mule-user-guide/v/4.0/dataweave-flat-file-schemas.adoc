...
Note that if you intend to use a fixed-width format, you can set up your data type directly through the Transform component using the `Fixed Width` type.
...
Note that the `tag` field identifies the type of record a row contains. 
...
Note that this example is not complete. It needs to <<Segment Definitions, define each of the referenced segments>> at the end. See <<Referenced vs in-lined Definitions>> to understand how these segments are being referenced in this example.
...
Note that if you're defining elements inline within a segment definition (as opposed to defining them at the end of the document and referencing them), the `id` field is not required.
...
Composites serve to reference a set of elements that are typically presented together. For example, `name` and `surname` could be bundled together into a single composite because they are likely to be referred to as a set.
...
|`tagLength` |Number of columns in segment identifier tags.  Required for flat file structures.
|`data` |List of segments (and groups) in the structure
|===

Note that the `tagStart` and `tagLength` parameters are required when using flat file structures. Using both is the only way for the parser to distinguish different segments. The only supported value for `tagStart` now is `0`.
...
Inline segment definitions use the `usage` and `count` key-value pairs as for references, and combine these with the <<Segment Definitions, segment definition>> key-value pairs.
...
A Ticket with its corresponding Checks makes up a group, and this group may be repeated any number of times within a message, while the `HeaderFile` and the `EndFile` segments are each present only once and mark the start and end of the message.
...
* link:/mule-user-guide/v/4.0/dataweave-formats#flat-file[DataWeave formats]
