...
This simple example changes the XML input to JSON output by using selector expressions without any operations. It also changes the order and names of some of the fields. Note that more complex transformations usually require the use of the `map` or `mapObject` operator.
