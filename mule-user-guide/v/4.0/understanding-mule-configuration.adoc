...
[NOTE]
Mule uses an XML configuration file that defines the constructs required to validate and run a Mule application. For more detail see link:/mule-user-guide/v/4.0/about-the-xml-configuration-file[About the XML Configuration File].
...
For more about Mule transformers, see link:/mule-user-guide/v/4.0/using-transformers[Using Transformers].
...
Filters once again can be configured as global elements and referred to where they are used, or configured at their point of use.  For more information, see link:/mule-user-guide/v/4.0/filters[Filters].

==== Expressions

For a current reference to using expressions in Mule, see link:/mule-user-guide/v/4.0/mule-expression-language-mel[Mule Expression Language MEL]. 
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:/mule-user-guide/v/4.0/logger-component-reference[Logger Component Reference]
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see link:/mule-user-guide/v/4.0/routers[Routing Message Processors].
...
<1> and <2> are Java components, specified by each's class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see link:/mule-user-guide/v/4.0/entry-point-resolver-configuration-reference[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

In #9 in <<Flows>>, the example calls `authorize.net` to authorize a credit card purchase, passing it information from the message.  For more about connectors, see link:/mule-user-guide/v/4.0/anypoint-connectors[Anypoint Connectors].
...
In #14 in <<Flows>>, the example writes the message that caused the exception to a JMS queue, where it can be examined.  For more about exception strategies, see link:/mule-user-guide/v/4.0/error-handling[Error Handling].
...
For more about configuration patterns, see link:/mule-user-guide/v/4.0/using-mule-configuration-patterns[Using Mule Configuration Patterns].
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see link:/mule-user-guide/v/4.0/transaction-management[Transaction Management].
...
For more about Mule security, see link:/mule-user-guide/v/4.0/configuring-security[Configuring Security].
