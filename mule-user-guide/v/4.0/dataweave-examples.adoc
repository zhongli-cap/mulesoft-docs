...
Follow along to see how to use the link:/mule-user-guide/v/4.0/dataweave-operators#map[map] operator. The ink:/mule-user-guide/v/3.8/dataweave-operators#coerce-to-number[as] operator is also featured in this example. Take an XML list of books and use DataWeave to output a JSON version of the book list. It goes through each `<book>` element from the XML input, simply using the `map` operator.

You can ensure the transformation generates the correct type for each element by using the `as` operator (used for type coercion).
...
This example takes in an XML file that is grouped separating two types of elements: teachers and students. This transform uses the link:/mule-user-guide/v/4.0/dataweave-operators#group-by[Group By] operator to make it into a new XML structure where the top level grouping is "class". Within each class, the students and teachers that are registered in it are listed.

This example goes through the whole set of elements in the input, using both a link:/mule-user-guide/v/4.0/dataweave-operators#map-object[map object] operator and a link:/mule-user-guide/v/4.0/dataweave-operators#map[map] operator. Note that they differ in that map object processes both key and value of its input, rather than just the keys. The example also uses the link:/mule-user-guide/v/4.0/dataweave-operators#filter[filter] operator to only show the attendees who have the class subject listed as a hobby. As each student may have multiple hobbies, the link:/mule-user-guide/v/4.0/dataweave-selectors#multi-value-selector[Multi value selector] is used to refer to them all.
