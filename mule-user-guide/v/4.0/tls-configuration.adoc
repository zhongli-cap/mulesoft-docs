...
For more information, see the XML reference later in this document.
...
The 'tls:trust-store' and 'tls:key-store' elements must reference existing certificates. If you don't provide any values for the trust-store, the default Java truststore is used. The default truststore is updated with the Java version, so it's recommended that you use an updated Java version to be sure it includes updates to well known CA certificates.

Mule supports many types of keystores. The following types of keystores are typically used:
...
Adding a truststore or a keystore to a TLS configuration implicitly implements the corresponding kind of authentication:

* Security based on certificates from a CA
* Security based on a private key and self-signed certificate
...
The `tls:context` is required to contain a `tls:key-store` element. You set the path field to the location of the keystore file that contains the private/public keys of the server.
