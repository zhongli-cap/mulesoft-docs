...
This document assumes that you are familiar with the link:/mule-fundamentals/v/3.8-m1/anypoint-studio-essentials[Visual Editor in Anypoint Studio], link:/mule-user-guide/v/3.8-m1/anypoint-connectors[Anypoint Connectors], and  link:/mule-fundamentals/v/3.8-m1/global-elements[Global Elements]. Review the link:/mule-fundamentals/v/3.8-m1/first-day-with-mule[First Day with Mule] chapter to learn more about developing Mule Applications.
...
[NOTE]
If you need to deploy to different environments (for example, production, development) where these parameters need to have different values, see link:/mule-user-guide/v/3.8-m1/deploying-to-multiple-environments[Deploying to Multiple Environments].
...
[width="100%",cols="20%,10%,60%"]
|===
|*Field* |*Example Value* |*Description*
|*Domain* |<fullDomainStartofFlow> |Should be the domain assigned to the listener at the start of your OAuth callback flow.  When in production, this is the domain on which your application is hosted.
|*Local Port* |`8081` |The port on which your authorization flow is hosted. This is the port configured on your HTTP listener.
|*Remote Port* | <CallbackHostFullDomain> |The port of the listener at which your OAuth callback is hosted.
|*Path* |`callback` |The path of the listener at which your OAuth callback is hosted.
|*Default Access Token Id* |n/a |Not used in this example. Read more about <<Managing OAuth Tokens,Managing OAuth Tokens>>.
|===
...
* Learn how to access an OAuth API using the link:/mule-user-guide/v/3.8-m1/http-request-connector[HTTP Request Connector]
