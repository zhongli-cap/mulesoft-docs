...
Install connectors using link:https://www.mulesoft.com/exchange#!/?types=connector&sortBy=name[Anypoint Exchange], which is accessible from within Anypoint Studio. For more information on connector installation, see link:/mule-fundamentals/v/3.8-m1/anypoint-exchange#installing-a-connector-from-anypoint-exchange[Installing Connectors from Anypoint Exchange].

== Using a Connector

You can configure and use a connector in Anypoint Studio following the guidelines particular to that connector. For a tutorial on basic usage, see our link:/mule-fundamentals/v/3.7/anypoint-connector-tutorial[tutorial] using a simple connector
