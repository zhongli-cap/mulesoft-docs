...
The example below illustrates a very simple Mule application in Studio that meets the minimum configuration requirements:

* configured Database connector,
* configured global Database connector element, and
* installed database driver.

However, only one of these three required parts is visible in the graphical representation of the application is the Database connector itself. 

image:basic-db-transform-to-file.png[basic-db-transform-to-file]

* *Poll scope* - regularly polls for data
* *Database connector* - connects and then submits a query to a database. For example, selects employee IDs
* *Transform Message / Dataweave* - transforms data to what is expected to be output to the file
* *File connector* - writes data to a file, such as a CSV

To prepare an application to use a Database connector as in the example above, you must first add the database's driver to your Mule project, then configure, at minimum, the required parameters for the Database connector and a Global Database connector element. The diagram and section below describe how to address these requirements.
...
*Required:*  The following table describes the attributes of the Global Database connector element that you  _must_ configure to be able to connect, then submit queries to a database. For a full list of elements, attributes, and default values, see
 link:/mule-user-guide/v/3.8-m1/database-connector-reference[Database Connector Reference]. See also <<Fields That Support MEL Expressions, Fields That Support MEL Expressions>>. 
...
====
|*SQL Statement* OR *Template Query Reference* |If you choose to use a *Parameterized* or *Dynamic* query type, use this attribute to define the SQL statement itself.
If you choose to use a *From Template* query type, use this attribute to reference the template (defined in a global Template Query element) in which you defined a SQL statement.
|===
*Examples:*
