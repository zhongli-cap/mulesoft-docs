...
For more information, see the link:/mule-user-guide/v/3.8-m1/windows-gateway-services-guide[Windows Gateway Services Guide].
...
In a traditional scenario, after a message is read in the gateway, it is deleted from the queue. If there is a connection failure or another issue in Mule that prevents the message from being read, that message is lost.
...
The MSMQ connector is integrated with the Mule logging infrastructure for logging errors and relevant information for the user.
...
|Not Acceptable. "The connector and proxy versions do not match" |This is unlikely to happen. The version of the connector running in Mule is not compatible with the version of the Windows Gateway. Make sure to update the connector or gateway to use the same version.
