...
[IMPORTANT]
====
Since 3.8-m1, Mule allows you to configure specific block sizes for each batch job and custom job instance Ids.
====
=== Custom batch Job instance Id

You can now use a Mule expression to specify a unique identifier for each batch job by passing a *Job Instance Id* attribute that takes a MEL expression. +
Please note that constant values are not allowed for batch jobs and if the MEL expression returns a previously seen value, an exception will be thrown and the job instance will not be created.

If you don't set a job instance Id; by default Mule will auto generate a unique identifier.
...
This will name the execution instance [green]`Job From 15/01/16 05:23:12`.

=== Batch configurable block sizes
Due to performance reasons, batch records are, by default, queued and scheduled in blocks of 100 records before executing the job.
After 3.8-m1, Mule allows you to configure the block size in the batch job properties tab.
This does not only lightens the amount of I/O requests, but also helps us keep a block:thread relationship, instead of a queue:thread relation to improve performance.

For most use cases, the default value of 100 records per block should work. However, if you need to process really heavy payloads, then queueing a hundred records will demand a large amount of working memory and it will be more convenient to lower the block size of this job.
On the other hand, if you have many tasks with really small payload size, then setting a larger block size will improve your batch job time without sacrificing working memory load.
