...
Regardless of the processing type (either synchronous, asynchronous, one-way, request-response or even if the requests are temporarily buffered before being processed), servers usually end up in this 1:1 relationship between a request and a running thread.

But when it comes to a batch job, all records are first stored in a persistent queue before the link:https://docs.mulesoft.com/mule-user-guide/v/3.8-m1/batch-processing#process[processing phase] begins, so this traditional threading model wouldn't apply.
Check our *Basic Anatomy* section below to learn more about how batch processing works different than a regular threading model.
...
Since Batch was designed to work with millions of records, we need to store them in a persistent storage queue. However, consider having 1 million records to place in a queue for a 3 step batch job. You might be seeing at 3 million I/O operations to take an re-queue each record as it moves through the link:https://docs.mulesoft.com/mule-user-guide/v/3.8-m1/batch-processing#batch-processing-phases[job's phases].
In order to overcome this performance repercussion, batch records are, by default, queued and scheduled in blocks of 100 records before executing the job. This does not only lightens the amount of I/O requests, but also helps us keep a block:thread relationship, instead of a queue:thread relation to improve operation's load. +
...
Consider, however, that in order to process 16 records in parallel, you need enough memory to move a grand total of 1600 records from a persistent storage into RAM, so always keep in mind that this performance enhancement entails working memory.
...
However, let's consider three use cases where you might need to increase or decrease the block size:

. Let's assume you have only 200 records process through a batch job. With the standard size of 100 block size, you'll only have 2 records being processed in parallel at any given time. +
If you hit it with less than 101, then your processing becomes sequential.
. If you need to process really heavy payloads, then queueing a hundred records will demand a large amount of working memory. +
Consider a batch job that needs to process images, and let's take an average image size of 3MB. You'll then have 100 blocks with payloads of 3Mb, being processed in 16 threads. Hence your default threading-profile setting would require around 4.6GB of working memory just to keep the blocks in memory. +
In these cases, set a lower block size to distribute each payload through more jobs and lessen the load of your available memory.
. On the other hand, consider having 5 million records with payloads so small, that you could actually fit blocks of 500 in your memory without problems. Setting a larger block size will improve your batch job time without sacrificing working memory load.

By configuring a new block size in the properties tab, you can overcome all the use cases mentioned above.
In order to take full advantage of this feature, you need to understand how the block sizes affects your batch job. Running comparative tests with different values and testing performance will definitely help you finding an optimum block size before moving this change into production.

Remember that modifying this value is optional, and if you apply no changes, you'll always have a default value of 100 records per block.
...
If you don't set a job instance Id; by default Mule will auto generate an UUID to identify your job instance.
...
As you can see, the logs of a properly identified instance is much more easier to read.
