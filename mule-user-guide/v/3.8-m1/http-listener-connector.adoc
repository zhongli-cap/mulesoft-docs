...
=== Creating a Global Element for the HTTP Connector Configuration
...
[NOTE]
To listen on an specific port for every network interface in the server, you can use the host to `0.0.0.0`. If you set it to `localhost` then you are only able to listen for incoming requests generated at your computer's localhost IP address.
