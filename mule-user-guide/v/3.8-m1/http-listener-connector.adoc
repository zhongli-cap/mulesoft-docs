...
In Studio, to instantiate the connector as an HTTP Listener Connector (and not an link:/mule-user-guide/v/3.8-m1/http-request-connector[HTTP Request Connector]), you must place it into the _Source_ section of a flow.
...
You can also use wildcards (e.g. `/*`) in the value for *Path*, to listen for all incoming requests to any path within the specified base path. You can also specify a partial path that ends in `/*`, such as `mypath/*`, pointing to any path that begins as defined but that could also be extended with anything else.
