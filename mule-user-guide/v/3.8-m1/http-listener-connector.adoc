...
You can also use `/*` as the value for *Path*, to listen for all incoming requests to any path within the specified base path. You can also specify a partial path that ends in `/*`, such as `mypath/*`, pointing to any path that begins as defined but that could also be extended with anything else.
