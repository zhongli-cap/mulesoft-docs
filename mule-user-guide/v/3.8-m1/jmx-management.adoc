...
There are several link:/mule-user-guide/v/3.8-m1/mule-agents[agents] provided with Mule for JMX support. The easiest way to configure JMX is to use the default JMX support agent.

NOTE: This version of Mule uses log4j2, which is configured in the `$MULE_HOME/conf/log4j2.xml` file. References in this document to log4j no longer are applicable. For more information, see link:/mule-user-guide/v/3.8-m1/logging-in-mule[Logging in Mule].
