...
Following the example above, by simply adding a For Each scope with an expression component containing the expression: `record.recordVars['sfdcld'] = payload.wrapped.id`, you can automatically get a variable called `record` that will hold a reference to one of the aggregated records.
...
And although this is valid for most use cases, if you need to access random records, we expose a variable called _records_. This is a random access list accessible all across the commit block.
