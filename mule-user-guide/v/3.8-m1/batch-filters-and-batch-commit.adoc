...
In previous versions of Mule, the commit block exposed only the grouped record’s payload and didn't allow you to change those payloads nor to retrieve the associated link:/mule-user-guide/v/3.8-m1/record-variable[record variables].
Now, Mule allows you to access the grouped records' payloads and variables from within the Batch commit block and process them using the Mule Expression Language (MEL).

==== Sequential Access

You can persistently go over the aggregated records' payloads in a sequential order and store them as record variables to then replace, manipulate, or store the data during the link:/mule-user-guide/v/3.8-m1/batch-processing#process[process phase] of your batch job.

In this case, by adding a link:/mule-user-guide/v/3.8-m1/foreach[Foreach Scope] the user iterates trough the records in a fixed size commit block, and through an link:/mule-user-guide/v/3.8-m1/expression-component-reference[Expression Component Reference] modifies the payload, and creates a record variable for each collected record.
...
Additionally you can use apply this for streaming commits:
...
The sequential access method consorts with this assuming that:
...
In case you need to access random records without the need of iterating, we expose a variable called _records_, which is a random access list that is accessible across the commit block.
...
As long as you are able to handle list indexes, you can discretionary access every record within the Commit block.
