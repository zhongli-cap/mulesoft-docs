...
image:authentication-in-http-requests-af80b.png[basic auth for http request]
....
...
When this is the case, the parameters will be automatically extracted and you can easily use link:/mule-user-guide/v/3.8-m1/mule-expression-language-mel[MEL expressions] later in the flow to reference these values in the Mule Message that was generated from the response to the request to the token URL.
...
It’s possible to customize when Mule will perform one of these requests to obtain a new access token. This is configured through a link:/mule-user-guide/v/3.8-m1/mule-expression-language-mel[MEL Expression] that is evaluated against the Mule Message that is generated from the response of the HTTP Request Connector call.
...
By default, the token manager uses an in-memory object store to store the credentials. You can customize the token manager object store by using the objectStore-ref attribute. link:/mule-user-guide/v/3.8-m1/mule-object-stores[See how to configure a custom object store].
