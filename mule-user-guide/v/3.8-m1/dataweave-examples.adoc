...
In this example, the input contains the same sensitive information as in the previous one, but instead of entirely removing the key:value pairs that contain it, the values are replaced with the string **. The transform replicates the inbound structure but uses a simple link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#when-otherwise[*when* operation] to replace values when specific keys occur.
...
[TIP]
See link:/mule-user-guide/v/3.8-m1/dataweave-selectors#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
The example goes through the whole set of elements in the input using  a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operator]. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#when-otherwise[*when* operator] to only select instances that include the field 'insurance'.
...
The example goes through the whole set of elements in the input using  a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map-object[*map object* operator]. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#when-otherwise[*when* operator] together with an link:/mule-user-guide/v/3.8-m1/dataweave-operators#and[*and*] to pick out the specific keys that need to change, and the link:/mule-user-guide/v/3.8-m1/dataweave-operators#as[*as* operator] to coerce its type to string.
...
In this example, there are three different input JSON files, these three all arrive in one single Mule message, occupying the payload and two link:/mule-user-guide/v/3.8-m1/dataweave-selectors#flow-variables-from-a-mule-message[flow variables]. The payload contains an array of book objects, one flow variable has a set of currency exchange rates, and the other one a query. The transform filters the first input using the conditions passed in the third input, then performs a link:/mule-user-guide/v/3.8-m1/using-dataweave-in-studio#map[*map* operation] to deal with each remaining object separately. Within this map, it defines two variables: *it* and *props*. Through the use of @, attributes are added into the XML tags. A second map operation inside the first one calculates the price of each book for each of the currencies provided in the second input. Another map operation displays each element in the "author" array as a separate <author></author> tag.


[TIP]
See link:/mule-user-guide/v/3.8-m1/dataweave-selectors#flow-variables-from-a-mule-message[flow variables] for how to access flow variables from the input and link:/mule-user-guide/v/3.8-m1/dataweave-selectors#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
The example defines a link:/mule-user-guide/v/3.8-m1/dataweave-types#functions-and-lambdas[function] in the link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#dataweave-header[dataweave header] and then calls it on the body. It goes through the whole set of elements in the input using  a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map-object[*map object* operator]. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#when-otherwise[*when* and *otherwise* operators] to pick out only instances where an XML attribute exists, and to do nothing otherwise.
...
The example goes through the whole set of elements in the input using a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map-object[*map object* operator]. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#dataweave-language-introduction#when-otherwise[*when* and *otherwise* operators] to pick out only instances where an XML attribute exists, and to do nothing otherwise.
It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-operators#match[*match* operator].
...
The example goes through the whole set of elements in the input using a link:/mule-user-guide/v/3.8-m1/dataweave-operators#map[*map* operator]. It also uses the link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction#default[*default* operator].
