...
This document assumes that you are familiar with the link:/mule-fundamentals/v/3.7/anypoint-studio-essentials[Anypoint Studio Essentials], link:/mule-user-guide/v/3.8-m1/anypoint-connectors[Anypoint Connectors] and link:/mule-fundamentals/v/3.7/global-elements[Global Elements]. Review the link:/mule-fundamentals/v/3.7/basic-studio-tutorial[Getting Started with Anypoint Studio] chapter to learn more about developing with Mule ESB's graphical user interface.

== Configuring Auto-Paging

The table below lists the link:/mule-user-guide/v/3.8-m1/anypoint-connectors[Anypoint Connectors] which support auto-paging functionality. (Additionally, you can use auto-paging with any custom-built connector which supports auto-paging functionality.)
...
. To make the *Paging* section visible in a connector, you must first select an *Operation* which outputs a collection, for example `Get groups`. Otherwise, Studio does not display the Paging section in the properties editor.
...
When you use a connector that employs paging, Mule creates an iterator object that splits the payload into manageable pages. Regardless of the page-size, the iterator pushes out objects one at a time, then fetches the next pages on demand. To process the paginated output, you must use elements in your flow that can handle collections, such as a link:/mule-user-guide/v/3.8-m1/foreach[Foreach scope], a collection splitter, a link:/mule-user-guide/v/3.8-m1/dataweave[Transform Message component], etc. Because Mule processes the set of pages one page at a time, it prevents memory usage from exceeding its limits.
...
. Add a new *Google Calendars* connector in the new flow.
...
* Learn more about link:/mule-user-guide/v/3.8-m1/scopes[Scopes].

* Learn more about the link:/mule-user-guide/v/3.8-m1/foreach[Foreach] scope.

* Need to handle really large payloads? Learn about link:/mule-user-guide/v/3.8-m1/mule-high-availability-ha-clusters[Mule High Availability HA Clusters].
