...
+
[NOTE]
JDBC Inbound Endpoints can only be set to a one-way exchange pattern. JDBC Outbound Endpoints can be set to either a one-way or request-response exchange pattern. For details about exchange patterns, see link:/mule-fundamentals/v/3.7/anypoint-studio-essentials[Anypoint Studio Essentials].
...
For details on setting the properties for a JDBC endpoint using an XML editor, consult the link:/mule-user-guide/v/3.8-m1/jdbc-transport-reference[JDBC Transport Reference].
