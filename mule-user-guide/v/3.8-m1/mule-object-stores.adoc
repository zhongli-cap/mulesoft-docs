...
* *In-memory store* – Persistent store is the default. For more information, see "Object Stores and Clustering" in the link:/mule-user-guide/v/3.8-m1/cache-scope[Cache Scope] document.
...
[TIP]
The default object store created for all new caching strategies supports cluster mode out of the box, with the exception of the cache scope which uses the old default caching strategy. For details, see the Object Stores and Clustering section in link:/mule-user-guide/v/3.8-m1/cache-scope[Cache Scope].
