...
The syntax of Mule configurations is defined by a set of XML schemas. Each configuration lists the schemas it uses and provides their designated URLs. The majority of these will be the Mule schemas for the version of Mule being used, but in addition there might be third-party schemas, for instance:
...
*Reference the numbers indicated in the list below, against the numbers indicated in the code block above. Use the slider to see the full code block, if necessary.
...
. The location for the Mule stdio schema
. THe last line provides the location for the Mule core schema
...
For more about Mule transformers, see link:/mule-user-guide/v/3.8-m1/using-transformers[Using Transformers].
...
For a current reference to using expressions in Mule, see link:/mule-user-guide/v/3.8-m1/mule-expression-language-mel[Mule Expression Language MEL]. 
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:/mule-user-guide/v/3.8-m1/logger-component-reference[Logger Component Reference]
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see link:/mule-user-guide/v/3.8-m1/routers[Routing Message Processors].
...
❶❷ This is a Java component, specified by its class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see link:/mule-user-guide/v/3.8-m1/entry-point-resolver-configuration-reference[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

❾ Calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about connectors, see link:/mule-user-guide/v/3.8-m1/anypoint-connectors[Anypoint Connectors].
...
❶❺ Writes the message that caused the exception to a JMS queue, where it can be examined.  For more about exception strategies, see link:/mule-user-guide/v/3.8-m1/error-handling[Error Handling].
...
For more about configuration patterns, see link:/mule-user-guide/v/3.8-m1/using-mule-configuration-patterns[Using Mule Configuration Patterns].
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see link:/mule-user-guide/v/3.8-m1/transaction-management[Transaction Management].
...
For more about Mule security, see link:/mule-user-guide/v/3.8-m1/configuring-security[Configuring Security].
