...
In the case of *Composite*, the embedded building blocks are actually message sources (i.e., link:https://docs.mulesoft.com/mule-user-guide/v/3.2/studio-endpoints[inbound endpoints]) that listen in parallel on different channels for incoming messages (See: **above, left**). Whenever any of these receivers accepts a message, the *Composite* scope passes it to the first message processor in the flow, thus triggering that flow.
...
The *Sub Flow* scope exists as a _synchronous_ branch flow with respect to the parent flow (See: *below*). After it receives a message from a link:https://docs.mulesoft.com/mule-user-guide/v/3.2/flow-ref-component-reference[Flow Reference component], it processes that message through the entire sequence of embedded message processors, then passes the message back to the next message processor in the parent flow for further processing. This process repeats each time a Flow Reference component in the main flow "calls" the Sub Flow processing block.
...
|image:async.png[async] |Async |Creates a block of message processors that execute asynchronously while the rest of the flow continues to execute in parallel. For instance, you can populate an Async scope with a sequence of building blocks that perform logging so that logging does not slow down the rest of the application flow. +
 ...
 +
 For specific information on configuring the Async Scope, see the link:https://docs.mulesoft.com/mule-user-guide/v/3.2/async-scope-reference[Async Scope Reference] page.
