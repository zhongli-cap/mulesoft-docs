...
In the case of *Composite*, the embedded building blocks are actually message sources (i.e., link:studio-endpoints[inbound endpoints]) that listen in parallel on different channels for incoming messages (See: **above, left**). Whenever any of these receivers accepts a message, the *Composite* scope passes it to the first message processor in the flow, thus triggering that flow.
