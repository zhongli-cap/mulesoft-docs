...
You can also use the XPath Filter from the link:/mule-user-guide/v/3.2/xml-module-reference[XML Module Reference], which supports some additional properties. Mule also provides a Jaxen XPath Filter based on the Jaxen library that may provide faster performance in some scenarios.
