...
For reference to the configuration of each filter, see link:filters-configuration-reference[Filters Configuration Reference].
...
You can also use the XPath Filter from the link:xml-module-reference[XML Module Reference], which supports some additional properties. Mule also provides a Jaxen XPath Filter based on the Jaxen library that may provide faster performance in some scenarios.
...
You can also use the JXPath filter from the link:xml-module-reference[XML Module Reference], which supports some additional properties.
...
Several Mule transports and modules provide their own filters. For example, the link:xml-module-reference[XML Module Reference] includes a filter to determine if a message is XML. For more information, see link:transports-reference[Transports Reference] and link:modules-reference[Modules Reference]. Also, there are additional filters on link:https://www.mulesoft.com/exchange#!/?searchTerm=filters[Anypoint Exchange].
