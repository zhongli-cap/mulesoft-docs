...
Many of the examples are distributed with Mule as pre-packaged application archives. To run them just copy the example zip archive from the example directory into the `<Mule Home Directory>/apps` directory and Mule will automatically deploy the example. Batch scripts are no longer provided to run the examples. Be sure you have completed all the instructions in link:hello-mule[Hello, Mule!] before you attempt to run the example.
