...
For more about Mule transformers, see link:/mule-user-guide/v/3.2/using-transformers[Using Transformers].
...
Filters once again can be configured as global elements and referred to where they are used, or configured at their point of use.  For more about Mule filters see link:/mule-user-guide/v/3.2/using-filters[Using Filters]
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:/mule-user-guide/v/3.2/logger-element-for-flows[Logger Element for Flows]
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see link:/mule-user-guide/v/3.2/routing-message-processors[Routing Message Processors].
...
❶❷ This is a Java component, specified by its class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see link:/mule-user-guide/v/3.2/entry-point-resolver-configuration-reference[Entry Point Resolver Configuration Reference].

==== Cloud Connectors

A cloud connector calls a cloud service.

❾ calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about Cloud Connectors, see  link:/mule-user-guide/v/3.2/integrating-with-cloud-connect[Integrating with Cloud Connect].
...
For more about configuration patterns, see link:/mule-user-guide/v/3.2/using-mule-configuration-patterns[Using Mule Configuration Patterns] .
...
The bridge pattern is the simplest, with flows not far behind. The service isn't hard to read, but it's significantly longer and more complicated for no real gain.  For more about services, see  link:/mule-user-guide/v/3.2/service-configuration-reference[Service Configuration Reference] .
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see link:/mule-user-guide/v/3.2/transaction-management[Transaction Management].
...
For more about Mule security, see link:/mule-user-guide/v/3.2/configuring-security[Configuring Security].
