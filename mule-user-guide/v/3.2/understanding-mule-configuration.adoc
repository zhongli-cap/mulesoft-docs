...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:logger-element-for-flows[Logger Element for Flows]
...
❾ calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about Cloud Connectors, see  link:integrating-with-cloud-connect[Integrating with Cloud Connect].
...
The bridge pattern is the simplest, with flows not far behind. The service isn't hard to read, but it's significantly longer and more complicated for no real gain.  For more about services, see  link:service-configuration-reference[Service Configuration Reference] .
