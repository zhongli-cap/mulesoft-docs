...
Screen shots should added for every new step. If you are working on a building example - such as a Core Concepts example like link:adding-business-logic-to-a-flow[Adding Business Logic to a Flow] - don't add screenshots for steps already covered previously unless the details change in some way.
