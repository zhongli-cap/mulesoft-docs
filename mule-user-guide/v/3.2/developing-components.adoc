...
For details on configuring entry point resolvers, see link:entry-point-resolver-configuration-reference[Entry Point Resolver Configuration Reference].
...
See link:service-messaging-styles[Service Messaging Styles] for more detail about the different configuration options that affect message flow.
...
* Design your component in such a way that interface methods can be mapped to outbound endpoints and then use bindings to map these in configuration. For information on how to configure bindings, see link:configuring-java-components[Configuring Java Components].
