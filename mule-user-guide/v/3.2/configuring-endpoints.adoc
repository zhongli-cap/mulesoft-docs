...
This page describes how to configure an endpoint. For details on the various attributes and elements you can configure on an endpoint, see link:https://docs.mulesoft.com/mule-user-guide/v/3.2/endpoint-configuration-reference[Endpoint Configuration Reference].

== Basic Configuration

In its most basic form, an endpoint consists of a link:https://docs.mulesoft.com/mule-user-guide/v/3.2/connecting-using-transports[transport] and a transport-specific channel/destination/resource used to identify the channel and location where two services can exchange information. For example:
...
Starting in Mule 3, an outbound endpoint can also be dynamic. This means that the endpoint's URI is the value of an link:https://docs.mulesoft.com/mule-user-guide/v/3.2/using-expressions[expression], which is evaluated just before a message is sent to it. This allows the target of a message to be determined by its contents or the value of any message property. Dynamic endpoints can use either of the endpoint formats shown above.
...
For more information on configuring messaging styles on an endpoint, see link:https://docs.mulesoft.com/mule-user-guide/v/3.2/service-messaging-styles[Service Messaging Styles].

=== Transaction

A transaction can begin or commit when an event is received or sent via an endpoint. The endpoint must be synchronous, and transaction support depends largely on the particular transport being used. For more information see link:https://docs.mulesoft.com/mule-user-guide/v/3.2/transaction-management[Transaction Management].
...
link:https://docs.mulesoft.com/mule-user-guide/v/3.2/using-transformers[Transformers] can be configured on an endpoint encapsulating transformation logic in an endpoint that can then be reused as required.
...
See link:https://docs.mulesoft.com/mule-user-guide/v/3.2/inbound-routers[Inbound Routers].
...
See link:https://docs.mulesoft.com/mule-user-guide/v/3.2/outbound-routers[Outbound Routers].
...
A single "catch-all" endpoint can be configured for certain types of routers. See link:https://docs.mulesoft.com/mule-user-guide/v/3.2/catch-all-strategies[Catch-all Strategies].
...
A single error endpoint can be configured on an exception strategy. See link:https://docs.mulesoft.com/mule-user-guide/v/3.2/error-handling[Error Handling].
