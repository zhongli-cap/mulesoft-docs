...
. Choose between a one-way or request-response exchange pattern. (For details, see: link:mule-studio-essentials[About Main Flows]).
...
See the link:sftp-transport-reference[SFTP Transport Reference] for details on setting the properties for an SFTP endpoint using an XML editor.
