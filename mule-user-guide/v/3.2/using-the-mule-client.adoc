...
. The Mule URL endpoint: any valid link:/mule\-user\-guide/v/3\.2/mule-endpoint-uris[Mule Endpoint] used to determine the transport, endpoint, and other information about delivery of the message. This can also be an endpoint name stored in configuration.
...
For SOAP support, you can use the Mule link:/mule\-user\-guide/v/3\.2/cxf-module-reference[CXF Module Reference] or #[AXIS:Axis Transport]#. For more information about Mule Axis support, see #[AXIS:Axis Web Services and Mule]#.
...
Some outbound routers such as the link:/mule\-user\-guide/v/3\.2/outbound-routers[List Message Splitter], link:/mule\-user\-guide/v/3\.2/outbound-routers[Multicaster], and link:/mule\-user\-guide/v/3\.2/outbound-routers[Recipient List] may return more that one result message in the following cases:
...
If you do not set the wire format, the serialization format is used. For more information on transformers, see link:/mule\-user\-guide/v/3\.2/using-transformers[Using Transformers].
