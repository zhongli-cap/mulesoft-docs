...
The inbound endpoints can be any valid link:/mule\-user\-guide/v/3\.2/configuring-endpoints[Mule Endpoint], so you can receive JMS messages, SOAP requests, files, HTTP and servlet requests, TCP, multicast, and more.
...
For more information on publishing events, see the link:/mule\-user\-guide/v/3\.2/error-handler-example[Error Handler Example].
