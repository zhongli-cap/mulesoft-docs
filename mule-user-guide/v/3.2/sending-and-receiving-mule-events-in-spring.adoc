...
The inbound endpoints can be any valid link:configuring-endpoints[Mule Endpoint], so you can receive JMS messages, SOAP requests, files, HTTP and servlet requests, TCP, multicast, and more.
...
For more information on publishing events, see the link:error-handler-example[Error Handler Example].
