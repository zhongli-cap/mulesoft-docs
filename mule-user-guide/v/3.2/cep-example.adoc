...
* Using link:quartz-transport-reference[Quartz] to generate a periodic input signal
* Using the link:ajax-transport-reference[AJAX transport] to update a web page from Mule
...
We declare Drools as the Rules Engine implementation, for more info. on this, refer to the link:drools-module-reference[Drools Module Reference].
...
Declare the link:ajax-transport-reference[AJAX connector] which will be used to update the Web UI.
...
To keep things simple for the example, we define link:vm-transport-reference[in-memory ("VM") queues] to transport messages. If this were a real application, the incoming stock ticks would probably be received via a web service and the outgoing alerts would probably be delivered to a link:jms-transport-reference[JMS queue] or via link:xmpp-transport-reference[IM]/SMS/link:email-transport-reference[E-mail].
...
In this flow, we use link:quartz-transport-reference[Quartz] to generate a periodic signal which triggers our `TickFeed` component. The `TickFeed` component simulates a new stock tick by reading in a line from a text file and putting it on the `stockTick` queue.
...
For more info. on the configuration of the Rules Engine, refer to the link:drools-module-reference[Drools Module Reference].
