...
The module archetype will link:creating-module-archetypes[ask various questions] and then generate the files. You can also use this plug-in without user prompts by entering all the arguments at the command line. For a full list of arguments that can be passed in, see the Command Line Options.
...
* *siso*: The pattern receives messages from a single inbound endpoint and dispatches to a single outbound endpoint. The link:bridge-pattern[Bridge], link:validator-pattern[Validator] and link:proxying-web-services[Web Service Proxy] patterns are of this kind.
