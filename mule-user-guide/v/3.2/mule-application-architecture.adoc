...
The units from which Flows are constructed are known generically as Building Blocks. In general, these correspond to icons on the Studio graphical link:https://docs.mulesoft.com/mule-user-guide/v/3.2/the-studio-interface[canvas] or link:https://docs.mulesoft.com/mule-user-guide/v/3.2/the-studio-xml-editor[XML elements] within the Mule application configuration file.
...
Under certain conditions, flows do not need to be triggered by message sources. For instance, a link:https://docs.mulesoft.com/mule-user-guide/v/3.2/flow-ref-component-reference[Flow Reference Component] can trigger a private, child flow, which can execute synchronously or asnchronously, be resuable, or not, and inherit the exception strategy used by the parent flow, or not. For details, see <<Child Flows>>, below. Similarly, the link:https://docs.mulesoft.com/mule-user-guide/v/3.2/async-scope-reference[Async Scope] can trigger a child flow that executes asynchronously, (i.e., in parallel with the parent flow).
...
For further details, see link:https://docs.mulesoft.com/mule-user-guide/v/3.2/flow-processing-strategies[Flow Processing Strategies]
...
For details, see: link:https://docs.mulesoft.com/mule-user-guide/v/3.2/error-handling[Error Handling]
