...
** Spring property placeholder return values specified in property files. By default, this is mule-app.propertiies, though it’s possible to configure Spring to read them from elsewhere. See link:using-parameters-in-your-configuration-files[Using Parameters in Your Configuration Files] .
...
If a message will be sent to different locations depending on its contents, use Mule Expressions in a dynamic outbound endpoint. Seelink:configuring-endpoints[Configuring Dynamic Endpoints] for examples.
...
(Note: the following examples can be copied as is from the existing page link:using-expressions[Using Expressions])
...
For more information on filters, see link:using-filters[Using Filters].
...
* Create a registry bootstrap file that will add it to the Mule registry, as described at link:bootstrapping-the-registry[Bootstrapping the Registry].
...
Where evaluators are objects used to retrieve data (generally from messages), enrichers are objects used to set data in messages. They are used as targets in the message enricher (see link:message-enricher[Message Enricher]. Currently, Mule supports two of them.
