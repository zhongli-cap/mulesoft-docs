...
If a message will be sent to different locations depending on its contents, use Mule Expressions in a dynamic outbound endpoint. Seelink:https://docs.mulesoft.com/mule-user-guide/v/3.2/configuring-endpoints[Configuring Dynamic Endpoints] for examples.
...
(Note: the following examples can be copied as is from the existing page link:https://docs.mulesoft.com/mule-user-guide/v/3.2/using-expressions[Using Expressions])
...
For more information on filters, see link:https://docs.mulesoft.com/mule-user-guide/v/3.2/using-filters[Using Filters].
...
|groovy |Returns the result of running a groovy script |#[groovy:payload.items.value] | |image:check.png[check] |
...
|ognl |Uses ognl to evaulate expressions on the current message payload |#[ognl:item.equals(42)] | |image:check.png[check] |
...
|regex |Returns true if the current message matches the specified regular expression |#[regex:prefix.*suffix} | |image:check.png[check] |
...
* Create a registry bootstrap file that will add it to the Mule registry, as described at link:https://docs.mulesoft.com/mule-user-guide/v/3.2/bootstrapping-the-registry[Bootstrapping the Registry].
...
Where evaluators are objects used to retrieve data (generally from messages), enrichers are objects used to set data in messages. They are used as targets in the message enricher (see link:https://docs.mulesoft.com/mule-user-guide/v/3.2/message-enricher[Message Enricher]. Currently, Mule supports two of them.
