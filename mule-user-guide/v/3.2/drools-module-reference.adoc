...
If the business logic for your Mule application is relatively simple, you are probably best off implementing it using Mule's orchestration functionality such as link:message-sources-and-message-processors[flows, routers, and custom components].
...
To see Drools with Mule in action, take a look at the link:cep-example[CEP Example] (available in the full Mule distribution).
