...
. Choose between a one-way or request-response exchange pattern. (For details, see: link:mule-studio-essentials[About Main Flows]). The default exchange pattern for the HTTP/HTTPS endpoints is request-response. Also, decide whether you want to implement security through HTTPS.
...
See the link:http-transport-reference[HTTP Transport Reference] for details on setting the properties for an HTTP endpoint using an XML editor.
