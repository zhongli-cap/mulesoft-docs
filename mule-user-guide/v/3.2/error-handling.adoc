...
This page describes these various error handling techniques. For information on setting retry policies, which control how a connector behaves when its connection fails, see link:configuring-reconnection-strategies[Configuring Reconnection Strategies].
...
[TIP]
For details on configuring exception strategies, see: link:exception-strategy-configuration-reference[Exception Strategy Configuration Reference]. +
 +
For examples of the most common exception strategy configurations, see link:exception-strategy-most-common-use-cases[Exception Strategy Most Common Use Cases]
...
For more information on routers, see link:using-message-routers[Using Message Routers].
...
If you are using link:transaction-management[transactions], you can use the `<commit-transaction>` and `<rollback-transaction>` elements to specify whether a transaction gets committed or rolled back on the basis of the name of the exception that is caught.
