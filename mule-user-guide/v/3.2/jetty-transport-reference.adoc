...
The Jetty transport provides support for exposing services over HTTP by embedding a light-weight Jetty server. The link:https://docs.mulesoft.com/mule-user-guide/v/3.2/jetty-ssl-transport[Jetty SSL Transport] works the same way but over SSL. You can only define inbound endpoints with this transport.
...
Jetty endpoints are configured the same way as link:https://docs.mulesoft.com/mule-user-guide/v/3.2/http-transport-reference[HTTP endpoints].
