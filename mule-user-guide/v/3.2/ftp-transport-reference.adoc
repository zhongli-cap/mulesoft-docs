...
The filename parser used in the FTP transport should be sufficient in 99% of the cases. It is an instance of link:/docs/site/current/apidocs/org/mule/transport/file/ExpressionFilenameParser.html[ExpressionFilenameParser] which allows to use link:expressions-configuration-reference[arbitrary expressions] to compose the filename that is used when storing files on the FTP server.
