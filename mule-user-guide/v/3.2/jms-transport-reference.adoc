...
To see JMS with Mule in action (using ActiveMQ), take a look at the link:error-handler-example[Error Handler Example] (available in the full Mule distribution).

[TIP]
====
*WebSphere MQ*

Mule Enterprise includes an link:mule-wmq-transport-reference[enhanced transport for WebSphereMQ] which is recommended if you are using WebSphereMQ as your JMS provider.
====
...
❶ New implementations are recommended to use link:using-flows-for-service-orchestration[flows], but Mule 2.x users are more familiar with `<service>`.

== Vendor-specific Configuration

Mule Enterprise includes an link:mule-wmq-transport-reference[enhanced transport for WebSphereMQ] which is recommended if you are using WebSphereMQ as your JMS provider.

Mule MQ is an enterprise-class JMS implementation officially supported by MuleSoft and has link:mulemq-integration[tight integration with Mule].

http://activemq.apache.org[ActiveMQ] is also widely-used with Mule and has link:activemq-integration[simplified configuration].
...
* link:fiorano-integration[FioranoMQ]
* link:hornetq-integration[HornetQ]
* link:jboss-jms-integration[JBoss MQ]
* link:openjms-integration[OpenJms]
* link:open-mq-integration[Open MQ]
* [Oracle AQ]
* link:seebeyond-jms-server-integration[SeeBeyond]
* link:sonicmq-integration[SonicMQ]
* link:sun-jms-grid-integration[Sun JMS Grid]
* link:swiftmq-integration[SwiftMQ]
* link:tibco-ems-integration[Tibco EMS]
* link:weblogic-jms-integration[WebLogic JMS]
