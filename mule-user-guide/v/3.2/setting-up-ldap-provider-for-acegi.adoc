...
The Acegi module is deprecated. Please link:upgrading-from-acegi-to-spring-security[upgrade] to the link:configuring-the-spring-security-manager[Spring Security module] instead, which is a drop-in replacement. As of Mule 3.2 the Acegi module is removed from the distribution.
====
...
* Used by Acegi/Spring to perform link:component-authorization-using-acegi[Component Authorization]

For information on configuring an in-memory DAO provider, see link:configuring-security[Configuring Security].
...
The configuration for component authorization is similar to the one described in link:component-authorization-using-acegi[Component Authorization Using Acegi]. A key point of configuration is `ObjectDefinitionSource`:
