...
The request-response link:https://docs.mulesoft.com/mule-user-guide/v/3.2/http-endpoint-reference[*HTTP Inbound Endpoint*] in this flow receives an end user request. Because it has a two-way message exchange pattern, this HTTP endpoint is responsible for both receiving and returning messages.
...
Then, Mule employs a content-based router to direct the message for further processing. The link:https://docs.mulesoft.com/mule-user-guide/v/3.2/choice-flow-control-reference[*Choice Flow Control*] routes each message to one of two processing pathways according to its payload contents.
...
Notice that the Loan-broker-sync flow also contains a [*Catch Exception Strategy*]. Rather than use Mule’s link:https://docs.mulesoft.com/mule-user-guide/v/3.2/error-handling[default exception strategy] this flow uses a customized exception strategy to handle errors. If an error occurs in the flow, the exception strategy’s *Set Payload Transformer* sets an error message on the payload. The application sends this error message, which reads, `Error processing loan request`, as a response to the end user.
...
Mule leverages http://cxf.apache.org[Apache’s CXF framework] to build Web services. The Processor Chain that wraps the link:https://docs.mulesoft.com/mule-user-guide/v/3.2/soap-component-reference[*SOAP Component*] and HTTP outbound endpoint is a CXF requirement. It ensures that Mule completes all processing activities prior to logging the processing result.

Last in this flow, the link:https://docs.mulesoft.com/mule-user-guide/v/3.2/logger-component-reference[*Logger Component*] logs the payload of the Web service’s response on the message payload as the `Credit Profile`.
...
* a link:https://docs.mulesoft.com/mule-user-guide/v/3.2/java-component-reference[*Java Component*] which produces random data to mimic Web service processing. 
