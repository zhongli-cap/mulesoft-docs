...
You can specify the namespace on any XML-based functionality in Mule, including the JXPath filter, Jaxen filter, XPath filter, filter-based splitter, expression splitter, round-robin splitter, JXPath extractor transformer, and XPath expression transformer in the link:xml-module-reference[XML Module Reference], link:xpath-annotation[XPath Annotation], as well as the SXC filter and filtering router in the link:sxc-module-reference[SXC Module Reference].
