...
Go here if you are looking for information about creating a transformer with the new link:https://docs.mulesoft.com/mule-user-guide/v/3.2/transformer-annotation[Transformer Annotation].
...
When Mule starts, it discovers this bootstrap file before loading any configuration and installs any objects listed in the file into the local registry. For more information, see link:https://docs.mulesoft.com/mule-user-guide/v/3.2/bootstrapping-the-registry[Bootstrapping the Registry].
...
The link:https://docs.mulesoft.com/mule-user-guide/v/3.2/hello-world-example[Hello World example] defines a custom transformer called `StringToNameString`, which wraps Java string in a custom class called `NameString`:
