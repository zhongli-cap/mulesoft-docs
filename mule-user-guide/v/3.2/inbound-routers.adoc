...
The WireTap router is based on the Selective Consumer router, so it can take any link:/mule-user-guide/v/3.2/using-filters[filters] supported by `SelectiveConsumer`. In this example, only messages that match the filter expression are copied to the vm://ToTapper endpoint.
