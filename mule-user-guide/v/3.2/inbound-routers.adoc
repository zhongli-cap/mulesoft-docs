...
The rest of this page describes the inbound routers you can use. For more detailed information on inbound router configuration elements and attributes, see the link:inbound-router-configuration-reference[Inbound Router Configuration Reference].
...
For information on using filters with this router, see link:using-filters[Using Filters]. Note that by default the filter is applied to the message after the inbound transformers are applied. If you need to execute filters on the message without applying any transformation, you can set the `transformFirst` property on this router to control whether transformations are applied.
...
After an outbound router such as the link:outbound-routers[List Message Splitter] splits a message into parts, the message chunking aggregator router reassembles those parts back into a single message. The aggregator uses the correlation ID, which is set by the outbound router, to identify which parts belong to the same message. This aggregator is based on the <<Selective Consumer>>, so filters can also be applied to messages.
...
There is an AbstractEventAggregator that provides a thread-safe implementation for custom aggregators, which you can can use to write a custom aggregator router. For example, the link:loan-broker-example[Loan Broker] examples included in the Mule distribution use a custom BankQuotesInboundAggregator router to aggregate bank quotes.
...
The WireTap router is based on the Selective Consumer router, so it can take any link:using-filters[filters] supported by `SelectiveConsumer`. In this example, only messages that match the filter expression are copied to the vm://ToTapper endpoint.
...
* Transactions are supported, so the forwarding of messages can either start or join a transaction provided that the endpoint link:transaction-management[supports transactions].
