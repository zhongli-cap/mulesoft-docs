...
This example requires that you run through the earlier example link:transforming-data-in-a-flow[Transforming Data in a Flow].

=== Building the Example

1. We are going to continue editing the link:transforming-data-in-a-flow[example] and add a File component to the flow. Drag the File endpoint from the palette to the canvas after the Echo component.
