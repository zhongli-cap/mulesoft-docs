...
It has always been possible to connect Mule to these services in the past using specific link:transports-reference[transports]. Mule Cloud Connect creates a layer on top of these services to simplify the use of these services even further.
...
To find documentation on how to install and use each cloud connector, see  link:studio-cloud-connectors[Mule Studio]. You can also learn about some scenarios in which Cloud Connectors are useful by reading link:integrating-with-cloud-connect[Integrating with Cloud Connect].
