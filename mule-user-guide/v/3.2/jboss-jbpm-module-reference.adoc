...
[WARNING]
This module provides a "Plug-in" for JBoss jBPM to be used with Mule's BPM support. If you have not yet read the general documentation for Mule's link:bpm-module-reference[BPM Support], please read that first and then come back to this page.
...
Refer to link:bpm-module-reference[BPM Module Reference] for a list of general features offered by Mule's BPM support.

To see jBPM with Mule in action, take a look at the link:loan-broker-bpm-example[Loan Broker BPM Example] (available in the full Mule distribution).
...
❶ New implementations are recommended to use link:using-flows-for-service-orchestration[flows], but Mule 2.x users will be more familiar with services.
...
This module uses the schema from the link:bpm-module-reference[BPM Module], it does not have its own schema.
...
Refer to link:bpm-module-reference[BPM Module Reference] for detailed information on the elements of the BPM schema.
