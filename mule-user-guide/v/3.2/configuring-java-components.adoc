...
For a real-world example of using `<spring-object/>`, see link:using-spring-beans-as-service-components[Using Spring Beans as Service Components].
...
You can configure entry point resolvers that determine how your component is invoked when a message is received by the service. See link:developing-components[Developing Components] for a more detailed description of their functionality.
...
See link:developing-components[Developing Components] for more information about lifecycles.
...
Binding can be used on Java components and script components. For more information see link:component-bindings[Component Bindings].
...
For more information about pooling and reference documentation for pooling configuration options, see link:tuning-performance[Tuning Performance].
