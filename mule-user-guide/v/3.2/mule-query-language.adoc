...
|===
|Name |Description
|link:mql-query-java-objects[Query Java Objects] |Query local POJOs and transform them into different objects.
|link:mql-merge-datasets[Merging Datasets] |In this example, see how you can take activity streams from Twitter and Yammer and merge them into a common format.
|link:mql-enrich-data[Enrich Data] |In this example, local data is enriched with data from SalesForce
|link:mql-service-versioning[Service Versioning] |Easily handle two different data formats (version 1.0 and version 2.0) for your service.
|===

== Resources

* link:mql-download[Download]
* link:mql-mule-integration[Mule Integration]
* link:mql-spring-integration[Spring Integration]
* link:mql-reference-guide[Syntax Guide]
* link:mql-roadmap[Roadmap]
