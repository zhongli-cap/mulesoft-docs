...
The Windows Gateway Services is configured by default to log *Information* events. This is configured in under the `<switches>` element. In case you want to log everything you should use the *Verbose* level, by changing it in the configuration element shown below.
...
=== Console Tracing and running from command line
