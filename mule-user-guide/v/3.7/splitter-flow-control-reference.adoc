...
[NOTE]
Weigh the pros and cons of using a splitter and aggregator in your flow, versus using a "Foreach" scope link:/mule-user-guide/v/3.7/foreach#using-foreach-versus-splitter-and-aggregator-pairs[here].
...
. Configure the Splitter as shown. In the *Expression* parameter, provide the link:http://www.freeformatter.com/xpath-tester.html#ad-output[XPath] expression `//*:actor/text()`, wrapped inside a MEL expression. This XPath expression selects every XML element named 'actor'. The splitter will make each of these (together with its children) into a new message. +
...
|===
. Add a Splitter below, to receive messages from the HTTP connector. In the *Expression* parameter provide the link:http://www.freeformatter.com/xpath-tester.html#ad-output[XPath] expression `//actor` , wrapped inside a MEL expression. This XPath expression selects every XML element named 'actor'. The splitter will make each of these (together with its children) into a new message.
+
