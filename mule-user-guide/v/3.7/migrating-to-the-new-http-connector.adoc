...
The new HTTP Connector has different syntax from the old one. Besides a different name and different attributes, the new connector must always reference a link:/mule-user-guide/v/3.7/global-elements[global element], this is consistent with how most of the other connectors in Mule work. Multiple connectors can reference one single global element that encapsulates many of the common configuration settings.
...
In both these cases, Mule parses the request to generate a Mule Message that is much simpler to consume. link:/mule-user-guide/v/3.7/http-listener-connector#the-request-body[Read More].
...
In both cases, Mule adds the corresponding headers to the HTTP request and builds the message body accordingly. link:/mule-user-guide/v/3.7/http-listener-connector#the-request-body[Read More].
