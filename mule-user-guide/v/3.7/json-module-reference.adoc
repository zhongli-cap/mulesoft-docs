...
Mule supports binding JSON data to objects and marshalling Java object to JSON using the link:http://jackson.codehaus.org[Jackson] Framework. Jackson uses annotations to describe how to map data to a Java object model. For example, use JSON data to map a person's information:
...
You can use JsonPath in link:/mule-user-guide/v/3.7/mule-expression-language-mel[Mule expressions] to query JSON message payloads for filtering or link:/mule-user-guide/v/3.7/message-enricher[enrichment].
