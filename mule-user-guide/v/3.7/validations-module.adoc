...
== Building with Maven
When building an application with Maven, the following dependency will need to be added to the application's pom.xml file:
...
where `${mule.version}` is the version of the Mule runtime that is being used.
