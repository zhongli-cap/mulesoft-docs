...
The IMAP connector supports all the link:/mule-user-guide/v/3.7/configuring-a-transport[common connector attributes and properties] and the following additional attributes:
...
IMAP and IMAPS endpoints include details about connecting to an IMAP mailbox. You link:/mule-user-guide/v/3.7/endpoint-configuration-reference[configure the endpoints] just as you would with any other transport, with the following additional attributes:

* user: the user name of the mailbox owner
* password: the password of the user. Note that for mule runtime prior to 3.8.x the character % needs to be escaped using %25 (html code for %).
