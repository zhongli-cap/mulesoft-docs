...
You must carry out the following steps, explained in detail bellow:
...
Before you can deploy your Mule application to WebLogic you must prepare it as a web application. This involves creating a `web.xml` file for your Mule application, a weblogic.xml for configuring the server and then creating a `.war` file of all the supporting files.
...
[TIP]
====
Read more about WebLogic Classloading link:http://docs.oracle.com/cd/E23943_01/web.1111/e13706/classloading.htm#WLPRG282[here].
====

=== Configuring Logging

Mule uses link:http://logging.apache.org/log4j/2.x/index.html[Log4j2]. WebLogic can be configured so that WebLogic's and Mule's logging both use log4j. For more information on using Commons logging and WebLogic, see link:http://e-docs.bea.com/wls/docs103/logging/config_logs.html#using_log4j.
