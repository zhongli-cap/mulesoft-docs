...
NOTE: The JDBC connector supports multi-resource transactions. Multi-resource transactions are also supported by the link:/mule-user-guide/v/3.7/database-connector[Database Connector]. Consider also the use of link:/mule-user-guide/v/3.7/xa-transactions[XA Transactions] to handle this use case.
...
+
[NOTE]
JDBC Inbound Endpoints can only be set to a one-way exchange pattern. JDBC Outbound Endpoints can be set to either a one-way or request-response exchange pattern. For details about exchange patterns, see link:/anypoint-studio/v/5/index[Anypoint Studio Essentials].
...
For details on setting the properties for a JDBC endpoint using an XML editor, consult the link:/mule-user-guide/v/3.7/jdbc-transport-reference[JDBC Transport Reference].
