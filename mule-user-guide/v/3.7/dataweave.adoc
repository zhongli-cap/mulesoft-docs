...
[TIP]
====
DataWeave is a new feature of the 3.7 runtime that replaces the link:/anypoint-studio/v/5/datamapper[DataMapper], which is a major component in previous versions. We recommend you use DataWeave to avoid the future obsolescence of Datamapper.
====
...
[TIP]
====
If you have projects that are built with the deprecated link:/anypoint-studio/v/5/datamapper[DataMapper], a migration tool is now included in Studio, which assists in converting a DataMapper map to DataWeave: Right click on a DataMapper, select *Migrate to DataWeave*, and follow the instructions.
