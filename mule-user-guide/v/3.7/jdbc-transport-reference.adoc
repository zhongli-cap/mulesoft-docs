...
Access the documentation for the link:/mule-user-guide/v/3.7/database-connector[Database Connector] to take advantage of the newly-designed functionality.
====
...
|JDBC |http://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/transport/jdbc/package-summary.html[JavaDoc] |image:check.png[(tick)] |image:check.png[(tick)] |image:check.png[(tick)] |image:check.png[(tick)](local,XA) |image:error.png[(error)] |image:check.png[(tick)] |one-way, request-response |one-way |org.mule.transport:mule-transport-jdbc
...
Note that stored procedures are only supported on outbound endpoints. If you want to set up a flow that calls a stored procedure at a regular interval, you can define a link:/mule-user-guide/v/3.7/quartz-transport-reference[Quartz] inbound endpoint and then define the stored procedure call in the outbound endpoint. For information on using Quartz to trigger flows, see the following link:http://blogs.mulesoft.com/dev/mule-dev/using-quartz-to-trigger-a-service/[blog post].
...
Transactions are supported on JDBC endpoints. See link:/mule-user-guide/v/3.7/transaction-management[Transaction Management] for details.
