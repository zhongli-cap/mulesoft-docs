...
[WARNING]
====
If you deploy multiple applications through a link:/mule-user-guide/v/3.7/shared-resources[Shared Resources] structure, don't set anything in the properties files, as there might potentially be conflicts between the various apps that share a domain. Instead, set environment variables over the scope of the deployed app, its domain, and other apps under that domain.

As explained in link:/mule-user-guide/v/3.7/shared-resources[Shared Resources], in Studio you can create these variables through the *Environment tab* of the *Run Configurations menu*, reachable via the drop-down menu next to the Play button.
====
...
For information on configuration builders, see link:/mule-user-guide/v/3.7/about-configuration-builders[About Configuration Builders].
...
If you deploy your application to Runtime Manager, you can also set properties through the Runtime Manager console. These can be defined when link:/runtime-manager/deploying-to-cloudhub[Deploying to CloudHub], or on an link:/runtime-manager/managing-applications-on-cloudhub[already running application].
