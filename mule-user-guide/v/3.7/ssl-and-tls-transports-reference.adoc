...
[WARNING]
This page refers to using SSL and TLS together with the old endpoint based link:/mule-user-guide/v/3.7/http-connector[HTTP connector]. For using TLS with the new HTTP Connector and the Web Service Consumer connector, see link:/mule-user-guide/v/3.7/tls-configuration[TLS Configuration].


The following Mule transports provide access to TCP connections:

* The link:/mule-user-guide/v/3.7/tcp-transport-reference[TCP Transport], which uses the basic TCP transport.
...
* Communicating with an external service that uses low-level unsecured TCP connections? If so, use the link:/mule-user-guide/v/3.7/tcp-transport-reference[TCP] protocol.

* Are you communicating with a flow always located in the same Mule application instance? If so, consider using the link:/mule-user-guide/v/3.7/vm-transport-reference[VM transport].
...
|===
|Name |Notes
|class |See link:/mule-user-guide/v/3.7/tcp-transport-reference[TCP Transport Reference] for an example of writing a custom protocol
