...
For other issues it is recommended to see the link:/mule-user-guide/v/3.7/windows-gateway-services-guide#windows-gateway-service-troubleshooting[troubleshooting] section at the Windows Gateway Services reference documentation. There are *message size* related settings there that you can increase if you suspect your issue is related with the response's payload of the operation you are executing. In addition, Within that section, there are many helpful tips on tracing/debugging that could give you insights on what could be causing of the problem.
