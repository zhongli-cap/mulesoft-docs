...
For other issues it is recommended to see the *troubleshooting* section at the link:/mule-user-guide/v/3.7/windows-gateway-services-guide[Windows Gateway Services] reference documentation]. There are *message sizes* related settings there that you can increase if you suspect that your issue is related with the response's payload of the operation you are executing, and within that section there are many helpful tips on tracing/debugging that could give you insights on what could be causing of the problem.
