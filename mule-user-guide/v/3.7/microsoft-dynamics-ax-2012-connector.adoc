...
* Install the link:https://repository-master.mulesoft.org/nexus/content/repositories/releases/org/mule/modules/anypoint-windows-gateway-service/1.9.0/anypoint-windows-gateway-service-1.9.0.zip[Windows Gateway Services]. Just go through the wizard by clicking *next* & *install* buttons if you plan to use this connector alone, otherwise take a look at its link:/mule-user-guide/v/3.7/windows-gateway-services-guide[reference documentation] for settings that apply to other connectors as well.
...
|*Gateway Router Service Uri* |The address (host and port) of the *Windows Gateway Services*. This address is usually similar to `https://[server_with_windows_gateway_services].com:9333/router` (refer to <<Prerequisites>>).
