...
If you receive a timeout while executing any operation using the connector, then you should properly set the `timeouts` parameters described at the connector's settings table shown above (<<Step 2: Create and Configure a New Microsoft Dynamics AX Global Element>>). In addition to the connector's timeout settings, there are timeouts that you can set at the *Windows Gateway Services* (who is the responsible of route your requests to the AX server). For more information please refer to the link:/mule-user-guide/v/3.7/windows-gateway-services-guide[Windows Gateway Services reference documentation].
