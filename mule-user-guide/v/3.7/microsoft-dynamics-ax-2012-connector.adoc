...
|*Gateway Router Service Uri* |The address (host and port) of the *Windows Gateway Services*. This address is usually similar to `https://[server_with_windows_gateway_services].com:9333/router` (refer to <<Prerequisites>>) if you haven't changed the link:/mule-user-guide/v/3.7/windows-gateway-services-guide[Windows Gateway Services] configuration, otherwise use the port you have chosen.
