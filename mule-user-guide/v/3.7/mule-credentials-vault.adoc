...
Administrative commands such as the `ps` command on UN*X or the `tasklist` command on Windows reveal the JVM arguments used to start a Mule runtime, including the secret encryption key value, so access to these commands should also be restricted to trusted administrators.
