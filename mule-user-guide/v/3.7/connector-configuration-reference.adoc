...
* operations-based connectors and
* <<Endpoint-Based Connector Configuration>> connectors. This document describes the general configuration needed for both of these types, but does not provide instructions for specific connectors; to obtain reference material for your connector, consult the desired connector's documentation.
