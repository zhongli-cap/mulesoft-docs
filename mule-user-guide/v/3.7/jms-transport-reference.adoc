...
NOTE: To set the `JMSCorrelationID`, use a transformer such as: +
`<set-property propertyName="MULE_CORRELATION_ID" value="1234" /> ` +
This creates the Outbound property which is then mapped to the `JMSCorrelationID` header. This header is then able to be consumed by any other non-Mule, but JMS compliant application. See also: <<Disable Reply Message>>, which helps if you are setting the correlation ID and want to avoid automatic responses.
