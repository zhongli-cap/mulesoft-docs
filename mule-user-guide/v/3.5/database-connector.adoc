...
After adding the database driver for a database engine not supported out of the box, you need to enter the fully qualified name of the driver class in the global element referenced by the database connector. For details, see hlink://mule-user-guide/v/3.5/database-connector#DatabaseConnector-generic[Configuring for Other Database Engines (Generic Configuration] below.
...
|*Driver Class Name*	|Fully-qualified driver class name of the driver for your database, which must be already imported into your project. (For details on importing the driver, see link://mule-user-guide/v/3.5/database-connector#DatabaseConnector-db_drv[Adding the Database Driver] above.) You can enter the full name in the empty field or click ... to browse the available driver classes. +
  +
When browsing the available driver classes, type the beginning of the driver class name (which you can check by clicking the driver file under Referenced Libraries in the Package Explorer). Studio will display the list of classes provided by the driver. +
 +
For a list of driver classes of commonly-used database engines, see link://mule-user-guide/v/3.6/database-connector#DatabaseConnector-drv_class[Common Driver Class Specifications] below.
