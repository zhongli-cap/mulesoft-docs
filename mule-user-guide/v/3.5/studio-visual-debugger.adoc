...
When you start Mule ESB in debug mode, _all_ applications that you launch in that server run in debug mode. Remote debugging does not work with link:/mule-user-guide/v/3.6/mule-high-availability-ha-clusters[clusters].
...
* link:/anypoint-studio/v/5/studio-in-eclipse[Eclipse plugin]
