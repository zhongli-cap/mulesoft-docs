...
* See DataSense in action! Examine two example applications which were built using DataSense: link:/anypoint-studio/v/5/Connect-with-Salesforce-Example[Connect with Salesforce Example] and link:/anypoint-studio/v/5/DataMapper-with-FlowRefLookup+Example[DataMapper with FlowRefLookup Example].
