...
This page describes how to use a standalone CXF-generated client with Mule. This approach is different from the link:consuming-web-services-with-cxf[outbound router] approach, which is typically a more appropriate fit for most applications.

[WARNING]
If you plan to connect to an external web service that exposes a WSDL file, the recommended connector to use is the link:web-service-consumer[Web Service Consumer].
...
Following is an example of how to construct a client using the service that was developed in link:building-web-services-with-cxf[Building Web Services with CXF]:
