...
* In addition to services, message processing can be configured using link:using-flows-for-service-orchestration[flows] and link:configuration-patterns[configuration patterns].
...
* The link:deprecated-http-transport-reference[HTTP transport] defines a new transformer element `body-to-parameter-map-transformer`, which transforms the body of an HTTP message into a map.
