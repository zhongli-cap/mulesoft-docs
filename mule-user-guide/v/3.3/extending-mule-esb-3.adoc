...
* A *module* is a package of related functionality in Mule, such as the XML module, which provides XML-based utilities like filters and routers. For a list of the available modules you can use, see link:modules-reference[Modules Reference].

* A *link:connecting-using-transports[transport]* is a type of module that carries messages between Mule flows via a specific protocol. For a list of the available transports you can use, see link:transports-reference[Transports Reference].

* An *example* is a sample application that you create to help users get up and running more quickly. Several link:mule-examples[examples] are provided with Mule.
...
When working with transports, note that you can link:configuring-a-transport[configure an existing transport], or you can link:creating-transports[create a new one]. The recommended approach is to try to use and configure an existing transport first.
...
The creation of custom configuration patterns can allow you to reach new levels of productivity with Mule. Follow this tutorial for link:creating-catalog-archetypes[starting to create your own patterns now].
