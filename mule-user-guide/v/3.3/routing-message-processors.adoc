...
Message Processors are used within flows to control how messages are sent and received within that flow. This is further described in link:https://docs.mulesoft.com/mule-user-guide/v/3.3/using-flows-for-service-orchestration[Using Flows for Service Orchestration].
...
The Message Filter is used to control whether a message is processed by using a link:https://docs.mulesoft.com/mule-user-guide/v/3.3/using-filters[filter]. In addition to the filter, you can configure whether to throw an exception if the filter does not accept the message and an optional message processor to send unaccepted messages to.
...
See link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-object-stores[Mule Object Stores] for further information about object stores in Mule.
