...
This document will help you understand some of the ways you can apply security to a SOAP Web service in Mule ESB applications. To understand more about Mule ESB’s ability to integrate services and systems, access the link:mule-examples[Mule examples] and see other applications in action.
...
. Complete the procedure in link:mule-examples[Examples and Exercises] to create, then run the Security template in Mule Studio, or the Security example in Mule Standalone (Mule ESB without Studio).
...
The request-response **link:http-endpoint-reference[HTTP Endpoint] **in this flow receives the end user request. Because it has a two-way message exchange pattern, this HTTP endpoint is responsible for both receiving and returning messages.
...
A JAX-WS service, the **link:soap-component-reference[SOAP Component]** in this flow evaluates the message according to its security configurations. In this case, the Web service is unsecure (see image below) so the SOAP component processes all requests it receives. +
...
The **link:java-component-reference[Java Component]** executes a simple script to prepare a personalized salutation for the end user.
...
* For more information on the SOAP Component, see link:soap-component-reference[SOAP Component Reference].
* For more information on WS-security, see link:enabling-ws-security[Enabling WS-Security].
