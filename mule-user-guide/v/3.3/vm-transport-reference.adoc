...
The VM transport can be used for intra-JVM communication between Mule link:mule-application-architecture[flows]. This transport by default uses in-memory queues but can optionally be configured to use persistent queues. (Note, however, that VM file persistency does not work on clusters.)
...
_One-way_ VM queues can take part in distributed link:transaction-management[XA Transactions]. To make a VM endpoint transactional, use a configuration like the following:
...
Using XA requires that you add a transaction manager to your configuration. For more information, see link:transaction-management[Transaction Management].
...
The transaction element configures a transaction. Transactions allow a series of operations to be grouped together so that they can be rolled back if a failure occurs. For more information, see link:transaction-management[Transaction Management].
...
Consider setting up your application following a link:reliability-patterns[reliability pattern].
