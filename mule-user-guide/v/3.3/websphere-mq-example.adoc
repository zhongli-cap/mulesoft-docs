...
. Complete the procedure in link:mule-examples[Examples and Exercises] to create WebSphere MQ template in link:mule-examples[Mule Studio], or the WebSphere MQ example in link:mule-examples[Mule Standalone] (Mule ESB without Studio). Create, but do not run the application.
...
. Start the application by following Step 8 in link:mule-examples[Examples and Exercises]. When the application starts you may see some exceptions in the output console, since WMQ is not configured (we will configure it in the next step).
