...
Install the link:installing-anypoint-enterprise-security[Anypoint Enterprise Security update] on your instance of Mule Studio Enterprise Edition 3.3.2.
...
* link:configuring-the-spring-security-manager[Mule Security Manager], client authentication and authorization on inbound requests as well as credential mapping for outbound calls
* link:setting-up-ldap-provider-for-spring-security[LDAP] and third party identity management system integration
* link:enabling-ws-security[Validation of inbound requests] through the SAML 2.0 federated identity standard
* link:sftp-transport-reference[Secure FTP (SFTP) Transport] that enables Mule flows to read and write to remote directories over the SSH protocol.
...
link:mule-secure-token-service[Apply Oauth 2.0 to my Mule Web service provider]

link:mule-secure-token-service[Apply Oauth 2.0 to my Mule Web service consumer]
...
link:mule-credentials-vault[Encrypt a properties file]
...
link:mule-message-encryption-processor[Encrypt a message payload]

link:mule-message-encryption-processor[Decrypt a message payload]
...
link:mule-digital-signature-processor[Add a digital signature to a message in Mule]

link:mule-digital-signature-processor[Sign part of a message payload]

link:mule-digital-signature-processor[Verify a digital signature in Mule]
...
link:mule-filter-processor[Filter unsecure messages]
...
link:mule-crc32-processor[Apply a CRC to a message]
