...
By default, Mule Studio will not log messages at the DEBUG or TRACE level to the console unless you create and configure a link:https://docs.mulesoft.com/mule-user-guide/v/3.3/logger-component-reference[`log4j.properties` file] in `src/main/resources` to lower the log level. |
[source, code, linenums]
----
level="INFO"
----
...
String |Optionally specify a category name and configure it in the link:https://docs.mulesoft.com/mule-user-guide/v/3.3/logger-component-reference[`log4j.properties file`] to behave per your use case. For example, you can route log messages based on category or set log levels based on category. |
[source, code, linenums]
----
...
* See the Logger in action in the link:https://docs.mulesoft.com/mule-user-guide/v/3.3/order-processing-example[Order Processing Example].
* Read more about link:https://docs.mulesoft.com/mule-user-guide/v/3.3/configuring-logging[Configuring Logging].
* Check the link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-expression-language-mel[Mule Expession Language] reference for more ideas on how to format helpful logger messages.
