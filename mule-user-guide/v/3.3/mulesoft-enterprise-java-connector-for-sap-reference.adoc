...
Port 3300 is predefined by SAP, so if you need to validate other port numbers based on your SAP instance number, you can check the complete list of service-to-port mappings, which can be found link:/mule-user-guide/v/3.3/sap-jco-server-services-configuration[here].
...
|jcoClientExtendedProperties-ref |A reference to `java.util.Map` containing additional JCo connection parameters. Additional information and a complete list of parameters can be found link:/mule-user-guide/v/3.3/sap-jco-extended-properties[here] | 
|===
...
[NOTE]
You can use link:/mule-user-guide/v/3.3/mule-expression-language-mel[Mule Expressions] inside the address attribute, just as you do for other Mule ESB transports.
...
|jcoServerExtendedProperties-ref |A Reference to `java.util.Map`, which contains additional JCo connection parameters. Additional information and a complete list of parameters can be found link:/mule-user-guide/v/3.3/sap-jco-extended-properties[here]. | 
|===
...
For more information, consult: link:/mule-user-guide/v/3.3/transactions-configuration-reference[Transactions Configuration Reference].
...
[source, code, linenums]
----
RfcException: [mc-vmware|a_rfc] message: (3) IDOC_ERROR_METADATA_UNAVAILABLE: The meta data for the IDoc type " with extension "  ORDSAPB6L B60CL" is unavailable.
...
In this example, JCo libraries are informing that the parameter with name _type_ is not valid. The complete list of valid property names can be found link:/mule-user-guide/v/3.3/sap-jco-extended-properties[here].
