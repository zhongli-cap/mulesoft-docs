...
. Complete the procedure in link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-examples[Examples and Exercises] to create, then run the Echo template in Mule Studio, or the Echo example in Mule Standalone (Mule ESB without Studio).
...
Next, the flow uses a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/logger-component-reference[Logger Component]** to log the message payload in the application’s log files. The logger component uses an** link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-expression-language-mel[Expression]** to determine what information in, or about, the message that it should log. In this case, because it needs to log the message payload, the instructions to log read `About to echo #[message:payload]`. +
 +
 image:logger_properties.png[logger_properties]

Finally, Mule moves the message to an **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/echo-component-reference[Echo Component] **which simply returns the message payload as the response to an end user. In other words, the response echoes the request.

== Related Topics

* For more information on using the Logger component, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/logger-component-reference[Logger Component Reference].
* For more information on the HTTP request-response endpoint, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/http-endpoint-reference[HTTP Endpoint Reference].
