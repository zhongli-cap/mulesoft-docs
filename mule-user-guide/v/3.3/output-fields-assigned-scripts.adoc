...
. Alternatively, click to drag any input field from the *Expression* pane or any function from the *Function* pane into the *Script* pane. (Refer to the link:output-fields-assigned-scripts#example-using-functions[Using Functions] example below for an illustration of this activity.)
...
[TIP]
For more details on using DataMapper's Preview feature to test your mapping, consult link:obtaining-a-preview-of-mapped-data[Obtaining a Preview of Mapped Data].
