...
This document will help you understand some of the ways you can integrate Web services in Mule ESB applications. To understand more about Mule ESB’s ability to integrate services and systems, access the link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-examples[Mule examples] and see other applications in action.
...
This document assumes that you are familiar with Mule ESB and the link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-studio-essentials[Mule Studio interface]. To increase your familiarity with Mule Studio, consider completing one or more link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-studio[Mule Studio Tutorials].
...
. Complete the procedure in link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-examples[Examples and Exercises] to create, then run, the Travel Agent template in Mule Studio.
...
The Mule ESB Travel Agent application contains multiple link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-application-architecture[flows and subflows] which cooperate to expose a *SOAP Web service*. The sections below offer flow-by-flow details of the Travel Agent Web service’s actions as it processes end user requests.
...
In Mule Studio, double-click a link:https://docs.mulesoft.com/mule-user-guide/v/3.3/studio-building-blocks[building blocks] in the canvas to open its *Properties Panel* , then examine its configuration details. Alternatively, click the *Configuration XML* tab to examine the application's XML configuration file.
...
The end user’s Web browser submits a request in http://en.wikipedia.org/wiki/JSON[JSON] format; the link:https://docs.mulesoft.com/mule-user-guide/v/3.3/ajax-endpoint-reference[*AJAX Endpoint*] receives the message, then passes it to the next **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-studio-essentials[message processor]** in the flow.

The TravelAgentRequest flow must, eventually, pass the message to the <<ConfirmReservations Flow>> flow via the request-response **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/http-endpoint-reference[HTTP Endpoint].** Because the Reservations flow only accepts XML messages, the TravelAgentRequest flow must first transform the data format of the message payload from JSON to XML. To perform the transformation, this flow uses an  **link:https://docs.mulesoft.com/anypoint-studio/v/6/datamapper-user-guide-and-reference[Anypoint DataMapper]**.

This flow next employs an **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/xslt-transformer-reference[XSLT Transformer]** to wrap the message in a SOAP envelope. Then, it uses the HTTP endpoint to push the message to the Reservations flow for processing.
...
The message source — an HTTP inbound endpoint — receives, then directs messages to a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/soap-component-reference[SOAP Component]** configured as a JAX-WS service. Mule unmarshalls the SOAP envelope into JAXB-annotated Java objects; Mule references these objects as parameters. From this point in the Reservations flow and beyond, the data format of message payloads is Java. (Note that Mule does not change the _content_ of a message’s payload, only its format.)

==== Flow References and Message Enricher

This flow invokes several **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-application-architecture[subflows]** in the Travel Agent application to book reservations and charge a commission.

Using a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/flow-ref-component-reference[Flow Reference Component]**, the Reservations flow invokes its first subflow to reserve seats on flights. The `Reserve flight` component sends the message to the ReserveFlight subflow to process, then enriches the message payload with the processing result.
...
Next, the Reservations flow passes the message through a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/message-filter[Message Filter]** to confirm that the preceding subflow identified a room. The message filter examines the message payload to determine whether Mule should process the message further.
...
Next, the **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/session-variable-transformer-reference[Session Variable Transformer]** adds the flight and hotel costs, then appends the total travel cost to the message as a session variable.
...
The last element in the flow, the `Create TravelResponse` **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/expression-transformer-reference[Expression Transformer]**, prepares a response to return to the SOAP component. Mule uses expressions to define the contents of the response (see image below); in this case, the expressions defines two:
...
Mule then subjects the message to a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/variable-transformer-reference[Variable Transformer]** which collaborates with a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/foreach[Foreach]** iterative processor to identify the least expensive room on the list.
...
This flow receives a message through a request-response *VM Endpoint*, then uses a flow reference component to invoke a subflow to authorize credit card payments. Once authorized to make payments, Mule passes the message to a set variable transformer. This transformer, `Set Rollback = CREDIT_CARD`, and the `Set Rollback = FLIGHT` transformer (further in the flow) collaborate with a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/choice-exception-strategy[Choice Exception Strategy]** to handle errors.
...
* If the variable contains a `CREDIT_CARD` value, the choice exception strategy directs the message through the first ** link:https://docs.mulesoft.com/mule-user-guide/v/3.3/catch-exception-strategy[Catch Exception Strategy] **which: +
...
In flows that conduct http://en.wikipedia.org/wiki/Transaction_processing[transactions], you would use a *link:https://docs.mulesoft.com/mule-user-guide/v/3.3/rollback-exception-strategy[Rollback Exception Strategy]* to handle errors (i.e. http://en.wikipedia.org/wiki/Rollback_(data_management)[rollback transactions]).
...
Both catch exception strategies in the ConfirmReservations flow invoke this *ThrowException* flow. Containing only a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/groovy-component-reference[Groovy Script Component]**, this flow follows a script to throw a `RuntimeException` and return a message that reads `Unable to confirm reservations`.
...
The `/commission` Web service passes requests through a request-response HTTP endpoint to a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/choice-flow-control-reference[Choice Flow Control]**. The flow control routes requests to one of two *Set Payload Transformers* to set a commission rate on the message.
...
Rather than use an expression transformer like the ConfirmFlight subflow, this subflow uses an link:https://docs.mulesoft.com/anypoint-studio/v/6/datamapper-user-guide-and-reference[Anypoint DataMapper] transformer to prepare a request for the hotels’ SOAP Web service.
...
* For more information on configuring the Anypoint DataMapper, see link:https://docs.mulesoft.com/anypoint-studio/v/6/datamapper-user-guide-and-reference[DataMapper Transformer Reference].
* For more information on using the JDBC endpoint, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/database-jdbc-endpoint-reference[Database (JDBC) Endpoint Reference].
* For more information on applying exception strategies to flows, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/error-handling[Error Handling].
* For more information on the specific exception strategies in this example, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/choice-exception-strategy[Choice Exception Strategy] and link:https://docs.mulesoft.com/mule-user-guide/v/3.3/catch-exception-strategy[Catch Exception Strategy].
* For more information on routing messages, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/choice-flow-control-reference[Choice Flow Control Reference].
* For more information on enriching messages, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/studio-scopes[Studio Scopes].
* For more information on setting variables on messages, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/variable-transformer-reference[Variable Transformer Reference] and link:https://docs.mulesoft.com/mule-user-guide/v/3.3/session-variable-transformer-reference[Session Variable Transformer Reference].
* For more information on caching Web service responses, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/cache-scope[Cache Scope].
* For more information on iterative processing, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/foreach[Foreach].
* For more information on configuring a SOAP component, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/soap-component-reference[SOAP Component Reference].
* For more information on filtering messages, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/message-filter[Message Filter].
