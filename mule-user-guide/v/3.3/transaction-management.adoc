...
For more details on the elements you configure for transactions, see link:transactions-configuration-reference[Transactions Configuration Reference].

For information on using exception strategies to determine whether a transaction gets committed or rolled back in case of an error, see link:error-handling[Error Handling with Transactions].
...
If you run Mule outside an application server, you can use link:jboss-transaction-manager-reference[JBoss Transaction Manager] to configure an embedded transaction manager.
...
* link:mule-wmq-transport-reference[Mule WMQ Transport Reference] (as of Mule Enterprise Edition 2.2)
...
, which is the default receiver on inbound JMS endpoints that use XA transactions. If you are using JBoss transactions, please read link:jboss-transaction-manager-reference[here] for information on how to configure the `timeout` value.
