...
For more about Mule transformers, see link:/mule-user-guide/v/3.3/using-transformers[Using Transformers].
...
Filters once again can be configured as global elements and referred to where they are used, or configured at their point of use.  For more about Mule filters see link:/mule-user-guide/v/3.3/using-filters[Using Filters]

==== Expressions

[WARNING]
The following description of expressions refers to the pre-Mule 3.3 expression evaluators. For a current reference to using expressions in Mule, see link:/mule-user-guide/v/3.3/mule-expression-language-mel[Mule Expression Language MEL].
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:/mule-user-guide/v/3.3/logger-component-reference[Logger Component Reference].
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see link:/mule-user-guide/v/3.3/routing-message-processors[Routing Message Processors].
...
❶❷ This is a Java component, specified by its class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see link:/mule-user-guide/v/3.3/entry-point-resolver-configuration-reference[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

❾ calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about connectors, see link:/mule-user-guide/v/3.3/anypoint-connectors[Anypoint Connectors].
...
For more about configuration patterns, see link:/mule-user-guide/v/3.3/using-mule-configuration-patterns[Using Mule Configuration Patterns] .
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see link:/mule-user-guide/v/3.3/transaction-management[Transaction Management].
...
For more about Mule security, see link:/mule-user-guide/v/3.3/configuring-security[Configuring Security].
