...
|image:http.png[http] |HTTP |Sends and receives messages via the HTTP transport protocol. Turn on security to send HTTPS messages via SSL. |one-way, *request-response* |link:https://docs.mulesoft.com/mule-user-guide/v/3.4/http-endpoint-reference[HTTP Reference]
