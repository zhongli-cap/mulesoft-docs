...
Access the link:embedding-mule-in-a-java-application-or-webapp[Bookstore example application] to witness Mule as an embedded as a web application inside a Servlet container.

== Interacting with Mule from Your Code

To interact with the Mule server from your application, JSP, or servlet, you can use the link:using-the-mule-client[Mule Client]
