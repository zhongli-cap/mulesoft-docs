...
This example should enable you to create an application that incorporates Java-platform scripts. To understand more about Mule ESB’s ability to integrate services and systems, access the Mule link:mule-examples[examples] and see other applications in action.
...
. Complete the procedure in link:mule-examples[Examples and Exercises] to create, then run the Scripting template in Mule Studio, or the Scripting example in Mule Standalone (Mule ESB without Studio).
...
The *Scripting* example application uses a link:mule-application-architecture[flow] which processes end user HTTP requests.
...
The *HTTP-inbound* flow makes use of several link:studio-building-blocks[building blocks] to process and respond to an end user request.
...
Mule ESB supports *Global Elements* within applications, such as the *Body to Parameter Map Transformer* and the `StringToNumber` *link:java-component-reference[Java Component]* in the Scripting example.
...
With its newly transformed content, the message next encounters the **link:choice-flow-control-reference[Choice Flow Control]**. This content-based router checks the payload of the message to discover the currency of the content, then makes a decision about where to route it:
...
. the USD path that ends with a *link:groovy-component-reference[Groovy Script Component]*
. the GBP path that ends with a **link:python-component-reference[Python Script Component]**  +
...
* For more information on using the Choice Flow Control, see link:choice-flow-control-reference[Choice Flow Control Reference].
* For more information on transformers, see link:studio-transformers[Studio Transformers].
* For more information on scripts, see link:studio-components[Studio Components].
