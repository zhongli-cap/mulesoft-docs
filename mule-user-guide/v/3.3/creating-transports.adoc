...
Each transport has a service descriptor that describes what classes are used to construct the transport. For complete information, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/transport-service-descriptors[Transport Service Descriptors].
...
Any exceptions messages used in your transport implementation should be stored in a resource bundle so that they can be link:https://docs.mulesoft.com/mule-user-guide/v/3.3/internationalizing-strings[internationalized]. The message bundle is a standard Java properties file and must be located at:
