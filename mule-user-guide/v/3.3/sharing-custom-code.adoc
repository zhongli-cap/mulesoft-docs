...
* Custom transformers - performing operations that none of the Mule stock transformers can perform (see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/creating-custom-transformers[here]), +
* Custom components - typically Mule-aware or non-business oriented components, as business components are usually simple POJOs coming from pre-existing projects (see: link:https://docs.mulesoft.com/mule-user-guide/v/3.3/developing-components[here]), +
* Custom expression evaluators - for the cases when a specific message extraction capacity is needed in several places of an application (see: link:https://docs.mulesoft.com/mule-user-guide/v/3.3/creating-non-mel-expression-evaluators[here]).
