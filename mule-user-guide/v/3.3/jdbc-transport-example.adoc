...
This example should enable you to create applications which input, retrieve and use data from a JDBC database. To understand more about Mule ESB’s ability to integrate services and systems, access the Mule link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-examples[Mule Examples] and see other applications in action.
...
. Complete the procedure in link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-examples[Examples and Exercises] to create, then run the JDBC template in Mule Studio.
...
. Complete the procedure in link:https://docs.mulesoft.com/mule-user-guide/v/3.3/mule-examples[Examples and Exercises] to create, then run the JDBC example in Mule Standalone (Mule ESB without Studio).
. Identify an Oracle or Sybase database to which you have access, and which you don’t mind populating with some sample data for the purpose of demonstrating this example application.
...
In Mule Studio, double-click an icon representing a link:https://docs.mulesoft.com/mule-user-guide/v/3.3/studio-building-blocks[building block] to open its *Properties Panel* and examine the configuration details. Alternatively, you can click the *Configuration XML* tab to access and examine the application's XML config file.
...
After receiving and converting the message payload with the Ajax endpoint, the flow uses a request-response **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/database-jdbc-endpoint-reference[JDBC Endpoint]** to save CSV data to the database. In this case, the CSV data contains information about Mules that live on end user’s ranches, such as the Mule’s name, color and weight.
...
The link:https://docs.mulesoft.com/mule-user-guide/v/3.3/jdbc-transport-reference[JDBC connector] — a global element in the JDBC example application — helps the JDBC Endpoint with its tasks (listed above).

. As a link:https://docs.mulesoft.com/mule-user-guide/v/3.3/connecting-using-transports[connector], it helps the JDBC endpoint access the database by providing specific details about how to connect to it, such as where it exists (the URL), and how to gain access to it (username and password for database). See image below, left.
...
Notice that the CSVLoader flow also contains a **link:https://docs.mulesoft.com/mule-user-guide/v/3.3/rollback-exception-strategy[Rollback Exception Strategy]**.
...
* For more information on transformers, see link:https://docs.mulesoft.com/mule-user-guide/v/3.3/studio-transformers[Studio Transformers].
