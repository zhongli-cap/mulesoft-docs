...
This example will help you to make informed decisions about incorporating iterative (also known as loop-type) processing into your Mule applications. To understand more about Mule ESB’s ability to integrate services and systems, access the Mule link:mule-examples[examples] and see other applications in action.
...
. Complete the procedure in link:mule-examples[Examples and Exercises] to create, then run, the Foreach template in Mule Studio, or the Foreach example in Mule Standalone (Mule ESB without Studio).
...
The *Foreach* example application contains two link:mule-application-architecture[flows].
...
In Mule Studio, double-click a link:studio-building-blocks[building block] to open its *Properties Panel* and examine its configuration details. Alternatively, click the *Configuration XML* tab to examine the application's XML configuration file.
...
This flow uses several link:studio-building-blocks[building blocks] to receive, process and respond to end user requests.
...
The request-response **link:http-endpoint-reference[HTTP Endpoint] **receives the end user request. Because it has a two-way message exchange pattern, this HTTP endpoint both receives requests from, and sends responses to, the end user.

The message then moves to a **link:database-jdbc-endpoint-reference[JDBC Endpoint]** which extracts a collection of records from the database; in this case, each record contains information about a car in the end user’s inventory:
...
Mule then subjects the message to a **link:foreach[Foreach]** iterative processor. With the help of a **link:choice-flow-control-reference[Choice Flow Control]**, foreach identifies which vehicles are 20th century cars, and which are 21st century cars.
...
Next, foreach delivers each element to an *link:studio-endpoints[Outbound Endpoints].*
...
* elements with a `21st century car TYPE` move through a **link:file-endpoint-reference[File Endpoint] **to record the information in an external file system.
...
Notice that the Process flow also contains a **link:catch-exception-strategy[Catch Exception Strategy]**. Rather than use Mule’s link:error-handling[default exception strategy], this flow uses a customized exception strategy to handle errors. If an error occurs, the exception strategy catches the exception and its message processors perform three actions:
...
As in Process flow, the request-response **link:http-endpoint-reference[HTTP Inbound Endpoint] **in the Populate flow receives the end user request. The message then moves to a **link:groovy-component-reference[Groovy Script Component]** which creates a table in a database, and populates the table with information about cars (see script, below).
