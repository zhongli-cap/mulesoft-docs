...
This document assumes that you are familiar with link:essentials-of-using-mule-esb-3[Mule ESB] and the link:mule-studio-essentials[Mule Studio] interface. To increase your familiarity with Studio, consider completing one or more link:getting-started-with-mule-studio[Mule Studio Tutorials].
...
* For more information on encryption in Mule, refer to link:mule-message-encryption-processor[Mule Message Encryption Processor].
* For more information on signatures in Mule, refer to link:mule-digital-signature-processor[Mule Digital Signature Processor].
