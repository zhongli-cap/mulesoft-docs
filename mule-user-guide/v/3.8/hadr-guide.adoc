...
Planned or unplanned, infrastructure and application downtime can come at any time, from any direction, and in any form. The ability to keep a company operational during a technology outage, facility destruction, loss of personnel or loss of critical third party services is critical to preventing irreversible damage to a business. With the increasing global shift to e-commerce models and their reliance on 24/7 application uptime, high availability (HA) and disaster recovery (DR)  impact the financial health of companies. 

The following table shows how uptime percentages map to time lost -- Even the maximum uptime can still cause critical damage to data:
...
On the other hand, disaster recovery (DR) is the process by which a system is restored to a previous acceptable state, after a natural (flooding, tornadoes, earthquakes, fires, etc.) or man-made (power failures, server failures, misconfigurations, etc.) disaster. 

While they both increase overall availability, a notable difference is that with HA there is generally no loss of service. HA retains the service and DR retains the data, but with DR, there is usually a slight loss of service while the DR plan  executes and the system is restored.
...
|All traffic has been routed to new environment.
|image:hadr-zero-downtime-3.png[hadr-zero-downtime-3]

|All traffic has been routed to the new environment so it has been promoted to the baseline production environment; previous baseline environment has been terminated.
|image:hadr-zero-downtime-4.png[hadr-zero-downtime-4]
|===

*Costs*

This deployment method may temporarily (could be a few mins to a few hours to a few days) add capacity to the service. 
...
If the application uses multiple workers, CloudHub by default deploys them in separate availability zones providing HA across availability zones. The distance between the availability zones is variable and in general it cannot be assumed that they are 350 miles or more apart.

image:hadr-am-web-services.png[hadr-am-web-services]

If an application uses a single worker, when the availability zone comes down, it  needs to be brought up manually. Alerting can be set up within `status.mulesoft.com` to receive alerts when any failure occurs at an availability zone or region level.
