...
To ensure reliable connectivity between cluster nodes, all nodes of a cluster should be located on the same LAN. Implementing a cluster with nodes across geographically separated locations, such as different datacenters connected through a VPN, is possible but not recommended.
...
*Distributed data centers clustering*
...
It's possible to use nodes of a cluster located in different datacenters and not necessarly located on the same LAN but some restrictions apply. 

To prevent this behaviour it's necessary to enable the *Quorum Protocol*. This protocol it's used to only allow one set of the nodes to continue processing data while the other part will not do anything with the shared data until they reconnect. Basically when a disconnection occurs only the portion with the higher number of nodes will continue to function. For instance, if we have two data center, one with three nodes and another one with two nodes, if there's a connectivity problem in the two nodes data center, then the three nodes data center will continue to function and the second data center won't. If you paid attention you may be thinking What happens if the three nodes data center goes offline?. In that case non of your nodes will be functional. To prevent this from happening you must create the cluster in at least three data center with the same amount of nodes. The likelihood of two data center crashing it's unlikely so if just one data center goes offline then the cluster will always be functional.

IMPORTANT: The cluster partition with no enough nodes to function will continue reacting to external system calls but all operations done over the object stores will fail preveting generating data.

.Limitaitions
* Quorum is only support in Object Store related operations. 
...
* In memory messaging is not suported which affects:
  - VM transport
  - SEDA queues
* Quorum feature can only be configured manually. See link:creating-and-managing-a-cluster-manually[Creating and managing a cluster manually]
...
* If your application processes messages from a non-transactional transport, use a link:/mule-user-guide/v/3.8/reliability-patterns[reliability pattern] to move them to a transactional store such as a VM or JMS store.
...
* Implement link:/mule-user-guide/v/3.8/reliability-patterns[reliability patterns] to create high reliability applications.
...
* link:/mule-user-guide/v/3.8/installing-an-enterprise-license[Install an Enterprise License] to start managing clusters in production.
