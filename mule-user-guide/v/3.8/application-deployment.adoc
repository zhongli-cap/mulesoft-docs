...
Mule runtimes run independently with the Management pane to executes integration logic and serve APIs. This architecture allows for the runtimes to be deployed strategically (on-prem and cloud) and ensures Mule is not a bottleneck for any communications. When/if an event occurs and the runtimes get disconnected from the Management pane they continue to running as designed and execute integration and/or serve API. However new or updated policies will not be pulled and updated until the connection is reestablished.
