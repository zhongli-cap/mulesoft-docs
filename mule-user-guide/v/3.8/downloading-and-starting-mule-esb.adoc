...
[NOTE]
--
Depending on your Gatekeeper configuration, OSx might throw an error message with the description: *Application is damaged and can't be opened. You should move it to the trash*. +
Gatekeeper displays this message because the application bundle is not digitally signed. It's recommended for you to follow the steps described above to disable Gatekeeper and run Studio for the first time. +
