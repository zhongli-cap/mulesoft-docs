...
In Mule 3.6 and newer, you can use link:/mule-user-guide/v/3.7/database-connector-reference#jdbc-data-types[JDBC data types] with the Database connector. This feature provides the ability to use JDBC supporteed types in a query; for example, array, struct, stored procedure parameters, and you can also reference these types from result sets.
...
In the case of struct values, the database connector returns `java.sql.Struct`. In order to obtain the information, invoke the `getAttributes` method of this Java type using MEL.
