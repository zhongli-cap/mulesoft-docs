...
To use a JDBC data type, define the type in the database configuration with the type ID corresponding to the structured data type. Here in the example below you can see `java.sql.STRUCT` is referenced using ID of 2002, and `java.sql.ARRAY` as 2003:
...
[NOTE]
The value returned in a result field can be used as an input value in another query. You can mine into the resultSet message using MEL and or the For Each component. For example, this MEL expression, `#[payload[0]['resultsetID']]` would give the first row. Omit the `[0]` index to get the whole result set.
