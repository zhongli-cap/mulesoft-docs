...
In Mule 3.6 and newer, you can use user-defined data types with the Database connector. This feature provides the ability to use JDBC supported types in a query; for example, array, struct, and other types. You can also reference these types from a `resultSet` when querying a database, e.g. this MEL expression would give the first row,`#[payload[0]['resultsetID']]`. Omit the `[0]` index to get the whole result set.
