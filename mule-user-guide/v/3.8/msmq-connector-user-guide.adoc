...
The MSMQ API used by the connector requires to specify the target address using the `Format Names` style mostly when working with remote queues, as well as queues having custom configuration options, and `Direct Format Names` is the recommended way:
...
For detailed information on how to address MSMQ please refer to the following link:https://msdn.microsoft.com/en-us/library/ms700996(v=vs.85).aspx[documentation].
