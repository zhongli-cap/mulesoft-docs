...
The Microsoft Dynamics 365 Customer Relationship Management (CRM) connector lets 
...
For OAuth 2.0, a patch for the Mule Runtime is needed, that can be provided upon request. There is an issue with the Mule DevKit Support module described in link:https://www.mulesoft.org/jira/browse/SE-5166[SE-5166] that can only be fixed with a Patch.
...
** Token Request Endpoint - The token endpoint that is called to get the access token. Example: `+https://login.windows.net/TENANT_ID/oauth2/token+` where TENANT_ID is the Azure AD ID.
+
...
* xref:authop[Authorize - only for OAuth 2.0 Connection]
* xref:unauthop[Unauthorize - only for OAuth 2.0 Connection]
...
. Set Operation to Authorize (only works with OAuth 2.0 Connection).
...
. Provide the endpoint (required) that issues the token: Example: `+https://login.windows.net/TENANT/oauth2/token+`, where TENANT is the Azure AD ID.
. Provide the Authorization URL (required) - Indicates where the resource owner is redirected to grant authorization to the connector. Example: https://login.microsoftonline.com/TENANT/oauth2/authorize, where TENANT is the Azure AD ID.
...
. Set Operation to Unauthorize (only works with OAuth 2.0 Connection).
