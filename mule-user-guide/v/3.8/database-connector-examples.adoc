...
Run this example to update a "bonus" field in a table where salary and bonus amounts are stored. This is not a recommended approach for updating a database, especially of this sort, but may prove useful for generating ideas on how to call stored procedures from your Mule applications.
...
. Here is the full code for a simple app that calls a stored procedure using an http parameter we set in the HTTP request.
+
[source,code,linenums]
----
