...
To instruct DataWeave how to process the MySQL data, right click the words "Payload" on the left side of the pane and click *Set Metadata*.

image:database-connector-examples-ab246.png[set metadata]

Define the metadata as *Map*, comprised of *List<Element>*, adding the attributes and their types per the database table attributes.
...
The resulting DataWeave code is auto-generated, and appears next to the above mapping, appearing like this:
...
Run this example to update a "bonus" field in a table where salary and bonus amounts are stored. This is not a recommended approach for updating a database, but may prove useful for generating ideas on how to call stored procedures from your Mule applications.

=== Set up "salary" Table

. First set up a new table in your MySQL database, `salary`, with the primary key as `emp_no`, with a `salary_amount` and `bonus` entry for each person.
+
----
CREATE TABLE salary (
emp_no INT NOT NULL,
...
----
+
. Create a stored procedure to update a bonus amount based on employee ID.
+
----
...
----
+
. Call this procedure to add an amount (for this example $500) to an employee's base bonus based on the employee's ID, `emp_id` as the first argument, with the second argument being the amount to add to the current bonus.
+
----
call add_bonus_by_emp_id(#[payload],500);
----
+
. Here is the full code for a simple app that calls a stored procedure using an http parameter we set in the HTTP request. `http://localhost:8081/getemps?emp=1065`
+
[source,code,linenums]
----
...
http://www.mulesoft.org/schema/mule/ee/dw http://www.mulesoft.org/schema/mule/ee/dw/current/dw.xsd">
    ...
    <db:mysql-config name="MySQL_Configuration" host="localhost" port="3306" user="root" password="testymule" database="company" doc:name="MySQL Configuration"/>
    ...
    <flow name="stored-proc-exFlow">
        ...
        <db:stored-procedure config-ref="MySQL_Configuration" doc:name="Database">
            <db:parameterized-query><![CDATA[call add_bonus_by_emp_id(#[payload],500);]]></db:parameterized-query>
        </db:stored-procedure>
