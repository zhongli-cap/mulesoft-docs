...
See how to configure a proxy request to the SOAP service in a Mule application. Let's start with this flow that grabs a country name as an HTTP parameter and stores it in a variable for the SOAP service request to a weather data service that spits back all cities for which their is data for the country given.
...
The proxy configuration would look similar to this XML block in your Mule application:
...
Find here the example flow for reference.
...
[TIP]
--
For a full list of elements, attributes and default values, consult the link:/mule-user-guide/v/3.8/web-service-consumer-reference[Web Service Consumer Reference].
--
+
