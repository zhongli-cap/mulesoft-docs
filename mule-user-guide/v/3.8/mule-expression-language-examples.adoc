...
This document assumes you are familiar with Mule Expression Language's link:/mule-user-guide/v/3.8/mule-expression-language-basic-syntax[basic syntax] and are comfortable building and running Mule applications using link:/mule-fundamentals/v/3.8/anypoint-studio-essentials[Mule Studio] or XML.

If you aren't yet familiar with how to access information about the Mule messages that pass through your applications, consider following this link:/mule-fundamentals/v/3.8/mule-message-tutorial[tutorial], which walks you through both examining your Mule message and its data structure and writing simple MEL expressions.
...
|*port* |`8081`
|*path* |greet1
|*http://docname[doc:name]* |`HTTP_Listener_Configuration`
|===
+
...
In this example, like in the previous one, the Mule application saves a CSV file with user data and returns a greeting. The call to the service includes two parameters, `username` and `age`. The service stores these two parameters and adds a third boolean parameter that evaluates if the user is above a certain age (if `age > 18`). In this case, however, the mapping between input and output fields and the generation of the third field is performed by a link:/mule-user-guide/v/3.8/datamapper-user-guide-and-reference[DataMapper] component.
...
. Open the HTTP connector's properties editor and give it the path `greet4` and select the global element you previously created in the Connector Config field.
. You access the connector with the `http://localhost:8081/greet4` URI.
. In the expression component, set the following MEL expression:
+
[source]
----
...
+
** In the Input, keep the default *Map<k,v>* type, and select the *User Defined* structure.
** In the Output, select the *CSV* type andthe *User Defined* structure.
+
...
|===
+
The connector can then be reached via the `http://localhost:8081/greet4` URI.
+
. After the connector, add an expression component that uses a MEL expression to record the inbound property `username` in a flow variable of the same name.
+
[source, xml, linenums]
----
<expression-component doc:name="Expression"><![CDATA[flowVars.username = message.inboundProperties.'http.query.params'.username]]>
...
+
[TIP]
Because the MEL expression is in an *expression component*, don't surround the expression with `#[]` brackets.
+
...
* In the Output, select the *CSV* type and the *User Defined* structure.
+
image:mel+ex+datamapper.png[mel+ex+datamapper]
+
...
[source, xml, linenums]
----
include::_sources/mule-expression-language-examples_4.xml[]
----
+
. Save and run your project.
. In a browser, access the URL `http://localhost:8081/greet4?username=yourName&age=22` +
...
[TIP]
For your convenience, you may download the link:_attachments/mel_example_project.zip[complete project]. Note that this project is configured specifically for the *Mule 3.6.0 runtime* and newer. Refer to the link:/mule-user-guide/v/3.5/mule-expression-language-examples[Mule Expression Language Examples - 3.5] documentation for a 3.5.X compatible version.
...
* Access the Mule Expression Language link:/mule-user-guide/v/3.8/mule-expression-language-reference[Reference] and link:/mule-user-guide/v/3.8/mule-expression-language-tips[Tips].
* Want to learn more about how to get information about the Mule message so that you can work with it using MEL? Try out this link:/mule-fundamentals/v/3.8/mule-message-tutorial[tutorial]. 
