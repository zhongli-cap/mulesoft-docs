...
In order for DataWeave to process the file stream, the input must enter DataWeave as a stream. For example, flows that start with a link:/mule-user-guide/v/3.8/file-transport-reference[file transport inbound endpoint] must have streaming enabled.
