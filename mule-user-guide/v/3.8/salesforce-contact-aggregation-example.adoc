...
Each Salesforce connector references a Salesforce link:/mule-user-guide/v/3.8/global-elements[global element], which is configured with the connection parameters (user ID, password, etc.) for each Salesforce organization. To see the configuration for each global element, see the <<Salesforce Global Elements>> section.
...
* Learn more about link:/mule-user-guide/v/3.8/mule-message-structure[flow variables].
