...
When you need to restrict the access to your resources, and you set a security policy with permission just to the resource/s you want to access, the connector cannot perform that connectivity test when it is starting up as it targets a standard endpoint which might be forbidden by the selected policy on the 'shared access key'. For these scenarios, you need to skip the connectivity test with the configuration option available to this purpose.
