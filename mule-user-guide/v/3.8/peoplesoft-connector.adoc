...
----
NOTE: In this example, all input parameters for the FIND operation are optional. If none of them are defined (`http://localhost:8081/find?id=&name=&last_name=&name_ac=`), then PeopleSoft retrieves the first 300 records available (the maximum limited by the server).
...
* For additional technical information regarding the PeopleSoft Connector, visit the link:http://mulesoft.github.io/peoplesoft-connector[github.io documentation].
