...
[IMPORTANT]
Do *NOT* change the names of any of the SAP JCo library files from their original names, as they won’t be recognized by JCo. Since *JCo 3.0.11*, the JAR file *cannot be renamed* or repackage from `sapjco3.jar`. Read more about link:/mule-user-guide/v/3.8/sap-connector-troubleshooting[SAP Troubleshooting].
...
[NOTE]
*Stateful transactions*, involving multiple outbound endpoints, only work from *Mule 3.3 and up*, by setting the *transactional scope*. Read more about link:/mule-user-guide/v/3.8/sap-advanced-features#transactions[SAP Transactions].
...
|*Address* |`address` |The standard way to provide endpoint properties. | For more information see link:/mule-user-guide/v/3.8/sap-advanced-features#endpoint-address[Endpoint Address]. 
...
[NOTE]
With *DataSense* support, the recommended way to generate the XML definitions is using link:/mule-user-guide/v/3.8/dataweave[DataWeave]. However, if you are using a *Mule 3.3* application, see  link:/mule-user-guide/v/3.8/datamapper-user-guide-and-reference[DataMapper].
...
If you intend to employ an SAP endpoint in conjunction with a link:/mule-user-guide/v/3.8/datamapper-user-guide-and-reference[DataMapper] or link:https:/mule-user-guide/v/3.8/dataweave[DataWeave] transformer to map and transform data, you can make use of Anypoint Studio's link:/mule-user-guide/v/3.8/datasense[DataSense] functionality.
