...
* Securiry token obtained from Microsoft Online
...
If case where is not possible to configure a connection strategy that depends on username/password, the security token connection strategy allow using a security token obtained from a Security Token Service (STS). This configuration requires the following information to use for authenticating:

* *Security Token*: (Optional) The security token (full XML document) obtained from your STS to authenticate the connection with your Sharepoint instance. If you don't configure the security token in this setting, you need to provide a `Security token provider` as defined in the setting detailed below.
* *Instance type*: Indicates if the connector should connect to an On-premises or an On-line instance as each one provides different endpoints and methods to provide the security token and authenticat the connection.

Additionaly to the basic configuration shown above, there is an advanced setting where you can provide a security token provider instance of your own. This mechanism allows you to obtain the security token using your own custom implementation and allows the connector refreshing the connection when the token has expired (which it is not the case for when just providing the `Security Token` as XML document).

* *Security token provider*: (Optional) This parameter should be reference to spring bean which should be an object that implements the `org.mule.module.sharepoint.connection.providers.SecurityTokenProvider` interface. Within the implementation you are in charge of providing a security token each time the connector requests for it. In order to provide a reference to your own implementation as a srping bean please refer to the following link:/https://docs.mulesoft.com/mule-user-guide/v/3.7/using-spring-beans-as-flow-components[documentation].
