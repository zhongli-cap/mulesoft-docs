...
* Securiry token obtained from Microsoft Online STS
...
* *Security token provider*: (Optional) A reference to an instance that implements the `org.mule.module.sharepoint.connection.providers.SecurityTokenProvider` interface where you are in charge of providing a security token each time the connector requests for one. In order to provide a provider as a srping bean please refer to the following link:/https://docs.mulesoft.com/mule-user-guide/v/3.7/using-spring-beans-as-flow-components[documentation].
