...
* If you are using Windows, download a curl command capable of posting JSON data to your application.
...
From a command line prompt, use the *curl* command to create a URL to send to Object Store. 
...
* The key name is translated into serialized base64 format. You can decode the key name using a site such as link:https://www.base64decode.org/[base64decode.org] or from a Chrome browser extension. The translated key string contains 5 bytes of initial data you can ignore:
+
image:osv2-base64-decode.png[osv2-base64-decode]
====
+
. Click the Object Store name. You can then click a key name to view its value.
