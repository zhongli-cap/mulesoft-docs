...
|===
|Property |Usage
|Requests |The requests are taken from the message Payload. To set these parameters correctly for each operation, a data mapper is typically used before the connector is called.
...
*Output*:  `BulkOperationResult<OrganizationResponse>` object (from the link:https://msdn.microsoft.com/en-us/library/microsoft.xrm.sdk.organizationresponse.aspx[OrganizationResponse] hierarchy tree) containing the results of the method executed.
