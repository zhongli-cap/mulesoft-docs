...
*Input*: In addition of using a POJO (from the link:https://msdn.microsoft.com/en-us/library/microsoft.xrm.sdk.organizationrequest.aspx[OrganizationRequest] hierarchy tree) as `Request Parameters`, this operation supports providing the payload as a Map<String, Object> (which is the most efficient/compact way of usage).
