...
The NetSuite connector v7.0.0 is compatible with:
...
Contact the link:https://www.mulesoft.com/lp/contact[MuleSoft Sales Team] to obtain either of these. (Read more about link:/mule-user-guide/v/3.8/installing-an-enterprise-license[Installing an Enterprise License].) 
...
In this release, we have upgraded the NetSuite connector to DevKit 3.7.2. As in the previous release we use Connection Strategies. Not much has changed on this front, and as before (v6.0.0) you are presented with a number of different Global Elements, each representing a different way of authenticating to NetSuite. The following connection strategies are supported now:
...
. In the Choose Global Type wizard, expand  *Connector Configuration,* and then select  *NetSuite: Login Authentication, NetSuite: Request Level Authentication,* or *NetSuite: SSO Login Authentication,* depending on your NetSuite authentication.
+
...
        keyFile="The name of the privateKey file" doc:name="Netsuite" applicationId="Your integration record applicationId">
</mule>
----
+
...
NetSuite connector is an operation-based connector, which means that when you add the connector to your flow, you need to configure a specific web service for the connector to perform. The NetSuite connector v7.0.0 supports 50 operations.
...
In NetSuite one can add different types of custom fields and have these customizations apply to different record types. NetSuite Connector DataSense is able to retrieve and show these fields. Note however that we do not fully support all the customization types that NetSuite users are able to define.
...
*_Transaction Item Options_*

Currently these are not handled by NetSuite Connector's DataSense.

*_Item Number Fields_*

Currently these are not handled by NetSuite Connector's DataSense.

*_Other Custom Fields_*
...
 
<netsuite:config-login-authentication name="NetSuite__Login_Authentication" email="${netsuite.email}" password="${netsuite.password}" account="${netsuite.account}" roleId="${netsuite.roleId}" applicationId="${netsuite.applicationId}" doc:name="NetSuite: Login Authentication"/> (1)
 
...
    <netsuite:async-add-list config-ref="NetSuite__Login_Authentication" recordType="__customRecordType__customrecordcustomaccount__22" doc:name="Async Add List"> (3)
...
    <netsuite:get-async-result config-ref="NetSuite__Login_Authentication" jobId="#[flowVars.jobId]" doc:name="Get Async Result"/> (11)
...
In NetSuite Connector version 7.0.0 we added pagination support to `search`.

We agreed to unify searching under just one operation and remove all the other search-related processors. Thus, `search` will always retrieve the whole set of results. Therefore users will not need to work with `searchNext` or `searchMore` in order to get the rest of the records from subsequent pages. The return type is also different; the processor will output a List of Maps representing each and every record obtained by your search criteria.
