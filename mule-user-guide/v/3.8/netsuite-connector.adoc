...
image:netsuite-connector-icon.png[NetSuite Connector Icon]
...
Read through this user guide to understand how to set up and configure a basic flow using the connector. Track feature additions, compatibility, limitations and API version updates with each release of the connector using the link:/release-notes/netsuite-connector-release-notes[Connector Release Notes]. Review the connector operations and functionality using the link:/mule-user-guide/v/3.8/netsuite-apidoc[Technical Reference] alongside the link:https://www.mulesoft.com/exchange#!/?filters=Netsuite&sortBy=rank[demo applications].
...
This document assumes that you are familiar with Mule, link:/mule-user-guide/v/3.8/anypoint-connectors[Anypoint Connectors], and the link:/mule-user-guide/v/3.8/anypoint-studio-essentials[Anypoint Studio interface]. To increase your familiarity with Studio, consider completing one or more  link:/mule-user-guide/v/3.8/basic-studio-tutorial[Anypoint Studio Tutorials]. Further, this page assumes that you have a basic understanding of link:/mule-user-guide/v/3.8/mule-concepts[Mule flows] and link:/mule-user-guide/v/3.8/global-elements[Mule Global Elements]. 
...
To use the NetSuite connector in your Mule application, you must configure a global NetSuite element that can be used by all the NetSuite connectors in the application (read more about link:/mule-user-guide/v/3.8/global-elements[global elements]).
...
+
image:user-manual-26ef5.png[NetSuite choose global elem]
+
. Click *Ok*.
...
NetSuite connector is an operation-based connector, which means that when you add the connector to your flow, you need to configure a specific web service operation for the connector to perform.

See the full list of operations for the latest connector in the latest link:/mule-user-guide/v/3.8/netsuite-apidoc[API reference]. All versions of the connector reference can be accessed from link:http://mulesoft.github.io/netsuite-connector/[here].
...
image:user-manual-c4bcf.png[flow 1]
...
|*Connector Configuration* |Configure a global element with the desired host and port, we use 0.0.0.0 and 8081 respectively
|*Path* |/addEmployee
|===
+
...
+
image:user-manual-803b2.png[add employee config]
+
[%header,cols="30,70"]
|===
|Field |Value
...
. From a browser, enter the employee's e-mail address, externalId, lastname, and name in the form of the following query parameters:
+
...
[source, code, linenums]
----
http://localhost:8081/accountWithCustomFields?email=<EMAIL_ADDRESS>&externalId=<EXTERNAL_ID>&lname=<LAST_NAME>&name=<FIRST_NAME>
----
. Mule conducts the query, and adds the Employee record to NetSuite.
...
http://www.mulesoft.org/schema/mule/ee/tracking http://www.mulesoft.org/schema/mule/ee/tracking/current/mule-tracking-ee.xsd">
	<netsuite:config-login-authentication name="NetSuite" email="${email}" password="${password}" account="${account}" roleId="${roleId}" applicationId="${applicationId}" doc:name="NetSuite: Login Authentication"/>
...
image:user-manual-d6887.png[netsuite_async1]
...
image:user-manual-a28c3.png[basic search]
...
image:user-manual-fbc6a.png[custom java component]
...
image:user-manual-8d66f.png[netsuite_code_example_search_advanced]
...
image:user-manual-restlet-calls.png[netsuite_code_example_restlet_calls]
