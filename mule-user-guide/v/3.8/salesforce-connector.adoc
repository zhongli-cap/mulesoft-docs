...
*Note:* You can use the `payload.success` flag to determine if the external Salesforce system is working correctly. If `payload.success` is `true`, the external Salesforce system is working correctly. If `false`, there is an issue in the external Salesforce system (not a failure in the connector) and it is a customer's responsibility to treat this issue.
