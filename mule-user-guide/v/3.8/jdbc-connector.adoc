...
NOTE: The JDBC connector supports multi-resource transactions. Multi-resource transactions are also supported by the link:/mule-user-guide/v/3.8/database-connector[Database Connector]. Consider also the use of link:/mule-user-guide/v/3.8/xa-transactions[XA Transactions] to handle this use case.
...
For details on setting the properties for a JDBC endpoint using an XML editor, consult the link:/mule-user-guide/v/3.8/jdbc-transport-reference[JDBC Transport Reference].
