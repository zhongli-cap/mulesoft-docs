...
Mule runs on operating systems that support the Java versions listed in link:/mule-user-guide/v/3.8/hardware-and-software-requirements#supported-java-versions[Supported Java Versions]. The following operating systems have been tested for use with Mule:
...
Mule Runtime runs as a standalone server and does not require an application server. You can run Mule Runtime in the following application servers.
...
Mule Runtime does not require a database unless you need to access a data store. The following database servers are recommended:
