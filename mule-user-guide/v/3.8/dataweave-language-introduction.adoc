...
In this example, the payload includes two elements in an array, and in both cases the link:/mule-user-guide/v/3.8/dataweave-operators#map[Mpa operator] to cycle through the array. It then evaluates each element agains a regular expression and outputs a different object depending on what kind of match is found.
