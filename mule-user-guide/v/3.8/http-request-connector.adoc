...
This connector can also implement HTTPS protocol and encrypt your communications via link:/mule-user-guide/v/3.8/tls-configuration[TLS], it can also implement link:/mule-user-guide/v/3.8/authentication-in-http-requests[authentication] via basic authentication, OAuth, NTLM or digest.
...
|Enable Datasense |The *Enable DataSense* checkbox allows Studio to use link:/mule-user-guide/v/3.8/datasense[DataSense] to expose metadata from the RAML file and interact with it on other elements in Studio.
|===
