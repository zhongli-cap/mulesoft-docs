...
This document assumes that you are working with Maven outside of Anypoint Studio, and thus are using archetypes to build Mule applications. See link:/mule-user-guide/v/3.8/using-maven-with-mule[Using Maven with Mule] for an overview of using Maven to manage your Mule projects.
...
This repository includes 3rd party libraries used by Mule components. If Maven cannot resolve all of the library dependencies needed by the Mule components using solely the official MuleSoft release and snapshot repositories, then please consider adding the following MuleSoft Public repository:
