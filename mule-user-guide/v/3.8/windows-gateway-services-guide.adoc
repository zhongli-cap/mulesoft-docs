...
link:https://repository.mulesoft.org/nexus/content/repositories/releases/org/mule/modules/anypoint-windows-gateway-service/1.8.5/anypoint-windows-gateway-service-1.8.5.zip[Install Windows Gateway Services]
...
For information on MSMQ, see the link:/mule-user-guide/v/3.8/msmq-connector-user-guide[MSMQ Connector User Guide].
...
. Download the Windows Gateway Services installer from link:https://repository-master.mulesoft.org/nexus/content/repositories/releases/org/mule/modules/anypoint-windows-gateway-service/1.8.5/anypoint-windows-gateway-service-1.8.5.zip[here]. Unzip the software distribution.
...
. Copy the Authentication Token that displays for use when you configure the connection to the Gateway Service from Anypoint Studio.. For more information, see the link:/mule-user-guide/v/3.8/windows-powershell-connector-guide[Windows PowerShell Connector Guide.] After you copy the token, click *Install*.
...
For all connectors leveraging the Windows Gateway Services, the service first authenticates the direct caller, which is the connector running in the Mule.
