...
|
baseRef icon:envelope[] | BaseRefType | An instance of BaseRefType | #[payload] | {nbsp}
...
|
status  | CalendarEventAttendeeResponseEnum | +++The new status to set+++ |  | *x*{nbsp}
...
|
searchRecord icon:key[] | String | The search type - SearchRecordTypeEnum |  | *x*{nbsp}
...
|
records icon:envelope[] | List<BaseRefType> | +++A list of BaseRefType that references the objects to be deleted.+++ | #[payload] | {nbsp}
...
|
records icon:envelope[] | List<BaseRefType> | +++A list of RecordRef that references the objects to be retrieved.+++ | #[payload] | {nbsp}
