...
[NOTE]
Using the <<DataWeave Quickstart,DataWeave Quickstart>> you can see how to connect a service that sends *XML* order requests to a different service that expects *JSON* - we can do this via the Transform Message component's UI.
...
This Quickstart introduces the key functional features of the Transform Message component and DataWeave:
...
== DataWeave Quickstart Overview

Here are the steps we take in this quickstart to design our DataWeave transformation and run in the context of a REST request and handled response.
...
[TIP]
====
To walk through the steps of this Quickstart, you'll need to download and extract the contents of the link:_attachments/dw-quickstart-bundle.zip[dw-quickstart-bundle.zip]
...
The service API is defined using link:http://raml.org/[RAML], the API-modeling language whose format is supported by our HTTP connector, so you can easily link to an HTTP connector with the corresponding metadata appropriately exposed to the Anypoint Studio user at design time.

The raw data from your sales tracking software follows this structure:
...
This XML format won't work as raw input to our web service, because the web service requires a different data structure, and it expects this structure to be serialized as JSON. Further, only a subset of these fields are needed, they would need to be renamed them and used to infer some other needed values. You then need to select only a few of these fields, rename some, and set a value for "size" (since it's not provided by the input).

== Steps to Perform Metadata Mapping

[NOTE]
The Transform Message component picks up all of the available metadata about the input and output on either side of the component in the flow -- this metadata awareness feature helps you build the transformation.
...
. Select the *Metadata* tab from the HTTP connector's properties window and click the *Set Metadata* button.
+
image::dataweave-quickstart-702fd.png[]
. Click the *Edit* icon that appears next to the Payload element
. Click the green plus sign to add a new metadata type and name it `XML-order-input`
+
image::dataweave-quickstart-c8ae5.png[]
. Select *XML* as the type, pick *example* and point it to the location of the `dw-tshirt-inputsample.xml` file, available in the bundle you downloaded at the start of this guide (link:_attachments/dw-quickstart-bundle.zip[dw-quickstart-bundle.zip]). Then click *Select* to use this Metadata type.
+
image::dataweave-quickstart-26f37.png[]
+
[TIP]
If you open the Transform Message component, the input section should show the fields that will be present in the incoming payload, this will come in handy later.
+
image::dataweave-quickstart-4a1db.png[]
+
. Click on the second HTTP connector to open its properties editor. Click the green plus sign to create a new configuration for the HTTP connector. In the "RAML Location" field, give the proper path to the `t-shirt.raml` file, which was available in the bundle you downloaded at the <<DataWeave Quickstart,start>> of this guide. Then click *Ok*.
+
...
. In the *path* field write `get-products`. Once deployed, this will make the endpoint reachable at `0.0.0.0:8081/get-products`
