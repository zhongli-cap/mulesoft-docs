...
Using `mule.oauth2.externalRedirectUrl is particularly useful for deploying applications to CloudHub, for example. In the configuration of authentication, you need to specify the redirectionURL as shown in the link:/mule-user-guide/v/3.8/authentication-in-http-requests#creating-the-mule-client-app[previous example]:
...
To create the endpoint for CloudHub, the following format is required:

`+http://<app>.cloudhub.io/<redirect Uri>+`

To create the endpoint for CloudHub in the correct format, you include the system property in your Mule properties file. For example:
