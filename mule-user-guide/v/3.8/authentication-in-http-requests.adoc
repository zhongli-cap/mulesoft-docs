...
Configure your APIs for authentication in HTTP requests, as described in this section, when your Mule app is sending requests to a service that requires authentication. In this case your app is the client. When your API is receiving requests, use an API Manager policy, such as the link:/api-manager/external-oauth-2.0-token-validation-policy[OAuth 2.0 Access Token Enforcement Using External Provider] policy.
...
This example builds an app in Studio that sends a request to the Github API for user information. The Github API accepts requests for user information on port 443 to https://api.github.com/user. Github requires the following header in requests: 
...
+
* *Parameters*: Click *Add Parameter*, and enter the key and value as follows: Key = `Accept` and Value = `application/vnd.github.v3+json`
+
