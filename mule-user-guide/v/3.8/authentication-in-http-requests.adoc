...
This example builds and runs an app in Studio that sends a request to the link:https://developer.github.com/v3[Github API] for user information. The Github API accepts requests for user information on port 443 to https://api.github.com/user. Github requires the following header in requests: 
...
. In Connector Configuration, click image:Edit-16x16.gif[Edit-16x16].
+
...
+
* *Parameters*: Click *Add Parameter*, select `Header` from the drop-down, and enter the key = `Accept` and Value = `application/vnd.github.v3+json`
+
...
* Set *Host* to api.github.com.
* Set *Port* to 443.
+
. On the TLS/SSL tab, accept *Default TLS Config*, and click OK.
...
. Check the *Preemptive* check box.
. Click OK.
. Save changes.
...
. When the console shows that the app is deployed, call the app using the following URL: `+http://localhost:8081/+`
+
The Github API returns your user information.
+
`{"login":"krishahn","id":16125343,"avatar_url":"https://avatars.githubusercontent.com/u/16125343?v=3"`
`...`
