...
* Submitting an HTTP request for user data to the Mule client app
+
The client app redirects the request to the Github authentication server. Github prompts you to login and approve the client app. 
* Using your Github login account credentials to log in and authorize the application
+
The Github authentication server calls the *Redirect URL* that you configured in the Mule client app, providing an *authorization code*. In response, the Github authentication server returns an *access token*. 
+
image::authentication-in-http-requests-42011.png[authentication-in-http-requests-42011]
+
* Requesting the secured user data using the access token
+
