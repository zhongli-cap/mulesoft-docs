...
. Download and install the Windows Gateway Services Software. A link to the software is in the link:/mule-user-guide/v/3.8/windows-gateway-services-guide[Windows Gateway Service Guide] .
...
The PowerShell connector uses a client-server architecture with two tiers, the connector or client running in the Mule, and the Windows Services Gateway running on the server side. The latter exposes an HTTP Web API for receiving and executing PowerShell commands.
...
Yes, the PowerShell connector passes the script contents to the link:/mule-user-guide/v/3.8/windows-gateway-services-guide[Windows Gateway Service] for execution so can be run on any platform.
