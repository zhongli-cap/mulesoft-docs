...
You query the Yahoo! REST API at http://query.yahooapis.com/v1/public/yql and specify the format using the `format=<format>` parameter: `format=XML`.

The API receives the reply and extracts some values such as stock name and price. Finally, the API inserts the values into a table in an external Postgres database.
...
[tab,title="XML Representation of Flow"]
....
...
The splitter divides the incoming message into parts based on a user-defined expression. The expression conforms to the link:/mule-user-guide/v/3.6/xpath[XPath syntax supported by MEL]:
...
`xpath_expression` is the `<quote></quote>` element in the XML returned for ticker symbol BAC.
`input_data` is the payload received from the Byte Array to String.
`return_type` is of the NODESET type, which returns a DOM NodeList object.
...
The Expression component evaluates the an expression. Create expressions to represent the Name and BookValue for Bank of America stock to extract from the XML and store in the database.
