...
This example queries the link:http://baconipsum.com/[Bacon Ipsum] REST API, which generates link:http://en.wikipedia.org/wiki/Lorem_ipsum[lorem ipsum] text. You don't have to use HTTPS or set up an account. The example consists of the an HTTP listener and an HTTP requester initially.

image::rest-api-examples-ec226.png[rest-api-examples-ec226]

Later, you add a File component to the example to save the output of the request for lorem ipsum text to a file on your file system.

*View the XML*

The XML without the File Component is:
...
[tab,title="Studio Visual Editor"]
....
. Create a Connector Configuration and set up the *Host*, *Port*, *Base Path*:
+
...
+
. Click OK. Then set up the path and method:
+
[cols=",",options="header"]
|===
|Parameter |Value
...
+
. Add two parameters using *Add Parameter*. Leave both as the default type *query-param*. For the `value` field of each, write a mule expression that takes the specified value from the query parameters of the request that first reaches the HTTP Listener:
+
