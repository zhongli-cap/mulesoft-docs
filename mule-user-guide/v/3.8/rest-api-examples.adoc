...
These examples of consuming REST APIs show you how to perform typical tasks using Studio or standalone XML:

* link:/mule-user-guide/v/3.8/rest-api-examples#consuming-a-rest-api-studio-visual-editor[Consuming a REST API]
+
Describes how to write an API that retrieves data from a publicly available REST API
+
* link:/mule-user-guide/v/3.8/rest-api-examples#extracting-and-storing-xml-data-from-a-rest-api-studio-visual-editor[Extracting and Storing XML Data from a REST API]
+
Covers retrieving data from a Yahoo REST API and inserting the data in a local Postgres database

== Overview of Consuming a REST API--Studio Visual Editor
...
For example, you enter a local browser request `http://localhost:8081?type=meat-and-filler` and the application issues this query: `http://baconipsum.com/api/?type=meat-and-filler`.
...
+
These values are transformed into inbound properties when they reach the HTTP Request Connector.
+
...
. Set the directory and file name for storing the output: `/tmp/output`
+
image::rest-api-examples-2d8e6.png[rest-api-examples-2d8e6]
+
...
== Extracting and Storing XML Data from a REST API--Studio Visual Editor
...
You specify the XML format when querying the Yahoo! REST API at http://query.yahooapis.com/v1/public/yql.

The application receives the reply and extracts some values, such as stock name and price. Finally, the API inserts the values into a table in an external Postgres database.
...
Configure the splitter as follows:
...
Accept the default values on the Advanced tab.

==== DOM to XML Transformer

The DomToXml transformer converts DOM objects to XML. Accept the default values for the DOM to XML connector.

==== Expression

The Expression component evaluates an expression. Create expressions to represent the Name and BookValue for Bank of America stock to extract from the XML and store in the database.
...
|*Expression* |`flowVars.dbName=xpath3('//Name'); flowVars.dbBookValue=xpath3('//BookValue');`
|===
...
The Variable component creates a variable that represents the insert statement. Configure this component as shown in the following screenshot and table:
...
+
Use a Web browser or an HTTP client such as the link:http://curl.haxx.se/download.html[curl] command-line utility to call the HTTP listener on localhost port 8081.
+
This example retrieves a quote for Bank of America by calling the HTTP listener using the following URL or curl command:
+
----
...
----
+
Remove any line breaks from the URL or `curl` command.
+
Calling the listener triggers the flow.
+
PgAdmin III Postgres interface shows the data inserted in the database row.
+
