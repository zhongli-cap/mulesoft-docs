...
The *Dom To Xml* transformer converts DOM objects to XML. Accept the default values in the properties editor.
...
. In the properties editor, create the following expressions to represent the Name and BookValue for Bank of America stock:
+
[source]
----
...
+
The application extracts the values from the XML in preparation for storage in the database.
+
. Accept the default values on the Advanced tab.
