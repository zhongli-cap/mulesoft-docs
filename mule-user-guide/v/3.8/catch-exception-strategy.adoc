...
[tab,title="Studio Visual Editor"]
....
. After xref:configcatchstrat[Configuring a Catch Exception Strategy], open the flow in the XML editor by clicking the *Configuration XML* tab at the bottom of the canvas.
. Locate the `<catch-exception-strategy` to `</catch-exception-strategy>` block in your XML flow.
