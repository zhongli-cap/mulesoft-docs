...
IMPORTANT: Before creating a Global Catch Exception Strategy, you must have first created a catch exception strategy as described in <<Configuring a Catch Exception Strategy>>. The Global version of the exception strategy requires copying text from your current exception strategy to create the global version.

[tabs]
------
[tab,title="Studio Visual Editor"]
....
. After <<Configuring a Catch Exception Strategy>>, open the flow in the XML editor by clicking the *Configuration XML* tab at the bottom of the canvas.
