...
[TIP]
====
You can enter the following example expressions to the *Execute When* field:
...
[TIP]
====
You can enter the following example expressions to the `when` attribute:
...
. Create a new Mule configuration file to use as a global file. This file appears in your Studio project under `src/main/app`. For this example, it's called `global.xml`. The global file has <mule and <flow XML elements the same as your project. You can add building blocks in Studio by searching for each and dragging what you want to the global file the same as in Studio for your Mule project.
. Click *Message Flow* and copy the building blocks you want in the global file. The catch exception strategy should be in the global file.
. Save your project.
+
You can now reference your global file from other flows.
...
For example, an exception strategy may handle only those messages which throw an org.mule.example.AlreadyProcessedException.
