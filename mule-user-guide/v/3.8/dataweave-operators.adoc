...
The usage above is performing the filter operation after any transformation has occurred, if this is not a requirement then filter can be done prior to the transformation.
...
----
<2> Filters the all key:value pairs with "data" value "b" => {data:b} before any transformation has occurred.
For large, complex transformations this is more efficient than filtering after the transformation has occurred.
