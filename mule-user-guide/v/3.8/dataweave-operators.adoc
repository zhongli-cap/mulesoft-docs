...
The mapping below performs exactly the same transform, but it defines custom names for the properties of the operation, instead of using $ and $$. Here, `position` is defined as referring to the array index, and `money` to the value in that index.  The `position` property reference is optional. 
...
----
 ...
 The `position` property reference is optional. This is also a valid example: 
