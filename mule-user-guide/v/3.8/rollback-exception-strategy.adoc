...
. Refer to step 2 xref:cares[Configuring a Rollback Exception Strategy] to configure your global rollback exception strategy.
...
. Follow steps 3 - 5 xref:cares[Configuring a Rollback Exception Strategy] to build your global rollback exception strategy flow and redelivery exhausted flow.
....
[tab,title="XML Editor or Standalone"]
....
