...
An excel workbook is a sequence of sheets, in DataWeave this is mapped to an object where each sheet is a key. Only one talbe is allowed per excel sheet. A table is expressed as an array of rows. A row is an object where its keys are the columns and the values the cell content.
