...
<<<<<<< HEAD
. Drag and drop input fields across to the output fields to map and transform data format to Salesforce-friendly values. Note how the link:/mule-user-guide/v/3.8/dataweave-reference-documentation[DataWeave Code] on the right is written automatically to match every field you drag along. +
=======
