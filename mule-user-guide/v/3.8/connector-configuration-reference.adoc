...
|===
|Field a|Description
|*Connector Configuration* |Define connection parameters. This configuration gets saved in a link:/mule-user-guide/v/3.8/global-elements[Global Element] that defines the connector configuration you set up.

|*Operation* |Select an operation from the dropdown to specify the function that the connector should perform. For example, in the case of the GitHub connector, you have many operations to choose from, for example, *"Get collaborator"* or *"Get commits"*
|===
[tabs]
------
