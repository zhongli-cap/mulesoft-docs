...
Follow along to see how to use the operators link:/mule-user-guide/v/3.8/dataweave-operators#map[`map`] and `as` in this example, which takes an XML list of books and outputs a JSON version of the list. It goes through each `<book>` element in the input using the `map` operator.

You can ensure the transformation generates the correct types on the output side by using the `as` operator (used for type coercion - see an https://docs.mulesoft.com/mule-user-guide/v/3.8/dataweave-operators#coerce-to-number[`as` operator example]).
