...
In this example, we are merging the result of a scatter-gather which returns a collection of results.

The first item in our payload contains an array of price objects uniquely identified by a book id, the second item in our payload contains an array of author objects identified by a book id. The objectve is two merge the two arrays to have a single array of objects with author and price data by book id using the id to merge the two sets of data.
