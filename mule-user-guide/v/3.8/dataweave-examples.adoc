...
Below are a series of advanced examples using the link:/mule-user-guide/v/3.8/dataweave-language-introduction[DataWeave Language]. For a more basic and closely guided step-by-step series of exercises that walk you through this feature as accessed via the Anypoint Studio UI, see the link:/mule-user-guide/v/3.8/dataweave-quickstart[DataWeave Quickstart Guide].

== Using the `map` Operator

This example takes an XML list of books and outputs a JSON version of the same. It goes through each 'book' element in the input using the link:/mule-user-guide/v/3.8/dataweave-operators#map[`map` operator].  Values that are numeric are defined as numbers using the link:/mule-user-guide/v/3.8/dataweave-operators#as[`as` operator] to avoid treating numeric types as strings. Since JSON can't have duplicate keys, elements that may be duplicated in the inbound XML, will present only the first to be defined as an element in the output.

.Transform Defined in DataWeave
...
== Using DataWeave to Remove Fields
...
The example goes through the whole set of elements in the input using a link:/mule-user-guide/v/3.8/dataweave-operators#map[`map` operator].
...
== Using DataWeave to Replace Values
