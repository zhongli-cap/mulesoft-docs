...
This guide offers information about how to use link:https://www.mulesoft.org/what-mule-esb[Mule Runtime], commonly called *Mule*, or *Mule ESB* to integrate systems, orchestrate Web services, or connect any system with another. The visual representations of Mule applications you see throughout are usually taken within the Anypoint Studio IDE. 

Integration developers write Mule applications to tie together various systems. Mule applications run on Mule runtime to process inbound information, and ingest this information in a pre-defined manner. An expected request to a running Mule application naturally triggers Mule to pass the event and data along a single or multiple threads. In this process Mule transforms and routes the Mule message to its destination passing to the recipient the requisite data. 
