...
The units that make up flows may be referred to as *flow elements*. In general, each flow element corresponds to an icon in the Anypoint Studio GUI, which in turn represents a message source, processor, _or_ component, which is written into the application's XML file as an XML element. Each XML element inside a flow element within the Mule application's XML configuration file usually has a corresponding icon that is displayed in the flow, indicating the flow element's position in the XML file's flow element.
