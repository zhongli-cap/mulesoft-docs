...
[NOTE]
This configuration is only valid when using Mule Runtime 3.8.2 and later, together with the Runtime Manager agent 1.5.2 and later.

If you want to log your Runtime Manager Agent state in a separate file from the other Mule log info, you can set up your log configuration file to do this.

You can configure your '$MULE_HOME/conf/log4j2.xml' file to include a 'mule-agent-appender', if included, the Runtime Manager Agent plugin logs messages about its state on a separate file named `mule_agent.log`.
...
<Appenders>
  ...
  (...)
        ...
        <Syslog name="mule-agent-appender" host="localhost" port="514" protocol="UDP" appName="hybrid-rest" facility="LOCAL6"/>
</Appenders>
----

This will make the Runtime Manager agent log its syslog data on a file named `mule_agent.log` as well as the `mule_ee.log` file where it logs everything by default.
