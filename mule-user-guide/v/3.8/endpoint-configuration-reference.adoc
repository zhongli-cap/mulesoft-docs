...
If something later in the flow throws an exception, the file won't be consumed, and is reprocessed. The link:/mule-user-guide/v/3.8/idempotent-filter#idempotent-redelivery-policy[idempotent-redelivery-policy] ensures that it won't be reprocessed more then 3 times; after that, it's sent to `vm:error-queue`, where it can be handled as an error case.
