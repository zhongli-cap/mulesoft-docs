...
To the Mule flow that involves a Web service, add a link:/mule-user-guide/v/3.6/cxf-component-reference[*CXF Component*] to publish, consume, or proxy a Web service. Complete the following steps to configure WS-security on the SOAP component.
...
* Review an the https://developer.mulesoft.com/docs/display/35X/SOAP+Web+Service+Security+Example[Mule example application] which demonstrates the use of WS-security.
* Review a http://www.mulesoft.org/documentation/display/current/XML-only+SOAP+Web+Service+Example[Mule example application] which demonstrates a SOAP Web service.
