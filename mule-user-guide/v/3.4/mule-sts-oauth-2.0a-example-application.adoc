...
This document assumes that you are familiar with the link:/anypoint-studio/v/5/index[Mule Studio interface]. To increase your familiarity with Mule Studio, consider completing one or more link:/docs/display/34X/Getting+Started+with+Mule+Studio[Mule Studio Tutorials].
...
* For more information on Oauth 2.0 in Mule, refer to link:/mule-user-guide/v/3.4/mule-secure-token-service[Mule Secure Token Service].
