...
* Communicating with an external service that uses low-level unsecured TCP connections? If so, use the link:tcp-transport-reference[TCP] protocol.

* Are you communicating with a flow always located in the same Mule application instance? If so, consider using the link:vm-transport-reference[VM transport].
...
* Are there advantages to a higher-level protocol built on top of TCP, for instance, the request-response features of link:https-transport-reference[HTTP], or the store-and-forward features of link:email-transport-reference[Email]? If so, use the transport for that protocol instead.
