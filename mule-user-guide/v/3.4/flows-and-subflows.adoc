...
* In clusters of Mule servers, messages can migrate between nodes when sent to an asynchronous flow. This allows for *load balancing between nodes* and higher performance of application. (See our link:flow-architecture-advanced-use-case[advanced use case] for an example.)
...
Read more about setting up link:transactional[Transactional units] in Mule applications.
====
...
|Asynchronous Flow |Flow Reference wrapped within an link:async-scope-reference[Async Scope] |asynchronous |not inherited
|===

== See Also

* *NEXT STEP:* Learn the various ways you can link:https://docs.mulesoft.com/getting-started/index[deploy Mule applications].
* Read about some alternative ways to control message processing within a flow using link:routing-message-processors[routing message processors].
* Refer to the link:flow-ref-component-reference[Flow Reference Component Reference] and link:async-scope-reference[Async Scope].
* Read more about link:flow-processing-strategies[Flow Processing Strategies].
* Read more about setting up link:transactional[transactional units] in Mule applications.
