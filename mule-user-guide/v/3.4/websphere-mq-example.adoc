...
The WebSphere MQ example demonstrates how to use the link:mule-wmq-transport-reference[Websphere MQ Transport]  to send and receive transactional messages to WebSphere MQ. This example is available in Mule Enterprise Edition.
...
Mule configures the `WMQ connector` with the values previously defined on the `wmq.properties` file. In MuleStudio, this configuration is stored as a link:global-elements[Global Element], named wmqConnector. To view the configured global element, click the *Global Elements* tab under the canvas, then double-click the `wmqConnector` Global Element. Studio displays the *Global Element Properties*, shown below:
...
The first building block in the flow is an link:ajax-endpoint-reference[AJAX Inbound Endpoint], which listens for messages on the configured channel, `/services/wmqExample/enqueue`. The flow adds the incoming messages to the "in" queue.
...
* Learn more about the link:wmq-endpoint-reference[WMQ Endpoint] in Mule Studio.
* Learn more about the link:mule-wmq-transport-reference[WMQ transport] in Mule.
* Learn more about the link:ajax-endpoint-reference[AJAX Endpoint] in Mule Studio.
* Learn more about the link:ajax-transport-reference[AJAX transport] in Mule.
