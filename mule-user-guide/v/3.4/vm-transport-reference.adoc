...
The VM transport can be used for intra-JVM communication between Mule link:https://docs.mulesoft.com/mule-user-guide/v/3.4/flows-and-subflows[flows]. This transport by default uses in-memory queues but can optionally be configured to use persistent queues. (Note, however, that VM file persistency does not work on clusters.)

[NOTE]
Note that VM queue persistence behaves differently on CloudHub. See the link:https://docs.mulesoft.com/runtime-manager/cloudhub-fabric[CloudHub Fabric] documentation for more details.
...
_One-way_ VM queues can take part in distributed link:https://docs.mulesoft.com/mule-user-guide/v/3.4/xa-transactions[XA Transactions]. To make a VM endpoint transactional, use a configuration like the following:
...
Using XA requires that you add a transaction manager to your configuration. For more information, see link:https://docs.mulesoft.com/mule-user-guide/v/3.4/transaction-management[Transaction Management].
...
|queue-profile |0..1 |Configures the properties of this connector's queue (see link:https://docs.mulesoft.com/mule-user-guide/v/3.4/configuring-queues[Configuring Queues]).
|===
...
The transaction element configures a transaction. Transactions allow a series of operations to be grouped together so that they can be rolled back if a failure occurs. For more information, see link:https://docs.mulesoft.com/mule-user-guide/v/3.4/transaction-management[Transaction Management].
...
Consider setting up your application following a link:https://docs.mulesoft.com/mule-user-guide/v/3.4/reliability-patterns[reliability pattern].
