...
The VM transport can be used for intra-JVM communication between Mule link:flows-and-subflows[flows]. This transport by default uses in-memory queues but can optionally be configured to use persistent queues. (Note, however, that VM file persistency does not work on clusters.)
...
_One-way_ VM queues can take part in distributed link:xa-transactions[XA Transactions]. To make a VM endpoint transactional, use a configuration like the following:
...
|queue-profile |0..1 |Configures the properties of this connector's queue (see link:configuring-queues[Configuring Queues]).
|===
