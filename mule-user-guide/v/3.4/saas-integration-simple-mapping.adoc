...
Before it can do anything with the contact information, the first thing the application has to do is actually accept a CSV file. The *link:file-endpoint-reference[File endpoint]* in the flow behaves as the inbound link:message-sources[message source], accepting files that get put into a specified folder. Once in a link:mule-concepts[Mule flow], the content of the file becomes a link:mule-concepts[Mule message] with a link:mule-concepts[header] and a link:mule-concepts[payload]. The other link:message-processors[message processors] in the flow will act upon this message by adding to it, transforming it, or otherwise manipulating it.
...
The Salesforce connector also requires a link:global-elements[Global Element], which contains global configurations. At minimum, the global configuration should include a Username, Password, and Security Token, shown in the orange box (below). 

image:Studio_SFglobal.png[Studio_SFglobal]

As you may have suspected, the real magic happens in the blue box (above). The** link:mule-datasense[Enable DataSense]** checkbox does two thrilling things:
...
. If the connector you have selected offers link:mule-datasense[DataSense], be sure that the "Enable DataSense" box is checked when you configure your connector's global element. You're now two-thirds of the way there, and DataMapper is about to do the rest of the hard work.
...
. Configure the DataMapper transformer. If your connector is DataSense-enabled, you should only need to configure the input values. Otherwise, you can quickly map inputs to outputs by clicking and dragging input fields to output fields in the link:datamapper-concepts[Data Mapping Console]. 
. Save and run your application. 
. Bask in the glory of your success.

== Dive Deeper

* Walk through this example step by step using the link:connect-with-salesforce-example[Connect with Salesforce Example], included with your instance of Mule Studio.
* Try out more complex link:choosing-mel-or-ctl2-as-scripting-engine[DataMapping using Mule Expression Language] to extract specific data from your message header or payload and dynamically map to your desired format.
* Learn how to customize your link:error-handling[error handling] strategies for more fine-grained control of what kinds of errors are captured and where the data is sent.
