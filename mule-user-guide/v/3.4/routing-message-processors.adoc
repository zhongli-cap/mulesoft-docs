...
Message Processors are used within flows to control how messages are sent and received within that flow. This is further described in link:https://docs.mulesoft.com/mule-user-guide/v/3.4/using-flows-for-service-orchestration[Using Flows for Service Orchestration].
...
|link:https://docs.mulesoft.com/mule-user-guide/v/3.4/until-successful-scope[Until Successful] |Repeatedly attempt to process a message until successful
...
The Message Filter is used to control whether a message is processed by using a link:https://docs.mulesoft.com/mule-user-guide/v/3.4/using-filters[filter]. In addition to the filter, you can configure whether to throw an exception if the filter does not accept the message and an optional message processor to send unaccepted messages to.
