...
Message Processors are used within flows to control how messages are sent and received within that flow. This is further described in link:using-flows-for-service-orchestration[Using Flows for Service Orchestration].
...
|link:until-successful-scope[Until Successful] |Repeatedly attempt to process a message until successful
...
*From 3.1.0* you can further customize the behavior of this router by specifying a _'failureExpression'_ that allows you to use link:non-mel-expressions-configuration-reference[Mule Expressions] to define a failure. The _failureExpression_ attribute is configured as follows:
