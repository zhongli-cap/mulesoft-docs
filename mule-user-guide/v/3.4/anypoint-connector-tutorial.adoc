...
However, if you wanted to expose such an application publicly, you would need to configure your application to link:using-a-connector-to-access-an-oauth-api[authenticate via OAuth].
====
+
...
|*link:foreach[Foreach scope]* |processes each item in a POJO iteratively
|*link:logger-component-reference[Logger component]* |logs messages or activity based on the value of a Mule expression; find this in the Components section of the palette at the right of the screen
|===
...
Read more about link:global-elements[Global Elements in Mule].
...
There are a few ways to accomplish this task, one of which is to add the extra text to the tweet inside a new variable. However, you can take a shortcut using the link:set-payload-transformer-reference[set payload] transformer. Replace the content of the entire payload using an expression composed of multiple parts, such as the following the following:
...
* *NEXT STEP:* Learn how to link:starting-and-stopping-mule-esb[start and stop Mule] from the command line.
* See link:https://www.mulesoft.com/exchange#!/?types=connector&sortBy=name[specific documentation for each connector].
* Import link:installing-connectors[additional connectors] into your instance of Studio.
