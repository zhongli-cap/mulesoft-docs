...
[%autowidth.spread]
|===
|[link:configuring-components[Simple Components]] |[link:configuring-components[Java Components]] |[link:configuring-components[Other Components]] |[link:configuring-components[Customizing Behavior with Interceptors]] |[link:configuring-components[Lifecycle]]
|===

Components contain the business logic for working with the message passed through Mule ESB. A component can be any type of object, including a link:using-spring-beans-as-flow-components[Spring bean], link:configuring-java-components[POJO], link:scripting-module-reference[script], link:using-web-services[web service]. or REST call.
...
For detailed information on the elements you configure for components, see link:component-configuration-reference[Component Configuration Reference]
...
|*<test:component/>* |Configures the `MuleFunctionalTestComponent`, which allows more complex testing scenarios to be created. For more information, see link:functional-testing[Functional Testing].
|===
...
The explicit syntax is required instead of the shortcut `<component class>` syntax if you add interceptor to the component. For further configuration options and information on the default settings that are applied, see link:configuring-java-components[Configuring Java Components].

== Other Components

These are several other components available that allow you to use different technologies such as link:using-web-services[web services] for your components. These components are often included as part of link:transports-reference[transport] or link:modules-reference[modules].

[%header%autowidth.spread]
|===
|Configuration |Description
|*<http:rest-service-component/>* |Proxies a remote call to a link:rest-component-reference[REST-style web service].
|*<cxf:wrapper-component/>* |Proxies a remote call to a web service using link:building-web-services-with-cxf[CXF].
|*<script:component/>* |Configures a JSR-223 link:scripting-module-reference[script] for the component.
|===
