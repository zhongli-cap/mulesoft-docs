...
link:pattern-based-configuration[Configuration patterns] are elements that simplify configuring specific and recurring activities. Mule comes with some link:using-mule-configuration-patterns[ready made patterns] that cover generic scenarios.
...
[WARNING]
A pattern catalog is actually a regular Mule module. If you already have an existing module (most probably created with the link:creating-catalog-archetypes[Module Archetype]) and are happy storing your patterns in it, you can directly proceed to the next section.
...
The module archetype will link:creating-module-archetypes[ask various questions] and then generate the files. You can also use this plug-in without user prompts by entering all the arguments at the command line. For a full list of arguments that can be passed in, see the link:creating-catalog-archetypes[Command Line Options].
...
* *si*: The pattern receives messages from a single inbound endpoint. It can optionally be configured with inbound transformers. The link:simple-service-pattern[Simple Service] pattern is of this kind.
* *siso*: The pattern receives messages from a single inbound endpoint and dispatches to a single outbound endpoint. The link:bridge-pattern[Bridge Validator] and link:proxying-web-services[Web Service Proxy] patterns are of this kind.
