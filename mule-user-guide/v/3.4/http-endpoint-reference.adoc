...
To learn more about endpoints, refer to the link:studio-endpoints[Studio Endpoints] document.
...
. In the *Connector Reference* field, use the combo-box to select a *Global Connector* for the HTTP endpoint to reference. The endpoint uses the link:http-transport-reference[connector] configurations you define within the global connector.  +
