...
The sections below describe how to use Mule's link:error-handling[*Default Exception Strategy*], a link:rollback-exception-strategy[*Rollback Exception Strategy*] or a link:mule-user-guide/v/3.4/catch-exception-strategy[*Catch Exception Strategy*] to handle errors in a flow with a CXF Web service.
