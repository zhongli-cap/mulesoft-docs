...
. link:hardware-and-software-requirements[Check hardware and software requirements.]
. link:downloading-and-launching-mule-esb[Download, then launch Mule.]
. link:adding-community-runtime[Add Community runtime to Mule Studio] (_optional_).
. link:installing-an-enterprise-license[Install an Enterprise license] (_optional_).
. link:installing-extensions[Extend Mule with plugins, modules, runtimes and connectors] (_optional_).
