...
For more about expressions, see link:mule-expression-language-mel[Mule Expression Language MEL].
...
Several Mule transports and modules provide their own filters. For example, the link:xml-module-reference[XML Module Reference] includes a filter to determine if a message is XML. For more information, see link:transports-reference[Transports Reference] and link:modules-reference[Modules Reference]. 
