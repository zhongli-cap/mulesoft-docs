...
* See the Logger in action in the link:service-orchestration-and-choice-routing-example[Service Orchestration and Choice Routing Example]. 
* Read more about link:configuring-logging[Configuring Logging].
* Check the link:mule-expression-language-mel[Mule Expression Language] reference for more ideas on how to format helpful logger messages.
