...
This annotation controls how the current message inbound attachments are passed into a method. The annotation supports, Map, List, single attachment, wildcards and optional entries. It can be used on component entry points and link:/mule\-user\-guide/v/3\.6/transformer-annotation[@Transformer] methods.
