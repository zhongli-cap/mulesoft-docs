...
To do this, you set a *breakpoint* at any building block in your flow that you wish to check or test (see image below). When you run your application in Debug mode, the application stops immediately after executing the building block with the breakpoint. Using the *Mule Debugger View*, you can browse through the contents of the message as it exists at that point in the flow, and evaluate link:https://docs.mulesoft.com/mule-user-guide/v/3.4/mule-expression-language-mel[Mule Expressions] against the message.
...
The left pane displays the message payload (in this case, an HTTP GET request received via an link:https://docs.mulesoft.com/mule-user-guide/v/3.4/http-endpoint-reference[HTTP inbound endpoint]). The right pane displays various metadata associated with the message:
...
A conditional breakpoint is activated only if a user-defined condition evaluates to _true_ when Studio executes the building block with the breakpoint. You define the condition using a link:https://docs.mulesoft.com/mule-user-guide/v/3.4/mule-expression-language-mel[Mule Expression].
...
When you start Mule ESB in debug mode, _all_ applications that you launch in that server run in debug mode. Remote debugging does not work with link:https://docs.mulesoft.com/mule-user-guide/v/3.5/mule-high-availability-ha-clusters[clusters].
...
* link:https://docs.mulesoft.com/mule-user-guide/v/3.4/studio-in-eclipse[Eclipse plugin]
