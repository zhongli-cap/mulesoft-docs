...
The left pane displays the message payload (in this case, an HTTP GET request received via an link:http-endpoint-reference[HTTP inbound endpoint]). The right pane displays various metadata associated with the message:
...
A conditional breakpoint is activated only if a user-defined condition evaluates to _true_ when Studio executes the building block with the breakpoint. You define the condition using a link:mule-expression-language-mel[Mule Expression].
...
* link:mule-datasense[Mule DataSense]
* link:studio-in-eclipse[Eclipse plugin]
