...
You can create one or more link:error-handling[global exception strategies] to reuse in flows throughout your entire Mule application. First, create a global exception strategy, then add a *Reference Exception Strategy* to a flow to apply the error handling behavior of a specific global exception strategy.
...
....
. If you have not already done so, create a *global* link:catch-exception-strategy[catch], link:rollback-exception-strategy[rollback], or link:choice-exception-strategy[choice] exception strategy to which your reference exception strategy can refer.
...
[NOTE]
====
You can create a global exception strategy (i.e. access the Choose Global Type panel) from the reference exception strategy’s pattern properties panel. Click the (plus) button next to the Global Exception Strategy drop-down and follow the steps to create a global link:catch-exception-strategy[catch], link:rollback-exception-strategy[rollback], or link:choice-exception-strategy[choice] exception strategy.
