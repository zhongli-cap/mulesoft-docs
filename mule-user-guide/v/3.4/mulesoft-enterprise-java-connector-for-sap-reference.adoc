...
This procedure assumes that you already have a Mule instance installed on your host machine. If not, then follow the link:installing[instructions] for downloading and installing Mule.
...
|jcoClientExtendedProperties-ref |A reference to `java.util.Map` containing additional JCo connection parameters. Additional information and a complete list of parameters can be found link:sap-jco-extended-properties[here] |  |1.0
...
The complete documentation for the different XML documents can be found link:xml-definitions[here]:

* link:xml-definitions[JCo Function]
** link:xml-definitions[Version 1]
** link:xml-definitions[Version 2]
* link:xml-definitions[IDoc]
...
|jcoClientExtendedProperties-ref |A Reference to `java.util.Map`, which contains additional JCo connection parameters. Additional information and a complete list of parameters can be found link:sap-jco-extended-properties[here]. |  |1.0
|===
...
After the flow executes and if the SAP inbound endpoint is request-response, then Mule will return values to the calling SAP instance. Return values can be represented with the XML representation of the link:xml-definitions[JCo Function]. +
...
* link:outbound-endpoint-transactions[sRFC stateful]
* link:outbound-endpoint-transactions[sRFC stateful BAPI transaction]
* link:outbound-endpoint-transactions[tRFC stateful]
* link:outbound-endpoint-transactions[qRFC stateful]

[WARNING]
If a transaction is not specified, then all calls (execute function or send IDoc) are stateless.

For examples, please take a look at link:outbound-endpoint-transactions[Outbound Endpoint Transactions].
...
* link:troubleshooting-sap-connector[Checking log files]
* link:troubleshooting-sap-connector[Enabling JCo trace]
* link:troubleshooting-sap-connector[Common errors]
