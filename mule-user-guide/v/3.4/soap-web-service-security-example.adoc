...
The request-response **link:https://docs.mulesoft.com/mule-user-guide/v/3.4/http-endpoint-reference[HTTP Endpoint] **in this flow receives the end user request. Because it has a two-way message exchange pattern, this HTTP endpoint is responsible for both receiving and returning messages.

A JAX-WS service, the **link:https://docs.mulesoft.com/mule-user-guide/v/3.4/soap-component-reference[SOAP Component]** in this flow evaluates the message according to its security configurations. In this case, the Web service is unsecure (see image below) so the SOAP component processes all requests it receives.
...
The link:https://docs.mulesoft.com/mule-user-guide/v/3.4/java-component-reference[Java Component] executes a simple script to prepare a personalized salutation for the end user.
