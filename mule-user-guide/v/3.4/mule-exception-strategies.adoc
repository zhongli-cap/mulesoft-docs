...
* *NEXT STEP*: Read on about the structure of a link:mule-message-structure[Mule message].
* See the link:foreach-processing-and-choice-routing-example[full example application].
* Learn more about configuring the link:catch-exception-strategy[catch], link:rollback-exception-strategy[rollback], link:reference-exception-strategy[reference] and link:choice-exception-strategy[choice] exception strategies.
