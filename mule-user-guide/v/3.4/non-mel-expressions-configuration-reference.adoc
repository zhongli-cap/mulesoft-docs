...
[TIP]
The new *Mule Expression Language (MEL)* obviates the need to use old expressions in Mule. Refer to link:mule-expression-language-mel[Mule Expression Language MEL] for details on how to use MEL.
...
|ognl |`#[ognl:[MULE:0].equals(42)]` |Set the `evaluator` attribute on the `<expression-filter>` element to `ognl` when specifying an link:using-filters[OGNL filter].
