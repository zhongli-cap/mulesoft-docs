...
[TIP]
The new *Mule Expression Language (MEL)* obviates the need to use old expressions in Mule. Refer to link:https://docs.mulesoft.com/mule-user-guide/v/3.4/mule-expression-language-mel[Mule Expression Language MEL] for details on how to use MEL.
...
|ognl |`#[ognl:[MULE:0].equals(42)]` |Set the `evaluator` attribute on the `<expression-filter>` element to `ognl` when specifying an link:https://docs.mulesoft.com/mule-user-guide/v/3.4/using-filters[OGNL filter].
