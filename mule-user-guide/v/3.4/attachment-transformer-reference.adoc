...
Use an *attachment transformer* to set, remove, or copy attachments on the outbound scope of a message. To learn more about message scopes, refer to link:mule-concepts[Mule Concepts].
...
The attachment transformer is very similar in functionality to the link:property-transformer-reference[property transformer]. However, attachments are only supported by the following transports:
...
* Read about related transformers, the link:property-transformer-reference[property transformer], the link:variable-transformer-reference[variable transformer]
, and the link:session-variable-transformer-reference[session variable transformer], which you can use to set properties and variables for different scopes.
