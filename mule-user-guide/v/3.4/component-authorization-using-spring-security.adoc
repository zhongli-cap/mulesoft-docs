...
* link:component-authorization-using-spring-security[Securing Flow Components]
* link:component-authorization-using-spring-security[Setting Security Properties on the Security Provider]
...
In addition to the `daoAuthenticationProvider` and `inMemoryDaoImpl` beans (see link:configuring-security[Configuring Security]), the following beans must be configured:
