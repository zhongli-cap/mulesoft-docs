...
Proceed to define your DB Server configuration in a new link:/munit/v/1.2/munit-suite[MUnit Suite].
...
In order for it to run, the database server must be started in a link:/munit/v/1.2/munit-suite#defining-a-before-suite[before-suite]. You start the server using the `start-db-server` message processor.
...
To stop the DB server gracefully, it needs to be stopped in an link:/munit/v/1.2/munit-suite#defining-an-after-suite[After Suite], using a `db-server` message processor with the `Stop db server` operation.
