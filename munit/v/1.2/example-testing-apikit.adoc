...
Create a new Mule Studio project and create a RAML definition file in the `api` folder (for details on how to do this, see the link:https://docs.mulesoft.com/apikit/apikit-tutorial[APIkit Tutorial]).
...
This is illustrated in the link:https://docs.mulesoft.com/getting-started/implement-and-test[implementing and testing an API quickstart] section.

Based on link:https://docs.mulesoft.com/getting-started/_attachments/t-shirt.raml[complete t-shirt API definition], the MUnit's scaffolder automatically creates the following test Suite:
...
Using a link:mock-message-processor#loading-payloads-from-files-and-scripts[getResource MEL expression] it then references those request and responses from the MUnit test:
...
[NOTE]
--
The Scaffolder that generates the MUnit test from the RAML definition does not consider if your application uses link:https://docs.mulesoft.com/mule-user-guide/v/3.8/tls-configuration[TLS configuration]. +
...
You can use APIkit in Mule runtime with or without API Gateway. API Gateway Runtime 1.3 - 2.x has a link:https://docs.mulesoft.com/api-manager/api-gateway-domain[domain named api-gateway].
