...
Another feature we provide is the ability to ignore a flow. That means that it won't count as coverage data, not the overall number of message processors nor will it fail the build if it's not tested or reach the coverage metrics. 
...
As you can see this is a list, so you can ignore as many flow as you need. 
