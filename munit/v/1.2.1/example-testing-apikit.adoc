...
<mule xmlns="http://www.mulesoft.org/schema/mule/core" xmlns:doc="http://www.mulesoft.org/schema/mule/documentation" xmlns:http="http://www.mulesoft.org/schema/mule/http" xmlns:mock="http://www.mulesoft.org/schema/mule/mock" xmlns:munit="http://www.mulesoft.org/schema/mule/munit" xmlns:spring="http://www.springframework.org/schema/beans" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.mulesoft.org/schema/mule/core http://www.mulesoft.org/schema/mule/core/current/mule.xsd http://www.springframework.org/schema/beans http://www.springframework.org/schema/beans/spring-beans-3.1.xsd http://www.mulesoft.org/schema/mule/http http://www.mulesoft.org/schema/mule/http/current/mule-http.xsd http://www.mulesoft.org/schema/mule/munit http://www.mulesoft.org/schema/mule/munit/current/mule-munit.xsd http://www.mulesoft.org/schema/mule/mock http://www.mulesoft.org/schema/mule/mock/current/mule-mock.xsd ">
    ...
    <munit:test name="get:/orders/status:api-config-200-application/json-FlowTest" description="Verifying functionality of [get:/orders/status:api-config-200-application/json]">
...
The new RAML scaffolder bundled in MUnit 1.2.1 generates external files located in `src/test/resources/` based on the requests and responses examples in the API definition.
...
If your API definition does not have examples set, you can configure a set of defaults value for your MUnit test to validate. +
...
Additionally you can choose to disable the request/response file creation in case you choose to define them manually, or you don´t need to validate the resulting payload.
...
== RAML Coverage Report

//TODO - Check this syntax
App coverage refer to how much of your application gets executed as a result of your test being executed. Is a measure of quality .

RAML (API) coverage refers of how many cases your RAML definition has and how many are being tested.
There is no hard correlation between all the posible RAML API scenarios and the actual API implementation.
The scaffolding of tests tries to create a test scenario for each posible path in the API.
