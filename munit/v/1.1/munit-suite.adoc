...
Each MUnit suite is meant to test a particular set of Mule flows. To do that, you need to tell MUnit where those flows are defined in your application structure. For this we use _imports_, which are in fact link:https://docs.mulesoft.com/mule-user-guide/v/3.7/adding-and-removing-user-libraries#adding-user-libraries[spring imports].
