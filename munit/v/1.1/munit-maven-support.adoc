...
TIP: If you are using Anypoint Studio, Maven integration is already solved for you. See link:https://docs.mulesoft.com/anypoint-studio/v/5/using-maven-in-anypoint-studio[Using Maven in Anypoint Studio] for details.
...
NOTE: For information on redirecting logs to a file, see link:/munit/v/1.1/logging-in-munit#redirecting-logs-to-a-file[Logging in MUnit].
