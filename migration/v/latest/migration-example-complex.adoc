...
image::m3-apikit-based-example-main.png[api-main in Mule 3]
...
// Now do the same for the next migrated feature, and so on.
. Add `APP:NOT_FOUND` exception type to the ApiKit-generated 404 mapping to handle the exception thrown by the `validation:is-true` element in `get-product-by-id-flow`.
+
