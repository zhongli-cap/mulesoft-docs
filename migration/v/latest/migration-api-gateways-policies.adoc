...
Another example is presented, where the policy does a client ID validation and if it fails, an error HTTP response
is returned.
...
In the Mule 4 example, it is shown how the policy can make use of its own Error Handler, something that in Mule 3 was not possible
