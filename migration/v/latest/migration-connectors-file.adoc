...
TODO: BRIEF OVERVIEW OF CHANGES

In Mule 3:

* The connector can serve as an inbound or outbound endpoint depending on where you place the connector in the flow. Many configuration options depend on the endpoint type.
* etc. TODO

In Mule 4:

* Th connector can read, write, and perform many new file operations, but it never serves as an inbound endpoint that triggers the flow.
...
However, the File connector configurations can also these properties:
 ** Working Directory where your files are located, which is required.
 ** Reconnection strategies for cases where the initial attempt to connect fails.
 ** A default write encoding for all files to which you write content. You can override this default encoding when specifying a particular File connector operation.
