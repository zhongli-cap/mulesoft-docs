...
In *Mule 4* the DB Connector also provides the capability of configure the connection properties
as a DataWeave expression. E.g.: `connectionProperties="#[{ 'userId' : 123123 }]"`
...
* <<tx_actions>>
* Query settings
* Target variable output settings (<<output_target>>)
* Reconnection strategies
...
In Mule 4, in the same way as Mule 3, the DB Connector supports Dynamic Queries which means
the usage of Expressions, *MEL* in Mule 3 and *DataWeave* in Mule 4, to generate parts
of the query to execute.

The only difference between Mule 4 and Mule 3, is that doesn't exist anymore an exclusive
element to execute dynamic queries `<db:dynamic-query>`, in *Mule 4* the plain `<db:sql>`
supports to embed DataWeave expressions to dynamically create queries.
...
[source,xml, linenums]
----
<db:select config-ref="db_config" streaming="true">
    ...
</db:select>
----
...
TIP: Note that making streams repeatable allow you to use DataWeave and other components to process the a stream many times, even in parallel.
