...
////
[[database_insert_update_delete]]
== Migrating Insert, Update, and Delete Operations

In Mule 3, the Insert (`<db:insert />`), Update (`<db:insert />`), and Delete (`db:delete />`) operations supports these features:

* Bulk Mode (<<database_operation_bulk>>)
* Parameterized, Dynamic, Templated (meaning parameterized or dynamic queries specified in a template) queries

The Insert, Update, and Delete operations also support the use of DataWeave parameters to get results from dynamic queries.

TODO: NEED MULE 3 COUNTERPARTS
...
In Mule 4, you use dedicated Bulk Insert (`<bulk-insert>`), Bulk Update (`<bulk-update>`), and Bulk Delete (`<bulk-delete>`) database, instead. This means that operations no longer change their behavior depending on the received payload.
