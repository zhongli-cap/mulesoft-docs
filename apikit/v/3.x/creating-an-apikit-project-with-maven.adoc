...
The link:/apikit/v/3.x/apikit-basic-anatomy[`api.raml`] file in `src/main/api` is initially empty. To populate the file, complete one of the following tasks:

* Edit the `api.raml` file to define your API.
* Use the provided link:/apikit/v/3.x/_attachments/sample_RAML.raml[sample RAML file] as an example.
...
You need to generate link:/apikit/v/3.x/apikit-basic-anatomy#backend-flows[backend flows] for each resource-action pairing in the RAML you create.
...
. link:/apikit/v/3.x/apikit-using#backend-first-design-process[Enhance backend flows].
