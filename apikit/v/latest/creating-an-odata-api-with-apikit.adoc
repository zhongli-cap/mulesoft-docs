...
APIKit OData Extension (Beta) is in beta, and requires Studio 5.4. All bugs and feedback must be submitted through the link:https://beta.mulesoft.com/home.html[MuleSoft Beta Portal]. If you would like to provide beta feedback and don't have an account, send an email to mailto:beta@mulesoft.com[beta@mulesoft.com].
...
This document describes APIKit support for OData.

image:apikit-odata-cycle.png[APIKit Odata Extension Development Cycle]

== API Gateway Support

APIKit OData Extension API Gateway 2.2.0 compatible, just by updating the versions of the dependencies in the `.pom` file to `APIKit 1.7.3` and `APIKit OData 0.9.1`. To use with API Gateway 2.2.0, right-click in Anypoint Studio, and select `Create Shared Domain`.
...
Using the template is the quickest, easiest way to build your OData-enabled application; however, it is also possible to do so without the template.
