...
When you import an API from Anypoint Platform, APIkit includes any RAML dependencies in the Studio project. You can then keep the API in both locations link:/anypoint-studio/v/6/api-sync-reference[in sync] by pushing or pulling changes from one to the other.
...
Included in the project are backend flows containing REST resource-action mappings or SOAP message templates. You can simulate calls to the API using APIkit Console when you import a RAML. 
...
In a RAML-based API, if you change or add error handling, you need to adjust references to error handling.
...
You can add APIkit components to an existing design, for example a legacy project that you want to modernize using RAML and APIkit. Create the RAML code within Studio as follows:
...
=== Generate REST Backend Flows and Error Handling

When you create a RAML or WSDL from within Studio, you have the option of generating backend flows, or not. Error handling is generated for a RAML-based API. You need to reference these flows from the main flow. This procedure assumes you have already created a main flow.
