...
In releases of APIkit 1.7.3 and earlier, applying a policy to the API that denied any request rendered APIkit Console inaccessible. The APIkit Console flow xml in Anypoint Studio 6.0 and later consists of two configurations to accommodate policies applied to the API endpoint:
...
The first configuration consists of a listener and is not affected by API policies. Use a URL that is not part of the api namespace, for example `/console` to access a console when using a policy that blocks access. By using this path, you can apply policies to the API. Policies will be applied to `/api/*`. Using the Try It button will result in an error.
