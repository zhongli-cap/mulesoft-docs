...
APIkit supports RAML-based development out-of-the-box on API Gateway Runtime or Mule runtime version 3.5.0 or later. For WSDL-based development, install the APIkit for SOAP plugin on Studio 6.x or later. APIkit projects function with on-premises and CloudHub runtimes.
...
Included in the project are backend flows containing REST resource-action mappings or SOAP message templates. APIkit Console is included when you import a RAML, but not a WSDL. APIkit Console is similar to link:/api-manager/designing-your-api#api-console[API Console] for simulating calls to the API.

After working with an RAML-based API in APIkit Console and improving or changing behavior, you alter the backend flow to act upon real resources with real data.

== Changing Generated APIkit Functionality
...
In a WSDL-based API, if you change the WSDL, regenerate backend flows. Click *Mule* > *Generate SOAP Flows*.

== Backend-First Design Process

If you create the RAML or WSDL code within Studio, you need to perform a number of tasks to generate APIkit components:
...
image::apikit-beyond-the-basics-3887d.png[apikit-beyond-the-basics-3887d]
...
By default, APIkit automatically opens an *APIkit Console* panel in Studio when you run a RAML-based APIkit project. APIkit Console gives you access to the auto-generated, fully testable, interactive documentation for the API. 
...
Because the APIkit Console is a client that is accessing your API by making calls against it, hosting the console in the same URI as the API itself can cause policies to restrict calls more than you intended. Calls to populate the console and the favicon.ico count can have an impact, depending on applied policies. For example, if you have a rate-limiting policy applied, loading the APIkit console consumes two of your allotted API calls – one to load the console in the browser and one for the favicon.ico. If your HTTP listener is protected with Basic Authentication, you need to authenticate in order to access or use the console. In particular, the APIkit Console doesn't know the steps of the OAuth dance, so if you apply an OAuth policy to the API, you will not be able to access the APIkit Console at all if you attempt to host multiple consoles in the same URI.
