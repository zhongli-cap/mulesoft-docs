...
Included in the project are backend flows containing REST resource-action mappings or SOAP message templates. You can simulate calls to the API using API Console when you import a RAML, but not a WSDL. You need to use a SOAP testing tool, such as SoapUI.
...
=== How to Expose API Console

There are two ways to expose the APIkit Console. The recommended way is to create a flow dedicated to exposing the console. For example:
...
The path of the console is not a part of the api namespace as console uses its own listener. Then you will be able to apply policies to your api without affecting the console.

The second way, now deprecated, is configuring the apikit:config in the following way:

==== Enabling and Disabling API Console
