...
CloudHub requires that application name is unique no matter the region where the API is deployed. For that reason, if the API needs to be deployed in multiple region, the region name needs to be added to the application name. For instance,

`test-customers-eu`

`test-customers-na`

By default, APIKit add base path to each API, namely `/api`.

Thus, the final URL of the APIs deployed on CloudHub will be as follows:
...
The MuleSoft best practice is to drop an environment prefix for production. For example:

`http://customers-v1.eu.cloudhub.io/api/customers`

You can setup desired URLs for each CloudHub application by using custom load balancer. For instance, you can configure `http://api.yourcompany.com/customer/v1` to point to this URL:

http://customers-v1.eu.cloudhub.io/api/customers

=== Naming Conventions for Deploying APIs on Premise

By default, APIKit generates a listener having a base path of `/api`.  In this case, a different application deployed on the same server will attempt to listen on the same context. For example:

`http://server:port/api/`

A conflict occurs. To avoid such conflicts, configure each Mule application needs with a different base path. For example:

`/api/customers`

`/api/products`

If multiple versions of the same application need to run on the same server, then version needs to be added to the base path:
...
Avoid hard-coding URLs in base path. Use different property files for different deployments as shown here:

image::apikit-using-7d817.png[apikit-using-7d817.png]

A custom load balancer can be configured with desired URLs. For example, `http://api.yourcompany.com/customer/v1` can point to `http://server:port/api/customer/v1`.
