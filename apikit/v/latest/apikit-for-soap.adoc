...
Mule EE Runtime is required for using APIkit for SOAP. APIkit for SOAP supports consuming a remote WSDL, such as the link:http://devkit-cookbook.cloudhub.io/soap?wsdl[Cookbook WSDL file]. The following tutorial, however, uses a local WSDL.

== Create a New Project

To create an APIkit for SOAP using Studio, follow the instructions in this document. The link:/apikit/apikit-reference[APIkit Reference] contains information about the configuration.xml for using APIkit for SOAP from the command line.
...
+
. Create the request to order the t-shirt by entering some values. For example:
+
* Enter *L* between the tags for the `<size>` element in the envelope.
+
...
The SoapUI, the request window shows the soap Envelope header.

image::apikit-for-soap-53dc2.png[apikit-for-soap-53dc2]

In this part of the tutorial, you modify the API to use header information sent by the request.

. In the SoapUI, modify the request to order the t-shirt by entering some values. For example, enter `987654321` between the `<apiKey>` tags.

. In Studio, edit the *Transform Message* mapping by adding the following DataWeave code:
+
[source,xml,linenums]
----
{
...
To add a header to the outgoing message to fulfill the expected contract:
...
+
. Select *property* from the Variable drop-down.
. In *Variable name*, paste the contents of the clipboard, and add soap. as a prefix:  `soap.APIUsageInformation`.
. Click OK.
...
. Change `null` to `10`.
. Save and rerun the project.
. In the SoapUI, hit the same endpoint by executing the OrderTshirt request. The response envelope from APIkit for SOAP is:
+
[source,xml,linenums]
----
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
...
APIkit for SOAP supports typed SOAP Faults. For the `tshirt2.wsdl` file, there is just one typed fault named `TshirtFault`, which is mapped to every operation that the WSDL has: OrderTshirt, ListInventory, and TrackOrder.
