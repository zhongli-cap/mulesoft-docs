...
After generating backend flows, you integrate them with functional processes that take the expected input and produce output. Studio infers from the RAML information about the metadata for the input of a flow and exposes this information through link:/mule-user-guide/v/3.8/datasense[DataSense].
...
Backend flows mock responses of the REST API for simulating calls to the API. Each RAML-based backend flow has one or more message processors for simulating calls to the API in APIkit Console. The simulation is based on the content of the example responses in RAML.
