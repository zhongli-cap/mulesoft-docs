...
For example purposes, add over-simplified logic to the project to use later on Anypoint Platform. Unlike the APIkit logic generated for the simulation, the flow you create during this procedure returns a result, a payload message, when you call the API from a browser.
...
+
. Click OK.
. In the properties editor, change the Path from `/` to `/test-policy`.
...
You can deploy the project to Anypoint Platform to enhance the RAML using API Designer or to apply a policy to the API, for example. First, in Studio, prepare the API for auto-discovery, and then run the project as a Mule application. 

To deploy an APIkit project to Anypoint Platform:

. link:/api-manager/api-auto-discovery[Configure and trigger auto-discovery] by entering auto-discovery settings for a global element as follows:
+
* API Name: vending
+
...
INFO  2016-08-20 12:54:28,774 [main] com.mulesoft.module.client.autodiscovery.AutoDiscoveryDeploymentListener: Successfully registered source http://192.168.1.127:8081/remote-vending/api/* to API vending with version 1.0development
INFO  2016-08-20 12:54:29,038 [main] com.mulesoft.module.client.autodiscovery.AutoDiscoveryDeploymentListener: Successfully added root RAML to vending with version 1.0development
INFO  2016-08-20 12:54:29,356 [main] com.mulesoft.module.client.autodiscovery.AutoDiscoveryDeploymentListener: Successfully created API named test-policy with version 1.0development
INFO  2016-08-20 12:54:29,553 [main] com.mulesoft.module.client.autodiscovery.AutoDiscoveryDeploymentListener: Successfully registered source http://192.168.1.127:8081/remote-vending/test-policy to API test-policy with version 1.0development
INFO  2016-08-20 12:54:29,847 [main] com.mulesoft.module.client.autodiscovery.AutoDiscoveryDeploymentListener: Successfully added root RAML to test-policy with version 1.0development
----
+
. Sign into Anypoint Platform, or if already signed in, refresh the browser.
. Go to API Manager.
+
