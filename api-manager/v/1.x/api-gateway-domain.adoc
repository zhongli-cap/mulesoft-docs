...
You can link:/api-manager/v/1.x/setting-up-an-api-proxy#modifying-a-proxy[modify the proxy] for your application to reference the domain, or not.
...
If you are using the link:/api-manager/v/1.x/api-auto-discovery[API Autodiscovery] feature, your endpoint registers itself using the local IP address. If your IP changes, update your endpoint accordingly. When you deploy the proxy or app on CloudHub, the Autodiscovery mechanism assigns privileges to the Cloudhub domain.
