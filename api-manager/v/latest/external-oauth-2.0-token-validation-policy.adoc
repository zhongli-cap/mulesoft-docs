...
* *`authorizationUri`* |Example: https://my-auth2-provider.cloudhub.io/authorize
* *`accessTokenUri`* |Example: https://my-oauth2-provider.cloudhub.io/access_token

In the mule-app.properties file, host must be 0.0.0.0. Port should be set to `${http.port}`. For HTTP, specify `http.port=8081` and for HTTPS, specify `http.port=8082`.
