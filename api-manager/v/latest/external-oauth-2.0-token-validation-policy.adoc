...
You need to configure a policy to match the grant types specified in your RAML securitySchemes. The RAML syntax for the authorizationGrants setting is:
...
The following table shows the mapping of the RAML grant types to the grant type names in the policy configuration. 
...
After a token is obtained, you must include it in all requests sent to the API. There are two ways you can include it:
