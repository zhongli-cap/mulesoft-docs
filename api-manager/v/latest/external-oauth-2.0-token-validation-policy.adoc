...
This document describes how to protect an API using the OAuth 2.0 Access Token Enforcement Using External Provider policy and to create an OAuth2 provider. To access an API on an OAuth2-protected server that is not managed by API Manager, configure authentication in link:/mule-user-guide/v/3.8/authentication-in-http-requests[the HTTP Requester connector].  
