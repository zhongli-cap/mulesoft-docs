...
You can apply one or more policies to govern your API version at runtime. You can also remove the policies at runtime. API Manager enforces policies on APIs having an HTTP/HTTPS endpoint. To apply policies to an API, you need link:/api-manager/creating-your-api-in-the-anypoint-platform[to register] the API in API Manager.
...
* An API deployed to an link:/api-manager/deploy-to-api-gateway-runtime[on-premise or cloud API Gateway] Runtime 2.x.
