...
A policy is a mechanism for enforcing filters on traffic. These filters generally control authentication, access, allotted consumption, and SLAs. API Manager offers a number of pre-built policies, listed in link:/api-manager/using-policies#available-policies[Available Policies]. You can build custom policies to support your needs if out-of-the-box policies do not. 

You can apply one or more policies, such as Client ID Enforcement, to govern your API version at runtime. This policy allows only authorized applications to access the deployed API implementation. Applications are authorized to use a specific API Version. Each authorized application gets credentials: client_id and client_secret. At runtime, authorized applications need to provide these credentials with each request to the API implementation in one of the following ways, depending on the policy configuration:

* In the headers of the HTTP request
* In the URL as query parameters, this depends on the policy configuration
...
API Manager enforces policies on APIs having an HTTP/HTTPS endpoint. To apply policies to an API, you need link:/api-manager/creating-your-api-in-the-anypoint-platform[to register] the API in API Manager.
