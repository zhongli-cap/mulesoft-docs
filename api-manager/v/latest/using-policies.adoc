...
You can apply one or more policies to govern your API version at runtime. You can also remove the policies at runtime. API Manager enforces policies on APIs having an HTTP/HTTPS endpoint that link:/api-manager/creating-your-api-in-the-anypoint-platform[are registered] in API Manager. Here are a few examples:

* Apply a policy to a project having an HTTP endpoint, such as an APIkit project. 
+
For example, deploy the APIkit project to Anypoint Platform using auto-discovery and apply a policy, as shown in the link:/apikit/apikit-tutorial#deploying-the-project-to-anypoint-platform[simple REST tutorial]. 
+
* Apply a policy to an API running on CloudHub.
+
...
* Apply a policy to an API running on API Gateway 2.x.
+
Deploy a proxy using the link:/api-manager/setting-up-an-api-proxy#deploying-a-proxy[automatic on-premises procedure], and apply a policy.

* Apply a policy to an API running on Mule Runtime 3.8.x 
+
Deploy a proxy using the link:/api-manager/setting-up-an-api-proxy#deploying-a-proxy[manual on-premises procedure], and apply a policy.
