...
You can apply one or more policies to govern your API version at runtime. API Manager enforces policies on APIs having an HTTP/HTTPS endpoint. To apply policies to an API, you need link:/api-manager/creating-your-api-in-the-anypoint-platform[to register] the API in API Manager.
