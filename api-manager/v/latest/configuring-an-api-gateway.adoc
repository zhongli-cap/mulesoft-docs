...
+
. When deploying the application, select the runtime *Gateway 1.3* (or a higher version of the Gateway runtime) in the the *Mule Version* field.
...
For all endpoints that you register in API Manager that point to proxies running on CloudHub, specify your host and port names according to the link:/runtime-manager/developing-a-cloudhub-application[CloudHub standards]. For the HTTP or HTTPS connector, specify the host as *0.0.0.0* and the port `${http.port}` or `${https.port}` in your application, as described in link:/runtime-manager/developing-a-cloudhub-application[Developing a CloudHub Application]. If you use HTTPS, there are link:/runtime-manager/building-an-https-service[additional steps] you need to take.
