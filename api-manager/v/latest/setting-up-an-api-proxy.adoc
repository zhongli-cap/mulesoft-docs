...
[code]
----
$ cf bind-route-service apps.pcf.mulesoft.com anypoint_gateway_service --hostname myapp -c input.json
----
...
|`appName` | The name with which you want your proxy application to be registered on Runtime Manager. It will also register an API on API Manager under this same name.
|`appHost` | The full address where you want to host your application. It should match the same domain and hostname that you provided as properties on the command. So if your domain was `apps.pcf.mulesoft.com` and your hostname was `myapp`, your appHost should be `myapp.apps.pcf.mulesoft.com`
|===
...
----
{
    ...
    "appName": "proxyApp",
    "appHost": "myapp.apps.pcf.mulesoft.com"
}
----
