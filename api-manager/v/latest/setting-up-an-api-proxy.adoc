...
Before deployment, you set up an API proxy to control access to a web service. To accomplish this, configure an endpoint from the Status section of the API version details page of an API.

Select configuration options as follows:
...
* Accept the default Type of endpoint, RAML for example, or select one of the other type options.
* In Implementation URI, accept the default based on the baseUri from the RAML definition, or enter one of the other URI options.
...
To deploy a proxy to API Gateway 2.x, follow instructions in the API Gateway archive documentation.

== Downloading a Proxy

You can download the latest, or a legacy API Gateway Runtime, in ZIP file format. The file is a deployable proxy application. Click *API Status* > *Download proxy*. A .zip file is downloaded.
+
...
Deploy the proxy application. After deployment, the yellow circle in the status indicator turns green.
