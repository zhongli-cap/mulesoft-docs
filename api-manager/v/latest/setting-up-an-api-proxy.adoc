...
* link:/api-manager/creating-your-api-in-the-anypoint-platform[Create an API version] in API Manager.
...
* *On-premises deployment*: Access to an on-premises Mule 3.8.0 Runtime instance or an API Gateway instance that has been configured to pair with your organization. link:/api-gateway-rt/configuring-an-api-gateway[Configure the on-premises API Gateway] if using versions of Mule earlier than 3.8.0.
...
+
* Select the *Reference user domain* option. This option requires creation of link:/api-gateway-rt/api-gateway-domain[an API Gateway domain].
+
...
. Now it's time to link:/api-manager/deploying-your-api-or-proxy[deploy your proxy application] to the API Gateway.
+
