...
API Manager is a component of Anypoint Platform for managing Mule APIs. API Manager acquires APIs to manage in these ways:

//update link to API Platform API

* Publishing an API to API Manager from Design Center
* Setting up a reference to an API in Exchange
* Importing an API in ZIP format from the file system
+
The ZIP is an API object, such as an API exported from Exchange or API Manager.
+
