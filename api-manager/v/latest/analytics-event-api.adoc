...
The following steps show you how to run a report. Anypoint Analytics lists the starting point for when data is available for the API, including a warning message if no data is available for the API.
...
The following steps use curl to call the report's API endpoint and programmatically retrieve the analytics data. If you're using Windows, you can use PowerShell as an alternative to curl.
...
. In Anypoint Platform > Access Management > Organization, click your organization name:
+
