...
On the link:/api-manager/tutorial-set-up-and-deploy-an-api-proxy#navigate-to-the-api-version-details-page[API version details page], you can configure the inbound endpoint of your API so that Anypoint Platform can manage it, either directly or by proxy. 
...
|*WSDL* |Select this option for SOAP APIs. Provide the source where Anypoint Platform can extract the URL. For example, you can set `http://myapiendpoint.com` if your WSDL is available at `http://myapiendpoint.com?wsdl`.
