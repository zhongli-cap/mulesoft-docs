...
Users who you can access your API portal can request access from their applications to your API. The applications that have access are listed on API Manager > Client Applications. To see the Client Applications link from all Business Groups, you need to be an Organization Administrator.
...
image::api-manager-client-app-controls[client app controls]
...
If you want to delete an application that you gave access to your API, click API Manager > Client Applications. Select the name of the application to delete. Click Delete image:trash.png[trash icon].
...
After you have approved a client app to access your API, you can access information about the app from API Manager > Client Applications. You see and can reset the unique client ID and client secret for your application, which the client app owner needs to pass to the API it calls if the API is protected with policies. 
