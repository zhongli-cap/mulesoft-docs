...
+
Anonymous users can view any API portals that you made public.
+
* My applications
+
You can search for and sort the list of apps approved to access the API.

To manage an app, click an app name in the list. The app page appears. Subject to role and permissions, you can use controls on the developer portal for removing app access privileges. Delete the app from the portal to remove access privileges. You can reset the client ID and client secret, update app properties, such as the name and URL, and view information about the app, such as its SLA tier.
