...
The OAuth Token Enforcement policy protects the API by restricting access. Unless the policy validates the access token it receives, access is denied. The token is not defined on the OAuth provider, it has been granted by it.
