...
Do _not_ use a placeholder, such as {userid}, in an expression. Using `{userid}` placeholder in an expression fails because the placeholder does not match an actual ID, which looks something like this:
...
To apply a policy to the resource `/api/users/{userid}` itself, which represents a single user, and all nodes below the resource, use the following expression:

`/api/users/.*`

To apply a policy to only the permissions resources, `/api/users/{userId/permissions}` and `/api/users/{userid}/permissions/{premissionId}`, use the following expression:
