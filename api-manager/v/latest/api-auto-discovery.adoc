...
Auto-discovery is a configuration in Studio that registers and starts an API in API Manager when you run the API as a Mule Application from Studio. After an API is registered and started in Anypoint Platform, API Manager tracks and shows the API status, enforces policies if you apply any to the API, and reports analytic data about the API.

If an API is not registered on Anypoint Platform, you can create the API on Anypoint Platform by following the auto-discovery procedure in this document. 
