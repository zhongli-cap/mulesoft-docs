...
* Install link:/api-gateway-rt/install-studio-gw[API Gateway Runtime from Studio].
...
. link:/api-gateway-rt/install-studio-gw[Install a current API Gateway Runtime]. For example, install API Gateway Runtime 2.2.0.
. In a browser, log in to the Anypoint Platform as an administrator, click the gear icon at the top-right, and then select the *Organization* tab.
+
The client ID and client secret appear.
+
