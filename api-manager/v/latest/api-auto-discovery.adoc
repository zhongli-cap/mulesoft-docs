...
Auto-discovery is the capability of Anypoint Studio to create and register an API in API Manager. Using auto-discovery ensures that the API Manager manages the API in the following ways:
...
* Install link:/anypoint-studio/v/6.0/install-studio-gw[API Gateway Runtime from Studio].
...
. link:/anypoint-studio/v/6.0/install-studio-gw[Install a current API Gateway Runtime]. For example, install API Gateway Runtime 2.2.0.
. In a browser, log in to the Anypoint Platform as an administrator, click the gear icon at the top-right, and then select the *Organization* tab.
+
The client ID and client secret appear.
+
