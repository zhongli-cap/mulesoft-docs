...
Auto-discovery is the technology employed by Studio and API Manager that registers an API in API Manager when you deploy the API, properly configured, from Studio. Using auto-discovery ensures that the API Manager manages the API in the following ways:
...
You need to configure auto-discovery for each flow that you want to manage in a Studio project. For example, 
...
* Create a global element for each flow in Studio that you want to manage, and configure the element for API auto-discovery.

Deploying the API project as a Mule application in Studio triggers auto-discovery, and an API appears on the API Administration page in Anypoint Platform for each flow you configure for auto-discovery. 
...
* Install link:/anypoint-studio/v/6/install-studio-gw[API Gateway Runtime from Studio].
...
. link:/anypoint-studio/v/6/install-studio-gw[Install a current API Gateway Runtime]. For example, install API Gateway Runtime 2.2.0.
...
You must reference that global element in the inbound endpoint that you wish to register in API Manager. Do this by adding a nested element in the inbound endpoint that includes an `api-ref` or `apikit-ref` that references the global element by name or ID.
...
When you deploy applications to an API Gateway runtime paired with API Manager, the agent performs the following steps to auto-discover and register your service version and/or endpoints in API Manager.
