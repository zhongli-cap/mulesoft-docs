...
A policy can modify message content. Starting from Mule 4.1.0, if the message is modified before the `execute-next` in `source` policies
...
there is a new flag called `propagateMessageTransformations` that can be enabled in each policy.
