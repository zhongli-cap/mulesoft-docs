...
----
. Download and unpack the link:_attachments/custom_oauth2_provider.zip[mSquaredMVP project].
...
. Encode the client ID and client secret pair in base64 per the HTTP Basic Authentication specification.
+
This specification has been superseded by 7235, but is better describes HTTP Basic Authentication. These following example shows the syntax for encoding these credentials. 
+
...
----
+
The output looks something like this and returns the access token:
+
----
HTTP/1.1 200 
Content-Length: 250
...
----
. In a browser, use the access token obtained to validate a call to the provider:
+
----
