...
*To build and exercise template functionality in Mule Runtime 3.8.x:*
...
+
. Request a token using the user name/password credentials specified in the project: user max, password `mule`. Include the basic auth header from the output of the last step.
+
----
...
----
+
The access token appears in the output. For example:
+
----
HTTP/1.1 200 
Content-Length: 250
...
----
. In a browser, validate a call to the provider using the access token:
+
----
