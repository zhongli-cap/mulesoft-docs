...
. Import the downloaded OAuth2 provider template .zip file into Anypoint Studio as an *Anypoint Studio Generated Deployable Archive (.zip)*, make sure it is using Mule 3.8.0 or API Gateway 2.x Runtime.
...
. Open the project’s `config.xml` file in Studio.
