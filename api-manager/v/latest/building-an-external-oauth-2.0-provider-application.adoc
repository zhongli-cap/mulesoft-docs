...
Another key configuration task is specifying scopes of access using the link:https://tools.ietf.org/html/rfc6749#page-23[scope] request parameter. Remove the default `READ WRITE` from *Scopes*, if you want to use API Console to simulate the API, and specify no scopes using an empty string ("").
