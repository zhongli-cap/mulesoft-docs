...
API Manager is the key component of Anypoint Platform™, which offers a secure and flexible architecture that supports cloud-based, on-premise, and hybrid use cases. API Manager can use the link:https://docs.mulesoft.com/api-manager/api-auto-discovery[auto-discovery process] to register and start a Mule app deployed from Studio. Registration and start up occurs without user intervention. 
