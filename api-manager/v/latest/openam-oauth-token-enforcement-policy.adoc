...
OpenAM and PingFederate define and support two kinds of OAuth clients: 
...
In Mule 3.8.x, you optionally enable or disable the proxy network connection between the API and PingFederate by setting the following parameter:
...
In earlier versions of Mule, use the following parameter:

`anypoint.platform.ping_federate_enable_proxy_setting`

This parameter is located in the following wrapper configuration file for API Gateway Runtime 2.1.x and 2.2.x: `/conf/wrapper.conf` file from the API Gateway software distribution.
