= Spike Control Policy Reference (Nov 2017)
...
The request delay is what softens spikes in traffic, as in high contention scenarios the backend will continue to function properly (it will have served no more than X request in the last Y milliseconds), and users querying the API may still see their requests being served, though with higher latency. Of course, the exact configuration depends exclusively on your API, and its throughput.
