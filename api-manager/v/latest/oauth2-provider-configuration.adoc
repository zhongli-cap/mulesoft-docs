...
Key OAuth2 provider configuration tasks are:
...
In the `config.properties` of the custom link:/api-manager/building-an-external-oauth-2.0-provider-application[OAuth2 provider example], you find these endpoint paths.

You also specify the validation endpoint path when apply the policy to the API, specifying the endpoint of the provider in `Access Token validation endpoint url`:
