...
API Designer for the RESTful API Modeling Language (RAML) is the legacy web-based editor for authoring RAML specifications. Using Design Center to author RAML specifications is recommended. API Designer supports RAML and the Open API Specification (OAS) 2.0, also known as Swagger. When you import an OAS specification, API Designer converts OAS code to RAML.
