...
+
When you link:/api-manager/designing-your-api#importing-an-oas-2-0-specification[import an OAS specification], API Designer converts OAS code to RAML. Importing and converting OAS to RAML is a beta feature and not recommended for production usage.
+
* link:/apikit/apikit-using[APIkit] 3.8.1 and later
+
* RAML-interpreting frameworks
+ 
Frameworks, such as Osprey and JAXRS-Codegen, can consume APIs that you create in API Designer.
...
+
API Designer appears:

image::designing-your-api-c9fdd.png[designing-your-api-c9fdd]

* Files panel
+
Shows the default file name `api.raml` for RAML code. A preceding asterisk indicates unsaved content.
+
* RAML editor panel
+
Contains generated code: the default RAML version 1.0 and names of the API and API version, which you provided to API Manager.
+
* API Console
+
This panel displays data types, resources, and methods in your API. Until you define the API, the Console displays an empty Resources list. Turn on the Mocking Service on the top, right-hand side to simulate calls to the API.
...
The capability to import an OAS 2.0 specification is at the beta stage of development in API Designer 2.0 and not recommended for production usage. The following example shows how to import the Swagger pet store example.
