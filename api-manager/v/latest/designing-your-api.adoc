...
Library information is lost. A single file with library and root raml information is generated. The parser generally handles the conversion of overlays and extensions, but there is a potential for loss because no actual mapping occurs.
