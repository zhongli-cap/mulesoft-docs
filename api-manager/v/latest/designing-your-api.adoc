...
API Designer supports the capability to import an OAS 2.0 specification. To import the Swagger pet store example, first validate the OAS document, using the link in the following section, before attempting to import the document into API Designer. Next, import an OAS 2.0 using Project > Import. Select OAS spec from the drop-down. Finally, in the text entry box, enter either the URL of an OAS spec or the path and file name of an OAS .zip file. The converted OAS code appears in RAML in API Designer.
