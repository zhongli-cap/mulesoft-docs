= Rate Limiting and Throttling - SLA-Based Policies (Nov 2017 and Jul 2017)
:keywords: sla, portal
...
SLA-based throttling works much like SLA-based rate limiting, except requests that exceed the SLA limits aren’t rejected, but instead are queued for later reprocessing. This requests will be retried periodically according to the configuration until they are either accepted, or rejected as they were retried the maximum amount of times.
