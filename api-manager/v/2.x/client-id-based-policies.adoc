...
*Note:* When you ask for a token to the OAuth provider, ensure that the specified client ID and secret belongs not only to a client application in the Anypoint Platform organization where the API you are trying to access resides, but also that client application is registered to and has a contract with the API being targeted. If the token issued provides credentials from a client app not registered to your API, then even if the token is valid, you get a 403/Forbidden response.
