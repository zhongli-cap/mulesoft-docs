...
This topic describes how to implement access to the object store in a custom policy as the underlying cache, and how that Object Store can be used by multiple policies applied in different APIs that are running in different instances when they are deployed in Runtime Manager.
...
The following code shows a generic piece of code that allows to retrieve something from the Object Store and in case nothing is found for a specific key, then a new value is retrieved from the external system and then stored in the Object Store.
...
In this example, the `<os:retrieve key="token" target="myVar" objectStore="objectStore"/>` statement executes the retrieve operation of the Object Store connector using the value token as the key to look for and assign the retrieved value to the new `myVar` variable.

If there is no value associated with a key, an error is thrown by the retrieve operation. This is why the operation is enclosed in a `<try>` block, so that should an error occur, it can be handled by retrieving a fresh value and storing it in the Object Store. 
...
After this block executes, myVar contains a value that was retrieved from either  the cache or the external system and stored in the cache for later use. You can later use that value for any purpose such as adding it as new header to a Request or Response, or anything needed by the user.

== Defining an Object Store

When using the Object Store connector operations, it is necessary to define the Object Store that those operations use. 
