...
The application ports that must be exposed through a load balancer or proxy from the cluster are:

[%header%autowidth.spread]
|===
|Protocol |External Port |Cluster Port |Purpose
|TCP |80 | 30080 |HTTP redirects to HTTPs
|TCP (TLS) |443 | 30443| HTTPS port
|TCP (TLS) |8889 | 30889 | WebSocket Port for Runtimes to connect
|===
