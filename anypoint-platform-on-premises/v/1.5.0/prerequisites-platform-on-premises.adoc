...
Unless specified, the system directory (`/var/lib/gravity` by default) is created on whatever device /var/lib is mounted on. This directory will be automatically formatted and prepared for use.
...
[TIP]
====
You can check if a device is unmounted with the following command:
---
lsblk --output=NAME,TYPE,FSTYPE -P --bytes /dev/sdc
---
...
In order to reset a device, you will have to work backwards:
  ...
  * remove physical volume and reset device with `pvremove /dev/sdc` (where /dev/sdc is used as an example - use `pvdisplay` to find the physical volume to remove and the device name it is on)
