...
SELinux should not interfere with the installation or running of Anypoint Private Cloud Edition.

Early in the installation, two Systemd unit files are created: `/etc/systemd/system/gravity__gravitational.io__planet-master__*.service` and `/etc/systemd/system/gravity__gravitational.io__teleport__*.service`. If these files are not created, it may be because the action is restricted by the user's SELinux context. To rule this out, you can review SELinux's audit log, in `/var/log/audit/audit.log`. If any actions were restricted, they'll be logged there, and with the assistance of a system administrator, you should be able to allow the creation of those files.
...
Note, that the platform only supports the specific device-types shown in the drop-down list, as shown below:
|===
