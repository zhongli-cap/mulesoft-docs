...
In the general case, SELinux should not interfere with the installation or running of Anypoint Private Cloud Edition.

Early in the install, two Systemd unit files are created: `/etc/systemd/system/gravity__gravitational.io__planet-master__0.1.36-138.service` and `/etc/systemd/system/gravity__gravitational.io__teleport__1.2.0.service`. If these files are not created, it's possible that the combination of the configuration your administrator has chose and the install user's SELinux context that this action was restricted. In that case, it's helpful to review SELinux's audit log, in `/var/log/audit/audit.log`. If any actions were restricted, they'll be logged there, and with the assistance of a system administrator, you should be able to allow creation of those files.
