...
`Error from server: container "anypoint-backup" in pod "anypoint-backup-d96d79-zcld0" is waiting to start: PodInitializing`

This simply means that the node that performs the backup is not up and running yet.
====
