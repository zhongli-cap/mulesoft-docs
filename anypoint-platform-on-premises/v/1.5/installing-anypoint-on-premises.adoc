...
You must direct your local installation of Anypoint Platform to this server, by configuring the link:/access-management/on-premises-features#smtp[Access Management panel], as explained in link:/anypoint-platform-on-premises/v/1.5/setting-smtp-manually[Setting SMTP Manually].

[NOTE]
SMTPS is currently not supported by the Anypoint Platform On-premises Edition, only regular SMTP.
