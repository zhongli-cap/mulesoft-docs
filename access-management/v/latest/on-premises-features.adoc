...
The Anypoint Platform On-premises Edition requires that you set up your own SMTP email server to deliver email alerts for both *API Manager* and *Runtime manager*. The SMTP section on the Access Management menu lets you reference the server that you set up.
...
[CAUTION]
--
Currently, only unsecure connections are supported for SMTP, meaning that a server that uses SMTPS can't be used.
--
