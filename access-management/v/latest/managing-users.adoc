...
After choose an IdP, follow this procedure to configure SAML.

Check your identity provider's documentation before configuring identity management in Anypoint Platform. For the providers tested my MuleSoft, the 'Assertion Consumer Service' or 'SAML Assertion URL' is likely `https://anypoint.mulesoft.com/accounts/login/receive-id` and the 'entityID' or 'Audience URL' is an arbitrary string value that identifies your organization. The conventional value for this string is `<organizationDomain>.anypoint.mulesoft.com`.
