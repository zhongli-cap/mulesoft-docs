...
== Inviting an External User to Access an API on a Public API Portal

You can invite users who are not part of your organization to access a public portal. This enables external users to view assets of your public portal and register client applications against a public API portal.

External users are listed in Access Management under the External Users tab of the Users page. This tab lists users from external Anypoint organizations who have registered API client applications against your public API portals.

Before you can invite an external user, the user must:

* Be a registered Anypoint Platform user.
* Login to your public portal.

=== Inviting an External User

. Send an invitation to a user.
. Ensure that the user logs in to your portal.

After logging in, the user appears in the External Users. After logging in, the external user has access to your public portal. You can also disable or enable external users from this tab.
