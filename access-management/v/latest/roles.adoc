...
Depending on the product to which the role is associated, these options may vary. For example, API roles cannot be removed and their permissions cannto be modified, however you can add a description and add users to that role.
...
. Choose the product whose permissions you want to assing (a full list of permissions can be found in our link:/access-management/managing-permissions[permissions] section)
