...
=== To Assign Application Permissions to a Custom Role

=== To Assign API Permissions to a Custom Role

You can also associate an API or an Anypoint application to the custom role. This enables you to create roles that give specific users access to specific applications or APIs. For applications, you can also specify the environment users in a role have access to. You can associate the following to a custom role:
...
. To assign a user to a custom role:
.. Click the Users tab.
.. In the search text box, enter the email address or name of the user you want to add.
.. Click the + icon to the right of the search text box.
. To associate an API or an application component to a custom role:
..
