...
This document assumes that you are familiar with the connector architecture presented in link:/anypoint-connector-devkit/v/3.9[Anypoint Connector DevKit], and that you are familiar with link:https://docs.mulesoft.com/anypoint-studio/v/5/datasense[DataSense] from an end-user's perspective.
...
[NOTE]
The *fundamental advantage of DataSense* is the ability to extract metadata from the data model exposed by an application through the connector. Learn more about link:https://docs.mulesoft.com/anypoint-studio/v/5/datasense[DataSense].
