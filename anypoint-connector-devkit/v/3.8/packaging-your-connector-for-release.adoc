...
. Provide the *Connector version* in its field in Exchange. Consumers of the connector only see the shorter version number. For example if the version number you provide is 2.0.1.201606101417, they only see 2.0.1. (The full version number displays ONLY when editing the entry in Exchange.)
