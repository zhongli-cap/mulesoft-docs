...
As you move forward you will learn the features that DevKit provides to help you build link:/mule-user-guide/v/3.8/anypoint-connectors[Anypoint Connectors], and how easy it can be to extend Mule.
...
== Connection Configuration using @Config
...
On your `@Connector`-annotated class in the `Connector.java` file use the `@Start` annotation on a method to initialize the client.
...
. Remove the dummy `@Processor` operation from the `Connector.java`.
+
...
+
. Change it to reflect the `getRecentlyAdded` method signature (see example implementation link:https://github.com/mulesoft/mule-cookbook/blob/master/cookbook-connector/src/main/java/org/mule/modules/cookbook/CookbookConnector.java[here]), and at this point you have the code in place to build your first connector, which should then be ready for testing.
...
[NOTE]
Connectors built with DevKit 3.8 and require specification of examples in a different format. See the new specification and generation format at https://docs.mulesoft.com/anypoint-connector-devkit/v/3.8/connector-reference-documentation[Connector Reference Documentation].

Follow along to add an example use for an operation (annotated with `@Processor`).

Create the file indicated by the code block in the `Connector.java` file following `{@sample ...}` if the file does not exist already.
...
. Create a Mule application in Studio and add an HTTP listener and specify `/get-recently` for the path. If this is your first Mule app, take a look at our link:/mule-fundamentals/v/3.7/build-a-hello-world-application[Hello World Application].
...
----
+
. Make sure you have set the MuleCookbookClient in the `ConnectorConfig.java` with a getter and a setter.
+
[source,java,linenums]
----
...
The `@Handler` feature is useful to avoid duplicating code for handling exceptions, and makes your code easier to read.
...
To see how it works, let's create a handler for the `InvalidEntityException` thrown by the Cookbook SDK `create()` call.
