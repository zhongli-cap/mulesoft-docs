...
==== Adding Javadoc Example Code in Connector Source
...
[NOTE]
When you add an example, use the same name as the name that narrows the example in the file. Inside of it you have to put an example of the `@Processor`.
...
=== Using the Connector in Mule App
...
. Create a Mule application in Studio and add an HTTP listener. If this is your first Mule app, take a look at our link:/mule-fundamentals/v/3.7/build-a-hello-world-application[Hello World Application].
. Drop the connector onto the canvas and configure.
+
...
. Run *mvn clean package*. This builds the connector. If the build succeeded, you should see the directory where the connector's "UpdateSite.zip" was generated, which is usually what you point Studio to in order to install a connector manually.
