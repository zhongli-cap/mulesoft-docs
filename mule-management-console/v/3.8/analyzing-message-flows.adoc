...
See link:http://www.mulesoft.org/documentation/display/MULE3USER/Routing+Message+Processors[Using Message Processors to Control Message Flow] to find out more about these standard message processors and how to use them to control message flow.
...
For more information on reading stream-based payloads, such as CXF streams, see how to apply an object-to-byte-array stream transformer in the section "Viewing Message Payload When Using Streams" in the link:/mule-management-console/v/3.8/analyzing-flow-processing-and-payloads[Analyzing Flow Processing and Payloads] topic.
