...
* xref:macandlinux[Download and Start MMC Using Mac or Linux]
* xref:winin[Windows Instructions]
...
. Go to link:https://www.mulesoft.com/dl/mmc[MMC], click the checkbox to agree to link:https://www.mulesoft.com/legal/terms/trial[the terms], and click *Download*.
. Choose where to save the MMC distribution zip file.
...
----
+
. To start MMC and Mule, follow these steps:
...
+
. The startup script prompts you for the listening port for Mule. Enter the desired port, or press Enter to accept the default, 7777. 
+
. The startup script launches an embedded Tomcat server (listening on port 8585 by default) and deploys MMC to it. Then, it starts Mule. During startup, your terminal console fills with startup messages. After startup is complete, the script finishes and you regain your terminal prompt. The last few messages indicate successful startup.
+
...
* For Mule: `INSTALL_DIR/mmc-distribution-mule-console-bundle-3.7.0/mule-enterprise-3.7.0/logs`
...
By default, the startup script uses port 7777 for Mule and 8585 for Tomcat. Before launching, the script checks to see that these ports are not in use. If the ports are being used, the script exits with an error message. In Windows, the startup script does not prompt you for the Mule port, since it always uses the default, port 7777.
...
+
. The startup script launches an embedded Tomcat server (listening on port 8585 by default) and deploys MMC to it. Then, it starts Mule. During startup, your terminal console lists startup messages:
+
----
...
* For Tomcat and MMC: `INSTALL_DIR\mmc-distribution-mule-console-bundle-3.7.0\mmc-3.7.0\apache-tomcat-7.0.52\logs`
* For Mule: `INSTALL_DIR\mmc-distribution-mule-console-bundle-3.7.0\mule-enterprise-3.7.0\logs\mule_ee.log`
