...
If you want to enable an MMC user to stop Flow Analyzer, assign the `Audit Flows - Admin Functions` permission to the user group that the user belongs. Refer to link:/mule-management-console/v/3.8/managing-mmc-users-and-roles[Managing MMC Users and Roles] for details on assigning permissions.
...
* link:/mule-management-console/v/3.8/tracking-flows[Tracking Flows] provides more information on using the Flows tab and setting up flow tracking, a prerequisite for flow analysis.
* link:/mule-management-console/v/3.8/analyzing-message-flows[Analyzing Message Flows] takes you through using the Flow Analyzer panes in detail.
* link:/mule-management-console/v/3.8/debugging-message-processing[Debugging Message Processing] illustrates the general steps for debugging an application's message processing.
* link:/mule-management-console/v/3.8/debugging-the-loan-broker-example-application[Debugging the Loan Broker Example Application] shows how to use the console Flow Analyzer screen to find an application logic bug.
* link:https://docs.mulesoft.com/mule-user-guide/v/3.8/routers[Using Message Processors to Control Message Flow] provides reference information for the available Mule message processors or routers.
