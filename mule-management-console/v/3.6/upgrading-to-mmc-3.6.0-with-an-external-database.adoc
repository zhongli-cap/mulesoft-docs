...
[NOTE]
If your current installation of MMC link:/mule-management-console/v/3.6/setting-up-and-managing-users-via-ldap[uses LDAP for managing users], you will also need to migrate your LDAP configuration. See link:/mule-management-console/v/3.6/upgrading-to-mmc-3.6.0-with-ldap[Upgrading to MMC 3.6.0 with LDAP] for details.
...
* Read an overview of link:/mule-management-console/v/3.6/configuring-mmc-for-external-databases-quick-reference[configuring MMC for external databases], which includes links to detailed instructions for each supported database server.
* Learn how to link:/mule-management-console/v/3.6/upgrading-to-mmc-3.6.0-with-ldap[upgrade to MMC 3.5.0 with LDAP].
