...
+
The instructions on this page are valid _only_  for MMC 3.4.1. MMC 3.4.2 and later versions use a simpler configuration in which data persistence on MS SQL server is fully supported. If persisting data on MS SQL Server, it is recommended to use MMC 3.4.2 or later. For details, see the latest version of this document, link:/mule-management-console/v/3.6/persisting-mmc-data-to-ms-sql-server[Persisting Data on MS SQL Server].
+
* This document lays out the instructions for deploying the Management Console on a production-ready environment using Apache Tomcat. All of the instructions should work with other link:https://docs.mulesoft.com/mule-user-guide/v/3.4/hardware-and-software-requirements[supported application servers] as well, but this has not been tested.
...
* *a* **Web application server such as Tcat, Tomcat, or JBoss**. Please refer to the link:https://docs.mulesoft.com/mule-user-guide/v/3.4/hardware-and-software-requirements[Hardware and Software Requirements] for the latest information about hardware requirements and supported application servers, Java Runtime Environments, and browsers.
...
* link:/mule-management-console/v/3.4/persisting-mmc-data-to-ms-sql-server#configuration-file-code-applicationcontext-xml-code[applicationContext.xml]
* link:/mule-management-console/v/3.4/persisting-mmc-data-to-ms-sql-server#configuration-file-code-jackrabbit-repo-xml-code[jackrabbit-repo.xml]
