...
The Mule Management Console in a Web app designed to run in an application server or container, such as link:/display/tcat/home[Tcat Server] or link:https://www.mulesoft.com/tcat/understanding-apache-tomcat[Apache Tomcat], and usually packaged in a `.war` file.
