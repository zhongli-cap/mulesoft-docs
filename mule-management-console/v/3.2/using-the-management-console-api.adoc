...
If you are programming in Java, it's recommended that you use the link:http://wikis.sun.com/display/Jersey/Main[Jersey client]. Or, for JSON support, use the link:http://hc.apache.org/httpclient-3.x/index.html[HttpClient] with link:https://github.com/FasterXML/jackson[Jackson], the high-performance JSON processor.

== An Example

As an example, here is how to use a REST API to perform a console-related operation - in this case, listing all servers. The URL for this resource (method) is `/mmc/api/servers` and the HTTP method to invoke the request is GET. Here's what the request would look like if you submitted it as a command using link:http://curl.haxx.se/[cURL], the command line HTTP tool:
