...
From the Management Console or the Console's link:/mule-management-console/v/3.7/rest-api-reference[REST API] you can dynamically add or remove nodes to or from a cluster without disbanding it.

This page details the procedure to add or remove a node in the Management Console. For details on performing these operations using the REST API, consult the link:/mule-management-console/v/3.7/clusters[REST API Clusters] documentation.
