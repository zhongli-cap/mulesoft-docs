...
Custom alerts are executed as Groovy script snippets. The execution context is exactly the same as for other scripts in Mule. The context bindings for these scripts are described in link:/mule-user-guide/v/3.5/scripting-module-reference#script-context-bindings[Script Context Bindings].
