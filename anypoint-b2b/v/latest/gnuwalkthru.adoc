...
. *AAZFasteners* and *AcmeHomeSupply* do not use standards-based identifers, so *ZZ (Mutually Defined)* is the appropriate choice.
...
[[img-identifiers-x12, Identifiers Page]]

image::img-identifiers-x12.png[img-identifier-x12, title="Identifiers Page"]
...
+
More identifier boxes appear. (The specific boxes that appear depend on your choice in this step.)
. In the AS2 Identity box, click the down arrow.
+
A list of qualifiers appears.
. *AAZFasteners* and *AcmeHomeSupply* do not use standards-based identifers, so *ZZ (Mutually Defined)* is the appropriate choice.
. In the *Interchange ID (ISA)* box, type *1234566*.
+
This ID is also based on agreement between the two partners.
