= Anypoint Manager Basic Tutorial

The Anypoint Manager (APM) Basic Tutorial is intended for anyone who wants to get an idea of the role APM plays in B2B transaction processing. The Tutorial provides instructions for configuring APM so that, in conjunction with two downloadable applications, you can simulate a transaction.
...
. Download link:_attachments/anypoint-manager-basic-tutorial.zip[anypoint-manager-basic-tutorial.zip].
=== Zip
