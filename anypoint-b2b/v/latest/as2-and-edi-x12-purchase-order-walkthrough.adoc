...
. In the *Company Name* field, enter *MySupplier*.
. Click *Save*.

=== Create AS2 Identity
. In the left navigation pane, click *Identifiers*. +
The AS2 Identity Settings Page (for the partner you created in <<Create Partner>>).
. In the *AS2 Identity* field, enter *MySupplier*.
. Click *Save*.
...
. In the *Endpoint* section, complete fields as shown in the table that follows.
+
[%autowidth.spread]
|===
|*Field* |*Value*
...
. Enter *MySupplier* as *Interchange ID (ISA)*, Select *ZZ (Mutually Defined)* from the *Interchange ID qualifier (ISA)*.
