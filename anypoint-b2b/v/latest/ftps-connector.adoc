...
Current instances of the FTPS Connector will continue to be supported in accordance with the https://www.mulesoft.com/legal/support-maintenance-terms[Product Support and Maintenance Terms] but will not be available for new customers or upgrades. Contact your MuleSoft representative for more information on MuleSoft's B2B solutions.
...
If you wish to use the FTPS Module in conjunction with Maven, follow the instructions link:http://modusintegration.github.io/mule-connector-ftps/[here].
////
...
. *General Options*  +
Enter the path and filename of the file you aim to read from the server. In our case we’re transferring _test-read.txt_ which is located at the _/demo_ directory of the FTP user we’ve set.
. **Post-processing**  +
...
Enter the File pattern as well; bear in mind this is a wildcard pattern. 

////
In our case we’re transferring every file that starts with “<name>”.
////
