...
B2B Transaction Processing Framework enables developers to build a single set of Mule Runtime  flows that can process a wide range of messages from many different partners by dynamically applying rules and configuration data stored in Partner Manager. This allows non-programmers such as data analysts to onboard new trading partner relationships and support new transaction types using the Partner Manager website without the need to develop, test, and deploy new Mule Runtime  flows. This section identifies the high-level framework components and delineates their roles in end-to-end B2B transaction processing.
...
The Partner Manager Connector (PMC) is a component that runs inside of Mule Runtime  and coordinates all interaction with the Partner Manager APIs (TPM and Tracking).
