...
. Download the link:_attachments/partner-manager-tracking-transmission-walkthrough.zip[application] and import it into Anypoint Studio.
...
+
. Drop the file link:_attachments/b2b-tracking-po.xml[b2b-tracking-po.xml] in the `outbox` directory. You should see the file moved to the `inbox` directory.
+
