...
The X12 EDI Connector enforces these code sets for HIPAA documents, signalling an error if a transaction set uses an undefined value for a field (i.e., a value not listed as a key in the `codeSet`) either when parsing or writing. In some cases, such as the first element of the BHT definition, only a single value is allowed. In other cases there may be many potential values.
...
specfications, and the list of items gives the numbers of the values governed by the rule.
