...
* <<Configure Partners, Identifiers and Certificates>>
...
. On the xref:img-partner-configuration[], Click *Create New Partner*. 
...
=== Add Identifiers and Certificates
...
. In the *Interchange ID (ISA)* box, type *Acme*.
+
...
. On the <<img-identifiers-acme-x12>>, click *New*.
+
The <<img-identifier-new>> appears.
...
. In the selection window, navigate to the *Certificates* folder within the folder to which you expanded downloaded tutorial materials in <<Download Tutorial Materials>>.
+
. Select *partner.cer*. 
...
===== Switch the Partner you are Configuring
...
Thus, to add an X12-ISA Identifier for the Supplier, perform <<Add Buyer X12-ISA Identifier>>, substituting *AAZ* for *Acme* in the *Interchange ID (ISA)* value in the corresponding step.

To add an AS2 Identifier, perform <<Add Buyer AS2 Identifier>>, substituting  *supplier* for *buyer* for the *AS2 Identity* value in the corresponding step.

==== Add Supplier Certificate
...
. In the *CONFIGURATION* section of the left-hand navigation pane on the xref:img-documents[] (or any other APM Configuration page), click *Endpoints*.
+
