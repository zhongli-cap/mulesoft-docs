...
The <<Partner Manager Connector>> posts tracking events from within <<Customer-built flows>> to the <<Tracking API>>. The manner in which these components, each a part of the <<Functional Architecture>>, interact to facilitate tracking is shown here:

image:

For a given message, APM can facilitate storage of either the content of the message - the _payload_ - or data about the message - that is, _metadata_.
...
* Who has access to payload

This ensures ultimate customer control of messages and messaging history.
...
* Sender
* Recipient
* Payload URL - The URL points to the message payload and is part of metadata tracking; the message payload itself is <<Payload Tracking>>.



=== Tracking Data Storage Methods

You can store tracking data in either of two ways.

==== S3 Bucket

S3 bucket is a turn-key data storage service.

==== Custom Flow

This method enable customer control of storage.
