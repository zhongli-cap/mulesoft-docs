...
configurations can be created at two distinct levels: at the partner or at
the home organisation level. APM will attempt to resolve a configuration at the
home organisation level should it fail to resolve it at the partner level. The
next sub-sections describe in more detail the resolution process for each type
of configuration.
