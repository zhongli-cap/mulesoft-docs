...
The <<img-lookup-tables-detail>> appears. The first row on this page is the Search Row. Use this row to search specific columns for a specific string. You can search multiple columns; for example, if you type *a* in the first column and *b* in the second, the page displays all rows that include both an *a* in the first column and a *b* in the second.
+
...
. On <<img-lookup-tables-detail>>, click the pen icon at the right locate the row or rows you want to edit.
. Edit table cells
...
. Enter values for the new row in the appropriate boxes.
. When you have entered all values, click the pen icon at the right end of the new row to save it.
