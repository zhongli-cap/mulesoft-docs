...
RNIF Send Settings pertain to trading partner-specific RNIF settings such as *certificate* and *URL*.
...
|URL to which you will send RNIF transmissions for this partner, including port +


Required?::
Yes, if RNIF is the transport protocol
...
|Content type for messages sent over RNIF.
...
| Type of encoding to use when sending a message with RNIF
...
| MDN Async
| Allows RNIF MDNs to be returned to the RNIF message sender's server at a later time.
Typically used when large files are involved, or when a trading partner's RNIF server has poor Internet service. If this checkbox is selected the *MDN Async URL* box appears.
