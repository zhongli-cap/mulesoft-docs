...
. Click *Save*.
+
The <<td-source-channel-in-progress-acme-endpoint-added-acme>> appears.
+
Note that *Resolution* status is *Complete*.

[[img-td-source-channel-in-progress-endpoint-added-acme]]

image::td-source-channel-in-progress-acme-endpoint-added-acme.png[img-td-source-channel-in-progress-endpoint-added-acme, title="Source Channel in-progress"]
...
+
The <<img-new-map-acme>> appears, with *JSON-PurchaseOrder-AAZFasteners* in the *Target Document* box.
...
image::partner/td--save-transaction.png[img-td-save-transaction, title="Transaction Complete Page"]
...
image::td-source-channel-in-progress-map-added-acme.png[img-td-source-channel-in-progress-map-added-acme, title="Source Channel in-progress"]
...
* The link:/anypoint-b2b/as2-connector[AS2 Connector]
 ...
 APM enables you to view a transmission payload. For example, in <<img-transaction-monitoring-successful-transaction>>, click the Payload icon in the right pane (identified in the figure by the gold arrow). The <<transmission-payload>> appears, presenting payload data.
