...
In the following sections, we add identifiers for the supplier. We will add buyer identifiers in <<Create the Buyer Identifiers>>.
...
substituting *Acme* for *Supplier*.
...
To import the mock partner application, repeat the steps in <<Import the Routing Engine Application>>, substituting *apm-acme-mock.zip* for the Routing Ending App in Step 7.
