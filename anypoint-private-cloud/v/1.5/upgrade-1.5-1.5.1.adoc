...
Before migrating, ensure that you have met the following prerequisites:

* Your system meets all of the system and network requirements described in link:system-requirements[About Minimum System Requirements]

* You have enabled TCP port 5973 intra-node to allow communication to the database cluster.

* You have permission to run the `sudo` command on the node where you launch the migration tool.

* The `kubectl` command is available in the node where you are performing the migration. To verify that kubectl is installed:
+
----
$ sudo gravity enter
$ kubectl
