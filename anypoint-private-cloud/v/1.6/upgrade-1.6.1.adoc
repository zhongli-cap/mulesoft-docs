...
. Drain and uncordon each node in your cluster by running the following commands: 

.. Login to each node of your cluster.
.. Drain each node by passing the node name of the node you are logged into:
+
----
...
.. Make each of the nodes in your cluster schedulable by uncordoning each node: 
+
