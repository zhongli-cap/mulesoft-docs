...
----
+
Node names are displayed in the name column. This name corresponds to the internal IP address of the node.
...
+
Draining a node stops all containers running on the node.
+
.. Login to each node of your cluster using the public IP address of the host.
.. Run the following command on each node by passing the node name (internal IP address) of the node:
+
----
