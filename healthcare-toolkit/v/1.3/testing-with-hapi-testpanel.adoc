...
After the HL7 inbound endpoint, an link:https://docs.mulesoft.com/mule-user-guide/v/3.8/all-flow-control-reference[All Flow Control] sends the HL7 message to two Cache Scopes for parallel processing.
...
In the `To XML and file` scope, the first building block, an link:/healthcare-toolkit/v/1.3/hl7-encoding-transformer[HL7 Encoding Transformer] transforms the original HL7 message from ER7 to XML, and a link:https://docs.mulesoft.com/mule-user-guide/v/3.8/file-connector[File Endpoint] writes the output to a file (in this example, `/tmp/hl7-2test`).
