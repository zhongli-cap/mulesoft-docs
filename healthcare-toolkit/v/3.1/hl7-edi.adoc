...
The schema simplification tool is distributed as the https://s3-us-west-2.amazonaws.com/mulesoft-sites-vendorcontent/edi-schemas-hl7-1.3.0-simplify.jar[edi-schemas-hl7-1.3.0-simplify.jar]. It takes a message structure schema, and one or more example messages (as separate files) as input, and generates an output schema reduced down to only those segments and components present in one or more of the sample messages.
