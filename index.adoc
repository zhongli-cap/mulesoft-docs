...
MuleSoft is a cloud software company that helps organizations change how they connect their data and applications to adapt to rapidly evolving digital landscapes.

== What is Mule runtime?

Basically, Mule runtime is an engine for processing and orchestrating data. Mule runtime is the layer that facilitates reliable, highly available connections to diverse systems. More specifically, Mule runtime is a Java-based enterprise service bus (ESB) based on SEDA (staged-event driven architecture). Using Mule runtime you can build enterprise-grade integration applications, called Mule applications.

[NOTE]
Mule runtime is often referred to as Mule ESB. Call it Mule for short.
...
A Mule application is a collection of functional components that are placed in sequences called *Mule flows*. Data passes through a Mule flow inside a *Mule message*. The message is transformed or accessed by components in the flow per design. It is up to the integration developer to design the flows and configure how their elements operate on this message. Elements can be configured to hook into existing applications on-premises or in the cloud to perform desired operations on data.

Once a Mule app is deployed to Mule runtime, the app's flows are ready to be kicked off by the event(s) that the component is set to listen for. You can deploy Mule applications to Mule runtime servers anywhere, and configure security and policies for them as well. In Mule apps *you write less custom code, and get more done through reuse.*

[TIP]
Consider APIs your ally when connecting assets. Starting with API design allows developers to decouple business logic from the points of contact that link together systems.
...
[TIP]
An link:/anypoint-platform-on-premises/[on-premises] version of the platform is available, enabling you to securely host it on your infrastructure.
...
. link:/anypoint-studio/v/6/download-and-launch-anypoint-studio[Download and Install Anypoint Studio]. Studio is comes with a test Mule server.
