...
Once a Mule app is deployed to Mule runtime, the app is ready to be kicked off by the event(s) the first flow component listens for. You can deploy Mule applications to Mule runtime servers anywhere, and configure security and policies for them as well. In Mule apps *you write less custom code, and get more done through reuse.*
