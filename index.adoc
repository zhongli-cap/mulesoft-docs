...
A Mule application is a collection of functional components that are placed in sequences called *Mule flows*. Data passes through a Mule flow inside a *Mule message*. The message is transformed or accessed by components in the flow. It is up to the integration developer to design the flow(s) and configure how the components operate on this message. Components can be configured to hook into existing applications on-premises or in the cloud to perform desired operations on data.
