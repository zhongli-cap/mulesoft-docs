...
You can create and configure an HL7 Connector as a link:/mule-user-guide/v/3.7/global-elements[Global Element] which the connectors in your flow will reference. An HL7 connector configured as a global element defines connection parameters for all HL7 connections that reference the connector.
