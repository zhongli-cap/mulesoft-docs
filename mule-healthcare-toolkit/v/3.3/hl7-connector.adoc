...
You can create and configure an HL7 connector as a link:/mule-user-guide/v/3.3/understand-global-mule-elements[Global Element] which the endpoints in your flow will reference. An HL7 connector configured as a global element defines connection parameters for all HL7 connections that reference the connector.
...
|Enables link:/mule-user-guide/v/3.3/mule-server-notifications[Dynamic Notifications] for this connector. a|`dynamicNotification="true"`
a|
*Valida* *te Connections*
...
The HL7 Connector also accepts all attributes configurable for TCP connectors. See the link:/mule-user-guide/v/3.3/tcp-transport-reference[TCP Transport Reference] for details.
