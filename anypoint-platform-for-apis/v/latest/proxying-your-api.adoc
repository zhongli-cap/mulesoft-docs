...
START HERE
. In the *Deploy proxy* dialog, choose an application name that becomes the access name at cloudhub.io, choose an environment, and if needed, change the API Gateway version. By default, Mule uses the API name you chose when you created your API, but you can change the CloudHub application name as needed.
+
