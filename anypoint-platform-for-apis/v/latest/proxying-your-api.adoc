...
You can generate a proxy for an API version, optionally modify it with additional code, and deploy the proxy. Mule 3.8.0 Runtime, or API Gateway in earlier versions, registers a proxy endpoint with Anypoint Platform for APIs. Subsequently, you can govern the API by applying runtime policies. You can also build your own proxy.
...
Before you can use API Platform to generate a proxy, you must first link:/anypoint-platform-for-apis/creating-your-api-in-the-anypoint-platform[create an API version] in the Anypoint Platform.

To generate a proxy for your API version, you need to have API Version Owner permissions for the API, or be an Organization Administrator. 

To deploy a proxy, you need a valid URL where your API is currently running. You also need one of the following:

* *On-premises*: Access to an on-premises Mule 3.8.0 Runtime instance or an API Gateway instance that has been configured to pair with your Anypoint Platform for APIs organization. link:/anypoint-platform-for-apis/configuring-an-api-gateway[Configure your on-premises API Gateway] if necessary.
* *Automatic On-premises deploy*: Valid permissions for both the Runtime Manager and the Anypoint Platform for APIs of your organization. Also, your on-premises server must be registered with the Anypoint Platform (see link:/runtime-manager/managing-servers#add-a-server[Managing Servers]).
* *Automatic CloudHub deploy*: Valid permissions for both the Runtime Manager and the Anypoint Platform for APIs of your organization.
...
== Autodeployment (Mule 3.8.0 or Gateway 2.0)

If you are using Mule 3.8.0 Runtime or API Gateway 2.0, you don't need to generate a proxy. Skip this section and go directly to the *Automatic - On Premises* tab below.
...
.. You can choose to not have a proxy by selecting *Basic endpoint*; otherwise accept the default the option *Endpoint with a proxy*.

.. Choose a *Type* for your proxy: +
...
+
[WARNING]
*Note*: When deploying to the CloudHub, you can only perform an automatic deploy if you use an HTTP listener that doesn't have HTTPS enabeled. If you use HTTPS, you must customize the proxy to add your HTTPS credentials and then perform a manual deploy to CloudHub, or deploy on premises.
