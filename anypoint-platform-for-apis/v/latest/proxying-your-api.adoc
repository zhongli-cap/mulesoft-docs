...
You can create a proxy for an API version, modify it with additional code, and deploy the proxy. Mule 3.8.0 Runtime, or API Gateway in earlier versions, registers a proxy endpoint with Anypoint Platform for APIs. Subsequently, you can govern the API by applying runtime policies. You can also build your own proxy.
...
To use Anypoint Platform to generate a proxy, you need to meet the following prerequisites:
...
If you use Mule 3.8.0 Runtime or API Gateway 2.0, you don't need to generate a proxy. Skip this section and go directly to the *Automatic - On Premises* tab below.
...
*** If your API is not RAML- or WSDL-based, choose *HTTP URL*.
*** If you have a RAML for your API, choose <<Basic Anatomy of a RAML Proxy, RAML>>.
*** If you have a WSDL to expose, choose WSDL.
...
.. Check the *Configure proxy for CloudHub* checkbox if you plan to deploy your proxy  to CloudHub. This option fixes the port number to a specific value required by CloudHub. 
.. For *Scheme*, set *HTTP* or *HTTPS*. 
+
[WARNING]
*Note*: When deploying to the CloudHub, you can only perform an automatic deploy if you use an HTTP listener that doesn't have HTTPS enabeled. If you use HTTPS, you must customize the proxy to add your HTTPS credentials and then perform a link:/anypoint-platform-for-apis/deploying-your-api-or-proxy#deploying-to-api-gateway[manual deployment] to CloudHub, or deploy on premises.

.. Specify a *Port* value and a *Path*, or use the default settings.
+
The following screenshot shows a possible configuration:
+
