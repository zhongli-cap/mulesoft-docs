...
You deploy an APIkit project or an API proxy to a Mule 3.8.0 runtime, or to an API Gateway if you are running Mule versions earlier than 3.8. This tutorial uses features that might not be visible or accessible to you, depending on the link:/release-notes/anypoint-platform-for-apis-release-notes#april-2016-release[entitlements you purchased].

== Assumptions

To deploy one of the following things:

* Built a new API using link:/anypoint-platform-for-apis/apikit[APIkit]
* Set up link:/anypoint-platform-for-apis/proxying-your-api[a proxy] for the endpoint on which your API is hosted. To deploy on premises or manually on Runtime Manager, download the .zip file of the proxy.
...
For more information about registering a server, see link:/runtime-manager/managing-servers-[Managing Servers].
...
----
+
Use your client id and client secret, which you can obtain from an Organization Administrator. Log in to the Anypoint Platform as an administrator, click the menu icon on the top-left and select the *Access Management* section, then select the *Organization* tab. The client id and secret id appear.
+
...
* If you deployed to an API Gateway, return to the API Details Page to link:/anypoint-platform-for-apis/managing-your-api[manage your endpoint] with policies and SLAs.
