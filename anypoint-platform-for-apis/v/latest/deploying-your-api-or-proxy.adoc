...
You deploy an APIkit project or an API proxy to a Mule 3.8.0 runtime, or to an API Gateway if you are running Mule versions earlier than 3.8. This walkthrough uses features that might not be visible or accessible to you, depending on the link:/release-notes/anypoint-platform-for-apis-release-notes#april-2016-release[entitlements you purchased].
