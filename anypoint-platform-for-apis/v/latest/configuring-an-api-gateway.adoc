...
If you are managing your on-premises Mule runtime with Mule Management Console (MMC) instead of using Studio, link:/anypoint-platform-for-apis/configuring-an-api-gateway#configuring-your-production-api-gateway-for-integration-with-the-anypoint-platform[configure API Gateway] to provide these credentials.
