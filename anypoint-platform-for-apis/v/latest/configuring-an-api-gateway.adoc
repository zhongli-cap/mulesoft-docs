...
. In the *Work with:* field in the Install wizard, enter the following URL: `http://studio.mulesoft.org/r4/api-gateway/`
...
+
[TIP]
A link is then provided to the actual CloudHub application for further management. You can change the worker type, the environment, set advanced settings, and so on. Under the API Status section, a new link appears labeled *Manage CloudHub proxy* that takes you there.
+
