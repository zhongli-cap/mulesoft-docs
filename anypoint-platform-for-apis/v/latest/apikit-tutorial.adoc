...
+
. Select one of the *API Gateway Runtime* versions to download the version. If you have the latest Studio installed, select the latest API Gateway Runtime version, for example 2.2.0, and then click *Next*.
. Follow through the rest of the wizard, and accept the terms and conditions.
+
The API Gateway Runtime downloads and installs. When finished, Studio needs to restart.
