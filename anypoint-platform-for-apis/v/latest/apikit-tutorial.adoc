...
This tutorial shows you how to build REST APIs using APIkit. In Anypoint Studio, you create the API based on RAML file. The API exposes information about the contents of vending machines in various locations. You run the API in Anypoint Studio and use the Web console.

These main tasks you perform in this tutorial are:

* Create a new Studio project, importing the RAML file, which defines the API's interface, into Studio.
...
This tutorial is based on a scenario involving a vending machine provider, SnackVendor, that needs to track the contents of all the vending machines deployed in the state of California. To better manage their resources, SnackVendor wants to send drivers out only to replenish the stock of the vending machines that are running low on content. The API addresses two use cases:

* Vending machine updates: to update the central database on each sale
* Inventory tracking: to find out if a machine is low on stock

== Assumptions

This document assumes you are familiar with the following things:

* The basics of creating a REST API and some understanding of link:https://en.wikipedia.org/wiki/Representational_state_transfer[REST] as a Web service paradigm.
...
This tutorial requires the *Gateway Runtime* that you install in the next section or a Mule runtime version *3.5.0* or later. The Mule Runtime beta version 3.8.0-M1 can also be used. As of 3.8, the both the Mule Runtime and the Gateway Runtime are merged into one.
...
+
. In the field labeled *Work with*, copy the following update site address to the field and press Return. The URL also works as `http://`... +
+
...
+
. Select one of the *API Gateway Runtime* versions to download the version. For example, select the latest version, and then click *Next*.
. Follow through the rest of the wizard, and accept the terms and conditions.
+
The gateway runtime downloads and installs, and Studio needs to restart when done.
