...
* API Gateway version 1.3 and later
...
|`type` |string (optional) - The kind of policy. A custom policy must always have type set to "custom".
|`standalone` |boolean - True if the policy can work on its own or false if the relies on other policies being active.
...
In the API Gateway version 2.0 and later and Mule 3.8 unified runtime, add properties `id` and `policyName` to the `<policy>` element parameters to gather data about the API for analytics.
...
For more examples, navigate to the directory where you installed the API Gateway or the Mule 3.8 unified runtime, search for the folder `examples` > `policies` to find a set of example XML files that refer to the default policies available in the platform.
