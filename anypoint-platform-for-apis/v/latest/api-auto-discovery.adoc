...
In Mule Runtime 3.8.0 and later, no setup is required for auto-discovery. You deploy the API or app to Anypoint Platform, and API Manager discovers it.


[tabs]
------
[tab,title="API Gateway Runtime 2.x"]
....
...
* Pair API Gateway runtime with an Anypoint Platform organization by adding organization credentials.
...
. In a browser, log in to the Anypoint Platform as an administrator, click the menu icon on the top-left and select the *Access Management* section, and then select the *Organization* tab.
+
...
+
This action pairs API Gateway runtime with your Anypoint Platform organization.
+
...
=== API Gateway Runtime 1.3.2 or Later
