...
When creating or modifying an API project in Anypoint Studio, you can set an application to be automatically discovered and registered to one or more APIs in an Anypoint Platform for APIs organization.

To enable auto-discovery, follow the step-by-step configuration procedure in the next section to perform these tasks:

* Install API Gateway Runtime from Studio.
* Pair API Gateway with an Anypoint Platform organization.
* Create a global element to configure API auto-discovery.

Running the application in your API project triggers auto-discovery, and the API appears on the API Administration page in Anypoint Platform. 
...
For example purposes, this procedure refers to the API created in the link:/anypoint-platform-for-apis/apikit-tutorial[APIkit Tutorial].

To configure and trigger auto-discovery:

. After creating the API, link:/anypoint-platform-for-apis/install-studio-gw[install a current API Gateway Runtime]. For example, install API Gateway Runtime 2.2.0.
...
+
. In Anypoint Studio main menu, select *Anypoint Studio > Preferences > Anypoint Studio > Anypoint Platform for APIs*.
+
The Preferences dialog for Anypoint Platform for APIs appears.
. Copy the client ID and client secret from the browser to the Preferences dialog.
+ 
...
+
. Create a global element to declare the details of the API version you want to register in the Anypoint Platform:
+
...
* In *Flow Name*, select the name of the main flow in your API from the drop-down. In the case of the APIkit tutorial, select `api-main`.
...
* In *Description*, describe the API.
+
image:auto-disc-conf.png[auto-disc-conf]
+
* Click OK.
+ In Project Explorer, right-click the app, `api.xml` in `src/main/app` in this example, and select *Run As > Mule Application*.
...
|*id* | No (2.0.2 and later) |The bean ID of the element. This value is not propogated to the platform.
...
functions the same as any other API version or endpoint. You can edit, update, or delete 
