...
|*id* |Yes |No longer used. Previously referenced in the deprecated `api-ref` attribute of an endpoint.
...
The apikitRef element is responsible for uploading the RAML to the platform after the API is registered.
This element is a string and is optional.
