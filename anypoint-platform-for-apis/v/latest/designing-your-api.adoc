...
. Deploy the API:
+
* Click *API administration*.
* Click the API version of placeholder.
+
The version Administration page appears.
. In *Status*, click *Configure endpoint*, and accept the default option `Endpoint with a proxy`.
. Click *Save*.
. Click *Deploy proxy*.
image::designing-your-api-723b6.png[designing-your-api-723b6]
. Click to add the new API Gateway if necessary or deploy to Cloudhub.
. Open a browser, and run the application.
+
