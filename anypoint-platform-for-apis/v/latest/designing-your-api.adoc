...
RAML is a structured, YAML-based language for describing REST APIs. RAML is machine readable--you can create tooling, automation, and repeatable processes for designs. Human friendly, concise. You can describe resources, methods and other aspects of design, such as security, in a human-friendly, concise way. RAML 1.0 strives to fill the gap that exists in standards exist for API design. Having a standard is important for communicating about the API with other developers, getting feedback, arriving at agreements and contracts for API design. Facilitating communication gives RAML the edge in dealing with change. Getting insight into design at an early stage promotes refactoring on an exploratory basis and accessing the impact of change. You can communicate ideas to team members before coding starts, which promotes involvement in the development process.

RAML 1.0 includes a number of enhancements. The following list includes a few of the new capabilities:

* Data types
+
A efficient way to model data that uses simplified YAML-based, map syntax and can include examples and annotations. You can extend data types using inheritance.
+
* Examples
+
...
Although this example, which connects to a free online REST service, link:http://jsonplaceholder.typicode.com[JSONPlaceholder], uses early access RAML 1.0, you can link:_attachments/placeholder.raml[download] the RAML 0.8 version, deploy the 0.8 version in Cloudhub, and call the API. For simplicity, this example API has only one resource.
...
. On lines 5-8, type the lines shown below securitySchemes:
+
[source,yaml,linenums]
----
#%RAML 1.0
...
. On the line after `type: Basic Authentication` (line 10), add a `types` declaration to the root section of the API.
. On line 11, add a line indented one level from the root, specifying the `user.json` schema to include to validate the list of users returned by the `users` resource.
+
...
+
. On line 15, enter `responses:` followed by the  response (required) to the `get` method and the example. Indent these entries as shown in the following example.
+
----
...
    description: Retrieve a list of all the users
...
To run the RAML 0.8 version of this example in Anypoint Platform, link:_attachments/placeholder.raml[download] and deploy the RAML 0.8 version to Cloudhub. Cloudhub does not support Early Access RAML 1.0.  Enter the URL assigned to you by Anypoint Platform in a browser. For example:

`http://placeholder.cloudhub.io/users`

The user information from the JSONPlaceholder service appears:
