...
+
. Below the `get`, include an indented description followed by the response (required). The response consists of a map of the HTTP status codes the API returns on success. The description, response, and example below `get` for the `/users` resource is:
----
