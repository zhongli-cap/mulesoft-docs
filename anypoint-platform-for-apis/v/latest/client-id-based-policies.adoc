...
When you apply Client ID-regulating policies, the credentials are expected in the form of query parameters named client_id and client_secret. The default configuration of the policy requires the <<Required Fields in API Calls, Client Id and Client Secret expressions>>. In Mule 3.8.0, another option is available. you can also select HTTP Basic Authorization Header to use Basic Authenication as the origin of the credentials. 
