...
Every flow has a trigger that sets it into action, this can either be an operation that is externally called, or a Scheduler which actions your flow at a given time or interval.
...
* Refer to link:/mule-user-guide/v/4.0/mule-concepts[Mule Key Concepts] for an overview of basic Mule Runtime concepts.
