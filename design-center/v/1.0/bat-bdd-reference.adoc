...
== Using Secrets that are Stored in Secrets Manager

To authenticate to test systems and reporting tools using information that you have stored in Secrets Manager, you can the 'secret' function in your tests. The prerequisite to doing so, besides storing secrets in Secrets manager, is running the `bat grant` command to store an access grant in your `bat.yaml` file. This grant allows BAT to retrieve the information needed for authentication.

The syntax of the function is as follows:

[source,code,linenums]
----
secret(secrets, '<alias>'[, '<default>'])
----

* `<alias>`
+
Specifies the name to use to identify the grant in the `bat.yaml` file.
* `<default>`
+
Default value is optional. In the case secret is not found and default value is absent, the test will fail.


You can use the function in the `execute` block of a test, as in this example:
...
You can also, for example, store a secret in a variable, like this:

[source,code,linenums]
----
var s = secrets, 'myAlias')
----

== Controlling Execution and Delete Assets
