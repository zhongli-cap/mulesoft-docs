...
In this example, when you simulate a successful call to the API, the mocking service returns the status code 200 and example data as coded in the RAML. You see the response example when you simulate calling the /users resource. 
...
. Check Show Optional Parameters, and enter a query parameter 2. Click Send.
+
Comment 2 appears.
+
