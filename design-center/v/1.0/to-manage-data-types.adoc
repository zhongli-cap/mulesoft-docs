...
The _type_ of data in a Mule app refers to its *metadata*, which represents the structure of data regardless of its format. For example, the metadata of a message may define that the 'user' element has two child elements: 'name' and 'address'; it doesn't matter if the message is in JSON or XML format. The metadata does not adhere to any particular format.

// kris 9/4/2017 unclear When the metadata information is available to another component, it is interpreted as the expected input or output, depending on where each are placed relative to each other in a flow. 

Almost all components include *Input* and *Output* that describe the types of data that enter and leave the component. The data includes specifics about the Mule Event: the payload, its attributes, and variables. Any real data is shown here.

Many elements in projects expose metadata automatically.
