...
the HTTP functions in the REST specification. For saved search items, Exchange lists search terms specific to your business group. Below those terms are your saved searches.
* Right detail panel - Lists the asset type, when the asset was created and by which organization, when the asset was last published, the version, and the asset's dependencies.
