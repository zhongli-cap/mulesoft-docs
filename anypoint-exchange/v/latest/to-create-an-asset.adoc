...
+
. For the OAS and WSDL assets:
+
* click Choose File to locate the API specification file.
* select the Main file (root file) of the API extracted from file (if the file is not a zip or if it has only one main file, it will be selected automatically)
+
. Click Publish.
