...
.. Select the Main file (root file) of the API extracted from the file. If the file is not a zip or if it has only one main file, it's selected automatically. If the file is a zip, the root file should be in the root directory inside the zip.
+
. Click Publish.
