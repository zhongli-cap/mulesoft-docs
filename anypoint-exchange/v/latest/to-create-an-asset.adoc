...
. Once a file is chosen for the OAS and WSDL assets, select the main file of the API (if the file is not a zip or if it has only one main file, it will be selected automatically).
. Click Publish.
