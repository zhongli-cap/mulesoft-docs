...
. Click your top level organization name and view your Organization ID in the Organization Info screen.
...
`extension`, `template`, `example`, `rest-api`, `soap-api`, `http-api`, `raml-fragment`, `custom`.
+
*Note:* For connectors, do not specify a `<type>` value. Exchange automatically identifies connector assets. This applies to 
connectors for Mule 3 and Mule 4.
+
