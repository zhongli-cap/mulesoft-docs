...
** For Mule 4 assets, do not specify the <type> element. In the `mule-maven-plugin` configuration, set the `classifier` as `mule-application` (default), `mule-application-template`, `mule-application-example`, `mule-policy` or `mule-plugin`. For example: 
+
[source,xml,linenums]
----
...
