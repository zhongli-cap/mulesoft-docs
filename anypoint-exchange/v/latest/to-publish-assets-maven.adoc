...
*Notes:*
+
** For Mule 3 assets, the <type> `app` is currently the only supported value (but the asset won't be displayed in Exchange asset list).
** Mule 3 `templates` and `examples` can only be deployed using Anypoint Studio 6.3 or above for them to work correctly.
