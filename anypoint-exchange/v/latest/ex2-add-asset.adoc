...
* To add a connector, example, or template to Exchange, see link:/anypoint-exchange/ex2-maven[To Publish and Deploy Exchange Assets Using Maven]. *Note:* If you are importing a zip file, all the files must be at root level in the folder. Files nested in sub folders can't be imported.
