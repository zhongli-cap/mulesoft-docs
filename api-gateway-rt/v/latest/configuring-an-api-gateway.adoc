...
Mule 3.8.0 runtime unifies the API Gateway runtime with the core Mule runtime, eliminating the need to configure API Gateway. This document pertains to earlier installations, not to Mule 3.8 runtime deployments managed by link:/runtime-manager/[Runtime Manager]. 
...
You can  link:/api-gateway-rt/configuring-an-api-gateway#configuring-organization-credentials[configure Anypoint Studio] to provide organization credentials when you deploy applications to Anypoint Platform from Studio.

If you are managing your on-premises Mule runtime with Mule Management Console (MMC) instead of using Studio, link:/api-gateway-rt/configuring-an-api-gateway#configuring-your-production-api-gateway-for-integration-with-the-anypoint-platform[configure API Gateway] to provide these credentials.

=== Deploying a Proxy

Follow instructions on the Cloud Gateway tab in the link:/api-gateway-rt/configuring-an-api-gateway#setting-up-your-gateway[Setting Up API Gateway Runtime] to deploy a proxy.
...
You need to configure the client ID and client secret in the gateway before the gateway can connect with your Anypoint Platform for APIs organization. Read more about link:/api-manager/api-platform-architecture[how the API Gateway works with the agent].
...
Before you configure your production gateway, you may want to configure Anypoint Studio to work with your Anypoint Platform organization for testing. First, link:/api-gateway-rt/install-studio-gw[download and install API Gateway Runtime] from the Studio Update Site.
