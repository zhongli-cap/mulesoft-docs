...
|================
|Text  | Parameter 
| uses key | The link:https://github.com/raml-org/raml-spec/blob/raml-10/versions/raml-10/raml-10.md#annotating-scalar-valued-nodes[spec] supports the `uses` node in any fragment. At this moment the parser only allows the `uses` node in root level RAML documents and libraries.
