...
* Support for link:/anypoint-platform-for-apis/designing-your-api[RAML 1.0]
+
This release of RAML has a number of enhancements and new features, including:
+
** Data types
** Examples
** Annotations
...
+
link:/anypoint-platform-for-apis/client-id-based-policies[Client ID-based] policies are now easier to apply. The API can now extract the client id and the client secret from the Authorization header to challenge the user for the credentials when they don't come as part of the request. Previously, two expressions were required. 
