...
Mule 3.8.1 has the following new features and functions.

=== DataWeave Support for bigger files

DataWeave now support processing files up to 2 GB for XML, JSON and CSV. In order to do this it will use temporary files to store the data instead of loading everthing in memory.
No change is required on DataWeave to use this feature as is done automatically when a link:https://docs.mulesoft.com/mule-user-guide/v/3.8/dataweave-memory-management#memory-vs-disk-usage[threshold] (of 1.5MB) is reached.

=== DataWeave changed maven ArtifactId

From mule-plugin-weave_2.11 to mule-plugin-weave
