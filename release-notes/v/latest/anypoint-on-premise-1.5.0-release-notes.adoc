...
The on-premises edition of Runtime Manager is also enabled to deploy applications to Pivotal Cloud Foundry (Beta). See link:anypoint-platform-on-premises/v/1.5/configuring-anypoint-platform-for-pcf[Configuring Anypoint Platform for PCF].
...
See link:/anypoint-platform-on-premises/v/1.5/prerequisites-platform-on-premises[Prerequisites for Anypoint Platform] for installing the platform on premises.
