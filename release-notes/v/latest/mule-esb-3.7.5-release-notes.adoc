= Mule ESB 3.7.5 Release Notes (Preliminary)
...
XML entity expansion in Jersey is now disabled by default because it allows DoS attacks. To restore previous behaviour use the `mule.xml.expandInternalEntities=true` property.

=== MULE-10979

The default response timeout and default transaction timeout can't be configured using system properties on the command line or in the wrapper.conf file anymore. In replacement, use the configuration element. For example: 
