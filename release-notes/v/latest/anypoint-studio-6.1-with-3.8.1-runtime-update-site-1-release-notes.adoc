...
In this release, we are adding Single-Sign-On support inside Studio, for users who leverage 3rd party identity management to log into Anypoint Platform (e.g. Exchange, Cloudhub) from Studio. The users can choose to use their Anypoint Platform credential or SSO credential under “Preferences”. +
For more details, refer to documentation link:/anypoint-studio/v/6/setting-up-your-development-environment#configure-anypoint-platform-credentials[here].

In addition, we have made various improvements.
