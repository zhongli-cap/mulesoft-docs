...
[NOTE]
To migrate following these steps, you must have the link:/runtime-manager/runtime-manager-agent[Runtime Management Agent] installed. The agent comes bundled with Mule runtime as of version 3.6.0. In case you're migrating from a Mule runtime version that is is older than that, you must do this migration manually.
