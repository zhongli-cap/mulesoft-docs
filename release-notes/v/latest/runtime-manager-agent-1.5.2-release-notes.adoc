...
* *SE-4321*: Contains a configurable property to exclude the payload from the tracking notifications.
...
* *CHHYBRID-1772*: Added `amc_debug` script to the assembly and added support for multiple Mules in the same machine.
...
* Configure the proxy settings when the setup is executed with the -P option.
* DiskBuffer throws an error with CustomEventProperties
