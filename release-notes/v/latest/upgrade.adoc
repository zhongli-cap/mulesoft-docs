...
Anypoint Platform provides these steps to complete the upgrade to the latest Anypoint Platform software (November release):

. Group your APIs. (optional, recommended)
. Request a time to upgrade via the green banner which lets you access the self-service upgrade portal. Only the org owner can schedule the upgrade. If you do not see a banner, contact your Customer Success Manager to coordinate steps to upgrade.
...
The idea of grouping APIs is to consolidate APIs into a single design project. Grouping APIs allows you to remove duplicate API design projects and API portals you may have had. API design projects may contain multiple branches and API portals contain links to RAML files for grouped APIs. Portal APIs can be deployed to multiple environments.
...
Consider completing this step before scheduling your upgrade if you previously labeled APIs or versions using names of your environments.

Notes: 

* If you used Business Groups instead of environments to group your APIs, API grouping is not needed or recommended.
...
NOTE: To the right of each API you should see a message that reads Prepare To Group. Once you Group the API it should read "Grouped". You can click Edit to change the New API Name, New API Version, or to include or exclude your portal. *Important*: Only one API portal can be migrated for a group of APIs.
...
The upgrade is triggered automatically at the time you schedule in the self-service portal. Access this portal from the green banner on platform login. The estimated upgrade time is 15 min per 50 APIs.
...
After this process completes, the user who scheduled an upgrade will receive an email confirming a successful upgrade. In case we encounter issues, we will automatically roll back an upgraded account to its previous experience. You can contact MuleSoft Customer Support with any questions on resolving these errors. 
