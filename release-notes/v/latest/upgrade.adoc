...
* API promotion to environments is a new feature that can be introduced as a step in your CI/CD pipeline.
* The API Manager API can be used with APIs in the unclassified environment with some restrictions (see below).
* Because the November release upgrade move all portals and RAMLs cannot be used to modify, create, or delete them after the upgrade. After the upgrade, the API Manager API v2.x changes its behavior in the following way:
