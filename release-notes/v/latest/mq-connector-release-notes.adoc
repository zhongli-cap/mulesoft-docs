...
== Version 1.0.3 - December 3, 2016 -- Upcoming

Anypoint MQ connector version 1.0.3 release includes the following bug fixes and improvements:

* MQ-564: Fixes to support “None” Acknowledgement mode when using MQ connector
* SE-4433: Fixes to MQ connector to correct failures with “Manual” Acknowledgement mode
* SE-4189: Fixes to support MUnit test suite for the Anypoint MQ connector
* SE-4575: Fixes to honor the ACK timeout period
* SE-4378: Fix message redelivery exception with Mule Runtime 3.8.1
* SE-4468: Fix MQ broker error caused by a race condition on the auth token under heavy load
* SE-4793: Fixes to de-duplicate users agent and authorization headers
