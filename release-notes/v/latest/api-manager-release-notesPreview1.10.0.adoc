...
link:/api-manager[API Manager] release notes include information about changes, enhancements, and bug fixes.
...
+
On the link:/api-manager/engaging-users-of-your-api#creating-a-portal[Developer Portal], you can now search for and sort the list of API portals. Click the sort icon to sort alphabetically by name in ascending or descending.
...
* link:/api-gateway-rt/configuring-an-api-gateway[New API Gateway distribution] with enhanced API auto-discovery, including auto-discovery for APIkit projects, and improved HTTP transport performance using an NIO-based transport.
