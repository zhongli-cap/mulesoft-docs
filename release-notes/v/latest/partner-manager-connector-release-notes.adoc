...
* Now uses *If-Modified-Since* header to check if cache-refresh is needed; removed export download for cache refresh when no changes are made to configuration.
* Added automatic retries for all Tracker and TPM operations when not in Caching(TPM) or Async (Caching)
...
* Caching refresh issue (Java Class not found) resolved.
* Endpoint data now includes Partnership in PMC Search operation against *ENDPOINTS* resource with Cache turned on
* ErrorCode in response from Start Error Notification operation now:
** Reflects user updates to *ErrorCodes*
** Includes *Severity* and *MaxRetries* values


=== Known Issues

* Multiple configurations are not supported when *asynchronousMode* is enabled.
* Transactions are sometimes not marked as onHold when there is a status window for onhold - this was worked on but not completed in time for the release.
