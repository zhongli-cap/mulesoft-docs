...
Prior to this release, MuleSoft offered API Entitlements on a contract basis, but did not restrict access to the related features. Features were used by those who often had no knowledge of the purchase requirement. In this release, these features are now available only to customers who have purchased API Entitlements. Contact MuleSoft for information about purchasing or obtaining a trial of API Entitlements.

If you have not purchased API Entitlements, the following changes are effective on Anypoint Platform:

* The credentials for your organization are no longer available on the *Organization* tab.
* Deploying APIs to CloudHub is no longer offered for free. 
* Analytics and controls for deploying a proxy, listing applications, setting up SLAs, applying policies, and changing permissions no longer appear on the API version page.

If you have _not_ purchased API Entitlements, the API version page looks something like this:
