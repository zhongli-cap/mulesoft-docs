...
* Fixed the "View application" link that wasn't working with Hybrid (SE-4611)
* Obfuscated the client secret on the developer portal applications detail page
* Fixed the link on the notification email when a new application is pending approval and the API belongs to a sub-organization (SE-4528)
* Fixed an issue with uppercase WSDL on SOAP proxies
* Fixed the Basic Authentication template that was not working properly with Mule Runtime 3.8.1 and later when HTTP is used for LDAP instead of HTTPS (SE-4529)
