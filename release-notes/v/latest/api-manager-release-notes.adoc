...
// February 1, 2017
//
// This release primary fixes maintenance issues.
//
// * Bump raml-1-parser version to 1.1.13.
// * Fix filterAlertsByPermission unit test on csService.
// * Add a flag to disable 'apis' entitlement check via configuration.
// * Fix Kafka consumer tests.
// * Update kafka-node version to 1.3.1, which includes changes previously on forked version.
// * Update authentication-server version.
// * Temporarily remove entitlement check.
// * Split check permission for applications into chunks.
// * Prevent name from getting invalidated when posting API Version.
// * Remove CS permissions when deleting API.
