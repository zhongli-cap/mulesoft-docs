...
* You can now specify a client ID and client secret when you create a client application. There is no UI support for specifying the credentials. This is supported through the API. API Manager uses the credentials you specify instead of generating a unique 32-digit string.
