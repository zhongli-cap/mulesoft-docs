...
This release of API Manager includes new and enhanced features for users who create an Anypoint Platform account from this date forward. Existing users can choose to upgrade APIs to the new API Manager or continue to use the familiar API Manager features and UI. Anypoint Platform opens the right version of API Manager for you automatically, depending on your status as a new user, upgraded user, or not-upgraded user. If you are a new user or upgrade your APIs to this release, you use Design Center to author and update RAML specifications.

=== Key changes and Enhancements

* A tight integration of Anypoint Platform components extends the link:/api-manager/environments-concept[use of environments] to API management. API permissions are now environment-based.
* You can now link:/api-manager/create-instance-task[create multiple instances] of an API.
* APIs reside in Exchange and are link:/api-manager/manage-exchange-api-task[managed from API Manager].
