...
* The error message associated with a URI conflict during auto-discovery API deployment has been improved. Now, such a conflict results in a 409 (conflict) instead of a 500 (internal server error) HTTP error code.
* API Manager now supports the configuration of scopes for the OpenAM token enforcement policy.
