...
- Fixed metadata retrieval bug for datasense. When inbound/outbound ports are not configured over the net.tcp adapter their metadata is not retrieved and in consequence there are not being shown through datasense. In addition, net.tcp services that are not deployed within the AOS are not shown as well.
