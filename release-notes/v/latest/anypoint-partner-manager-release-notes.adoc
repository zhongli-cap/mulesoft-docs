...
* Moved Service Path setting from x to link:/anypoint-b2b/endpoints#http-receive-settings[Endpoints - HTTP Receive Settings]

==== RNIF Streamlining

* Automatic name generator for Rosetta Net Information Framework (RNIF) endpoints no longer prepends *SEND* or *RECEIVE* to RNIF names.
* Location codes are now optional.
* Changed ROSETTA_NET to ROSETTANET in document definition table.
...
* Retreiving DocumentMaps no longer introduces possiblity of latency or timeouts
