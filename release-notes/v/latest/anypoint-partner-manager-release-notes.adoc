...
*link:/anypoint-b2b/anypoint-partner-manager[Anypoint Partner Manager User's Guide]*
...
* Fixed - For Transmissions view make the Filename filter field be text instead of dropdown
* Fixed - Need to do permissions bypass for Statistics resource.
* Fixed - RosettaNet, loads page as Dirty and Version unselected
* Fixed - Error on Channels Page while choosing Map
* Fixed - Channels Currently very Buggy
* Fixed - During channel creation at partner level if I try to use a document defined at home org I get validation error
* Fixed - While creating a map, if source or target documents are not defined, the page still shows Save button but does not save upon clicking and does not display any error
* Fixed - On TPM document definition page, if schema file is already document, the label says no file uploaded should show the file name
* Fixed - B2B Transactions overview dashboard does not show correct Transmission/Tracking/Error count
* Fixed - B2B Transactions overview dashboard not reflected current transactions
