...
Note that Mule 4 is only supported on the new API Manager. See new API Manager documentation, link:/api-manager/v/2.x/configure-autodiscovery-4-task[Configuring API Auto-Discovery in a Mule 4 Application] (on `+https://docs.mulesoft.com/+`), for how to configure API Gateway.
