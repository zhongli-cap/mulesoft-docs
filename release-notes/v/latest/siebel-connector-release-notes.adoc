...
* Added *isOutputEnrichedWithInputPayload* checkbox in the Siebel configuration. If unchecked, it will only present the output structure and the output payload of the operation. If checked, alongside the output, it will also give the input structure and the input payload used for a given operation.
* Limited the exceptions that trigger the reconnection strategy to *SocketTimeoutException*, *ConnectTimeoutException*,
*SocketException*, *ConnectException* and other exceptions that could require a Reconnection Strategy in the connector configuration.
