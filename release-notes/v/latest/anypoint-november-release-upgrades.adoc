...
MuleSoft is _automatically_ upgrading customers with twenty (20) or fewer APIs unless your platform administrator opts out (<<opt_out>>). The automatic upgrades occur on a rolling schedule. A self-service upgrade process is available for other customers.

The upgrade schedule depends on the number of APIs in your organization:

* March 5th - 7th: Customers with twenty (20) or fewer APIs will be automatically upgraded.
* Starting March 12th: All other customers will be offered self-service experience to migrate.
