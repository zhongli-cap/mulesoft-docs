...
== November 6, 2017
3.9.0, 3.8.5 and 3.8.4 Runtime Updates comes with the following fix:

* Fixes an issue where PollingReceiverWorker did not clean RequestContext after performing poll [MULE-13698]

3.8.5 and 3.8.4 Runtime Updates comes with the follwing fixes:

* Fixes an issue where setting inboundValidationMessage true in soapkit resulted in error cannot be cast to org.codehaus.stax2.XMLStreamReader2 [MULE-13167]
* Fixes a problem where CXF Proxy is throwing NPE when Schemas are imported in WSDL [MULE-7794]

3.8.5 Runtime Update comes with the following fix: 

* Fixes an issue where SFTP DSA verification is rejected as Invalid Verification Code using jdk 1.8.0_121 onwards [MULE-13465]
