...
This release now supports Internet Explorer. It also includes in-app links to documentation. It also includes bug fixes in several components such as the Choice, Try, and Transform components. 
...
* Missing tooltips on the top (right) toolbar - the following were added "Download Mule Application"; "Support"
* Auto completion doesn't work in DataWeave function editor
* Dataweave unnecessarily underlines the script as having an error
* Improve transformer inline function editor error handling in DataWeave (Errors aten't shown in DataWeave inline Function Editor because the provided model caused false errors during scoping.)
* The Transform Message component forces you to select a mapping when you shouldn't have to
* Suggestions get cut off in Choice card
...
* Dictionary Editor not working in Windows chrome
