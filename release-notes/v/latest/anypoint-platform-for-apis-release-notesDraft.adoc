...
+
In addition to listing applied policies in the order that you apply them, Anypoint Platform now numbers policies the order of application.
* The audit log now reveals the permission level of the users when you assign a permission to a user.

*Bug Fixes*

* A tooltip is now displayed when the full API name is truncated in the UI. (AP-2258)
* The problem related to resetting the client secret when using Internet Explorer 11 has been resolved. (AP-2293)
* A list of grant types are now displaed in the application sidebar. (AP-2317)
* An example or default value that you set in the RAML is no longer hidden in the console. (AP-2378)
* The API Designer is no longer overriding example files. (AP-2384)
* The import process now overrides example files when configured to do so. (AP-2384)
