...
* DataWeave now includes supports for new file formats, allowing you to read and write fixed-width and other flat files using DataWeave. The structure of these files can be easily defined via a new link:/mule-user-guide/v/3.8-m1/dataweave-flat-file-schema[Flat File Definition file] format. You can also import COBOL Copybook files into this schema format using Studio (Early Access). Finally, if you have previous DataMapper transformations, you can use the link:/mule-user-guide/v/3.8-m1/dataweave-migrator[migration tool] to convert them to DataWeave transformations now.
* The link:/mule-user-guide/v/3.8-m1/dataweave-language-introduction[DataWeave language] also adds several new link:/mule-user-guide/v/3.8-m1/dataweave-operators[operators].
...
This release also features support for all the new capabilities in Mule 3.8, which are found in the link:/release-notes/mule-3.8.0-release-notes[Mule 3.8 release notes].
