...
[NOTE]
To obtain the required values of your Environment ID and your Organization ID, you’ll have to use the link:/runtime-manager/cloudhub-cli[Runtime Manager CLI]. Through this tool, run the commands `business-groups` and `environments`. Each returns a list with all of the available business groups / environments in your organization, together with the corresponding IDs for each.
