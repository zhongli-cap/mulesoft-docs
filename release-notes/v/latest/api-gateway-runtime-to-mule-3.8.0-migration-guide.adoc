...
As part of the link:/[Gatekeeper security improvement], on startup, managed API traffic is disabled until the online policies are successfully applied. Until traffic is enabled, the API receives a 503 status code in response to requests. 
