...
+
When you don't want to validate the client id, you can use a custom policy based on the original policy, removing the client validation step.

== Changes

Streaming is turned off by default in this release because users might not be consuming request streams in their existing apps. link:/mule-user-guide/v/3.8/configuring-properties#system-properties[Set the following system properties] to override this default for all HTTP requests in a container:
