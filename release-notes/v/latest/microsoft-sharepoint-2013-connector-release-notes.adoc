...
- *FileQuery* and *FolderQuery* operations are not supported when interacting with Sharepoint Online. These operations will be covered by the new Sharepoint Online Connector.
...
- Under certain conditions, login now works. Previously, under specific conditions, login could throw a 403 error because some cookies were missing in the login request. Now the connector is correctly handling those cookies.
