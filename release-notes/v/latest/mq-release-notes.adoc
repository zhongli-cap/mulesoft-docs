...
== Version 1.3.1 - November 5, 2016 -- Upcoming
...
[source]
forbiddenUserAgents=MQ-CONNECTOR/1.3,MQ-CONNECTOR/1.2
...
|===
|Issue |Description
|MQ-572 |Modify startDate and endDate in metrics to use a standard date format, such as `2016-11-01T19:00Z`
|MQ-556 |Possible resource leak with usage metrics
|MQ-548 |NPE sending a message with no body
|MQ-543 |Error 500 sending an Ack with no bearer token
|MQ-535 |Error 500 trying to create a queue with the same name as the dead letter queue
|MQ-534 |MQ should return an appropriate error when trying to configure the DLQ of a queue as itself
|MQ-316 |Ability for MQ administrators to forbid user agents in a broker. This is specified as a 
comma-separated property in the deploy properties of Anypoint MQ, 
such as `forbiddenUserAgents=MQ-CONNECTOR/1.3,MQ-CONNECTOR/1.2`
|===
