...
When you send a request that includes a query parameter without `=` to an APIkit 3.8.3 app, the value of the query parameter is null. For example, in http://localhost:8080/resource?queryParameter the queryParameter is null. Consequently, only query parameters of type nil pass APIkit validation; other query parameters fail.
