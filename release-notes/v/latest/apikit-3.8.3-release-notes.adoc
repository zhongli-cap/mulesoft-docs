...
APIkit 3.8.3 returns the expected HTTP status code when using a query parameter to call an implemented APIkit app. For example `+http://localhost:8080/resource?queryParam+` returns 200 (success) if queryParam is of type nil. Otherwise, queryParam is of type NULL, the request returns 400 (failure).
