...
[NOTE]
--
This is a pre-release notes for v1.48.1 scheduled for July 16th.
--

This release introduces two major new features: A new load balancer service and VPC setup self-service through the Anypoint CLI, VPC self service includes the ability to add and manage VPC’s including IP and port security setup for each VPC.
