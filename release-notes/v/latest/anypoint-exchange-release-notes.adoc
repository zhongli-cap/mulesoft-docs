...
* With this major upgrade we have kept the Exchange 1.7 still accesible for 90 days. The URL for old Exchange is now https://anypoint.mulesoft.com/exchange1/. Existing customers with content on Private Exchange can also access the old Exchange using a link on the Exchange 2.0. None of the content on old Exchange is deleted. Customers can migrate the content to Exchange 2.0 using link:https://docs.mulesoft.com/anypoint-exchange/ex2-migrate[migration instructions].
