...
Anypoint Analytics is compatible with all hardware and software as described in link:/mule-user-guide/v/3.9/hardware-and-software-requirements[Hardware and Software Requirements], as well as API Manager v1.0 and newer.
...
* API is updated to be environment-aware
** Events API supports environments using a new set of resource paths, while maintaining backwards-compatibility by keeping the current resource paths functional.
