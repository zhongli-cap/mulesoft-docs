...
This release of API Manager includes new and enhanced features for new users who create an Anypoint Platform account. Existing users can choose to upgrade APIs to the new API Manager or continue to use the familiar API Manager features and UI. Anypoint Platform opens the right version of API Manager for you automatically, depending on your new user, upgraded user, or not-upgraded user status. If you are a new user or upgrade your APIs to this release, you use Design Center to author and update RAML specifications.

== Key changes and Enhancements

* API Manager supports environments and API permissions are now environment-based.
...
* APIs reside in Exchange and are managed from API Manager.
* Mule 4-related changes to API Manager are:
** Classloader isolation exists between the application, the runtime and the connectors, and policies.
** Expect changes to the packaging, size, and scope of a policy.
...
* Custom policy development has changed. For more information, see the documentation.

== Changes to Out-of-the-Box Policies

* Mule 4 replaces the security manager policies two HTTP Basic Authentication policies are available: 
** The combination of HTTP Basic Authentication and Simple Security Manager 
** A combination of HTTP Basic Authentication and LDAP Security Manager
+
The policy protects an API by expecting and validating a username and password.
+
