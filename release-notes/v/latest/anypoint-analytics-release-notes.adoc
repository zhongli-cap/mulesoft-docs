...
This release also includes a major re-achitecture on the API Event backend. Effective starting with this release onward, the Analytics Event API provides a one month retention policy instead of 3 months for events. Also, a usage limit of 10 request/minute, with a maximum of 20,000 events per request is enforced. Request rates higher than 10 request/minutes will be throttled.
