...
This release includes also a major reachitecture on the API Event backend. Starting from this release, the Analytics Event API provides a one month retention policy instead of 3 months for events. Also, usage limit of 10 request/minute, with a maximum of 20,000 events per request is enforced. Request rates higher than 10 request/minutes will be throttled.
