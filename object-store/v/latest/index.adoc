...
* Persistent storage of objects with a flexible time to live (TTL) of up to 30 days, but the default TTL is 
14 days. (After the TTL expires, the data is deleted, but the object store continues to be available.) Each time the data is updated, the TTL is extended by the duration of the TTL. For more information, see link:/object-store/osv2-faq#how-long-can-data-persist-in-osv2[How long can data persist in OSv2?].
