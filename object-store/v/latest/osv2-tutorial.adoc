...
+
. Search for `http` and drag and drop the HTTP Connector onto the Studio Canvas. The HTTP connector listens for the HTTP packet that we will send using a curl command.
+
...
curl, https://www.getpostman.com/apps[Postman], or a browser extension. Any serializable data can be sent to the 
