...
After learning some of the basic functionality in the link:/anypoint-studio/v/6/basic-studio-tutorial[Basic Studio Tutorial] and link:/getting-started/build-a-hello-world-application[Build and Deploy a Hello World App], it's time to try your hand at enriching your application with an *Anypoint Connector*.
...
Anypoint Connectors are created as extensionsfor Mule that allow you to operate as you would normally in popular software as a service (SaaS) such as Salesforce, Google Contacts, ServiceNow, Workday, Facebook, and Twitter. Connectors simplify the interaction with the SaaS providers' APIs, providing methods for most or all of the available operations.

You typically define how the connector interacts with the SaaS app by storing the authentication and other settings in a link:/mule-user-guide/v/3.8/global-elements[Global Element] and referencing the global element from the connector. Additionally, properties files provide a convenient mechanism for storing variables that you set in the global element or connector. See link:/mule-user-guide/v/3.8/configuring-properties[Configuring Properties]

Much of the functionality of each individual connector is dependent upon its target; that said, there are many features common to most connectors. After creating and running the applications in this tutorial, you should be able to apply what you've learned to connect applications to various SaaS applications. If you need to create your own connector for an existing SaaS or other application, see link:/anypoint-connector-devkit/v/3.8/[Anypoint Connector DevKit].
