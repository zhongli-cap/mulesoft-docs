...
Learn to create a Mule project, then deploy it as a Mule application through Anypoint Studio's embedded Mule server.

As the objective of this quick tutorial is get something built and running fast, this document doesn't spend much time explaining components. You are about to create a basic application in Studio, deploy it to Studio's embedded server, then interact with the application using a web browser.
