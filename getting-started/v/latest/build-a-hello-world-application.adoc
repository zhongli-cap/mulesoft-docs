...
Before starting, make sure you link:https://www.mulesoft.com/platform/saas/cloudhub-ipaas-cloud-based-integration[enable CloudHub access] with your Anypoint Platform account.

. Export your Mule project from Studio as a Mule deployable archive `.zip` (see link:/anypoint-studio/v/6/importing-and-exporting-in-studio[Exporting Projects from Studio]) or have performed the export link:/mule-user-guide/v/3.8/maven-tools-for-mule-esb#creating-a-mule-application[using Maven].
. If you do not already have access to the Anypoint-CLI command line tool, follow the brief link:/runtime-manager/anypoint-platform-cli#installation[installation instructions].
...
. Use the link:/runtime-manager/anypoint-platform-cli#runtime-mgr-application-deploy[Deploy to CloudHub Command] providing the *name* of the app and the *location* of the deployable archive (`.zip`) file on your file system, for example:
+
----
...
. Go to the domain provided `helloworldtestdocs.cloudhub.io` and you should see "Hello World!"; recall the application sets the payload to that string and returns this as the response via the HTTP connector.
