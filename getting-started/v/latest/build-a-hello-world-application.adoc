...
The HTTP connector allows your Mule application to connect to Web resources through the HTTP or HTTPS protocol. You need to provide values for the following HTTP connector properties:
...
You also need to create a global element for the HTTP Connector to reference. A global element encapsulates some reusable configuration properties that multiple connectors can reference.

. In the canvas, click the HTTP connector to open the canvas to open its Properties Editor.
. Click the plus sign image:plus-1.png[plus] to configure the host and port number
. In the Global Element Properties dialog, set the host to *localhost*, accept the default port *8081*, click OK.
+
The properties window opens, with Path populated with " /" and Connector Configuration populated with a reference to the element you just created.
+
image:http-global-config.png[http-global-config]
+
. In the canvas, click the HTTP connector.
+
The red warning marks disappear from the properties editor and for the connector on the canvas.
+
...
+
. In Studio, stop the application from running by clicking Terminate image:stop.icon.png[] in the console.
