...
+
A chart that is grouped by Violated Policy Name includes events that triggered a policy violation on the API Gateway. A chart that does not group by Violated Policy Name filters out those events. For example, API X has 100 requests, of which 10 were rate-limited. Unless the chart groups by Violated Policy Name, the chart shows the number of requests over the 90 requests that were not rate limited. However, a chart filtered by the number of policy violations shows 10.
