...
+
A chart that is grouped by Violated Policy Name includes events that triggered a policy violation. A chart that does not group by Violated Policy Name filters out those events. For example, API X has 100 requests, of which 10 were rate-limited. Unless the chart groups by Violated Policy Name, chart computations are based on the 90 requests that were not rate limited. However, chart computations are based on 100 requests if the chart groups by Violated Policy Name.
