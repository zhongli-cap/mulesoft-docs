...
== Overview
The installation process is broken down into a few stages:
. Provisioning infrastructure
. Setting up each VM (operating system, disks)
. Installing Runtime Fabric across VMs
. Registering Runtime Fabric
. Adding your Mule license key

The installation uses the included Azure Resource Manager (ARM) templates with scripts to combine the first three steps. Below are more details on what occurs during installation to better understand the process:
