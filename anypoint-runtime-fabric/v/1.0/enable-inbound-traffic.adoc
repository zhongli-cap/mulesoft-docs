...
This configuration ensures high availability, protects against failures, and gracefully handles automated failover in the event a replica of the internal load balancer were to restart or become evicted and rescheduled on another controller VM.
...
. Runtime Fabric is behind a different type of load balancer. +
