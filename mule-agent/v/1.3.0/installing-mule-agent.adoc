...
[IMPORTANT]
====
Mule 3.7.x and API Gateway 2.x provide an link:/mule-agent/v/1.1.1/index[older version of the Mule Agent]. This version doesn't provide support for link:/runtime-manager/sending-data-from-arm-to-external-monitoring-software[Sending Data from Runtime Manager to External Monitoring Software]. Version 1.3.0 of the Agent must be downloaded separately.
====
...
This option configures the Mule agent to establish a TLS connection(HTTPS) with Agent API's clients or an on-premises administration console. You need to provide the truststore and keystore in JKS format. This option enables a TLS channel for REST communications only. Once you select the Secure connection Channel mode, you see the following menu:
...
Please note that this procedure will copy the provided Keystore and Truststore files to .`$MULE_HOME/conf/`. For the Truststore file, it will be under the name of `truststore.jks`, while the Keystore file will be copied as it is, since is being referenced by the configuration file located at .`$MULE_HOME/conf/mule-agent.yml`. This files should not be removed and the installation script will require the proper permissions to be able to write in the mentioned directory.
