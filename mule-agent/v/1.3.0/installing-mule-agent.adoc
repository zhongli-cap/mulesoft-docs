...
Mule agent version 1.3.0 installs as a plugin from a zip file, available in the latest version page or in the corresponding link:/release-notes/mule-agent-release-notes[release notes page].
...
[WARNING]
It is not supported to register a Mule runtime with both an older link:/mule-management-console/[Mule Management Console (MMC)] and a link:/runtime-manager/cloudhub-and-mule[CloudHub and Mule]. If the Mule runtime is currently managed in MMC, you should first unregister the Mule runtime with MMC before running the `amc_setup -H` script.
