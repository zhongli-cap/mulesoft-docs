...
Please note that this procedure will copy the provided Keystore and Truststore files to .`$MULE_HOME/conf/` under the name of `keystore.jks` and `truststore.jks` respectively. This files should not be removed and the installation script will require the proper permissions to be able to write in the mentioned directory.
