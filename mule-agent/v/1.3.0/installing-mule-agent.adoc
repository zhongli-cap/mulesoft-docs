...
NOTE: For information on starting and stopping API Gateway, see link:/anypoint-platform-for-apis/configuring-an-api-gateway[Configuring an API Gateway].
...
Note: It is not supported to register a Mule runtime with both an older Mule Management Console (MMC) and a Runtime Manager. If the Mule runtime is currently managed in MMC, you should first unregister the Mule runtime with MMC before running the `amc_setup -H` script. 
