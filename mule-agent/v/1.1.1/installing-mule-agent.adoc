...
Mule agent version 1.1.1 now installs as a plugin from a zip file available in the latest version page or the corresponding link:/release-notes/mule-agent-release-notes[release notes page].
...
. Stop Mule or the API Gateway runtime. You can stop Mule with the `./bin stop` command or by killing the process. Restart Mule with the `./bin start` command. For information on starting and stopping API Gateway, see link:/api-manager/configuring-an-api-gateway[Configuring an API Gateway].
