...
A test file must contain at least one step. The following example performs a `GET http://httpbin.com/get` and an [assertion].
...
You use the following functions, which follow Fowler's _GivenWhenThen_ structure, in tests:
...
=== Skipping a Test Block

Test blocks also have a shorthand name to _skip the block_ described as `x<name>`. For example, the skipped version of `describe` is `xdescribe`.
...
You do _not_ need to coerce the values into strings. Enclose a custom interpolation text and value in backticks (```).
...
If you get a failure in the middle of a test, you need to delete any created assets. For example, the user creates an asset, preforms a validation, and then deletes it.
...
You can prevent this problem by using the following recommended words in your test code:

* `should` when something _might_ fail, but is not mandatory for the test
