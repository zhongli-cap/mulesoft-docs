...
. Copy the `installer.tar.gz` to a single server or virtual machine. The installer URL can be found by navigating to Anypoint Runtime Manager, selecting the Worker Clouds tab, and clicking the Create Worker Cloud button.
. 
+
----
...
. Run the following command on a separate machine containing the `auto-installer.py` script and set up to ssh into each server or virtual machine. This script requires Python 2.7 to run. The private and public IP addresses, admin username for the servers, the block device path for _Disk 2_, and the private ssh key pair should be filled in before running.
+
----
...
+
[NOTE]
This step will install the Project Worker Cloud appliance across the servers to form a cluster. It may take 10-15 minutes or longer to complete.
