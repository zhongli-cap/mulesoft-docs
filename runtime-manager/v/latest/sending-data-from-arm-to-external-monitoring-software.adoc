...
If you deploy apps from Runtime Manager to Mule servers on-premises, you can send your Mule app event data to third party monitoring software such as *Elk* and *LogStash*.
...
You have the ability to leverage both options to monitor business events, custom business events, logs, and – in the case of apps deployed to an link:/api-manager/api-gateway-runtime-archive[API Gateway] Runtime – API Analytics and governance.
