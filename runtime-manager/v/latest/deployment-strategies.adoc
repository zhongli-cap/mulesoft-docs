...
* image:logo-server-active.png[link="/runtime-manager/deploying-to-your-own-servers", title="Anypoint Platform Private Cloud Edition"] An *On-premises* edition of Runtime Manager to an *on-premises server* that has Mule runtime.
+
link:/runtime-manager/deploying-to-your-own-servers[_See deployment instructions_]
