...
[TIP]
====
Note that the topics in the Runtime Manager section are tagged with these icons to signal what deployment strategies are relevant to the content explained on the page.

image:logo-cloud-active.png[cloud]image:logo-hybrid-active.png[hybrid]image:logo-server-active.png[server]

////
* An image:logo-server-active.png[pcf]
////
