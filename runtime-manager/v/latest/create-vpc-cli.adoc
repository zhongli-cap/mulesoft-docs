...
Once the VPC is created, All inbound traffic is blocked by default. You need to create firewall rules to allow traffic to the VPC. +
...
<1> Refer to the recently created `vpc-tutorial` VPC to set the new firewall rule.
...
Setting this firewall rule allows a load balancer to communicate to your workers using its default configurations.
