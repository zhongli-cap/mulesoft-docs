...
Specifying properties in Runtime Manager enables you to define properties in one place rather than having multiple application property files for a single application in multiple environments. By defining a properties template in a sandbox application, you can create a new application and configure it for a specific environment.
