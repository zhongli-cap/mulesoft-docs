...
The definitive solution for persistent batch jobs is Cloudhub is to use link:/https://docs.mulesoft.com/object-store/[Cloud Object Store]
...
When you deploy an application containing link:/mule-user-guide/v/3.8/batch-processing[batch jobs] to CloudHub with persistent queues enabled, the batch jobs use CloudHub's persistent queuing feature for the batch queuing functionality. However this is not recommended because introduces two limitations:
...
* Message loss can still occur in case of an impaired instance
====
