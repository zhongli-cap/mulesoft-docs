...
[IMPORTANT]
link:/mule-user-guide/v/3.7/batch-processing[Batch jobs] only run on a single worker at a time, and cannot be distributed across multiple workers.
...
When you deploy an application containing link:/mule-user-guide/v/3.7/batch-processing[batch jobs] to CloudHub with persistent queues enabled, the batch jobs use CloudHub's persistent queuing feature for the batch queuing functionality to ensure zero message loss. However, there are two limitations:
...
Although you can refer to the complete references for link:/mule-user-guide/v/3.7/transaction-management[Transaction Management], the link:/mule-user-guide/v/3.7/vm-transport-reference[VM transport], and link:/mule-management-console/v/3.7/reliability-patterns[Reliability Patterns], note that CloudHub imposes some key differences in the way that persistent queues are implemented. 
...
* Refer to the complete references for link:/mule-user-guide/v/3.7/transaction-management[Transaction Management], the link:/mule-user-guide/v/3.7/vm-transport-reference[VM transport], and link:/mule-management-console/v/3.7/reliability-patterns[Reliability Patterns].
