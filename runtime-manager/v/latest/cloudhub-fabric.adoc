...
When you deploy an application containing link:/mule-user-guide/v/3.8/batch-processing[batch jobs] to CloudHub with persistent queues enabled, the batch jobs use CloudHub's persistent queuing feature for the batch queuing functionality. However this introduces two limitations:
