...
Some additional *optional* parameters are available to configure a connection between a Mule runtime and an Anypoint Platform On-Premises Edition RUntime Manager. These additional parameters are only appropriate when using the `-H` parameter. 
