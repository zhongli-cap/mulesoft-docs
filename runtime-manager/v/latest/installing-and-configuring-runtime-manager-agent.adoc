...
These tables show you the different arguments you can add to the `amc_setup` command. The required arguments differ depending on if you're registering your server to be managed via the cloud console of Runtime Manager, or to be managed by the Anypoint Platform On-Premises Edition.
...
=== Register with a Runtime Manager with amc-setup
...
. Some additional optinal parameters are available to configure a connection between a Mule runtime and an Anypoint Platform On-Premises Edition RUntime Manager. These additional options only apply when using the `-H` option. 

////
[tabs]
------
[tab,title="Anypoint Platform On-Premises Edition - Optional Parameters"]
....
////
