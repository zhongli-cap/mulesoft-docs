...
[WARNING]
It is not supported to register a Mule runtime with both an older link:/mule-management-console/[Mule Management Console (MMC)] and Runtime Manager. If the Mule runtime is currently managed in MMC, you should first unregister the Mule runtime with MMC before running the `amc_setup -H` script.

[NOTE]
----
MuleSoft support can also provide you with some migration scripts to help you migrate from MMC to Runtime Manager.
----

For details, see link:/runtime-manager/managing-servers#add-a-server[Managing Servers].


== Registering with an On-Prem Runtime Manager

Some additional *optional* parameters are available to configure a connection between a Mule runtime and an On-Premises Runtime Manager, which runs in an Anypoint Platform On-Premises Edition installation.

These additional parameters are only appropriate when using the `-H` parameter.
