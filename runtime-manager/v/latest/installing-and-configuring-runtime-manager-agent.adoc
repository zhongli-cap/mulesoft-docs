...
To upgrade an existing Runtime Manager Agent, after you have decrompressed the Runtime Manager Agent zip file in your $MULE_HOME/bin folder (replacing the amc_debug, amc_setup and amc_setup.bat files), type `./amc_setup -U` to update the agent version, but keep your existing `$MULE_HOME/conf/mule-agent.yml` configuration.
