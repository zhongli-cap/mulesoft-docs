...
This document covers how to <<Installing the Agent from the Zip File, install>> or <<Updating a Previous Installation, update>> the Runtime Manager Agent, which mediates the communication between Mule Runtimes and the Runtime Manager.
...
Runtime Manager Agent v1.6.0 installs as a plugin from a zip file available link: http://mule-agent.s3.amazonaws.com/1.6.0/agent-setup-1.6.0.zip[here].
...
[IMPORTANT]
====
Mule 3.7.x and API Gateway 2.x provide an link:/runtime-manager/runtime-manager-agent[older version of the Runtime Manager Agent]. This version doesn't provide support for link:/runtime-manager/sending-data-from-arm-to-external-monitoring-software[Sending Data from Runtime Manager to External Monitoring Software].
====
...
Download the link: http://mule-agent.s3.amazonaws.com/1.5.1/agent-setup-1.6.0.zip[Runtime Manager Agent v1.6.0 zip file].
...
Compatible Mule and API Gateway runtimes ship with an older version of the `amc_setup` script. If you already ran an older version of this script to installed an earlier version of the Runtime Manager Agent, then follow these steps to keep your existing Runtime Manager Agent configuration and modifications:
...
You can always run `./amc_setup --help` to see the available properties for the installation command.

[TIP]
On the Runtime Manager console, you can see a full example of the code you need to run by clicking the link:/runtime-manager/managing-servers#add-a-server[Add a Server] button. This sample already includes your specific organization's ID, so it's ready to use in case you don't need to configure anything beyond the default settings.
