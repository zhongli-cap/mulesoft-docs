...
The configuration files created during the installation are $MULE_HOME/conf/mule-agent.jks and $MULE_HOME/conf/mule-agent.yml.
In order to uninstall the agent configuration, just remove these files and restart the instance.
