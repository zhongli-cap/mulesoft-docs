...
When the payload format isn't of a convertible type, the agent only passes on the full qualified name of the format MIME type, not its content.
...
The Runtime Manager agent only passes on payload messages where the *Converted to String* colum reads `TRUE` in the table below:
...
You can override the default behaviour make the agent forward all payload formats or none. This is achieved by adding certain properties to the `conf/mule-agent.yml` file.
