...
== Configuring a Connection to an Internal DNS

To connect to other networks from your VPC, you can specify custom private domains by adding the IP address and domain name for your network. This feature is only supported on the following versions of the Mule runtime: 3.5.x, 3.6.x, 3.7.4, 3.8.0, and 3.8.1 or later

When configuring the DNS to connect to your internal network, consider the following:
...
* The maximum number of characters of all the domains you configure must not exceed 256 characters. For example, adding the domains `mydomain1.com` and `mydomain2.com` would require 28 characters. Each domain name consumes an additional `.` character.
* You should avoid entering hostnames to ensure that you do not exceed the 256 character limit.

To Configure a Connection to an Internal DNS
