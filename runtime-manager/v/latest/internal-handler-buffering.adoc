...
[IMPORTANT]
====
The buffer maximum size is configured by amount of events and not in bytes, so the developer must be carefull when configuring these values as, for example, if the events have a considerable size and the user has a MEMORY buffer with a big maximumCapacity, the heap might suffer a big stress and this might impact the application performance.
====

As a note, if the user configures a `DISK` buffer, the agent will also instantiate an in-memory buffer in order to improve the performance of the I/O opperations. This memory buffer used in the middle has a default value of 10000 events and is flushed to disk every 500ms; these values are configurable with the system properties `agent.disk.buffering.cache.size` and `agent.disk.buffering.cache.flush.frequency`, respectively.
