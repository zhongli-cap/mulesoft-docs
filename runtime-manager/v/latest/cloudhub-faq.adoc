...
Yes!  link:mailto:cloudhub-support@mulesoft.com[File a support case] to enable global deployments for your account. Applications can be deployed to: US (East), US (West), Canada, South America (Sao Paulo), Europe (Ireland, Frankfurt, London),  and Asia-Pacific (Sydney, Tokyo, Singapore). Administrators can  link:/runtime-manager/managing-cloudhub-specific-settings[set the default region] on the Organization tab in Account Settings, but that region can be adjusted here when the application is deployed, if necessary.
