...
|<<exchange asset upload>> | Uploads an asset to Exchange
|<<exchange asset list>> | Lists all assets on Exchange
...
This Command copies the asset passed in `source` to the location passed in `target`. +
Both arguments `source` and `target` are represented using the format: +
...
[source,Example]
----
> exchange asset copy mulesoft-914/org.mulesoft/sample/1.0 mulesoft-914/com.mulesoft/sample/1.0-SNAPSHOT
----

Copies the sample asset from th org.mulesoft group Id on the mulesoft-914 organization, to the com.mulesoft group Id on the same organization and changes its version to 1.0-SNAPSHOT. +
If no organization Id is specified, the command defaults to the organization that the CLI is using.
...
[source,Example]
----
> exchange asset download [options] <directory>
----

This Command downloads an Exchange asset to the directory specified in `directory`. +
Besides the default `--help`, `-f`/`--fields` and `-o`/`--output` options, this command also takes:

[%header,cols="30a,40a,30a"]
|===
|Command | Description |  Example
| --groupId | The groupId of the asset to download. .3+<.<| `exchange asset download --groupId com.mulesoft --assetId sample --version 1.0`
| --assetId  | The assetId of the asset to download.
...
[source,Example]
----
> exchange asset upload [options] <zipfile>
----

This Command uploads the ZIP packaged asset from the path passed in `zipfile`.

[WARNING]
--
The ZIP file must include an "exchange.json" file describing the asset.
--
...
|===
|Command | Description | Example
|--groupId <val>    | Asset group ID .6+<.<| `exchange asset upload --groupId com.mulesoft --assetId sample --version 1.0 apiVersion 1.1 --classifier connector`
|--assetId <val>    | Asset ID
|--version <val>    | Asset version
|--apiVersion <val> | Asset API version
|--name <val>       | Asset name
|--classifier <val> | Asset classifier
|===
...
[source,Example]
----
> exchange asset list [options]
----

This Command lists all assets on your Exchange. +
You can type the name of an asset to display information about the specified asset.
...
[source,Example]
----
> exchange asset delete [options] <name>
----

This Command deletes the asset specified in <name>.
...
|===
|Command | Description | Example
|--groupId <val>    | Asset group ID .3+<.<| `exchange asset delete --groupId com.mulesoft --assetId sample --version 1.0`
|--assetId <val>    | Asset ID
|--version <val>    | Asset version
|===
