...
// Comment Out Deployment Logos (Outdated and not accurate anymore for this product).
...
[CAUTION]
In order for the Anypoint Platform CLI to recognize your target servers, each server needs to be manually registered with the platform using the link:/runtime-manager/anypoint-platform-cli#runtime-mgr-server-register[runtime-mgr server register] command.
