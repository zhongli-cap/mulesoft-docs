...
make sure the git protocol is not blocked by your firewall or network. To do this try running the commmand
...
[NOTE]
You may need to run these commands as an Administrator. On a UNIX machine, prepend each command with `sudo`.
...
[NOTE]
You may need to run them with Administrator permissions, so on a UNIX machine, prepend `sudo` to each command.
