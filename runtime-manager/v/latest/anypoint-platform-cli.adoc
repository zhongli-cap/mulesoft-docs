...
----
[NOTE]
For the installation, make sure to write `npm install -g anypoint-cli`, not `npm install anypoint-cli -g`.
...
If you get errors installing indicating 
[source,Example]
----
npm ERR! Cloning into bare repository 'C:\Users\lxm8426\AppData\Roaming\npm-cache\_git-remotes\git-github-com-mulesoft-minimist-git-267deee0'...
npm ERR! fatal: read error: Invalid argument
npm ERR!
----

make sure the git protocol is not blocked by your firewall or network. To do this try running the commment
[source,Example]
----
git clone git://github.com/npm/npm.git
----
It this fails, make sure you have the git client installed on your computer. 

If you cannot install git, change to another clean directory where you have write permission, then run these commands: 
[source,Example]
----
$ npm cache clean
...
----
[NOTE]
You may need to run these commands as an Administrator. On a UN*X machine, prepend each command with `sudo`. 


===Packaging Anypoint-CLI
You can pre-package the Anypoint-CLI (or any node.js module) with its dependencies to avoid requiring other host machines download dependencies during the install process. 

====Option 1. 
Create an executable using link:https://github.com/zeit/pkg[https://github.com/zeit/pkg].

To do this, change to a folder in which you want to work, then run the following commands.
[source,Example]
----
$ npm install -g pkg
$ npm install anypoint-cli
$ pkg node_modules/anypoint-cli/
----
[NOTE]
You may need to run them with Administrator permissions, so on a U*NIX machine, prepend `sudo` to each command.

This will create three standalone executables, one for each target: linux, macos, and windows. 

For example, on a Mac computer, you would run `anypoint-cli-macos` from any directory on your computer. 

[NOTE]
----
It also infers the node version to package and the OS architecture to target from the machine where the `pkg` command is run. The optional `--targets` argument lets you override this (https://github.com/zeit/pkg#targets). The executables are rather large ~100MB because they include a copy of nodejs. This also means that there is no need to have nodejs/npm installed on the host machine.
----

====Option 2. 
Create a tarball with link:https://github.com/majgis/npm-bundle[https://github.com/majgis/npm-bundle].
[source,Example]
----
$ npm install -g npm-bundle
$ npm-bundle anypoint-cli
$ npm install -g anypoint-cli-2.0.3.tgz
----
This creates an executable named anypoint-cli. You can move and run this from any directory by just running `anypoint-cli`. 

[NOTE]
----
Unlike the first option that does not require any software to be installed on the client machine, this option does require nodejs/npm to be installed on the host machine. However, it uses standard features of `npm` which would probably make it easier to troubleshoot in the event that there was an issue running the CLI on the host machine.
----
