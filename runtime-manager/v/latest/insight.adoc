...
[IMPORTANT]
====
Enabling Insight may cause a performance impact when processing application data. We strongly discoraged enabling Insight in production environments. To monitor a production environment, we recommend using the tools listed in the link:/runtime-manager/monitoring[monitoring section].
