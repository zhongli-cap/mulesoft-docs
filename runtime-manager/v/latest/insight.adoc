...
[NOTE]
Data for any new applications created after 9/9/2017 will be stored in the first region the application is deployed in. To change the storage region for Replay Data, the customer has to delete the application and redeploy it in the new region. All existing applications will continue to store replay data in the older default region: us-east-1. Make sure this doesn't violate any of your compliance requirements.
