...
[IMPORTANT]
====
Enabling insight introduces a significant performance impact on the processing of your data and it's stronly discoraged to be enabled in production deployment. For everyday monitoring, it's advisable to use the tools listed in the link:/runtime-manager/monitoring[monitoring section].
