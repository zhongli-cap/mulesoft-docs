...
Anypoint Platform enforces rate limits that determine how often an alert is triggered. These limits prevent excessive emails from being sent. The specific rate limit applied depends on if your application is deployed to CloudHub or a hybrid server:

* CloudHub: Using Cloudhub-Connector to trigger notifications and alerts is limited 5 custom application alerts per IP per second. If an Application uses multiple workers, then this limit is multiplied by the number of workers per Application, since the limitation is per IP. The Custom Notifictions that trigger e-mails are subject to the next section.

* Alert e-mails: both out-of-the-box and custom alerts have a rate limit where an alert is triggered at most once every 2 seconds and a half. This means that only one alert notification email is sent every 2.5 seconds. This limit is per the combination of Organization, Environment, Alert (Name/ID) and Resource (e.g.: Application(s), Server, Cluster or Group)
...
* link:/runtime-manager/custom-application-alerts[Custom Application Alerts] tutorial.
