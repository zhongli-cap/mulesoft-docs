...
This topic describes how to configure alerts using Runtime Manager. You can create an alert that sends an email notification when a specific event occurs. These can be events occur on an application or server. Runtime Manager provides multiple default alert types, including:

* Notification when an application exceeds a certain number of events processed within a certain time period.
* Notification when deployment completes with success or failure.
* Notification when CloudHub encounters a problem with the worker on an application via the worker monitoring system.
* Notification when a server is disconnected or deleted

== Custom Alerts

In addition to default alerts, CloudHub enables you to create a custom alert for an application. Custom alerts are triggered by notifications that your application sends to the Runtime Manager console using the CloudHub Connector. Custom alerts are only available for applications deployed to CloudHub. You cannot configure an alert for an application deployed to a customer-hosted runtime.

See link:/runtime-manager/custom-application-alerts[Custom Application Alerts] for more information.
