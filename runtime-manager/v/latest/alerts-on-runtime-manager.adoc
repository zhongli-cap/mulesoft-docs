...
* CloudHub: limits custom application alerts to 5 alerts per IP per second.
* Hybrid: both out-of-the-box and custom alerts have a rate limit where an alert is triggered at most once every five seconds. This means that only one alert notification email is sent every 5 seconds. This limit is per the combination of Organization, Environment, Alert (Name/ID) and Resource (e.g.: Application(s), Server, Cluster or Group)
