...
[NOTE]
These properties are only available for using in alerts for <<Conditions on CloudHub Applications, CloudHub deployed applications>> with the condition set to *Custom Notification Alert*. Other alert types don't expose this information.
