...
* Hybrid: both out-of-the-box and custom alerts have a rate limit where an alert is triggered at most once every five seconds. This means that only one alert notification email is sent every 5 seconds.
