...
Sometimes firewalls or infrastructure settings don't let Mule Runtimes reach the Runtime Manager's public services. This might be an issue when registering a Mule Runtime or when starting it and registering it to work with Runtime Manager. If you need to whitelist the IPs of the Runtime Manager's public services, you can obtain them from the table below:
