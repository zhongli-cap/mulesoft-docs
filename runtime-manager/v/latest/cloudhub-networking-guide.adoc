...
If you need to whitelist the IPs for the communication between the link:/runtime-manager/runtime-manager-agent[Runtime Manager Agent] and the Runtime Manager console, see link:/runtime-manager/installing-and-configuring-runtime-manager-agent#ports-ips-and-hostnames-to-whitelist[IPs to Whitelist].
...
|===
|DNS Record |Regions
|myapp.cloudhub.io | US West (Oregon), US West (N. California), US East (N. Virginia)
