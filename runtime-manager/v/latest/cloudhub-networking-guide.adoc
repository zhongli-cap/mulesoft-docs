...
If you need to whitelist the IPs for the communication between the link:/runtime-manager/mule-agent[Runtime Manager Agent] and the Runtime Manager console, see link:/runtime-manager/installing-and-configuring-mule-agent#ports-and-ips-to-whitelist[IPs to Whitelist].
