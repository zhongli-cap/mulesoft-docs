...
*Remember* that on the Mule worker, the CloudHub load balancer proxies port :80 to :8081 for HTTP and proxies port :443 to :8082 for HTTPS. The `http.port` value must be set to port 8081 for HTTP, and the `https.port` value must be set to port 8082 for HTTPS. No other port numbers are supported.
...
If you need to whitelist the IPs for the communication between the Mule Agent and the Runtime Manager console, see link:/mule-agent/v/1.4.0/installing-mule-agent#ports-and-ips-to-whitelist[IPs to Whitelist].
