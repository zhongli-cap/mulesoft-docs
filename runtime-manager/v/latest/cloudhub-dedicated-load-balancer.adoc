...
Each SSL Endpoint can only have one CA certificate. If the applications behind your dedicated load balancer are reached by clients that do not share the same CA certificate, you need to create one SSL endpoint per CA certificate, which requires non-overlapping server certificate common names for the various SSL endpoints
