...
Each Load Balancer has an internal domain name to be used by applications and clients within the VPC.
...
Additionally, each load balancer has an external domain name that you can ping from outside your VPC network: `<lb-name>.lb.anypointdns.net` where `<lb-name>` is the name you gave the load balancer when you created it.
...
The table below contains some mappikng rule examples:
