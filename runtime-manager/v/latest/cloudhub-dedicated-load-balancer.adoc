...
This wildcard ceritifcate is configured to support any subdomain request to example.com, so this would include `app1.example.com` and `app2.example.com`, as well as any future subdomain names, like `appN.example.com`. 
...
Each SSL Endpoint can have multiple CA certificates and CRLs (Certificate Revocation Lists). Each of these certificates needs to be provided in a single unencrypted, _pem_ encoded file. Ordering for independent CA certificates is not important, but certificates in a chain of trust must be concatenated together.
...
The example above ensures both ’example.com/bookings’ and ‘example.com/sales’ match to `app-bookings/data` and `app-sales/data` respectively, as the variable `mypattern` holds these values. +
...
For these examples, suppose you have already configured a DNS CNAME record to route `example.com` to `lb-name.lb.anypointdns.net`. 
