...
////
== Deploying in Cluster Mode

Need to bind to Hazelcast

Via UI or command line????


link:/anypoint-platform-on-premises/v/1.5.0/configuring-anypoint-platform-for-pcf#set-up-a-hazelcast-server[Set Up a Hazelcast Server]

Just bind you PCF Mule application to the Hazelcast user provided service. The Mule buildPack will automatically recognize the binding between the Mule application and the Hazelcast cluster and will start the mule server in cluster mode. Remember to bind the application to all the Hazelcast node services from the cluster.

To scale Mule applications in cluster mode just use the cf scale command as you would do usually with any other application. The new Mule nodes will be binded to the same services as the rest. Take into account that by scaling your Mule application you could be imposing too much on your Hazelcast cluster and might have to scale that as well.
