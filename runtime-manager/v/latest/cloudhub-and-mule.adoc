...
Although the link:/mule-user-guide/v/3.8/begin-with-the-basics[basics of building a Mule application] are the same, the different deployment modalities offer distinct management features. The key reason is that each modality uses a different link:/mule-agent/[Mule Agent] when communicating with servers.
