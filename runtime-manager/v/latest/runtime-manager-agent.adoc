...
When registering the Runtime Manager Agent with Runtime Manager (by using the `-H` option), the Runtime Manager Agent first makes an outbout HTTPS connection to the Runtime Manager. This connection is used to establish a new secure, full duplex Web Socket connection between the Runtime Manager and the Runtime Manager Agent. 

There is a reconnection strategy in place in cases the Web Socket connection drops.

Each Runtime Manager Agent can also be configured to allow other REST connections (either secure or in-secure), for use by other management tools outside of Runtime Manager. 
