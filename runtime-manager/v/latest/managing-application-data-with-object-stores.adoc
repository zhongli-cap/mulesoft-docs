...
Settings: Set *Connector Configuration*: `ObjectStore_Configuration`, in the plus menu, specify a Partion Name, which can be any unique name. You can leave the other fields empty. For *Operation*: `Store`, *Key*: `&#x0023;[payload.key]`, and *Value Reference*: `&#x0023;[payload.value]`
