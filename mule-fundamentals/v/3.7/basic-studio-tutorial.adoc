...
able to apply what you learn to create link:/mule-fundamentals/v/3.7/content-based-routing-tutorial[more complex applications].
...
For more information about how to use the visual editor, see this link:/mule-fundamentals/v/3.7/anypoint-studio-essentials[quick overview]

At the bottom of the canvas are Message Flow and Configuration XML tabs for alternating between visual and XML editing.
...
On the *Configuration XML* tab is a text editor that lets you view and change the XML file behind the visual representation.
...
the *Package Explorer*. This file stores flow information for the visual editor. Because you named the project "Basic Tutorial", Studio names this file `basic_tutorial.xml`:

image:packages.png[packages]

When you open a new project, Studio opens this .xml file and a blank canvas. The `mule-project.xml`
file at the bottom of the Package Explorer is the Mule project descriptor file. Open this
...
For example `0.0.0.0:8081/any path`. You need this flexibility later when you extend the application.
+
...
The default `Info: Basic Tutorial` message in the Logger component appears in the log after running the application as follows:
...
by other HTTP connectors on other flows. Observe that the configurations you set in the Configuration Element
and those in the XML view of this element are the same:

If you make any changes in the XML configuration view, for example if you change the value of the *port*
attribute on the `http-listener-config`, then this change is reflected if you switch back to the visual editor and open the HTTP Connector's properties editor. Studio's two-way editor automatically updates the configuration as you switch back and forth.
...
* Log the unique message ID that Mule automatically assigns to each message that passes through a flow.
* Write the message payload that you see in your browser to a new .txt file on your local drive. 
* Name each .txt file using the message ID.
...
You use a Mule expression to log and use a link:/mule-user-guide/v/3.7/mule-expression-language-reference[unique message ID]. The following procedure provides
...
Because you are entering the expressions within a larger Mule expression, the `\#[]` syntax is not required around these individual expressions. Anything that you enter inside of the `\#[]` syntax which is _not_ a Mule expression must be enclosed with quotes so that Mule reads it as a string.
...
+
. Return to your Web browser and go to `http://0.0.0.0:8081/world`, and then replace `world` with another word of your choice and refresh.
. Check the following results:
* The names of files that the application saved to `/Downloads` and the inbound properties messages, "Current path is /world" for example, appear in the console.
* The directory you specified for writing files, Downloads for example, Mule wrote `<UUID>.txt` files, 5ebf4320-d43b-11e5-a7b3-acbc32b4ecbd.txt for example.
* The contents of each .txt file match the inbound message specified by the payload, which includes the http request path, "Hello, /world. Today is 20/02/16" for example.

Congratulations! You're all set to go on to the link:/mule-fundamentals/v/3.7/content-based-routing-tutorial[Content-Based Routing Tutorial].
