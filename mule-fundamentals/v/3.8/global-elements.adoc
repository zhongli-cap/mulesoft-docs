...
A *Global Element* is an object that you define once, then reference as many times as needed from elements in one or many Mule flows. Rather than repeatedly writing the same code to apply the same configuration to multiple elements in flows, you can create one global element that details your configurations or transport details. Then, you can instruct any number of elements across flows in your Mule application to reference the global element for specifics on how to behave.  +

== Advantages

Global elements provide a number of benefits to you as a Mule application developer, such as:
...
This *Global Elements* primer assumes that you are familiar with the
 link:/mule-fundamentals/v/3.8/anypoint-studio-essentials[Visual Editor in Anypoint Studio]. Review the link:/mule-fundamentals/v/3.8/anypoint-studio-essentials[Anypoint Studio Essentials] document to learn more about developing Mule applications using the Anypoint Studio graphical user interface.

== How to Create a Global Element

Some elements _require_ their properties be stored in a global element. For example, some "message sources" must reference a global connector configuration in order to function. The majority of elements, however, use global elements merely as a matter of convenience and efficiency.
