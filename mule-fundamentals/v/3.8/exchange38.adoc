...
* On the link:/mule-user-guide/v/3.8/http-request-connector[HTTP Request Connector] 's global element, through the link *Search RAML Exchange*
...
. The example is now complete! You can deploy it to see what's its outcome, run it on link:/mule-user-guide/v/3.8/studio-visual-debugger[debug mode] to examine what occurs with the Mule Message on each step, or modify it at will.
