...
NOTE: The repository where you serve the connector from must support basic authentication for access.
...
. Use Studio to export the connector as a zip file. Right-click the project name in Package Explorer and click *Export* > *Mule* > *Anypoint Studio Project ...*, *Next*, and *Finish*.
. Open the zip file. Locate the Feature ID. See link:/anypoint-connector-devkit/v/3.8/packaging-your-connector-for-release#uploading-your-connector-to-exchange[Uploading your Connector to Exchange] for how to obtain the value.
. In Anypoint Exchange, create an item in your private Exchange for the connector. Click *Done* and *Save*.
. Publish the connector item.

Thereafter users can open Anypoint Studio and download the connector using the Exchange icon in the task bar.
