...
Java-based integration runtime engine of MuleSoft's *Anypoint Platform* that uses a staged-event driven architecture (SEDA) to enqueue information and process it inside of *flows* to orchestrate the capture and movement of information both inbound and outbound from the Mule environment in which a Mule application is deployed. Mule was commonly known as Mule ESB, and is now known also as Mule Runtime, or as the entry states, simply *Mule*. Mule is used to integrate systems and applications, old and new, and is built to scale.
