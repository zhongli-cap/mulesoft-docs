...
* In clusters of Mule servers, messages can migrate between nodes when sent to an asynchronous flow. This allows for *load balancing between nodes* and higher performance of application. (See our link:/mule-fundamentals/v/3.4/flow-architecture-advanced-use-case[advanced use case] for an example.)
...
* *NEXT STEP:* Learn the various ways you can link:/mule-fundamentals/v/3.4/deploying-mule-applications[deploy Mule applications].
