...
Just like with on-premises Mule runtime deployments, applications deployed to CloudHub can still bundle their own property placeholder or secure property placeholder files inside the deployable archive file and these properties are substituted into the application when the application is started. With an on-premises Mule runtime, there are several ways to override property values bundled inside the application. 
...
. You you can set Java system environment variables at deployment time to override properties. 
