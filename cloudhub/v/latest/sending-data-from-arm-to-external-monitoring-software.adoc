...
By simply configuring a few settings on the Runtime Manager, the event notifications produced by your running Mule Applications can be easily sent out to your existing monitoring and analytics third party software.
...
The Runtime Manager exists both as a component of the link:/mule-fundamentals/v/3.7/anypoint-platform-primer[Anypoint Platform] and as an link:/anypoint-platform-on-premises/v/1.0/installing-anypoint-runtime-manager-on-premises-edition[application that can be installed on premise]. Runtime Manager On-Premises Edition manages applications that are deployed on premise, the Cloud version of Runtime Manager allows you manage applications deployed to link:/cloudhub/index[CloudHub] as well as applications that are deployed on premises. Users of Runtime Manager, using the Cloud and On-Prem editions, are able to monitor their applications. Applications deployed on Cloud can either use Insights (MuleSoft’s Analytics & Monitoring feature) or be integrated into third party monitoring applications for a unified view of monitoring and analytics. Apps deployed on on-prem must be integrated into third party monitoring applications.
...
When using Runtime Manager as part of the Anypoint Platform, launch the agent with exactly the same command that you found on the Runtime Manager UI:
...
The argument `-H` is an id that references your organization in the Anypoint platform and Runtime Manager On-Premises Edition installation.
...
If you update the Agent version isn't necessary to undeploy the apps.
...
[tab,title="Rest API"]
....
This feature require 1.2.0 agent version or newer.
...
. Activate the *Splunk* switch, this will open a pop up menu where you can provide your Splunk user and password data; and the host and port for the connection.
