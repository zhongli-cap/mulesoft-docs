...
The placeholder value can come from a file. The placeholder value will became the string value of the mentioned file. For example:
...
Then the payload's value will be `"Some content"`. Just like properties files, these files must be located at `src/main/resources`, inside your Mule project, or you can also use absolute paths.
