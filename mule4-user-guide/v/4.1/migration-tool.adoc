...
(Mule apps written for the Mule 3 runtime) to Mule 4. Instead of performing a completely manual migration, the tool automates the initial migration step for supported component, connector, and module configurations. Specifically, the migrator runs a sequence of tasks over a Mule project and then outputs a Mule 4 project with a report about the migration.
