...
Mule 4 offers an improved way of doing that. The `<file:listener>` component (shows as `On New File` in the Studio palette) polls a directory and triggers a new message just like its predecessor did. However it has the following added capabilities:

* You can use a `<file:matcher>` to only accept files with match a certain criteria
* The listening `directory` has to be created before the app starts, or a reconnection strategy be configured. In Mule 3.x the directory was created if it did not exist when the app started. 
* You're no longer forced to either delete or move the file, although the option to do that is still available.
* In 3.x, the delete or move of the input file was not done if an error occurred and a transaction was rolled back, but it was done if there was no transaction. In Mule 4, an error causes the delete or move to not be done regardless of whether there is a transaction or not.
