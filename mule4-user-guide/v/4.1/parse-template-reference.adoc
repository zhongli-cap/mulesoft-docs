...
For example, perhaps you want a Mule flow to send an "order complete" HTTP response to a caller to indicate the new order's number and the number of widgets ordered (both values that you previously set on the Mule message as variables).
You can use a Parse Template to load a template (either embedded in the component's definition or loaded from an external file) – where is expecting values for "order number" and "number of widgets".
Then, the Parse Template extracts info from the Mule message variables to insert as values into the template and set the resulting contents as the message payload.
In essence, a Parse Template takes a template into a Mule flow, fills in the blanks, then sets results on the message as the payload. In the example, after the parse template sets the payload on the message, the Mule flow returns the HTTP response to confirm the order.
