...
Parse Template is a Mule component to use for processing a template and obtaining a result. A template is defined as text with embedded Mule expressions that are evaluated and replaced with their result.

The template can be configured through an external file reference, or it can be embedded in the component definition itself.
