...
For more detail see link:/mule-user-guide/v/3.9/about-the-xml-configuration-file[About the XML Configuration File].
...
<2> Shows the namespace for Mule's link:/mule-user-guide/v/3.9/stdio-transport-reference[stdio transport], which allows communication using standard I/O. It has the prefix "stdio". The convention for a Mule module or transport's schema is to use its name as the prefix. +
...
* define configuration properties, and so on. 
...
Mule configurations can contain references to configuration properties, to allow referencing values specified outside the configuration file. One important use case for this is usernames and passwords, which should be specified in a more secure fashion. The syntax for configuration properties is simple: `${name}`, where `name` is a property in a standard Java property file.

Here is an example of a configuration that uses configuration properties, together with the properties it references:
...
Note the the location given for the file is a location in the classpath. Another alternative would be a URL, for instance `+file:///etc/mule/conf/my-mule-app-override.yaml+`, or an absolute path. 
...
For more about Mule transformers, see link:/mule-user-guide/v/3.9/using-transformers[Using Transformers].
...
Filters once again can be configured as global elements and referred to where they are used, or configured at their point of use.  For more information, see link:/mule-user-guide/v/3.9/filters[Filters].

==== Expressions

For a current reference to using expressions in Mule, see link:/mule-user-guide/v/3.9/mule-expression-language-mel[Mule Expression Language MEL]. 
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:/mule-user-guide/v/3.9/logger-component-reference[Logger Component Reference]
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see link:/mule-user-guide/v/3.9/routers[Routing Message Processors].
...
<1> and <2> are Java components, specified by each's class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see link:/mule-user-guide/v/3.9/entry-point-resolver-configuration-reference[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

In #9 in <<Flows>>, the example calls `authorize.net` to authorize a credit card purchase, passing it information from the message.  For more about connectors, see link:/mule-user-guide/v/3.9/anypoint-connectors[Anypoint Connectors].
...
In #14 in <<Flows>>, the example writes the message that caused the exception to a JMS queue, where it can be examined.  For more about exception strategies, see link:/mule-user-guide/v/3.9/error-handling[Error Handling].
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see link:/mule-user-guide/v/3.9/transaction-management[Transaction Management].
...
For more about Mule security, see link:/mule-user-guide/v/3.9/configuring-security[Configuring Security].
