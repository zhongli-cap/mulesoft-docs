...
Note that in a Mule runtime cluster or multi-worker CloudHub deployment, Scheduler only runs (triggers) on the primary node (that is, only in one Mule runtime).
