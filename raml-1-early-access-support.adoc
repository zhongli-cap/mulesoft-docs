...
The link:https://github.com/raml-org/raml-spec/blob/master/versions/raml-10/raml-10.md#annotating-scalar-valued-nodes[spec] allows using `uses` in any fragment. The Java parser only allows its use in root level RAML documents and libraries.
...
RAML 1.0 loosens the syntax requirements for `examples / example` and replaces “content” with “value”. If a user specifies the following: 
...
* In the current RAML spec: VALID
...
* In the current RAML spec: INVALID
