...
* Viewer: enables a user to view the graph of an application network, but does not allow them to make changes. This permission can only be assigned to users of the master organization.
* Editor: enables a user to view and modify the graph of an application network. This permission has to be configured for each business group, to allow it's users to access Anypoint Visualizer.
