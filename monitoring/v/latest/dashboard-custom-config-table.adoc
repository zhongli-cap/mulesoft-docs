...
Note that this example uses color-coded <<thresholds, thresholds>> (shown in the Avg and Max cells).
|===
...
You can set pagination properties for tables so that they do not become to long and are more manageable.
...
Link settings are only available when _Render Value As Link_ is checked. You can use these to link from a value to a specified web page.
