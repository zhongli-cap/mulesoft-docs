...
+
* Creating a query incorporating predefined filters and operators. You can save
this query  as a filter.
+
image::log-filter-timestamp.png[Using a Filter Query]
+
See <<filters_predefined, Search Filters>> and <<operators, Operators>>.
+
Note that the UI can convert a filter query into an Elasticsearch Query DSL
query for you. This example show a query for log-level INFO:
+
...
----
+
The Logs UI provides a link to the query documentation so that you can perform more complex queries than otherwise available, for example:
+
...
----
+
Note that some complex Elasticsearch Query DSL queries do not have an equivalent in the Search Filter Values UI, so you can only view such queries in the Edit Query DSL UI.
