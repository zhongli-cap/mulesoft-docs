...
+
If the chart (widget) icons shown above are not visible, you can use the left-side menu on a new or existing row, for example:
+
image::dashboard-custom-add-move-delete.png[Adding a Chart (Widget) to a Custom Dashboard]
+
To open this menu, you need to mouse over the row and then click the three vertical dots next to the title of the left-most part of the row, for example:
+
