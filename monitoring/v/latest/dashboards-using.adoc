...
You can refresh data in the graphs automatically, or if your subscription permits, set a regular interval (such as 30s, 5m, 1h, 1d). Your refresh setting applies to all graphs in the dashboard.

.Auto and Manual Refresh Setting
image::dashboard-auto-refresh.png[Data Refresh Options]

Note that the auto-refresh setting is the circular (cycle) icon to the right.
