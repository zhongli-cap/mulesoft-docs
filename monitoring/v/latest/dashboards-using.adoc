= Monitoring Environments through Dashboards

//NOTE: MIGHT NEED TO MAKE THIS MORE USE CASE FOCUSED

Dashboards in the Monitoring Center provide visibility into Mule apps that are deployed to your environments (for example, Production, Sandbox, or Design).

Built-in dashboards contain a standard set of graphs, while
custom dashboards can display additional data that is important to your organization. Both dashboards provide many of the same display options.
//TODO: NEED EXAMPLE

== Viewing a Dashboard

You can select a built-in or custom dashboard from the Monitoring Center.

image::dashboard-select[Select a Dashboard]

. From the Monitoring Center, click Built-in Dashboards or Custom Dashboards.
. Select and environment to view, such as your Design or Sandbox environment.
. Select a Mule app.

//TODO: LIST OF BUILT-IN graphS?
...
image::dashboard-time-period-refresh[Configure Display]
...
image::dashboard-data-intervals[Data Point Display at Intervals]
...
image::dashboard-response-time-outbound[Example: Data from the Last 30 Minutes]

Custom settings are useful for isolating data over a given date and time range:

image::dashboard-data-range[Data Point Display over Date and Time range]
...
[cols="1,4"]
|===
| image:zoom-earlier-later[Zoom Out, Show Earlier and Later] a|
...
image::dashboard-mouseover[Graph: Select Time Period]
...
.Resulting Graph
image::dashboard-mouseover-result[Graph: Results of Time Period Selection]
...
image::dashboard-auto-refresh[Data Refresh Options]
