...
Graphs can contain configurable legends.
...
| As Table | Checking displays the selected Values (such as, Min and Max) in a table. Unchecked by default.
| To the Right | Checking display the legend to the right side. Unchecked by default.
| Width | Available when To the Right is checked. Sets the minimum width for the legend. You can leave it blank to allow for the automatic width selection.
|===
...
== Stacking and Hover (Advanced)

A hover tooltip displays data for a data point in a graph when you hover over it.
...
You can set one or more thresholds, such as a line or section to a graph, that indicate whether the data crosses a limit that you specify.
...
| Fill | *TODO* |
| Line | *TODO* |
| Fill Color  | For custom thresholds only. *TODO* |
| Line Color | *TODO* |
|===

== Time Range Shift (Advanced)

These settings are for overriding the global settings for *TODO*
