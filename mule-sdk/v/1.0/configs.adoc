...
All modules are required to have at least one configuration, but they can define as many as needed.
...
Depending on how you want to organize your code, you might want a single
...
Because an application might contain many listener elements, the elements will only work when
paired with a `<http:listener-config />` that contains general settings about how the
...
<1> The `@Extension` annotated class defines the two configs.
<2> The `HttpListener` class defines the inbound connection and the message source exclusive to the listener feature.
...
For logging or debugging reasons, you might want to know the name that this config instance
...
annotation. Here is an example that shows how to use it.

The example shows how to log the name of the component when it is getting initialized. 
