= About Technical Guidelines for Connector Certifications
...
+
.. Use the `mvn clean` package command.
+
...
* Add the following docs (`user-manual.adoc` and `release-notes.adoc`) to the `doc` folder.
+
. Add `License.md` if it is not in the root folder already:
+
[sounce,license,linenums]
----
...
----
+
You can develop or add operations to your by following the Mule SDK Doc.
+
...
+
----
MuleSoft Certified Connectors (Additional Fee)
@Extension(name = “SAP Hybris”, category = CERTIFIED, provider = “Admios")
@RequiresEnterpriseLicense
@RequiresEntitlement(name = “sap-hybris-connector")
----
...
+
image::pom_file[pom.xml file example]
+
+
