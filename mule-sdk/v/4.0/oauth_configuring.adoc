...
the OAuth protocol, things like the OAuth dance are unavoidable. The SDK's approach to this problem is to standardize the
experience of all OAuth modules to keep simplifying the user's experience.
...
this attribute is multi-tenancy.
...
* <<oauth_dance#, Performing the OAuth dance with an OAuth enabled module>>
