...
For logging or debugging reasons, we may want to know the name that this config instance
 has on the application. To this purpose, we can use the `@RefName` annotation. This annotation must
...
On this example we can see how to log the name of our component when it's getting initialized,
this way we can have traceability of the different config instances.
...
this config is initialized we will see this reflected on the logs with the message:
