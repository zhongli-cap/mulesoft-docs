...
<<configs#, Configurations>>, <<module_structure#components, Components>> and <<connections#, Connection Providers>>
...
* If `options` is not configured, it value will be `null` (see <<null_safe#, NullSafe>> for altering this behavior).
