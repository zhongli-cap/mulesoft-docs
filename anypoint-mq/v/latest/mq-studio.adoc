...
=== Additional Global Element Properties
...
|===
|Studio Field |XML Element |Description
|Name |`doc:name` |Configuration name.

*Required:* Yes +
*Default:* None
|Provider: +
Use Provider Config |`anypoint-mq:provider` |Sets a provider configuration for the current app consisting of the URL, Client App ID, and Client Secret.

*Required:* Yes +
*Default:* None
|URL |`url` |URL shown in Anypoint Platform.

*Required:* Yes +
*Default:* None

image:mq-ap-url.png[URL in MQ near top of UI]
|Client App ID |`clientId` |From Anypoint Platform > MQ > Client Apps. Click the app and copy the  Client App ID value.

*Required:* Yes +
*Default:* None
|Client Secret |`clientSecret` |From Anypoint Platform > MQ > Client Apps. Click the app and copy the Client Secret value.

*Required:* Yes +
*Default:* None
|Use Global Provider Config |`provider-ref` |Sets the URL, Client App ID, and Client Secret for all apps.

*Required:* No +
*Default:* None
|Basic Settings: +
Max Redelivery |`maxRedelivery` |Number of redeliveries
...
*Required:* No +
*Default:* -1
|[[am]]Acknowledgement Mode |`acknowledgementMode` |Defines how the Anypoint MQ messages are acknowledged when the message are removed from being inflight messages. If set to the default
...
*Required:* No +
*Default:* None
|Use custom worker threading profile |`worker-threading-profile` |See link:/mule-user-guide/v/3.8/tuning-performance[Tuning Performance].

*Required:* No +
*Default:* None
|Max Active Threads |`maxThreadsActive` |The maximum number of threads to use. See link:/mule-user-guide/v/3.8/tuning-performance[Tuning Performance].

*Required:* No +
*Default:* None
|Max Idle Threads |`maxThreadsIdle` |The maximum number of idle or inactive threads that can be in the pool before they are destroyed. See link:/mule-user-guide/v/3.8/tuning-performance[Tuning Performance]. The thread pool does not grow from Max Idle Threads towards Max Active Threads unless the queue is completely filled up.

*Required:* No +
*Default:* None
|Thread TTL |`threadTTL` |Thread time to live duration.

*Required:* No +
*Default:* None
|Pool Exhausted Action |`poolExhaustedAction` |When the maximum pool size or queue size is bounded, this value determines how to handle incoming tasks. Possible values are: WAIT (wait until a thread becomes available; don’t use this value if the minimum number of threads is zero, in which case a thread may never become available), DISCARD (throw away the current request and return), DISCARD_OLDEST (throw away the oldest request and return), ABORT (throw a RuntimeException), and RUN (the default; the thread making the execute request runs the task itself, which helps guard against lockup).

*Required:* No +
*Default:* None
|Thread Wait Timeout |`threadWaitTimeout` |Thread wait timeout in milliseconds.

*Required:* No +
*Default:* None
|Max Buffer Size |`maxBufferSize` |Max buffer size.

*Required:* No +
*Default:* None
|Operation |`anypoint-mq:<operation>` |Operation that this connector instance performs.
...
*Required:* Yes +
*Default:* None
|Destination |`destination` |Queue or message exchange name.

*Required:* Yes +
*Default:* None
|Message ID |`messageId` |Optional ID of a message to publish or consume.

*Required:* No +
*Default:* None
|Send Outbound Properties |`sendOutboundProperties` |Send properties when publishing.

*Required:* Yes +
*Default:* Checked
|Send Content Type |`sendContentType` |Send content type when publishing.

*Required:* Yes +
*Default:* Checked
|Property |`propertyName` |Optional property content to set for publish.

*Required:* No +
*Default:* None
|Value |`value` |Optional value content to set for publish.
