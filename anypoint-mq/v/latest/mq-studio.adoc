...
threading profile statement to start with anypoint-mq, and switch the order of the statement with the anypoint-mq:provider statement in the anypoint-mq:config block. 
...
|Polling Time |Duration in milliseconds that the MQ connector polls the queue or message exchange for messages. *Note:* This value overrides the *Pooling Time* field in the Global Elements Properties Basic Settings.
|===
...
times out. *Note:* This value overrides the *Acknowledgement Timeout* field in the Global Elements Properties Basic Settings.
|Polling Time |Duration in milliseconds that the MQ connector polls the queue or message exchange for messages. *Note:* This value overrides the *Pooling Time* field in the Global Elements Properties Basic Settings.
|Reconnection Attempts |Number of times MQ should attempt to re-establish a connection. -1 means try forever.
|===
