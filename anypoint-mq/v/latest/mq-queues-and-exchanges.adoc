...
* xref:dlqfifo[FIFO note] - Determine if a queue is FIFO or non-FIFO.
* link:/anypoint-mq/mq-studio#configuring-anypoint-studio[Configuring Anypoint Studio] > *Studio Visual Editor* tab >
*Advanced Global Element Properties* > *Message ID* field description - When publishing to FIFO queues,
if you specify a custom Message ID and the Message ID is the same on multiple messages, the
messages with the same Message ID will not be redelivered.
* link:/anypoint-mq/mq-studio#configuring-anypoint-studio[Configuring Anypoint Studio] > *XML Editor or Standalone* tab >
