...
For applications where the order of messages needs to be strictly preserved and enforced, Anypoint MQ provide first in, first out (FIFO) processing to enable ordering of messages. With FIFO ability, Anypoint MQ ensures that the order in which messages are placed on a queue is the order in which they will be retrieved.
...
FIFO queues are best fit for applications requiring strict message ordering and exactly once delivery but message delivery speed is of less importance.
Standard queues are best fit for applications where messages need to be delivered quickly.
