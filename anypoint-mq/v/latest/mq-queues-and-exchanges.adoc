...
* FIFO queues are currently only available in the US West region.
...
FIFO queues are best for when message order is more critical than the speed of delivery.
Non-FIFO queues are good when message speed needs to be as fast as possible.

IMPORTANT: FIFO queues are currently only available in the US West region. If your Anypoint MQ user interface points to an alternate region, you cannot create a FIFO queue.
