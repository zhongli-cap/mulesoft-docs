...
*Note:* The access token has a time to live of 1 hour. It's the same TTL as the tokens users get when they log into the Anypoint Platform. The TTL is set in the Access Management screen for an organization.
