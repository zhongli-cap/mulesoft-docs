...
*Note:* The organization from which you use the Administration portal must have an Anypoint MQ FIFO entitlement. FIFO is available in the Oregon and Northern Virginia regions. If needed, see xref:getoeb[Get an Organization ID, Environment ID, and Bearer Token]. See https://docs.mulesoft.com/anypoint-mq/ for the regions MQ is available in.
