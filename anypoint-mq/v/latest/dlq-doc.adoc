...
NOTE: A few caveats for a DLQ. The DLQ has to have the same encryption as the queues writing to it. If a DLQ is encrypted, all queues using the DLQ must also be encrypted or the messages in the DLQ may be unreadable. Both the DLQ and the queue writing to it must be in the same geographical region and owned by the same Anypoint Platform account.
