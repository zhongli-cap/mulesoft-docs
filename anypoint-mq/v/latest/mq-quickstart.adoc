...
+
You can delete the message by clicking the trash can icon. For other ways to delete a message, see link:/anypoint-mq/mq-ack-mode[Anypoint MQ Acknowledgement Mode].
+
...
To test your application, you can use link:https://www.mulesoft.com/platform/studio[Anypoint Studio] to create a connector that lets you send messages to your queue. Before using Studio, you need to generate a client ID and secret so that the Studio application can talk to the Anypoint MQ queue.
...
+
. In the Client Apps display, click the client name, The Client App ID and Client Secret values display to the right of the client name. You can use these values in Anypoint Studio. Note the links for how to click *Copy* that copy the value to your computer's clipboard.
...
link:https://www.mulesoft.com/platform/studio[Anypoint Studio] provides a graphic interface and web server for running MQ applications.
...
. In Studio, click the Exchange icon in the upper left of Studio to start Anypoint Exchange, which provides connectors, templates, and examples you can use with Studio.
+
