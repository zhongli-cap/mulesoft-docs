...
////
== FAQ: If I use AUTO acknowledgement mode without a rollback exception strategy, would the message ever be delivered back to the queue?

AUTO acknowledgement mode acks the message at the end of the flow. If the message gets to the end, 
then it’s ok to ack and the connector acks it. The message is purged once it gets an ack. 
In the meantime it's in flight until it gets the ack and gets back to the queue due to ack timeout.

If the flow is only ever returning an ACK, then the message is purged (unless the Lock TTL Timeout is fired), 
and the only time redelivery that occurs is if the Lock TTL Timeout is fired or if a NACK is sent. 
But in AUTO mode, the only time a NACK is implicitly sent is in a rollback exception strategy.

The Max Redelivery count is not relevant. The redelivery count only increases when a 
nack is received and when an ack timeout occurs.

If you have a dead letter queue, after the number of retries, the message goes to the DLQ.

When you fetch a message that you send to MQ a time, it's “locked” and not available for other consumers 
so if you fetch a message with Timeout set to zero, 
the message fetches repeatedly for all the consumers of that queue.

The Acknowledgement timeout is the time the message is in the flow before considering that the operation was not successful.

*Note:* The Default Lock TTL setting overrides the Acknowledgement Timeout.
////
