...
There is no maximum transactions per second (TPS). FIFO queues have a hard limit of 300 TPS, but regular queues or exchanges have no limit. The TPS is only throttled in the event of a denial of service attack, but even then it would be a high limit.
