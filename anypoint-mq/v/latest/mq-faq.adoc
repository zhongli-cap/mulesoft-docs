...
Anypoint MQ provides persistent data storage across multiple data centers in a region, ensuring that it can handle data center outages and provide full disaster recovery in case of an availability zone going down. MQ provides data recovery within a region by deploying services across multiple AZs. While MQ does not provide multi-region failover, even if all the MQ servers go down across all AZs, messages are durable and are not lost as they persist to additional internal networks.
