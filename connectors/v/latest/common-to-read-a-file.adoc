...
You use the File connector's Read operation to read a file.

The connector returns a MuleMessage with the following attributes:
* An `InputStream` as payload.
* A `FileAttributes` instance.

. After adding the File connector to your flow, select the Read operation.
