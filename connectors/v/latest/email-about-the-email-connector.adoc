...
Matchers in List operations are for identifying the email you want to list and filtering out what you do not. For example, you might accept emails from a given date range or use a regex pattern to identify a certain subject or author.

Email attributes, such as the subject, ID, from address, and so on form the metadata of the email message. You can use the metadata for data transformations and
