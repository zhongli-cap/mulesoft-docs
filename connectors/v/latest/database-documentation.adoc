...
<p> The Database connector can be configured with various criteria to limit or filter the results returned from a database table. The two options are to set an Id Column or a Watermark column in the database table to be used to limit the rows returned from the database based on certain conditions. 

A *Watermark* column set's a value in the SQL WHERE clause to filter the results returned from the database. 

An *Id Column* provides idempontent criteria to avoid duplicate processing of any row where the Id Column value matches any previously returned value. 

You can also set both of these filter columns at the same time, as each filter works in a different way. 
