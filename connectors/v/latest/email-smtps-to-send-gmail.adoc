...
+
. Add a Logger to the end of your flow with a Message such as `"My Message"`.
. Deploy, Sync, or Run the app.
. Once the app is deployed (or running), copy the link to the app to a browser, and append the Listener path (`/send`), for example:
+
