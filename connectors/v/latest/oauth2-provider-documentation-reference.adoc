...
Since the OAuth dance is donde through HTTP, the OAuth2 Provider makes use of the link:connectors/v/latests/http-connector.adoc[Mule HTTP Connector].
...
Two HTTP endpoints are created for listening to Authentication Code and Token request as stated by the OAuth2 definition. Those will work independently of the mule application and will respond via HTTP. +
...
==== Raises
* SERVER_SECURITY:TOKEN_UNAUTHORIZED
+
