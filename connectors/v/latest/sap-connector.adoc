...
=== Install Each SAP Library

. For each Library, click Add Dependency. 
. Click Install as for this IDOC library.
+
...
. Specify a unique Group ID, Artifact ID, and the 3.0.18 version. 
+
image:sap-connector-configuration-02-in.png[Maven dependency for IDOC]
+
. Repeat this same process to install the JCO library and the JCO Native
library.
+
When done, the configuration screen appears as follows with green check marks for each successfully installed library:
+
image:sap-connector-configuration-05.png[All dependencies added]
+
