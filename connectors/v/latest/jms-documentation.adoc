...
| User Properties a| Object |  +++The custom user properties that should be set to this Message. Can be written as a DataWeave object, such as `#[output application/json --- { userName: vars.user, appName: 'myApp'}]`+++ |  | {nbsp}
