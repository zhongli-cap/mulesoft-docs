...
Uses the File connector's List operation to list all files in a directory that match file properties. The operation can recurse through subdirectories.
...
. Perform any optional tasks you need:
  ...
  ** Provide File Matching Rules. For an example, see .
  ** Set a Target Variable to store output of the operation.
// TODO: example of setting a target variable

[[see_also]]
== See Also

* link:/connectors/file-documentation[Connector Technical Reference]
* link:common-to-match-file-properties[To Match File Properties with a Connector]
* link:file-to-set-up-a-file-connector-config[To set up a File connector]
