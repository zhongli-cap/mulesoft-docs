= To Query a Database (Design Center)
...
. Click Deploy.
. Select Copy Link, and go to the URL. For example, paste the link in a browser or a client, such as Postman, and press return.
+
...
+
. Open the database operation and select Output.
+
...
payload.productLine
+
. Click Deploy and select Copy Link, paste the link in a browser, and go to the URL.
