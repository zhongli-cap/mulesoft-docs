...
. To return a list that matches specific email properties, use *Match with* criteria. For an example, see:
include::include_link_list.adoc[match-file-props]
 _... TODO, PROVIDE AN EXAMPLE, LINK TO OR MENTION THE REF?_
. To change the number of emails on each page of results, change the *Page Size*. Defaults to `10` emails per page.
. To ... TODO ... *Delete After Retrieve* option (disabled by default).
