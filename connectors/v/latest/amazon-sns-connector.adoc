...
. Search for the Amazon SNS connector and click Install.
...
[[config]]
== To Configure a Global Element Using the Studio Visual Editor
...
== To Configure a Global Element Using the XML Editor
...
<sns:config name="Amazon_SNS__Configuration" accessKey="${amazon.accesskey}"
 secretKey="${amazon.secretkey}" topicArn="${amazon.sns.topic.arn}"
 doc:name="Amazon SNS: Configuration"/>
----

=== Connector Namespace and Schema

In Studio, when you drag the connector from the palette onto the Anypoint Studio canvas, it automatically populates the XML code with the connector namespace and schema location.
...
See the link:https://mulesoft.github.io/sns-connector/[full list of operations for this connector].

This connector supports these operations:
...
image:sns-use-case-flow.png[Sending messages to SQS Queue]
...
+
image:sns-http-props.png[sns-http-props]
+
[%header%autowidth.spread]
|===
|Parameter|Value
...
|===
+
. Save and run the project as a Mule Application. Right-click the project in Package Explorer. Click Run As > Mule Application.
