...
////
>>[BOGDAN] Read through this user guide to understand how to set up and configure a basic flow using the connector. Track feature additions, compatibility, limitations and API version updates using the link:/release-notes/salesforce-connector-release-notes[Salesforce Connector Release Notes]. Review the connector operations and see how they work be reviewing the link:http://mulesoft.github.io/salesforce-connector/[Technical Reference] alongside the link:https://www.mulesoft.com/exchange#!/?filters=Salesforce&sortBy=rank[Demo Applications]. >>[BOGDAN] duplicated << [/BOGDAN]
////
...
This document assumes that you are familiar with Mule, the link:/anypoint-studio/v/6/[Anypoint Studio Interface], link:/mule-user-guide/v/3.8/global-elements[Global Elements] and link:/anypoint-studio/v/6/datasense[DataSense]. Further, it assumes you are familiar with Salesforce.

////
This document assumes that you are familiar with Mule, the link:/anypoint-studio/v/6/[Anypoint Studio Interface], link:/mule-user-guide/v/3.8/global-elements[Global Elements], link:/anypoint-studio/v/6/datasense[DataSense], >>and link:/mule-user-guide/v/3.8/batch-processing[Batch Processing] in Mule>>[BOGDAN] Not migrated yet. So far not sure how is it going to be migrated << [/BOGDAN]. Further, it assumes you are familiar with Salesforce.
////
...
[source, xml]
----
xmlns:sfdc="http://www.mulesoft.org/schema/mule/>>[BOGDAN] sfdc>>"salesforce" is the new namespace << [/BOGDAN]
----
////
...
...
http://www.mulesoft.org/schema/mule/sfdc
http://www.mulesoft.org/schema/mule/sfdc/current/>>[BOGDAN] mule-sfdc.xsd>>mule-salesforce.xsd is the new name of xsd file << [/BOGDAN]"
----
////
...
<mule xmlns="http://www.mulesoft.org/schema/mule/core"
      xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      xmlns:sfdc="http://www.mulesoft.org/schema/mule/>>[BOGDAN] sfdc>>salesforce"
      xsi:schemaLocation="
               http://www.mulesoft.org/schema/mule/core
               http://www.mulesoft.org/schema/mule/core/current/mule.xsd
               http://www.mulesoft.org/schema/mule/sfdc
               http://www.mulesoft.org/schema/mule/sfdc/current/>>[BOGDAN] mule-sfdc.xsd>>mule-salesforce.xsd << [/BOGDAN]">
  
      <!-- Insert your configuration elements and your flow here -->
</mule>
----
...
|===
|Application/Service|Version
|Mule Runtime|Mule Enterprise Edition >>[BOGDAN] 3.5.2>>"4.0.0" I suppose << [/BOGDAN] and newer
|Anypoint Studio|Only works with Studio >>[BOGDAN] 5.4.0>>"7.0.0" I suppose << [/BOGDAN] and newer
|Salesforce|>>[BOGDAN] v37.0, v38.0>>"v37.0, v38.0, v39.0" should be << [/BOGDAN]
|===
...
>>[BOGDAN]  I am not sure that here is the best place for this << [/BOGDAN]
////
...
////
>>[BOGDAN] After you download and install the connector, use the following steps to make the Salesforce connector available to inside a Mule application for use and to package the application with the connector.
If you use Anypoint Studio, it will do this automatically for you.>>[BOGDAN]  Thisy is not useful any more as in latest studio all the projects are mavenized << [/BOGDAN]
////
...
----
<dependency>
    >>[BOGDAN] <groupId>org.mule.modules</groupId>
    <artifactId>mule-module-sfdc</artifactId>
    <version>RELEASE</version> << [/BOGDAN]
    >>[BOGDAN]     <groupId>org.mule.connectors</groupId>
    	<artifactId>mule-sfdc-connector</artifactId> this are the new artefact and group ids << [/BOGDAN]
</dependency>
----
////
+
...
<dependency>
    >>[BOGDAN] <groupId>org.mule.modules</groupId>
    <artifactId>mule-module-sfdc</artifactId>>[BOGDAN] >same as above << [/BOGDAN]
    <version>LATEST</version>
</dependency>
----
////
...
----
>>[BOGDAN]  I do not think that this is applicable any more << [/BOGDAN]
////
...
>>[BOGDAN]  this has to be updated. I do not know how to approach the thing that we have both Mozart and Studio << [/BOGDAN]
////
+
...
>>[BOGDAN] This has to be updated << [/BOGDAN]
////
...
====
>>[BOGDAN] I do not know if this is the right place to point this, as we have release notes and this only seems to polute user manual << [/BOGDAN]
////
...
>>[BOGDAN] This phrase is duplicated << [/BOGDAN]
...
>>[BOGDAN]  this is also duplicated << [/BOGDAN]
////
...
For all the configurations *except OAuth v2.0*, you can keep the session alive until it expires by checking the *Disable session invalidation* checkbox.

////
>>[BOGDAN] New in Salesforce Connector version 7.0.0, for all the configurations *except OAuth v2.0*, you have
the option to keep the session alive until it expires by checking the *Disable session invalidation*
checkbox.>>[BOGDAN] This is something that is not new, it was in here since few version << [/BOGDAN]
////
...
////
TODO?
>>[BOGDAN] If you’re currently using an older version of the connector, a small popup appears in the bottom right corner of Anypoint Studio with an "Updates Available" message:
...
We recommend that you keep Studio up to date with its latest version.>>[BOGDAN] As all the projects in studio 7 are maven based I do not know if this is still needed. << [/BOGDAN]
////
