...
The SAP Connector provides a wide range of functionalities. This document helps you harness the full potential of your SAP Mule application.

[[jco-extended-properties]]
== JCo Extended Properties

The SAP connector provides a wide set of additional JCo properties allowing you to configure the connection at the connector or endpoint level, inside the `mule-config.xml` file.

Important: The key names for these properties must match those expected by the JCo libraries. 
...
----
+
. Using MEL expressions for each attribute.
+
[source, xml]
----
...
* The properties defined at the connector or endpoint level override the values in the properties map.

* The properties defined at the endpoint level override those from the connector.
...
|jco.destination.repository_destination |Specifies the destination to be used as a repository, such as use this destination's repository.| |
|jco.destination.repository.user |Optional: If the repository destination is not set, and this property is set, it's considered to be a user for repository calls. This allows use of a different user for repository lookups.| |
...
|jco.destination.one_roundtrip_repository|1 = force usage of RFC_METADTA_GET in SAP Server, 0 = deactivate it. If not set, the destination makes initially a remote call to check if RFC_METADATA_GET is available.| |
|===
...
|jco.server.gwserv * |Gateway service, such as the port registration can be performed on |jcoGwService |
...
If you want to receive IDocs or be called as a BAPI but do not want to use the PORT number as the value for `jcoGwService`, modify the Operating System `services` file:
...
You need to add that SAP Gateway entry in the `services` file. So, for example, if you are trying to reach SAP instance 21, you need to add:
...
You must “escape” an ampersand in the address attribute and replace it with `&amp;`.
...
*Note:* The key to a fully working application in a cluster is the implementation of reliability patterns.
...
The inbound endpoint represents a bigger challenge when configuring your application in HA mode. The following sections provide information that  assists you in making the best decision.
...
Also, recall that in HA configurations the payload should be serializable. To ensure this is done, configure the inbound endpoint to output XML. This is easily achieved with the `outputXml` attribute set to `true`. 
[source, xml, linenums]
----
...
If the `rfcType` is configured to *srfc*, or it is not provided (thus defaulting to srfc), then no TID handler is configured. Furthermore, if a TID handler has been configured in the XML file, it is ignored.
...
* To configure more than one PropertyPlaceholder, the first one must have the property ignoreUnresolvablePlaceholders set to true (such as, `<spring:property name="ignoreUnresolvablePlaceholders" value="true" />`).
...
|bapiTransaction |When set to `true`, either BAPI_TRANSACTION_COMMIT or BAPI_TRANSACTION_ROLLBACK is called at the end of the transaction, depending on the result of that transaction.  |`false` |`1.0`
|===
...
The following example describes how to execute two BAPIs in a stateful transaction.
