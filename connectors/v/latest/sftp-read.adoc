...
* If the file does not exist, you receive a `SFTP:ILLEGAL_PATH` error.

Note that the path must point to an actual file. The operation does not read directories. If there is no file, you will receive a `SFTP:ILLEGAL_PATH` error.
