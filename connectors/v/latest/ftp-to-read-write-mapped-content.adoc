...
. Provide a Scheduler trigger for your flow.
+
...
.. From the Input tab in the Write operation, Add and Save a custom JSON data type example with a name such as `My JSON Input` that looks like this:
+
----
[
  {
...
----
+
.. From the Output tab in the Write operation, Add and Save a custom JSON data type example with a name such as `My JSON Output` that provides a subset of the full input example:
+
----
[
  {
...
| Customize Data Type button - If the Output column lacks your custom data type example, you can load it through this button (located at lower-left corner of the Output column).
|===
+
...
----
+
Note that you can change `(value0, index0)` to `(value0)` because the `index0` key is not needed for the transformation.
