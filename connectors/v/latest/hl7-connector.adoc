...
HL7 schema definitions are distributed inside the hl7-schemas-hl7-4.0.0.jar, 
...
HL7 defines the Message Type in the component values of MSH-09a composite 
structure of type `MSG`. The HL7 EDI Connector uses these component values 
to find the structure schema to be used for processing a receive message, 
...
supported Message Code to the actual Message Structure. Here's a sample of what this looks like:
