...
+
* Update the TOCs (top-level and product or product-version level) to reflect the archived state, for example: link:/mule-user-guide/v/3.4/_toc.adoc[Mule Runtime Doc Archives] and the link:_toc.adoc[Top-level TOC].
...
* Make sure that the `_config.adoc` file reflects the correct setting for `isLatest: true|false`. For example, if the latest archived version was 1.4, and you are now adding 1.5 docs to the archive, you need to change the setting to `false` for th 1.4 docs and to `true` for the 1.5 docs. 
...
. Test links in `master`, then commit and push in the changes `master`.
...
. Copy the directory that contains the files you want to archive, for example: `$ cp -r /4.1 /4.1.archive`
