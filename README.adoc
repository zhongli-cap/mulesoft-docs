...
*Note:* Not all links may work in the local preview as the preview server doesn't implement the rewrite rules used in production.
...
* For tables, use `[%header%autowidth.spread]` instead of specifying column widths. This lets the documentation builder
optimize the column widths.
* Use `|===` to start and end a table. Adding extra equal signs only adds unnecessary characters to files and makes table specifications more complex.
...
[CAUTION]
====
The value of the `cols` attribute should _never_ be a string of repeating commas (for example, `cols=",,,"`).

The only acceptable formats for the `cols` attribute value are (a) a single number followed by an asterisk (for example, `3*`) or (b) a comma-separated list of column specs (for example, `1h,^2,>1`).
====
+
* The simplest table convention is `[%header%autowidth.spread]` for headings and automatically spaced columns.
...
* Omit the `linenums` option on 1 line code examples.
* Put multi-word examples in a source block instead of a long tick-marked string.
* Don't use the [tabs] syntax. Instead use headings for 
 "Visual Studio Editor" and "XML Editor or Standalone".
...
|Bold |Don't use |Initial capitalize each letter in a button or field name regardless of how shown in the UI.
|`tick marks` |Reserved words or code examples, such as a MEL expression |`#[payload]`
|Italics |Emphasis |_Ensure the checkbox is set_
|Bold italics |Don't use |Initial capitalize each letter in a button or field name regardless of how shown in the UI.
|Bold links |Use sparingly and only for links such as Skip to Code | \*\<\<Skip to Code>>*
|Quotes |Don't use. |{nbsp}
|===
